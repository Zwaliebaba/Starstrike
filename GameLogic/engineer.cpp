...
void Engineer::SetWaypoint( LegacyVector3 const &_wayPoint )
{
    m_retargetTimer = 0.0f;
...
bool Engineer::AdvanceReprogramming()
{
    ...
    LegacyVector3 targetFront = (consolePos - m_pos).Normalise();
    m_front = (targetFront * SERVER_ADVANCE_PERIOD) +
              (m_front * (1.0f - SERVER_ADVANCE_PERIOD) );
...
void Engineer::BeginBridge( LegacyVector3 _to )
{
    ...
    LegacyVector3 componentSpan = bridgeSpan / (float) numComponents;
