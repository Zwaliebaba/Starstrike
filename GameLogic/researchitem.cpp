...
void ResearchItem::RenderAlphas( float _predictionTime )
{
    ...
    if( alpha > 0.0f )
    {
        ...
        glBegin( GL_QUADS );
            glColor4f( 0.1f, 0.2f, 0.8f, alpha);
            glTexCoord2i(0,0);      glVertex3fv( (m_pos + LegacyVector3(0,-50,0) - camRight * w).GetData() );
            glTexCoord2i(0,1);      glVertex3fv( (m_pos + LegacyVector3(0,-50,0) + camRight * w).GetData() );

            glColor4f( 0.1f, 0.2f, 0.8f, 0.0f );
            glTexCoord2i(1,1);      glVertex3fv( (m_pos + LegacyVector3(0,1000,0) + camRight * w).GetData() );
            glTexCoord2i(1,0);      glVertex3fv( (m_pos + LegacyVector3(0,1000,0) - camRight * w).GetData() );
        glEnd();
...
bool ResearchItem::DoesSphereHit(LegacyVector3 const &_pos, float _radius)
{
    return false;
}
...
bool ResearchItem::DoesRayHit(LegacyVector3 const &_rayStart, LegacyVector3 const &_rayDir,
