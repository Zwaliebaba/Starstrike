...
void Spider::StompFoot(LegacyVector3 const &_pos)
{
    for( int p = 0; p < 3; ++p )
    {
...
float Spider::IsPathOK(LegacyVector3 const &_dest)
{
...
bool Spider::FaceTarget()
{
	...
	if (dotProd < 0.999f)
	{
		LegacyVector3 rotation = m_front ^ toTargetNormalised;
...
bool Spider::AdvanceAttack()
{
    ...
    if( facingTarget )
    {
        ...
        else
        {
            ...
            LegacyVector3 up = ( m_pounceTarget - m_pos ).Normalise();
            up.y = 0.5f;
            up.Normalise();
            m_vel = up * force;
