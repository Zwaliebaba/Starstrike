...
LegacyVector3 EntityLeg::CalcFootHomePos(float _targetHoverHeight)
{
	...
	LegacyVector3 groundNormal = g_app->m_location->m_landscape.m_normalMap->GetValue(
...
float EntityLeg::CalcFootsDesireToMove(float _targetHoverHeight)
{
	...
	LegacyVector3 deltaHoriNorm = delta; deltaHoriNorm.HorizontalAndNormalise();
...
LegacyVector3 EntityLeg::CalcKneePos(LegacyVector3 const &_footPos, LegacyVector3 const &_rootPos, LegacyVector3 const &_centrePos)
{
...
LegacyVector3 EntityLeg::GetIdealSwingingFootPos(float _fractionComplete)
{
	...
	LegacyVector3 pos = m_foot.m_lastGroundPos * fractionIncomplete +
...
void EntityLeg::Render(float _predictionTime, LegacyVector3 const &_predictedMovement)
{
...
bool EntityLeg::RenderPixelEffect(float _predictionTime, LegacyVector3 const &_predictedMovement)
{
	LegacyVector3 predictedPos = m_parent->m_pos + _predictedMovement;
	LegacyVector3 rootPos(_predictedMovement + GetLegRootPos());
	LegacyVector3 footPos;
