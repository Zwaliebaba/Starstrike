...
bool LaserTrooper::Advance(Unit *_unit)
{
    ...
    if( !m_dead && m_onGround && m_inWater < 0.0f )
    {
        ...
        targetPos.y = 0.0f;
        LegacyVector3 offset = _unit->GetOffset(Unit::FormationRectangle, m_formationIndex );
        targetPos += offset;
...
//		LegacyVector3 targetPos = _unit->GetLeadVector(&m_wayPointId, 0.0f, m_pos);
        ...
        if( !aiControlled || m_obeyUnit != 0.0f )
        {
			...
			else // otherwise we are standing on the spot
			{
				...
				{
					LegacyVector3 toMouse = g_app->m_userInput->GetMousePos3d() - m_pos;
					toMouse.HorizontalAndNormalise();
					m_angVel = (toMouse ^ m_front) * 4.0f;
				}
				else
				{
					m_angVel = (LegacyVector3(1,0,0) ^ m_front) * 4.0f;
				}
    ...
    if( !m_dead )
    {
        ...
        if( alpha > 0.5f )
        {
			...
			g_imRenderer->Begin(PRIM_QUADS);
				...
				g_imRenderer->TexCoord2f(1.0f, 0.0f);     g_imRenderer->Vertex3fv( (predictedPos + entityFront*1.5f).GetData() );
