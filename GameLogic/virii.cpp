...
ViriiUnit::ViriiUnit(int teamId, int unitId, int numEntities, LegacyVector3 const &_pos)
...
bool Virii::AdvanceToTargetPos(LegacyVector3 const &_pos)
{
    ...
    if( arrived )
    {
        ...
        for( int i = 0; i < 3; ++i )
        {
            g_app->m_particleSystem->CreateParticle( m_pos,
                                                     LegacyVector3(  syncsfrand(15.0f),
...
WorldObjectId Virii::FindNearbyEgg( LegacyVector3 const &_pos )
{
    int numFound;
...
LegacyVector3 Virii::AdvanceDeadPositionVector( int _index, LegacyVector3 const &_pos, float _time )
{
    ...
    switch( _index % 8 )
    {
        case 0 :        result += LegacyVector3( -distance, 0, 0 );                  break;
        case 1 :        result += LegacyVector3( -distance, 0, distance );           break;
        case 2 :        result += LegacyVector3( 0, 0, distance );                   break;
        case 3 :        result += LegacyVector3( distance, 0, distance );            break;
        case 4 :        result += LegacyVector3( distance, 0, 0 );                   break;
        case 5 :        result += LegacyVector3( distance, 0, -distance );           break;
        case 6 :        result += LegacyVector3( 0, 0, -distance );                  break;
        case 7 :        result += LegacyVector3( -distance, 0, -distance );          break;
    }
...
void Virii::Render( float predictionTime, int teamId, int _detail )
{
    ...
    for( int i = 0; i < lastIndex; i ++ )
    {
        ...
        LegacyVector3 const &glowDiff = prevPos.m_glowDiff;
...
void Virii::RenderLowDetail( float predictionTime, int teamId )
{
    ...
    LegacyVector3 landNormal = g_app->m_location->m_landscape.m_normalMap->GetValue( predictedPos.x, predictedPos.z );
    ...
    for( int i = 0; i < m_positionHistory.Size(); i += 1 )
    {
        ViriiHistory *history = m_positionHistory[i];
        LegacyVector3 pos = history->m_pos;
        LegacyVector3 rightAngle = prevPos.m_right * width;
...
void Virii::RenderGlow ( float predictionTime, int teamId )
{
    ...
    for( int i = 0; i < m_positionHistory.Size(); i += 1 )
    {
        ...
        LegacyVector3 diff = (prevPos.m_pos - pos).Normalise();
