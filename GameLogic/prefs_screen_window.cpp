...
class ScreenResDropDownMenu : public DropDownMenu
{
  ...
  void SelectOption(int _option) override
  {
    ...
    auto refresh = static_cast<DropDownMenu*>(m_parent->GetButton(LANGUAGEPHRASE("dialog_refreshrate")));
    refresh->Empty();
...
class FullscreenRequiredMenu : public DropDownMenu
{
  ...
  {
    auto windowed = static_cast<DropDownMenu*>(m_parent->GetButton(LANGUAGEPHRASE("dialog_windowed")));
...
void RestartWindowManagerAndRenderer()
{
  ...
  EclRegisterWindow(new MessageDialog("Restart Required", "Please restart Darwinia for the screen\n" "settings to take affect"));
#else
