...
int AI::FindNearestTarget( LegacyVector3 const &_fromPos )
{
...
bool AITarget::DoesSphereHit(LegacyVector3 const &_pos, float _radius)
{
    return false;
}
...
void AISpawnPoint::RenderAlphas( float _predictionTime )
{
    if( g_app->m_editing )
    {
        ...
        g_editorFont.DrawText3DCentre( m_pos+LegacyVector3(0,20,0), 5.0f, "Next spawn in %d seconds", (int) m_timer );
    }
}
...
bool AISpawnPoint::DoesSphereHit(LegacyVector3 const &_pos, float _radius)
{
    return false;
}
