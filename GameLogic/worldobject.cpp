...
WorldObjectId::WorldObjectId(unsigned char _teamId, int _unitId, int _index, int _uniqueId)
{
...
Light::Light()
{
  ...
  SetFront(LegacyVector3(0, 0, 1));
}

void Light::SetColor(float colour[4])
{
  m_colour[0] = colour[0];
  m_colour[1] = colour[1];
  m_colour[2] = colour[2];
...
void Light::SetFront(LegacyVector3 front)
{
  m_front[0] = front.x;
  m_front[1] = front.y;
  m_front[2] = front.z;
