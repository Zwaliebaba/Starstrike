...
bool Triffid::DoesRayHit(LegacyVector3 const &_rayStart, LegacyVector3 const &_rayDir,
            ...
            g_app->m_locationEditor->m_selectionId == m_id.GetUniqueId() )
        {
            ...
            if( m_useTrigger )
            {
                ...
                for( int i = 0; i < numSteps; ++i )
                {
                    ...
                    LegacyVector3 thisPos = triggerPos + LegacyVector3( sinf(angle)*m_triggerRadius, 0.0f,
...
bool Triffid::Advance()
{
    ...
    if( m_damage <= 0.0f )
    {
        ...
        fireSpawn += LegacyVector3( sfrand(10.0f*m_size),
                              sfrand(10.0f*m_size),
                              sfrand(10.0f*m_size) );
        ...
        fireSpawn = m_pos + LegacyVector3( sfrand(10.0f*m_size),
                                     sfrand(10.0f*m_size),
                                     sfrand(10.0f*m_size) );
        g_app->m_particleSystem->CreateParticle( fireSpawn, g_zeroVector, Particle::TypeFire, fireSize );
