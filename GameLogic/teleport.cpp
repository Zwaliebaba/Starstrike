...
Teleport::Teleport(BuildingType _type)
  : Building(_type),
    m_timeSync(0.0f),
    m_sendPeriod(1.0f),
    m_entrance(nullptr) {}
...
void Teleport::RenderAlphas(float predictionTime)
{
  ...
  //    RenderSphere( pos, 3.0f, RGBAColor(255,0,0) );
  //    RenderSphere( pos, 15.0f, RGBAColor(255,0,0) );
...
void Teleport::RenderSpirit(const LegacyVector3& _pos, int _teamId)
{
    ...
    if (!_relay)
    {
      ...
      if (oldUnit)
      {
        ...
        if (newUnitId == -1)
        {
          ...
          newUnit = g_app->m_location->m_teams[_id.GetTeamId()].NewUnit(oldUnit->m_troopType, oldUnit->m_entities.NumUsed(),
                                                                        &newUnitId, m_pos);
          ...
          if (oldUnit->m_troopType == EntityType::TypeSquadie)
          {
            // Shut down the old task
            for (int i = 0; i < g_app->m_taskManager->m_tasks.Size(); ++i)
            {
              Task* task = g_app->m_taskManager->m_tasks[i];
              if (task->m_type == GlobalResearch::TypeSquad && task->m_objId == WorldObjectId(
                oldUnit->m_teamId, oldUnit->m_unitId, -1, -1))
              {
            ...
            Task* controller = g_app->m_taskManager->GetTask(static_cast<InsertionSquad*>(newUnit)->m_controllerId);
            if (controller)
            {
