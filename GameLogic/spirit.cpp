...
bool Spirit::Advance()
{
  ...
  if (m_state == StateFloating)
  {
    ...
    if (m_eggSearchTimer <= 0.0f)
    {
      ...
      int numNeighbours;
      WorldObjectId* ids = g_app->m_location->m_entityGrid->GetNeighbours(m_pos.x, m_pos.z, VIRII_MAXSEARCHRANGE, &numNeighbours);
...
void Spirit::Render(float predictionTime)
{
  ...
  if (m_state == StateBirth)
  {
    ...
    fractionBorn = std::min(fractionBorn, 1.0f);
