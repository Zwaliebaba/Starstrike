...
RadarDish::RadarDish()
  : Teleport(BuildingType::TypeRadarDish),
    m_receiverId(-1),
    m_range(0.0f),
    m_signal(0.0f),
    m_newlyCreated(true),
    ...
    m_movementSoundsPlaying(false)
{
  ...
  Shape* radarShape = Resource::GetShapeCopy("radardish.shp", true);
  Teleport::SetShape(radarShape);
...
void RadarDish::Initialize(Building* _template)
{
...
bool RadarDish::Advance()
{
  ...
  if (m_newlyCreated)
  {
    ...
    if (gb)
    {
      ...
      if (targetBuilding && targetBuilding->m_buildingType == BuildingType::TypeRadarDish)
      {
  ...
  if (!m_horizontallyAligned)
  {
    ...
    currentFront.HorizontalAndNormalize();
