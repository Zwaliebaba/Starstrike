...
bool Darwinian::AdvanceWatchingSpectacle()
{
    ...
    targetPos += LegacyVector3( sinf(g_gameTime) * 30.0f,
...
bool Darwinian::SearchForRandomPosition()
{
    ...
    if( syncfrand() < 0.7f )
    {
        ...
        m_wayPoint = m_pos + LegacyVector3( sinf(angle) * distance,
                                       0.0f,
                                       cosf(angle) * distance );
...
void Darwinian::GiveOrders( LegacyVector3 const &_targetPos )
{
    m_orders = _targetPos;
    m_ordersBuildingId = -1;
    m_ordersSet = true;
...
LegacyVector3 Darwinian::PushFromObstructions( LegacyVector3 const &pos, bool killem )
{
