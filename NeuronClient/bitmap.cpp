...
void BitmapRGBA::Write24BitLine(FILE* _out, int _y)
{
  ...
  {
    const RGBAColor& c = GetPixel(x, _y);
    fputc(c.b, _out);
    fputc(c.g, _out);
    fputc(c.r, _out);
    nbytes += 3;
...
// *** PutPixelClipped
void BitmapRGBA::PutPixelClipped(int x, int y, const RGBAColor& colour)
{
...
void BitmapRGBA::DrawLine(int x1, int y1, int x2, int y2, const RGBAColor& colour)
{
...
void BitmapRGBA::ApplyDilateFilter()
{
  ...
  memcpy(temp, m_pixels, sizeof(RGBAColor) * m_width * m_height);
    ...
    for (int y = 1; y < m_height - 1; ++y)
    {
      ...
      for (int i = -1; i <= 1; ++i)
      {
        for (int j = -1; j <= 1; ++j)
        {
          if (i != 0 || j != 0)
          {
            RGBAColor col = temp[(y + j) * m_width + (x + i)];
            adjacentRed += col.r;
            adjacentGreen += col.g;
            adjacentBlue += col.b;
          }
...
void BitmapRGBA::ConvertColorToAlpha()
{
  RGBAColor col;
  RGBAColor newCol(255, 255, 255, 0);
