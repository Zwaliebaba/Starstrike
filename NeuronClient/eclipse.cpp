...
void EclRegisterWindow(EclWindow* window, EclWindow* parent)
{
  ...
  if (parent && window->m_x == 0 && window->m_y == 0)
  {
    ...
    if (left > window->m_w / 2)
      window->m_x = static_cast<int>(parent->m_x + parent->m_w * (float)darwiniaRandom() / (float)DARWINIA_RAND_MAX);
    else
      window->m_x = static_cast<int>(parent->m_x - window->m_w * (float)darwiniaRandom() / (float)DARWINIA_RAND_MAX);
    if (above > window->m_h / 2)
      window->m_y = static_cast<int>(parent->m_y + parent->m_h * (float)darwiniaRandom() / (float)DARWINIA_RAND_MAX);
    else
      window->m_y = static_cast<int>(parent->m_y - window->m_h / 2 * (float)darwiniaRandom() / (float)DARWINIA_RAND_MAX);
  }
...
int EclGetAccurateTime()
{
  ...
  static bool initted = false; static struct timeval startTime; struct timeval tv; if (!initted)
  {
    initted = true;
    gettimeofday(&startTime, NULL);
    return 0;
  } gettimeofday(&tv, NULL); long diff_usec = tv.tv_usec - startTime.tv_usec; if (diff_usec < 0)
  {
    diff_usec += 1000000;
    tv.tv_sec--;
  } long diff_sec = tv.tv_sec - startTime.tv_sec; return 1000 * diff_sec + diff_usec / 1000;
