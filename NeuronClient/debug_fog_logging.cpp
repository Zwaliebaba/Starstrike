...
bool g_fogLogging = false;static const char* GLCapToString(GLenum _cap)
{
  switch (_cap)
  {
  case GL_FOG:
...
{
  public:
    DarwiniaGLEnableLogMsg(const char* _filename, int _line, const char* _function, bool _enable, GLenum _cap)
      : DarwiniaLogMsg(_filename, _line, _function),
        m_enable(_enable),
        m_cap(_cap) {}

  protected:
    void DisplayCall() { printf("gl%sable(%s)", m_enable ? "En" : "Dis", GLCapToString(m_cap)); }

  private:
    bool m_enable;
    GLenum m_cap;
};template <class T, bool t_isFloat>
inline void DisplayFogValue(GLenum _param, T _value)
{
...
{
  public:
    DarwiniaGLFogLogMsg(const char* _filename, int _line, const char* _function, GLenum _param, T _value)
      : DarwiniaLogMsg(_filename, _line, _function),
        m_param(_param),
        m_value(_value) {}
...
{
  public:
    DarwiniaGLFogvLogMsg(const char* _filename, int _line, const char* _function, GLenum _param, const T* _value)
      : DarwiniaLogMsg(_filename, _line, _function),
        m_param(_param)
    {
      for (int i = 0; i < 4; i++)
