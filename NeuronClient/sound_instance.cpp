...
void DspHandle::Advance()
{
  ...
  for (int i = 0; i < MAX_PARAMS; ++i)
  {
    m_parent->UpdateParameter(m_params[i]);
    int dataType;
    g_context->m_soundSystem->m_filterBlueprints[m_type]->GetParameter(i, nullptr, nullptr, nullptr, &dataType);
    switch (dataType)
    {
    case 0:
...
void SoundInstance::SetSoundName(const char* _name)
{
  if (_name)
  {
    strncpy(m_soundName, _name, sizeof(m_soundName));
...
void SoundInstance::SetEventName(const char* _entityName, const char* _eventName)
{
  ...
  m_eventName = static_cast<char*>(malloc(entityLen + eventLen + 2));
  ...
  if (m_adsrState != StateRelease)
  {
    ...
    if (m_loopType == LoopedADSR && !_final)
    {
      if (NearlyEquals(m_loopDelay.GetOutput(), 0.0f))
        g_context->m_soundSystem->TriggerDuplicateSound(this);
      else
...
void SoundInstance::OpenStream(bool _keepCurrentStream)
{
  ...
  const char* sampleName = m_soundName;
  if (m_sourceType == SampleGroupRandom)
  {
    SampleGroup* group = g_context->m_soundSystem->GetSampleGroup(m_soundName);
...
bool SoundInstance::StartPlaying(int _channelIndex)
{
  ...
  START_PROFILE(g_context->m_profiler, "StartPlaying");

  //
  // If we don't have our stream yet, load it now

  START_PROFILE(g_context->m_profiler, "OpenStream");
  OpenStream(false);
  END_PROFILE(g_context->m_profiler, "OpenStream");

  //
  // Set up our parameters

  START_PROFILE(g_context->m_profiler, "Set Parameters");
  ...
  g_soundLibrary3d->SetChannelMinDistance(m_channelIndex, m_minDistance);
  END_PROFILE(g_context->m_profiler, "Set Parameters");

  START_PROFILE(g_context->m_profiler, "Set freq/vol/pos");
  UpdateParameter(m_freq);
  ...
  END_PROFILE(g_context->m_profiler, "Set freq/vol/pos");

  //
  // Start our DSP effects

  START_PROFILE(g_context->m_profiler, "Setup DSP Effects");
  int filterTypes[16];
  ...
  END_PROFILE(g_context->m_profiler, "Setup DSP Effects");

  END_PROFILE(g_context->m_profiler, "StartPlaying");
  return true;
}
...
bool SoundInstance::Update3DPosition()
{
  ...
  case TypeInEditor:
    {
      LegacyVector3 relativePos(g_context->m_soundSystem->m_editorPos.x, 0.0f, g_context->m_soundSystem->m_editorPos.z);
      m_pos = relativePos;
      m_vel.Zero();
...
void SoundInstance::ForceParameter(SoundParameter& _param, float value)
{
  ...
  case SoundParameter::LinkedToHeightAboveGround:
    {
      ...
      if (g_context->m_location)
        landHeight = g_context->m_location->m_landscape.m_heightMap->GetValue(m_pos.x, m_pos.z);
  ...
  case SoundParameter::LinkedToCameraDistance:
    {
      if (g_context->m_camera) { m_pos = g_context->m_camera->GetPos() + g_context->m_camera->GetFront() * value; }
      break;
    }
  }
}
...
bool SoundInstance::UpdateParameter(SoundParameter& _param)
{
  ...
  {
    pos = g_context->m_camera->GetPos();
    vel = g_context->m_camera->GetVel();
  }
  ...
  if (_param.m_type == SoundParameter::TypeLinked)
  {
    ...
    case SoundParameter::LinkedToHeightAboveGround:
      {
        float landHeight = 0.0f;
        if (g_context->m_location)
          landHeight = g_context->m_location->m_landscape.m_heightMap->GetValue(pos.x, pos.z);
    ...
    case SoundParameter::LinkedToCameraDistance:
      {
        float camDist = 0.0f;
        if (g_context->m_camera)
          camDist = (g_context->m_camera->GetPos() - pos).Mag();
...
void SoundInstance::StopPlaying()
{
  ...
  for (int i = 0; i < g_context->m_soundSystem->m_numChannels; ++i)
  {
    SoundInstanceId soundId = g_context->m_soundSystem->m_channels[i];
    if (soundId == m_id)
      g_context->m_soundSystem->m_channels[i].SetInvalid();
  }
}
...
int SoundInstance::GetChannelIndex()
{
  ...
  if (m_channelIndex >= 0 && m_channelIndex < g_context->m_soundSystem->m_numChannels && g_context->m_soundSystem->m_channels[
    m_channelIndex] == m_id)
...
void SoundInstance::CalculatePerceivedVolume()
{
  ...
  {
    float distance = (m_pos - g_context->m_camera->GetPos()).Mag();
...
WorldObject* SoundInstance::GetAttachedObject()
{
  ...
  if (g_context->m_locationId != -1)
  {
    ...
    if (!g_context->m_location->GetWorldObject(m_objId))
    ...
    obj = g_context->m_location->GetWorldObject(m_objId);
  }
