...
float SoundParameter::GetSmooth() { return sqrtf(m_smooth); }

const char* SoundParameter::GetParameterTypeName(int _type)
{
  const char* names[] = {"TypeFixedValue", "TypeRangedRandom", "TypeLinked"};
...
{
  const char* names[] = {"Nothing", "HeightAboveGround", "Xpos", "Ypos", "Zpos", "Velocity", "CameraDistance"};
...
{
  const char* names[] = {"UpdateConstantly", "UpdateOncePerLoop"};
