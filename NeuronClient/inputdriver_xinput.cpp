...
static ControlAction s_controls[] = {
  // Name                  Type
  "lefttrigger", INPUT_TYPE_1D, "righttrigger", INPUT_TYPE_1D, "leftbumper", INPUT_TYPE_BOOL, "rightbumper", INPUT_TYPE_BOOL,
  "leftthumbstick", INPUT_TYPE_2D | INPUT_TYPE_BOOL, "rightthumbstick", INPUT_TYPE_2D | INPUT_TYPE_BOOL, "dpadleft", INPUT_TYPE_BOOL,
  "dpadright", INPUT_TYPE_BOOL, "dpadup", INPUT_TYPE_BOOL, "dpaddown", INPUT_TYPE_BOOL, "a", INPUT_TYPE_BOOL, "b", INPUT_TYPE_BOOL, "x",
  INPUT_TYPE_BOOL, "y", INPUT_TYPE_BOOL, "start", INPUT_TYPE_BOOL, "back", INPUT_TYPE_BOOL, "plug", INPUT_TYPE_BOOL, "leftthumbstickleft",
  INPUT_TYPE_BOOL, "leftthumbstickright", INPUT_TYPE_BOOL, "leftthumbstickup", INPUT_TYPE_BOOL, "leftthumbstickdown", INPUT_TYPE_BOOL,
  "rightthumbstickleft", INPUT_TYPE_BOOL, "rightthumbstickright", INPUT_TYPE_BOOL, "rightthumbstickup", INPUT_TYPE_BOOL,
  "rightthumbstickdown", INPUT_TYPE_BOOL, nullptr, NULL
};
...
bool XInputDriver::getInput(const InputSpec& spec, InputDetails& details)
{
  ...
  case XInputLeftThumbstick:
    {
      if (COND_UP == spec.condition || COND_DOWN == spec.condition || COND_PRESSED == spec.condition || COND_REPEAT == spec.condition)
  ...
  case XInputPlug:
    ...
    if (COND_DOWN == spec.condition)
      return (m_state.isConnected && !m_oldState.isConnected);
    if (COND_PRESSED == spec.condition)
...
InputMode XInputDriver::getInputMode() { return INPUT_MODE_GAMEPAD; }

bool XInputDriver::getButtonInput(WORD button, const InputSpec& spec, InputDetails& details)
{
  bool val, oldVal, rep;
  const XINPUT_GAMEPAD& gp = m_state.state.Gamepad;
  const XINPUT_GAMEPAD& oldGP = m_oldState.state.Gamepad;
  double now;
  ...
  case COND_REPEAT:
    now = GetHighResTime();
    rep = val && (!oldVal || (s_nextrepeat[spec.control_id] <= now));
    if (rep)
...
bool XInputDriver::getThumbInput(const InputSpec& spec, InputDetails& details)
{
  SHORT x, y, oldX, oldY;
  const XINPUT_GAMEPAD& gp = m_state.state.Gamepad;
  const XINPUT_GAMEPAD& oldGP = m_oldState.state.Gamepad;
...
bool XInputDriver::getTriggerInput(const InputSpec& spec, InputDetails& details)
{
  BYTE val, oldVal;
  const XINPUT_GAMEPAD& gp = m_state.state.Gamepad;
  const XINPUT_GAMEPAD& oldGP = m_oldState.state.Gamepad;
  ...
  case XInputLeftTrigger:
    val = gp.bLeftTrigger;
    oldVal = oldGP.bLeftTrigger;
    break;
  case XInputRightTrigger:
    val = gp.bRightTrigger;
    oldVal = oldGP.bRightTrigger;
    break;
  default:
    return false; // We should never get here!
  }
...
bool XInputDriver::getThumbDirection(control_id_t control, const InputSpec& spec, unsigned direction, InputDetails& details)
{
  ...
  case COND_REPEAT:
    now = GetHighResTime();
    rep = isPressed && (!wasPressed || (s_nextrepeat[control] <= now));
    if (rep)
...
bool XInputDriver::acceptDriver(const std::string& name)
{
  m_optTokenState = BEFORE_OPTIONAL_TOKENS;
  return (name == "XInput");
}

control_id_t XInputDriver::getControlID(const std::string& name)
{
  int i = 0;
  const char* cmd = s_controls[i].name;
  while (cmd && name != cmd) { cmd = s_controls[++i].name; }
  if (i < XInputNumControls)
    return i;
  return -1;
}

inputtype_t XInputDriver::getControlType(control_id_t control_id) { return s_controls[control_id].type; }

condition_t XInputDriver::getConditionID(const std::string& name, inputtype_t& type)
{
...
InputParserState XInputDriver::parseExtraToken(const std::string& token, InputSpec& spec)
{
...
bool XInputDriver::getInputDescription(const InputSpec& spec, InputDescription& desc)
{
