...
void glCopyTexImage2D(GLenum target, GLint level, GLenum internalFormat, GLint x, GLint y, GLsizei width, GLsizei height,
                      GLint border)
{
  GL_TRACE_IMP(" glCopyTexImage2D(%s, %d, %s, %d, %d, %d, %d, %d)", glEnumToString(target), level, glEnumToString(internalFormat),
  ...
  if (pTexture == nullptr)
  {
    HRESULT hr = g_pd3dDeviceActual->CreateTexture(width, height, 1, D3DUSAGE_RENDERTARGET, g_d3dpp.BackBufferFormat,
...
void glReadPixels(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLvoid* pixels)
{
  ...
  if (LegacyTexture* screenTexture = LegacyTexture::Create(TextureParams(width, height, D3DFMT_A8R8G8B8, TF_RENDERTARGET)))
  {
...
{
  GL_TRACE_IMP(" gluBuild2DMipmaps(%s, %d, %d, %d, %s, %s, %p)", glEnumToString(target), components, width, height,
...
{
  GL_TRACE_IMP(" glTexImage2D(%s, %d, %d, %d, %d, %d, %s, %s, %p)", glEnumToString(target), level, internalformat, width, height,
...
void glLightfv(GLenum light, GLenum pname, const GLfloat* params)
{
  ...
  switch (pname)
  {
    ...
    default: GL_TRACE("-glLightfv(%s, %s, (const float *)%p) not implemented", glEnumToString(light), glEnumToString(pname),
                      params);
      break;
  }
