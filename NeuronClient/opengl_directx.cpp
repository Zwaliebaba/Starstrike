...
namespace OpenGLD3D
{
  struct TextureState
  {
    ...
    TextureState()
      : target(0),
        lastTarget(static_cast<GLuint>(-1)),
        envMode(GL_MODULATE),
        lastEnvMode(0),
        rgbCombineMode(0) {}
  };
...
void OpenGLD3D::PrepareDrawState(D3D_PRIMITIVE_TOPOLOGY topology)
{
  ...
  {
    scissor = {
      static_cast<LONG>(s_renderState.scissorX), static_cast<LONG>(s_renderState.scissorY),
      static_cast<LONG>(s_renderState.scissorX + s_renderState.scissorW), static_cast<LONG>(s_renderState.scissorY + s_renderState.scissorH)
    };
  }
  else
...
void glClearColor(GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha)
{
  ...
  s_clearColor = (static_cast<UINT32>(alpha * 255) << 24) | (static_cast<UINT32>(red * 255) << 16) | (static_cast<UINT32>(green * 255) << 8)
...
void glClear(GLbitfield mask)
{
  ...
  {
    float clearColor[4] = {
      ((s_clearColor >> 16) & 0xFF) / 255.0f, ((s_clearColor >> 8) & 0xFF) / 255.0f, (s_clearColor & 0xFF) / 255.0f,
...
void glMatrixMode(GLenum mode)
{
  ...
  case GL_MODELVIEW:
    ...
    break;
  case GL_PROJECTION:
    s_pTargetMatrixStack = &s_projectionMatrixStack;
    break;
  }
}

void glLoadIdentity() { s_pTargetMatrixStack->LoadIdentity(); }

void gluOrtho2D(GLdouble left, GLdouble right, GLdouble bottom, GLdouble top)
{
  XMMATRIX m = XMMatrixOrthographicOffCenterRH(static_cast<float>(left), static_cast<float>(right), static_cast<float>(bottom),
                                               static_cast<float>(top), -1.0f, 1.0f);
...
{
  XMMATRIX m = XMMatrixPerspectiveFovRH(XMConvertToRadians(static_cast<float>(fovy)), static_cast<float>(aspect), static_cast<float>(zNear),
...
void glPushMatrix() { s_pTargetMatrixStack->Push(); }

void glPopMatrix() { s_pTargetMatrixStack->Pop(); }
...
{
  XMMATRIX m = XMMatrixPerspectiveOffCenterRH(static_cast<float>(left), static_cast<float>(right), static_cast<float>(bottom),
...
{
  /* No-op in D3D12 — matrices are handled via constant buffer */
}
...
void glEnd()
{
  ...
  case GL_TRIANGLE_FAN:
    // D3D12 has no triangle fan — decompose to triangle list
    {
      ...
      // Ensure space
      while (newVertCount >= s_allocatedVertices) { reallocateVertexList(); }
...
void glDisable(GLenum cap)
{
  ...
  {
  case GL_ALPHA_TEST:
    s_renderState.alphaTestEnabled = false;
    break;
  case GL_BLEND:
    s_renderState.blendEnabled = false;
    break;
  ...
  case GL_CULL_FACE:
    ...
    break;
  case GL_DEPTH_TEST:
    s_renderState.depthTestEnabled = false;
    break;
  case GL_FOG:
    s_renderState.fogEnabled = false;
    break;
  case GL_LIGHTING:
    s_renderState.lightingEnabled = false;
    break;
  case GL_LIGHT0:
  case GL_LIGHT1:
  case GL_LIGHT2:
  ...
  case GL_LIGHT6:
  case GL_LIGHT7:
    s_renderState.lightsEnabled[cap - GL_LIGHT0] = false;
    break;
  case GL_LINE_SMOOTH:
    s_renderState.lineSmoothEnabled = false;
    break;
  case GL_NORMALIZE:
    s_renderState.normalizeEnabled = false;
    break;
  case GL_SCISSOR_TEST:
    s_renderState.scissorTestEnabled = false;
    break;
...
void glEnable(GLenum cap)
{
  ...
  {
  case GL_ALPHA_TEST:
    s_renderState.alphaTestEnabled = true;
    break;
  case GL_BLEND:
    s_renderState.blendEnabled = true;
    break;
  case GL_COLOR_MATERIAL:
  ...
  case GL_CULL_FACE:
    ...
    break;
  case GL_DEPTH_TEST:
    s_renderState.depthTestEnabled = true;
    break;
  case GL_FOG:
    s_renderState.fogEnabled = true;
    break;
  case GL_LIGHTING:
    s_renderState.lightingEnabled = true;
    break;
  case GL_LIGHT0:
  case GL_LIGHT1:
  case GL_LIGHT2:
  ...
  case GL_LIGHT6:
  case GL_LIGHT7:
    s_renderState.lightsEnabled[cap - GL_LIGHT0] = true;
    break;
  case GL_LINE_SMOOTH:
    s_renderState.lineSmoothEnabled = true;
    break;
  case GL_NORMALIZE:
    s_renderState.normalizeEnabled = true;
    break;
  case GL_SCISSOR_TEST:
    s_renderState.scissorTestEnabled = true;
    break;
  case GL_TEXTURE_2D:
...
GLboolean glIsEnabled(GLenum cap)
{
  ...
  {
  case GL_ALPHA_TEST:
    return s_renderState.alphaTestEnabled;
  case GL_BLEND:
    return s_renderState.blendEnabled;
  case GL_COLOR_MATERIAL:
    return s_currentAttribs.colorMaterialEnabled;
  case GL_CULL_FACE:
    return s_currentAttribs.backCullingEnabled;
  case GL_DEPTH_TEST:
    return s_renderState.depthTestEnabled;
  case GL_FOG:
    return s_renderState.fogEnabled;
  case GL_LIGHTING:
    return s_renderState.lightingEnabled;
  case GL_LIGHT0:
  case GL_LIGHT1:
  case GL_LIGHT2:
  case GL_LIGHT3:
  ...
  case GL_LINE_SMOOTH:
    return s_renderState.lineSmoothEnabled;
  case GL_NORMALIZE:
    return s_renderState.normalizeEnabled;
  case GL_POINT_SMOOTH:
    return FALSE;
  case GL_SCISSOR_TEST:
    return s_renderState.scissorTestEnabled;
  case GL_TEXTURE_2D:
...
void glDepthMask(GLboolean flag) { s_renderState.depthWriteEnabled = flag != 0; }

void glDepthFunc(GLenum func) { s_renderState.depthFunc = func; }
...
void glShadeModel(GLenum mode) { s_renderState.shadeModel = mode; }

void glPolygonMode(GLenum face, GLenum mode) { s_renderState.fillMode = mode; }

void glFrontFace(GLenum mode) { s_ccwFrontFace = (mode == GL_CCW); }

void glColorMaterial(GLenum face, GLenum mode) { s_currentAttribs.colorMaterialMode = mode; }
...
void glGetLightfv(GLenum light, GLenum pname, GLfloat* params)
{
  ...
  {
  case GL_POSITION:
    memcpy(params, l.position, 4 * sizeof(float));
    break;
  case GL_DIFFUSE:
    memcpy(params, l.diffuse, 4 * sizeof(float));
    break;
  case GL_SPECULAR:
    memcpy(params, l.specular, 4 * sizeof(float));
    break;
  case GL_AMBIENT:
    memcpy(params, l.ambient, 4 * sizeof(float));
    break;
  default: DEBUG_ASSERT(false);
    break;
  }
...
void glMaterialfv(GLenum face, GLenum pname, const GLfloat* params)
{
  ...
  {
  case GL_SPECULAR:
    memcpy(s_renderState.matSpecular, params, 4 * sizeof(float));
    break;
  case GL_DIFFUSE:
  ...
  case GL_AMBIENT_AND_DIFFUSE:
    ...
    break;
  case GL_SHININESS:
    s_renderState.matShininess = params[0];
    break;
  default: DEBUG_ASSERT(FALSE);
    break;
  }
...
void glGetMaterialfv(GLenum face, GLenum pname, GLfloat* params)
{
  ...
  {
  case GL_SPECULAR:
    memcpy(params, s_renderState.matSpecular, 4 * sizeof(float));
    break;
  case GL_DIFFUSE:
    memcpy(params, s_renderState.matDiffuse, 4 * sizeof(float));
    break;
  case GL_AMBIENT:
    memcpy(params, s_renderState.matAmbient, 4 * sizeof(float));
    break;
  case GL_AMBIENT_AND_DIFFUSE:
    memcpy(params, s_renderState.matAmbient, 4 * sizeof(float));
    break;
  case GL_SHININESS:
    params[0] = s_renderState.matShininess;
    break;
  default:
    break;
  }
...
void glFogf_impl(GLenum pname, GLfloat param)
{
  ...
  {
  case GL_FOG_DENSITY:
    s_renderState.fogDensity = param;
    break;
  case GL_FOG_END:
    s_renderState.fogEnd = param;
    break;
  case GL_FOG_START:
    s_renderState.fogStart = param;
    break;
  case GL_FOG_MODE:
    break; // handled by glFogi
  default: DEBUG_ASSERT(FALSE);
    break;
  }
}
...
int gluBuild2DMipmaps(GLenum target, GLint components, GLint width, GLint height, GLenum format, GLenum type, const void* data)
{
  ...
  if (texIdx >= s_textureResources.size())
    return -1;
  ...
  HRESULT hr = device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &texDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
  ...
  auto destData = static_cast<UINT8*>(alloc.cpuPtr);
...
void glTexImage2D(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type,
                  const GLvoid* pixels) { gluBuild2DMipmaps(target, 4, width, height, format, type, pixels); }
...
void glCopyTexImage2D(GLenum target, GLint level, GLenum internalFormat, GLint x, GLint y, GLsizei width, GLsizei height, GLint border)
{
  ...
  texDesc.DepthOrArraySize = 1;
  texDesc.MipLevels = 1;
  texDesc.Format = Graphics::Core::Get().GetBackBufferFormat();
  texDesc.SampleDesc.Count = 1;
  ...
  auto& renderTarget = Graphics::Core::Get().GetRenderTarget();
...
void glTexEnvf(GLenum target, GLenum pname, GLfloat param)
{
  if (target == GL_TEXTURE_ENV)
  {
    ...
    {
      s_activeTextureState->envMode = static_cast<GLenum>(param);
      return;
    }
    if (pname == GL_COMBINE_RGB_EXT) { s_activeTextureState->rgbCombineMode = static_cast<GLenum>(param); }
  }
}
...
void glGetTexParameteriv(GLenum target, GLenum pname, GLint* params)
{
  ...
  {
  case GL_TEXTURE_MAG_FILTER:
    params[0] = s.magFilter;
    break;
  case GL_TEXTURE_MIN_FILTER:
    params[0] = s.minFilter;
    break;
  case GL_TEXTURE_WRAP_S:
    params[0] = s.wrapS;
    break;
...
void glEnableClientState(GLenum array)
{
  ...
  case GL_VERTEX_ARRAY:
    s_clientStateVertex = true;
    break;
  case GL_NORMAL_ARRAY:
    s_clientStateNormal = true;
    break;
  case GL_COLOR_ARRAY:
    s_clientStateColor = true;
    break;
  case GL_TEXTURE_COORD_ARRAY:
    s_clientStateTexCoord = true;
    break;
  }
}
...
void glDisableClientState(GLenum array)
{
  ...
  {
  case GL_VERTEX_ARRAY:
    s_clientStateVertex = false;
    break;
  case GL_NORMAL_ARRAY:
    s_clientStateNormal = false;
    break;
  case GL_COLOR_ARRAY:
    s_clientStateColor = false;
    break;
  case GL_TEXTURE_COORD_ARRAY:
    s_clientStateTexCoord = false;
    break;
  }
}
...
void glNewList(GLuint list, GLenum mode) { s_pDisplayListRecorder = new DisplayListRecorder(list); }
...
void glGetIntegerv(GLenum pname, GLint* params)
{
  ...
  case GL_ALPHA_TEST_FUNC:
    params[0] = s_renderState.alphaFunc;
    break;
  case GL_BLEND_DST:
    params[0] = s_renderState.blendDst;
    break;
  case GL_BLEND_SRC:
    params[0] = s_renderState.blendSrc;
    break;
  case GL_DEPTH_FUNC:
    params[0] = s_renderState.depthFunc;
    break;
  case GL_DEPTH_WRITEMASK:
    params[0] = s_renderState.depthWriteEnabled ? 1 : 0;
    break;
  ...
  case GL_FOG_HINT:
    ...
    break;
  case GL_FOG_MODE:
    params[0] = s_renderState.fogMode;
    break;
  case GL_FRONT_FACE:
  ...
  case GL_POLYGON_MODE:
    params[0] = params[1] = s_renderState.fillMode;
    break;
  case GL_POLYGON_SMOOTH_HINT:
    params[0] = s_polygonSmoothHint;
    break;
  case GL_SHADE_MODEL:
    params[0] = s_renderState.shadeModel;
    break;
  case GL_VIEWPORT:
...
void glGetFloatv(GLenum pname, GLfloat* params)
{
  ...
  case GL_CURRENT_COLOR:
    ...
    break;
  case GL_FOG_COLOR:
    memcpy(params, s_renderState.fogColor, 4 * sizeof(float));
    break;
  case GL_FOG_DENSITY:
    params[0] = s_renderState.fogDensity;
    break;
  case GL_FOG_END:
    params[0] = s_renderState.fogEnd;
    break;
  case GL_LIGHT_MODEL_AMBIENT:
    memcpy(params, s_renderState.globalAmbient, 4 * sizeof(float));
    break;
  }
}
               ...
               const GLint viewport[4], GLdouble* winx, GLdouble* winy, GLdouble* winz)
{
  ...
  for (int i = 0; i < 16; i++)
  {
    reinterpret_cast<float*>(&model4)[i] = static_cast<float>(modelMatrix[i]);
    reinterpret_cast<float*>(&proj4)[i] = static_cast<float>(projMatrix[i]);
  }
                 ...
                 const GLint viewport[4], GLdouble* objx, GLdouble* objy, GLdouble* objz)
{
  ...
  XMVECTOR win = XMVectorSet(static_cast<float>((winx - viewport[0]) / viewport[2] * 2 - 1),
                             static_cast<float>((winy - viewport[1]) / viewport[3] * 2 - 1), static_cast<float>(winz * 2 - 1), 1.0f);

  XMVECTOR source = XMVector4Transform(win, inv);
  XMFLOAT4 s;
  XMStoreFloat4(&s, source);
  if (s.w == 0.0f)
    return false;
  *objx = s.x / s.w;
  *objy = s.y / s.w;
