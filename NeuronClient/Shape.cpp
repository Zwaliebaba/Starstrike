    ...
    m_mostNegativeY(0.0f)
{
  ...
  while (_in->ReadLine())
  {
    ...
    if (_stricmp(firstWord, "ParentName") == 0) { m_parentName = _strdup(secondWord); }
    else if (_stricmp(firstWord, "front") == 0)
    {
      m_transform.f.x = static_cast<float>(atof(secondWord));
      m_transform.f.y = static_cast<float>(atof(_in->GetNextToken()));
...
ShapeFragment::~ShapeFragment()
{
  ...
  Resource::DeleteDisplayList(m_displayListName);
...
void ShapeFragment::BuildDisplayList()
{
  ...
  m_displayListName = Resource::GenerateName();
  int id = Resource::CreateDisplayList(m_displayListName);
...
void ShapeFragment::WriteToFile(FILE* _out) const
{
  ...
  if (_stricmp(m_name, "SceneRoot") != 0)
  {
    ...
    fprintf(_out, "\tColors: %d\n", m_numColors);
    for (i = 0; i < m_numColors; i++)
    {
      const RGBAColor& c = m_colours[i];
      fprintf(_out, "\t\t%d: %d %d %d\n", i, c.r, c.g, c.b);
    }

    // Write out the vertices
    fprintf(_out, "\tVertices: %d    # Position ID then Color ID\n", m_numVertices);
  ...
  for (i = 0; i < m_childFragments.Size(); ++i) { m_childFragments.GetData(i)->WriteToFile(_out); }

  // Write out all the child markers
  for (i = 0; i < m_childMarkers.Size(); ++i) { m_childMarkers.GetData(i)->WriteToFile(_out); }
}
...
void ShapeFragment::RegisterPositions(LegacyVector3* _positions, unsigned int _numPositions)
{
  ...
  // Calculate bounding sphere
  {
    ...
    for (i = 0; i < m_numPositions; ++i)
    {
      ...
      if (magSquared > radiusSquared) { radiusSquared = magSquared; }
    }
  ...
  // Calculate bounding vertical cylinder
  {
    ...
    for (i = 0; i < m_numPositions; ++i)
    {
      if (m_positions[i].y > m_mostPositiveY) { m_mostPositiveY = m_positions[i].y; }
      if (m_positions[i].y < m_mostNegativeY) { m_mostNegativeY = m_positions[i].y; }
      LegacyVector3 delta = m_center - m_positions[i];
      delta.y = 0.0f;
      float magSquared = delta.MagSquared();
      if (magSquared > radiusSquared) { radiusSquared = magSquared; }
...
void ShapeFragment::RegisterColors(RGBAColor* _colours, unsigned int _numColors)
{
  delete [] m_colours;
  m_colours = _colours;
  m_numColors = _numColors;
...
void ShapeFragment::Render(float _predictionTime)
{
  ...
  if (m_displayListName)
    id = Resource::GetDisplayList(m_displayListName);
  if (id != -1) { glCallList(id); }
  else
#endif
  {
    RenderSlow();
...
{
  if (_stricmp(_name, m_name) == 0) { return this; }
...
ShapeMarker* ShapeFragment::LookupMarker(const char* _name)
{
  ...
  for (i = 0; i < numMarkers; ++i)
  {
    ...
    if (_stricmp(_name, marker->m_name) == 0) { return marker; }
  }
...
void ShapeFragment::CalculateRadius(const Matrix34& _transform, const LegacyVector3& _center, float& _radius)
{
  ...
  if (distance + m_radius > _radius) { _radius = distance + m_radius; }
...
void Shape::Load(TextReader* _in)
{
  ...
  {
    if (_stricmp(allFrags[i]->m_parentName, "SceneRoot") == 0) { m_rootFragment->m_childFragments.PutData(allFrags[i]); }
    else
    {
