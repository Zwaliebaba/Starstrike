...
constexpr double DEFAULT_TRACK_AGE = 10; // 10 seconds
constexpr double SENSOR_THRESHOLD = 0.25;
...
Contact::Contact()
  : ship(nullptr),
    ...
    d_pas(0.0f),
    d_act(0.0f),
    probe(false) { acquire_time = Game::GameTime(); }
...
{
  acquire_time = Game::GameTime();
  SimObserver::Observe(shot);
}

Contact::~Contact() { delete [] track; }
...
bool Contact::Update(SimObject* obj)
{
...
std::string Contact::GetObserverName() const
{
  static char name[128];

  if (ship)
    sprintf_s(name, "Contact Ship='%s'", ship->Name().c_str());
  else if (shot)
    sprintf_s(name, "Contact Shot='%s' %s", shot->Name().c_str(), shot->DesignName().c_str());
  else
...
bool Contact::ActLock() const { return d_act >= SENSOR_THRESHOLD; }

bool Contact::PasLock() const { return d_pas >= SENSOR_THRESHOLD; }

void Contact::GetBearing(const Ship* observer, double& az, double& el, double& rng) const
{
  // translate:
...
double Contact::Range(const Ship* observer, double limit) const
{
  ...
  if (!ActLock())
  {
    constexpr int chunk = 25000;

    if (!PasLock())
      r = static_cast<int>(limit);
    ...
    {
      int r1 = static_cast<int>(r + chunk / 2) / chunk;
      r = r1 * chunk;
    }
  }
...
bool Contact::Threat(const Ship* observer) const
{
  ...
  if (observer && observer->Life() != 0)
  {
    if (ship && ship->Life() != 0)
    {
      threat = (ship->GetIFF() && ship->GetIFF() != observer->GetIFF() && ship->GetEMCON() > 2 && ship->
        IsTracking((Ship*)observer) && ship->Weapons().size() > 0);
...
void Contact::Reset()
{
  ...
  float step_down = static_cast<float>(Game::FrameTime() / 10);
