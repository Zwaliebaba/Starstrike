...
MsnEditNavDlg::MsnEditNavDlg(Screen* s, FormDef& def, MenuScreen* mgr)
  : NavDlg(s, def, mgr),
    ...
    btn_pkg(nullptr),
    btn_map(nullptr),
    mission_info(nullptr) { RegisterControls(); }

MsnEditNavDlg::~MsnEditNavDlg() {}

void MsnEditNavDlg::RegisterControls()
{
...
void MsnEditNavDlg::SetMissionInfo(MissionInfo* m) { mission_info = m; }

void MsnEditNavDlg::ScrapeForm()
{
  if (mission)
  {
    if (txt_name)
...
void MsnEditNavDlg::ShowTab(int tab)
{
  if (tab < 0 || tab > 2)
    tab = 0;
...
void MsnEditNavDlg::OnTabButton(AWEvent* event)
{
  if (!event)
    return;
...
void MsnEditNavDlg::OnSystemSelect(AWEvent* event)
{
...
void MsnEditNavDlg::OnCommit(AWEvent* event)
{
  if (mission)
  {
    ScrapeForm();
...
void MsnEditNavDlg::OnCancel(AWEvent* event)
{
  if (mission)
    mission->Load();
