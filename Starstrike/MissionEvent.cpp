...
bool MissionEvent::CheckTrigger()
{
  ...
  switch (trigger)
  {
    ...
    case TRIGGER_DAMAGE:
    {
      ...
      if (ship)
      {
        double damage = 100.0 * (ship->Design()->integrity - ship->Integrity()) / (ship->Design()->integrity);
...
void MissionEvent::Execute(bool silent)
{
  ...
  switch (event)
  {
    case MESSAGE:
      if (event_message.length() > 0)
      {
        if (ship)
        {
          auto msg = NEW RadioMessage(ship, src, event_param[0]);
        ...
        {
          auto msg = NEW RadioMessage(elem, src, event_param[0]);
          msg->SetInfo(event_message);
    ...
    case OBJECTIVE:
      if (elem)
      {
        ...
        auto obj = NEW Instruction(event_param[0]);
    ...
    case DAMAGE:
      if (ship)
      {
        ...
        if (ship->Integrity() < 1)
        {
          ...
          ship->DeathSpiral();
          DebugTrace("    {} Killed By Scripted Event {} ({})\n", ship->Name(), id, FormatGameTime());
        }
      }
      else
        DebugTrace("   EVENT {}: Could not apply damage to ship '{}' (not found).\n", id,
                   (event_ship));
      break;
    ...
    case END_MISSION:
      DebugTrace("    END MISSION By Scripted Event {} ({})\n", id, FormatGameTime());
      end_mission = true;
      break;
    ...
    case BEGIN_SCENE:
      DebugTrace("    ------------------------------------\n");
      DebugTrace("    Begin Cutscene '{}'\n", event_message.data());
      stars->BeginCutscene();
      break;

    case END_SCENE:
      DebugTrace("    End Cutscene '{}'\n", event_message.data());
      DebugTrace("    ------------------------------------\n");
      stars->EndCutscene();
      break;
    ...
    case CAMERA:
      if (stars->InCutscene())
      {
        ...
        if (event_target.length())
        {
          DebugTrace("Mission Event {}: setting camera target to {}\n", id, (event_target));
          Ship* s_tgt = nullptr;

          if (!event_target.starts_with("body:"))
            s_tgt = sim->FindShip(event_target);

          if (s_tgt)
          {
            DebugTrace("   found ship {}\n", s_tgt->Name());
      ...
      if (stars->InCutscene())
      {
        ...
        if (disp_view)
        {
          ...
          if (event_message.length() && event_source.length())
          {
            ...
            disp_view->AddText(event_message, FontMgr::Find(event_source), color, event_rect, event_point.y,
                               event_point.x, event_point.z);
          }

          else if (!event_target.empty())
          {
            ...
            {
              disp_view->AddImage(&image, color, Video::BLEND_ALPHA, event_rect, event_point.y, event_point.x,
                                  event_point.z);
            }
          }
        }
  ...
  {
    DataLoader* loader = DataLoader::GetLoader();
    DWORD flags = pan ? Sound::LOCKED | Sound::LOCALIZED : Sound::LOCKED | Sound::AMBIENT;
...
std::string MissionEvent::TriggerParamStr() const
{
  std::string result;
  char buffer[8];
...
int MissionEvent::EventParam(int index) const
{
...
int MissionEvent::NumEventParams() const { return event_nparams; }

int MissionEvent::TriggerParam(int index) const
{
...
int MissionEvent::NumTriggerParams() const { return trigger_nparams; }

static std::string_view event_names[] = {
  "Message",
  "Objective",
  "Instruction",
  "IFF",
...
static std::string_view trigger_names[] = {
  "Time",
  "Damage",
  "Destroyed",
  "Jump",
...
std::string_view MissionEvent::EventName() const { return event_names[event]; }

std::string_view MissionEvent::EventName(int n) { return event_names[n]; }

int MissionEvent::EventForName(std::string_view n)
{
...
std::string_view MissionEvent::TriggerName() const { return trigger_names[trigger]; }

std::string_view MissionEvent::TriggerName(int n) { return trigger_names[n]; }

int MissionEvent::TriggerForName(std::string_view n)
{
