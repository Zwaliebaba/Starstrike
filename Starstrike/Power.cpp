...
static const char* source_type[] = {"sys.power.battery", "sys.power.auxilliary", "sys.power.fusion"};

static int source_value[] = {1, 2, 4};

PowerSource::PowerSource(SUBTYPE s, double max_out, double f_ratio)
  : System(POWER_SOURCE, static_cast<int>(s), "Power", source_value[s], 0),
    max_output(static_cast<float>(max_out)),
    fuel_ratio(static_cast<float>(f_ratio)),
    route_changed(false),
    requested_power_level(1.0f)
{
  m_name = Game::GetText(source_type[s]);
  abrv = Game::GetText(std::string(source_type[s]) + ".abrv");

  if (fuel_ratio < 1)
  {
    switch (subtype)
    {    // enough to last for [n] hours at full power
      case BATTERY:
...
int PowerSource::Charge() const { return static_cast<int>(capacity); }

void PowerSource::SetFuelRange(double hours)
{
  if (hours > 0)
    fuel_ratio = static_cast<float>(max_output * hours * 3600 / 100);
}
...
void PowerSource::ExecFrame(double seconds)
{
  ...
  if (availability < 0.4 && capacity > 0)
    capacity -= static_cast<float>(0.03 * seconds);
  ...
  if (IsPowerOn() && capacity > 0)
  {
    ...
    {
      power_level += static_cast<float>(seconds * 0.03);     // thirty seconds to charge up
    ...
    {
      power_level -= static_cast<float>(seconds * 0.10);     // ten seconds to power down
  ...
  if (IsPowerOn() && capacity > 0 && !Game::Paused())
  {
    ...
    if (total_distrib > 0)
    {
      ...
      capacity -= static_cast<float>(total_distrib / effective_fuel_ratio);
    }
  }
...
void PowerSource::DrainPower(double to_level)
{
  if (to_level >= 0 && to_level < power_level)
    power_level = static_cast<float>(to_level);
}
