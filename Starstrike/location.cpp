...
void Location::InitialiseTeam(unsigned char _teamId, unsigned char _teamType)
{
  ...
  for (int i = 0; i < m_levelFile->m_instantUnits.Size(); i++)
  {
    ...
    WorldObjectId spawnedId = SpawnEntities(pos, iu->m_teamId, unitId, iu->m_type, iu->m_number, g_zeroVector, iu->m_spread, -1.0f,
  ...
  if (_teamType == Team::TeamTypeLocalPlayer)
  {
    for (int i = 0; i < m_levelFile->m_runningPrograms.Size(); ++i)
    {
      ...
      if (program->m_type == Entity::TypeEngineer)
      {
        ...
        auto engineer = static_cast<Engineer*>(GetEntitySafe(objId, Entity::TypeEngineer));
      ...
      if (program->m_type == Entity::TypeInsertionSquadie)
      {
        ...
        int unitId;
        auto squad = static_cast<InsertionSquad*>(GetMyTeam()->NewUnit(Entity::TypeInsertionSquadie, program->m_count, &unitId, pos));
  ...
  else
  {
    ...
    if (entity)
    {
      ...
      case Entity::TypeEngineer:
        {
          auto eng = static_cast<Engineer*>(entity);
          if (unitMove)
...
WorldObjectId Location::GetEntityId(const LegacyVector3& startRay, const LegacyVector3& direction, unsigned char teamId, float* _range)
{
  if (teamId == 255)
...
void Location::ThrowWeapon(const LegacyVector3& _pos, const LegacyVector3& _target, int _type, unsigned char _fromTeamId)
{
