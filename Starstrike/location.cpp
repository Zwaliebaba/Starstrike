...
WorldObjectId Location::SpawnEntities( LegacyVector3 const &_pos, unsigned char _teamId, int _unitId,
...
LegacyVector3 Location::FindValidSpawnPosition( LegacyVector3 const &_pos, float _spread )
{
    int tries = 10;
...
int Location::SpawnSpirit( LegacyVector3 const &_pos, LegacyVector3 const &_vel, unsigned char _teamId, WorldObjectId _id )
{
    DarwiniaDebugAssert( _teamId < NUM_TEAMS );
...
Entity *Location::GetEntity(LegacyVector3 const &_rayStart, LegacyVector3 const &_rayDir)
{
...
Building *Location::GetBuilding(LegacyVector3 const &_rayStart, LegacyVector3 const &_rayDir )
{
...
bool Location::IsVisible( LegacyVector3 const &_from, LegacyVector3 const &_to )
{
    LegacyVector3 rayDir = (_to - _from).Normalise();
...
bool Location::IsWalkable( LegacyVector3 const &_from, LegacyVector3 const &_to, bool _evaluateCliffs )
{
    ...
    LegacyVector3 diff = ( _to - _from ) / (float)numSteps;
...
void Location::AdvanceChristmas()
{
    if( m_christmasTimer < -90.0f )
    {
        for( int i = 0; i < 150; ++i )
        {
            ...
            LegacyVector3 spawnPos = LegacyVector3( syncfrand(sizeX), posY, syncfrand(sizeZ) ) ;
            Snow *snow = new Snow();
            snow->m_pos = spawnPos;
...
int Location::GetUnitId( LegacyVector3 const &startRay, LegacyVector3 const &direction, unsigned char team, float *_range )
{
...
WorldObjectId Location::GetEntityId( LegacyVector3 const &startRay, LegacyVector3 const &direction, unsigned char teamId, float *_range )
{
...
int Location::GetBuildingId(LegacyVector3 const &rayStart, LegacyVector3 const &rayDir, unsigned char teamId, float _maxDistance, float *_range )
{
    float closestRangeSqd = FLT_MAX;
    int buildingId = -1;
...
void Location::ThrowWeapon( LegacyVector3 const &_pos, LegacyVector3 const &_target, int _type, unsigned char _fromTeamId )
{
    ...
    LegacyVector3 front = ( _target - _pos ).Normalise();
    front.y = 1.0f;
    front.Normalise();
...
void Location::FireRocket( LegacyVector3 const &_pos, LegacyVector3 const &_target, unsigned char _teamId )
{
...
void Location::FireTurretShell( LegacyVector3 const &_pos, LegacyVector3 const &_vel )
{
...
void Location::FireLaser( LegacyVector3 const &_pos, LegacyVector3 const &_vel, unsigned char _teamId )
{
...
void Location::Bang( LegacyVector3 const &_pos, float _range, float _damage )
{
    int numCores = _range * _damage * 0.01f;
	numCores += syncfrand(numCores);
    for( int p = 0; p < numCores; ++p )
    {
        LegacyVector3 vel( syncsfrand( 20.0f ),
    ...
    {
        LegacyVector3 vel( syncsfrand( 30.0f ),
...
void Location::CreateShockwave( LegacyVector3 const &_pos, float _size, unsigned char _teamId )
{
