...
MapView::MapView(Window* win)
  : View(win),
    view_mode(VIEW_SYSTEM),
    seln_mode(SELECT_REGION),
    ship_filter(0xFFFFFFFF),
    zoom(1.1)
{
  for (int i = 0; i < 3; i++)
  {
  ...
  m_titleFont = FontMgr::Find("Limerick12");
  font = FontMgr::Find("Verdana");

  active_window = dynamic_cast<ActiveWindow*>(window);
...
void MapView::BuildMenu()
{
  ...
  if (ship || mission)
  {
    ...
    for (int i = 0; i < Instruction::NUM_ACTIONS; i++)
      action_menu->AddItem(Game::GetText(std::string("MapView.item.") + Instruction::ActionName(i)), MAP_ACTION + i);

    formation_menu = NEW Menu(Game::GetText("MapView.menu.FORMATION"));
    for (int i = 0; i < Instruction::NUM_FORMATIONS; i++)
      formation_menu->AddItem(Game::GetText(std::string("MapView.item.") + Instruction::FormationName(i)), MAP_FORMATION + i);
  ...
  else if (campaign)
  {
    ...
    speed_menu = nullptr;
    hold_menu = nullptr;
    farcast_menu = nullptr;
    objective_menu = nullptr;
    formation_menu = nullptr;
    nav_menu = nullptr;
  }
...
void MapView::ClearMenu()
{
  ...
  map_menu = nullptr;
  map_system_menu = nullptr;
  map_sector_menu = nullptr;
  ship_menu = nullptr;
  nav_menu = nullptr;
  action_menu = nullptr;
  objective_menu = nullptr;
...
void MapView::ProcessMenuItem(int action)
{
  ...
  {
    std::string rgn_name = m_regions[m_currentRegion]->Name();
    ...
    else if (current_elem && can_command)
    {
      ...
      if (m_regions[m_currentRegion]->Type() == Orbital::TERRAIN)
        init_pt.z = 10e3;
  ...
  else if (action >= MAP_SECTOR)
  {
    ...
    {
      Orbital* s = m_regions[m_currentRegion];
      SetupScroll(s);
    }
  }
...
void MapView::FindShips(bool friendly, bool station, bool starship, bool dropship, std::vector<std::string>& result)
{
  if (mission)
  {
    for (int i = 0; i < mission->GetElements().size(); i++)
    {
      ...
      if (elem->IsSquadron())
        continue;
      if (!station && elem->IsStatic())
        continue;
      if (!starship && elem->IsStarship())
        continue;
      if (!dropship && elem->IsDropship())
        continue;
    ...
    if (sim)
    {
      for (int r = 0; r < sim->GetRegions().size(); r++)
      {
        ...
        for (int i = 0; i < rgn->Ships().size(); i++)
        {
          ...
          if (!station && s->IsStatic())
            continue;
          if (!starship && s->IsStarship())
            continue;
          if (!dropship && s->IsDropship())
            continue;
...
void MapView::SetSelectionMode(int mode)
{
  ...
  if (mode != seln_mode && mode <= SELECT_FIGHTER)
  {
    ...
    if (system && view_mode == VIEW_SYSTEM) { SelectAt(rect.x + rect.w / 2, rect.y + rect.h / 2); }
  }
}
...
void MapView::SetSelection(int index)
{
  ...
  switch (seln_mode)
  {
    ...
    case SELECT_STATION:
    {
      if (mission)
      {
        MissionElement* selected_elem = nullptr;
        ...
        if (selected_elem && m_regions.size())
        {
          ListIter<Orbital> rgn = m_regions;
          while (++rgn)
          {
            if (selected_elem->Region() == rgn->Name())
            {
              Orbital* elem_region = rgn.value();
              m_currentRegion = m_regions.index(elem_region);
            }
          }
        }
      }
      ...
      else
      {
        ...
        if (selship)
        {
          ...
          m_currentRegion = m_regions.index(s);
        }
      }
    }
    break;
...
void MapView::SetSelectedShip(Ship* ship)
{
  if (scrolling)
    return;
  Orbital* s = nullptr;
...
void MapView::SetSelectedElem(MissionElement* elem)
{
  if (scrolling)
    return;
  Orbital* s = nullptr;
  ...
  switch (seln_mode)
  {
    ...
    case SELECT_STATION:
    case SELECT_STARSHIP:
    case SELECT_FIGHTER: { SelectElem(elem); }
    break;
  }
...
void MapView::SetupScroll(Orbital* s)
{
  switch (view_mode)
  {
    case VIEW_GALAXY:
...
bool MapView::IsVisible(const Point& loc)
{
  if (view_mode == VIEW_REGION)
  {
    ...
    int test_x = static_cast<int>(cx + sx + ox);
    int test_y = static_cast<int>(cy + sy + oy);

    bool visible = test_x >= 0 && test_x < rect.w && test_y >= 0 && test_y < rect.h;
...
void MapView::SetRegion(OrbitalRegion* rgn)
{
  if (scrolling || rgn == nullptr)
    return;

  int index = m_regions.index(rgn);

  if (index < 0 || index == m_currentRegion || index >= m_regions.size())
    return;
...
void MapView::SetRegionByName(std::string_view rgn_name)
{
  OrbitalRegion* rgn = nullptr;

  for (int i = 0; i < m_regions.size(); i++)
  {
    Orbital* r = m_regions[i];
    if (rgn_name == r->Name())
    {
      rgn = dynamic_cast<OrbitalRegion*>(r);
      break;
    }
  }
...
void MapView::SelectAt(int x, int y)
{
  if (scrolling)
    return;
  if (c == 0)
  ...
  else if (view_mode == VIEW_SYSTEM)
  {
    switch (seln_mode)
    {
      ...
      default: case SELECT_REGION:
      {
        if (m_regions.isEmpty())
          return;
        int index = 0;
        ListIter<Orbital> region = m_regions;
        while (++region)
        {
  ...
  else if (view_mode == VIEW_REGION)
  {
    ...
    if (mission)
    {
      Orbital* rgn = m_regions[m_currentRegion];
...
Orbital* MapView::GetSelection()
{
  ...
  switch (seln_mode)
  {
    ...
    case SELECT_REGION:
      if (m_currentRegion < m_regions.size())
        s = m_regions[m_currentRegion];
      break;
...
Ship* MapView::GetSelectedShip() { return current_ship; }

MissionElement* MapView::GetSelectedElem() { return current_elem; }
...
void MapView::DrawTabbedText(Font* font, std::string_view text)
{
  if (font && !text.empty())
  {
    Rect label_rect;
...
void MapView::Refresh()
{
  ...
  {
    DrawGrid();
    DrawTabbedText(m_titleFont, Game::GetText("MapView.item.no-system"));
    return;
  }
...
{
  m_titleFont->SetColor(active_window->GetForeColor());
  DrawTabbedText(m_titleFont, m_title);
}

void MapView::DrawGalaxy()
{
  m_title = Game::GetText("MapView.title.Galaxy");
  DrawGrid();
  ...
  while (++iter)
  {
    ...
    if (system == s)
    {
      ...
      {
        int sx = static_cast<int>(s->Location().x);
        int sy = static_cast<int>(s->Location().y);
  ...
  while (++iter)
  {
    ...
    int sx = static_cast<int>(cx + ox + s->Location().x * scale);
    int sy = static_cast<int>(cy + oy + s->Location().y * scale);
    ...
    while (++iter2)
    {
      ...
      if (s != s2 && s->HasLinkTo(s2))
      {
        ...
        int bx = static_cast<int>(cx + ox + s2->Location().x * scale);
        int by = static_cast<int>(cy + oy + s2->Location().y * scale);
        ...
        else
        {
          ...
          ax += static_cast<int>(8 * d.x);
          ay += static_cast<int>(8 * d.y);

          bx -= static_cast<int>(8 * d.x);
          by -= static_cast<int>(8 * d.y);
        }
...
{
  std::string caption = Game::GetText("MapView.title.Starsystem");
  caption += " ";
  caption += system->Name();
  ...
  active_window->SetFont(font);
  auto rc = Rect(4, 4, rect.w - 8, 24);
  active_window->DrawText(resolution, -1, rc, DT_SINGLELINE | DT_RIGHT);
}

void MapView::DrawRegion()
{
  auto rgn = static_cast<OrbitalRegion*>(m_regions[m_currentRegion]);

  std::string caption = Game::GetText("MapView.title.Sector");
  caption += " ";
  caption += rgn->Name();
  ...
  int size = static_cast<int>(rgn->Radius());
  int step = static_cast<int>(rgn->GridSpace());
  ...
  int left = static_cast<int>(-size * scale + ox + cx);
  int right = static_cast<int>(size * scale + ox + cx);
  int top = static_cast<int>(-size * scale + oy + cy);
  int bottom = static_cast<int>(size * scale + oy + cy);
  ...
  else if (ship)
  {
    ...
    if (simrgn)
    {
      ...
      while (++s_iter)
      {
        ...
        if (s && (s->IsStatic()) && !IsClutter(*s) && (s->GetIFF() == ship->GetIFF() || s->GetIFF() == 0))
          DrawShip(*s, (s == current_ship), rep);
      }
      ...
      while (++r_iter)
      {
        ...
        if (r != simrgn)
        {
          ...
          while (++s_iter)
          {
            ...
            if (s && !s->IsStatic() && !IsClutter(*s) && (s->GetIFF() == ship->GetIFF() || s->GetIFF() == 0))
            {
              DrawNavRoute(simrgn->GetOrbitalRegion(), s->GetFlightPlan(), s->MarkerColor(), s, nullptr);
            }
          }
        }
      }
...
void MapView::DrawOrbital(Orbital& body, int index)
{
  ...
  int ipx = static_cast<int>(cx + px + ox);
  int ipy = static_cast<int>(cy + py + oy);
  int ibo_x = static_cast<int>(bo_x);
  int ibo_y = static_cast<int>(bo_y);
  ...
  {
    case Orbital::STAR:
      color = Color(248, 248, 128);
      break;
    case Orbital::PLANET:
      color = Color(64, 64, 192);
      break;
    case Orbital::MOON:
      color = Color(32, 192, 96);
      break;
    case Orbital::REGION:
      color = Color(255, 255, 255);
      break;
    case Orbital::TERRAIN:
      color = Color(16, 128, 48);
      break;
  }

  int icx = static_cast<int>(cx + bx + ox);
  int icy = static_cast<int>(cy + by + oy);
  int ibr = static_cast<int>(br);
  ...
  if (type < Orbital::REGION)
  {
    ...
    {
      auto map_icon = (Bitmap*)&body.GetMapIcon();
  ...
  {
    label_rect.x = x1 - label_w + static_cast<int>(br);
...
void MapView::DrawShip(Ship& s, bool current, int rep)
{
  ...
  if (rep && view_mode == VIEW_REGION && rgn == s.GetRegion()->GetOrbitalRegion())
  {
    ...
    shiploc.x = static_cast<int>(cx + sx + ox);
    shiploc.y = static_cast<int>(cy + sy + oy);

    bool ship_visible = shiploc.x >= 0 && shiploc.x < rect.w && shiploc.y >= 0 && shiploc.y < rect.h;
    ...
    if (ship_visible)
    {
      ...
      else
      {
        ...
        int sprite_index = static_cast<int>((theta + THETA_OFFSET) * THETA_SLICE);
        ...
        if (nsprites)
        {
          ...
          window->DrawBitmap(shiploc.x - sprite_width, shiploc.y - h, shiploc.x + sprite_width, shiploc.y + h, &bmp,
                             Video::BLEND_ALPHA);
        }
  ...
  if (current && std::string(s.GetRegion()->Name()) == m_regions[m_currentRegion]->Name())
  {
    x1 = static_cast<int>(shiploc.x - sprite_width - 1);
    x2 = static_cast<int>(shiploc.x + sprite_width + 1);
    y1 = static_cast<int>(shiploc.y - sprite_width - 1);
    y2 = static_cast<int>(shiploc.y + sprite_width + 1);
  ...
  if (s.GetIFF() == 0 || ship && s.GetIFF() == ship->GetIFF())
    DrawNavRoute(rgn, s.GetFlightPlan(), s.MarkerColor(), &s, nullptr);
}

void MapView::DrawElem(MissionElement& s, bool current, int rep)
{
  if (!mission)
    return;

  bool visible = editor || s.GetIFF() == 0 || s.GetIFF() == mission->Team() || s.IntelLevel() > Intel::KNOWN;
  ...
  if (s.Region() == rgn->Name())
  {
    ...
    if (ship_visible)
    {
      ...
      else
      {
        ...
        if (s.Count() > 1)
          sprintf_s(label, "%s x %d", s.Name().c_str(), s.Count());
        else
          strcpy_s(label, s.Name().c_str());
  ...
  // draw nav route:
  // draw current ship marker:
  if (current && s.Region() == m_regions[m_currentRegion]->Name())
  {
    x1 = static_cast<int>(shiploc.x - sprite_width - 1);
    x2 = static_cast<int>(shiploc.x + sprite_width + 1);
    y1 = static_cast<int>(shiploc.y - sprite_width - 1);
  ...
  if (editor || s.GetIFF() == 0 || mission && s.GetIFF() == mission->Team())
    DrawNavRoute(rgn, s.NavList(), s.MarkerColor(), nullptr, &s);
}

void MapView::DrawNavRoute(OrbitalRegion* rgn, List<Instruction>& s_route, Color s_marker, Ship* ship,
                           MissionElement* elem)
{
  int x1, y1, x2, y2;
  double cx = rect.w / 2;
  ...
  for (int i = 0; i < s_route.size(); i++)
  {
    ...
    if (navpt->RegionName() == rgn->Name())
    {
      double nav_x = navpt->Location().x * scale;
      double nav_y = navpt->Location().y * scale;

      int isx = static_cast<int>(cx + nav_x + ox);
      int isy = static_cast<int>(cy + nav_y + oy);

      if (old_in && draw_route)
      {
        int iox = static_cast<int>(cx + old_x + ox);
        int ioy = static_cast<int>(cy + old_y + oy);
        ...
        if ((x1 * x1 + y1 * y1) > 2000)
        {
          ...
          int imx = static_cast<int>(cx + (old_x + nav_x) / 2 + ox);
          int imy = static_cast<int>(cy + (old_y + nav_y) / 2 + oy);
      ...
      if (draw_route)
      {
        ...
        if (navpt == current_navpt)
        {
          if (!navpt->TargetName().empty())
          {
            sprintf_s(buf, "%s %s",
                      Game::GetText(std::string("MapView.item.") + Instruction::ActionName(navpt->Action())).c_str(),
                      navpt->TargetName().c_str());
            window->Print(x2 + 3, y1 + 10, buf);
          }
          else
          {
            sprintf_s(buf, "%s",
                      Game::GetText(std::string("MapView.item.") + Instruction::ActionName(navpt->Action())).data());
            window->Print(x2 + 3, y1 + 10, buf);
          }

          sprintf_s(buf, "%s",
                    Game::GetText(std::string("MapView.item.") + Instruction::FormationName(navpt->Formation())).data());
...
void MapView::DrawCombatantSystem(Combatant* c, Orbital* rgn, int x, int y, int r)
{
  int team = c->GetIFF();
  int x1 = 0;
  int x2 = 0;
...
void MapView::DrawCombatGroupSystem(CombatGroup* group, Orbital* rgn, int x1, int x2, int& y, int a)
{
  ...
  if (group->GetRegion() == rgn->Name())
  {
    switch (group->Type())
    {
      ...
      case CombatGroup::DESTROYER_SQUADRON:
      {
        ...
        active_window->SetFont(font);
        auto rc = Rect(x1, y, x2 - x1, 12);
        active_window->DrawText(txt, 0, rc, a);
        y += 10;
      }
      break;
      ...
      case CombatGroup::MISSILE:
      {
        active_window->SetFont(font);
        auto rc = Rect(x1, y, x2 - x1, 12);
        active_window->DrawText(group->GetShortDescription(), 0, rc, a);
        y += 10;
      }
      break;
...
void MapView::DrawCombatGroup(CombatGroup* group, int rep)
{
...
bool MapView::IsCrowded(Ship& test)
{
...
void MapView::GetShipLoc(Ship& s, POINT& shiploc)
{
  double cx = rect.w / 2;
  double cy = rect.h / 2;
  ...
  if (view_mode == VIEW_SYSTEM || (view_mode == VIEW_REGION && rgn == s.GetRegion()->GetOrbitalRegion()))
  {
    double sx = (s.Location().x + rlx) * scale;
    double sy = (s.Location().y + rly) * scale;
...
bool MapView::IsCrowded(MissionElement& test)
{
  Orbital* rgn = m_regions[m_currentRegion];
  ...
  if (mission)
  {
    ...
    while (++s)
    {
      ...
      if (ref && ref != &test && ref->Region() == rgn->Name())
      {
...
void MapView::GetElemLoc(MissionElement& s, POINT& shiploc)
{
  double cx = rect.w / 2;
  double cy = rect.h / 2;
  ...
  if (view_mode == VIEW_SYSTEM || (view_mode == VIEW_REGION && s.Region() == rgn->Name()))
  {
    double sx = (s.Location().x + rlx) * scale;
    double sy = (s.Location().y + rly) * scale;
...
OrbitalRegion* MapView::GetRegion() const
{
  ...
  if (m_currentRegion < m_regions.size())
    result = static_cast<OrbitalRegion*>(m_regions[m_currentRegion]);
...
int MapView::OnMouseMove(int x, int y)
{
  if (captured)
  {
...
int MapView::OnRButtonDown(int x, int y)
{
  if (!captured)
    captured = SetCapture();
...
int MapView::OnRButtonUp(int x, int y)
{
  if (captured)
  {
    ReleaseCapture();
...
int MapView::OnClick() { return active_window->OnClick(); }
