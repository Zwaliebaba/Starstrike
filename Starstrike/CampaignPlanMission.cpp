...
void CampaignPlanMission::ExecFrame()
{
  if (m_campaign && m_campaign->IsActive())
  {
    player_group = m_campaign->GetPlayerGroup();
    if (!player_group)
      return;

    int missionCount = m_campaign->GetMissionList().size();
    ...
    if (missionCount > 0)
    {
      ...
      if (Campaign::Stardate() - m_execTime < 1)
        return;
    }
    ...
    if (player_group->IsFighterGroup())
    {
      ...
      CampaignMissionFighter generator(m_campaign);
    ...
    else if (player_group->IsStarshipGroup())
    {
      ...
      CampaignMissionStarship generator(m_campaign);
      generator.CreateMission(request);
      delete request;
    }
...
void CampaignPlanMission::SelectStartTime()
{
  const int HOUR = 3600;  // 60 minutes
  const int MISSION_DELAY = 1800;  // 30 minutes
  double base_time = 0;

  List<MissionInfo>& info_list = m_campaign->GetMissionList();
  ...
  if (base_time == 0)
    base_time = m_campaign->GetTime() + MISSION_DELAY;

  start = (int)base_time + MISSION_DELAY;
  start -= start % MISSION_DELAY;
}

CampaignMissionRequest* CampaignPlanMission::PlanStarshipMission()
{
  CampaignMissionRequest* request = 0;

  if (!request)
    request = PlanCampaignMission();
  if (!request)
    request = PlanStrategicMission();
  if (!request)
    request = PlanRandomStarshipMission();

  return request;
}

CampaignMissionRequest* CampaignPlanMission::PlanFighterMission()
{
  CampaignMissionRequest* request = 0;
  ...
  if (!request)
    request = PlanStrategicMission();
  if (!request)
    request = PlanRandomFighterMission();

  return request;
}

CampaignMissionRequest* CampaignPlanMission::PlanCampaignMission()
{
  CampaignMissionRequest* request = 0;
  ...
  while (++iter && !request)
  {
    ...
    if (action->IsAvailable())
    {
      ...
      if (action->ExecTime() > 0 && m_campaign->GetTime() - action->ExecTime() < 7200)
        continue;

      CombatGroup* g = m_campaign->FindGroup(action->GetIFF(), action->AssetType(), action->AssetId());

      if (g && (g == player_group || (player_group->Type() == CombatGroup::WING && player_group->
        FindGroup(g->Type(), g->GetID()))))
      {
        request = NEW CampaignMissionRequest(m_campaign, action->Subtype(), start, g);
...
CampaignMissionRequest* CampaignPlanMission::PlanStrategicMission()
{
  CampaignMissionRequest* request = 0;
  ...
  if (assignments.size())
  {
    ...
    request = NEW CampaignMissionRequest(m_campaign, a->Type(), start, a->GetResource());
...
CampaignMissionRequest* CampaignPlanMission::PlanRandomStarshipMission()
{
  ...
  {
    CombatGroup* freight = m_campaign->FindGroup(ownside, CombatGroup::FREIGHT);
    if (!freight || freight->CountUnits() < 1)
      type = Mission::PATROL;
  }

  CampaignMissionRequest* request = 0;
  request = NEW CampaignMissionRequest(m_campaign, type, start, player_group);
...
CampaignMissionRequest* CampaignPlanMission::PlanRandomFighterMission()
{
  CampaignMissionRequest* request = 0;
  ...
  if (type >= Mission::AIR_PATROL && type <= Mission::AIR_INTERCEPT)
  {
    ...
    {
      std::string air_region = zone->GetRegions().at(1);
  ...
  request = NEW CampaignMissionRequest(m_campaign, type, start, primary);
