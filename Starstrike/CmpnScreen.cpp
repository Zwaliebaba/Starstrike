...
CmpnScreen::CmpnScreen()
  : screen(nullptr),
    cmd_force_dlg(nullptr),
    cmd_orders_dlg(nullptr),
    cmd_missions_dlg(nullptr),
    cmd_intel_dlg(nullptr),
    cmd_theater_dlg(nullptr),
    cmd_msg_dlg(nullptr),
    cmp_file_dlg(nullptr),
    cmp_end_dlg(nullptr),
    cmp_scene_dlg(nullptr),
    isShown(false),
    campaign(nullptr),
    stars(nullptr),
    completion_stage(0)
...
CmpnScreen::~CmpnScreen() { CmpnScreen::TearDown(); }

void CmpnScreen::Setup(Screen* s)
{
  if (!s)
    return;
...
void CmpnScreen::SetFieldOfView(double fov)
{
  if (cmp_scene_dlg)
    cmp_scene_dlg->GetCameraView()->SetFieldOfView(fov);
}

double CmpnScreen::GetFieldOfView() const
{
  if (cmp_scene_dlg)
    ...
    if (completion_stage == 0)
    {
      ...
      if (campaign->IsTraining())
      {
        ...
        if (player->Trained() >= all_missions && player->Trained() < 255)
        {
          ...
          std::string msg_info = std::vformat(Game::GetText("CmpnScreen.congrats"), 
            std::make_format_args(unmove(Player::RankName(player->Rank())), unmove(player->Name())));
      ...
      else if (campaign->IsComplete() || campaign->IsFailed())
      {
        ...
        if (event && !event->Visited() && !event->SceneFile().empty())
        {
...
void CmpnScreen::ShowCmdDlg() { ShowCmdOrdersDlg(); }

void CmpnScreen::ShowCmdForceDlg()
{
  HideAll();
  cmd_force_dlg->Show();
  Mouse::Show(true);
...
void CmpnScreen::HideCmdForceDlg()
{
  if (IsCmdForceShown())
    cmd_force_dlg->Hide();
}

bool CmpnScreen::IsCmdForceShown() { return cmd_force_dlg && cmd_force_dlg->IsShown(); }

void CmpnScreen::ShowCmdOrdersDlg()
{
  HideAll();
  cmd_orders_dlg->Show();
  Mouse::Show(true);
...
void CmpnScreen::HideCmdOrdersDlg()
{
  if (IsCmdOrdersShown())
    cmd_orders_dlg->Hide();
}

bool CmpnScreen::IsCmdOrdersShown() { return cmd_orders_dlg && cmd_orders_dlg->IsShown(); }

void CmpnScreen::ShowCmdMissionsDlg()
{
  HideAll();
...
void CmpnScreen::HideCmdMissionsDlg()
{
  if (IsCmdMissionsShown())
    cmd_missions_dlg->Hide();
}

bool CmpnScreen::IsCmdMissionsShown() { return cmd_missions_dlg && cmd_missions_dlg->IsShown(); }

void CmpnScreen::ShowCmdIntelDlg()
{
  HideAll();
  cmd_intel_dlg->Show();
  Mouse::Show(true);
...
void CmpnScreen::HideCmdIntelDlg()
{
  if (IsCmdIntelShown())
    cmd_intel_dlg->Hide();
}

bool CmpnScreen::IsCmdIntelShown() { return cmd_intel_dlg && cmd_intel_dlg->IsShown(); }

void CmpnScreen::ShowCmdTheaterDlg()
{
  HideAll();
...
void CmpnScreen::HideCmdTheaterDlg()
{
  if (IsCmdTheaterShown())
    cmd_theater_dlg->Hide();
}

bool CmpnScreen::IsCmdTheaterShown() { return cmd_theater_dlg && cmd_theater_dlg->IsShown(); }

void CmpnScreen::ShowCmpFileDlg()
{
  cmp_file_dlg->Show();
  Mouse::Show(true);
}

void CmpnScreen::HideCmpFileDlg()
{
  if (IsCmpFileShown())
    cmp_file_dlg->Hide();
}

bool CmpnScreen::IsCmpFileShown() { return cmp_file_dlg && cmp_file_dlg->IsShown(); }

void CmpnScreen::ShowCmdMsgDlg()
{
  cmd_msg_dlg->Show();
  Mouse::Show(true);
}

void CmpnScreen::HideCmdMsgDlg()
{
  if (IsCmdMsgShown())
    cmd_msg_dlg->Hide();
}

bool CmpnScreen::IsCmdMsgShown() { return cmd_msg_dlg && cmd_msg_dlg->IsShown(); }

void CmpnScreen::ShowCmpCompleteDlg()
{
  cmp_end_dlg->Show();
  Mouse::Show(true);
}

void CmpnScreen::HideCmpCompleteDlg()
{
  if (IsCmpCompleteShown())
    cmp_end_dlg->Hide();
}

bool CmpnScreen::IsCmpCompleteShown() { return cmp_end_dlg && cmp_end_dlg->IsShown(); }

void CmpnScreen::ShowCmpSceneDlg()
{
  cmp_scene_dlg->Show();
  Mouse::Show(false);
}

void CmpnScreen::HideCmpSceneDlg()
{
  if (IsCmpSceneShown())
    cmp_scene_dlg->Hide();
}

bool CmpnScreen::IsCmpSceneShown() { return cmp_scene_dlg && cmp_scene_dlg->IsShown(); }
