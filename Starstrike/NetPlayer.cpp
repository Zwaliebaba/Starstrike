...
bool NetPlayer::DoObjHyper(NetObjHyper* obj_hyper)
{
  if (ship && obj_hyper)
  {
    ...
    {
      DebugTrace("NetPlayer::DoObjHyper ship: '%s' rgn: '%s' trans: %d (IGNORED)\n\n", ship->Name(),
                 obj_hyper->GetRegion().data(), trans);

      return false;
    }

    DebugTrace("NetPlayer::DoObjHyper ship: '%s' rgn: '%s' trans: %d\n\n", ship->Name(), obj_hyper->GetRegion().data(),
               trans);
    ...
    else
    {
      ...
      sim->CreateExplosion(ship->Location(), Point(0, 0, 0), Explosion::QUANTUM_FLASH, 1.0f, 0, ship->GetRegion());

      sim->RequestHyperJump(ship, rgn, obj_hyper->GetLocation(), trans, fc1, fc2);

      ShipStats* stats = ShipStats::Find(ship->Name().c_str());
...
bool NetPlayer::DoWepTrigger(NetWepTrigger* trigger)
{
  if (ship && trigger)
  {
    ...
    if (decoy)
      ...
      w = ship->GetProbeLauncher();
    else
      w = ship->GetWeaponByIndex(index);
...
bool NetPlayer::DoSysDamage(NetSysDamage* sys_damage)
{
  if (ship && sys_damage)
  {
    ...
    ship->InflictNetSystemDamage(sys, sys_damage->GetDamage(), sys_damage->GetDamageType());
...
bool NetPlayer::DoSysStatus(NetSysStatus* sys_status)
{
  if (ship && sys_status)
  {
    ...
    ship->SetNetSystemStatus(sys, sys_status->GetStatus(), sys_status->GetPower(), sys_status->GetReactor(),
...
bool NetPlayer::Update(SimObject* obj)
{
  if (obj == ship)
    ship = nullptr;
...
std::string NetPlayer::GetObserverName() const { return "NetPlayer"; }
