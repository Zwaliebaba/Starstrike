...
Sensor::Sensor()
  : System(SENSOR, 1, "Dual Sensor Pkg", 1, 10, 10, 10),
    ...
    target(nullptr)
{
  m_name = Game::GetText("sys.sensor");
...
Sensor::~Sensor() { ClearAllContacts(); }

void Sensor::ClearAllContacts() { contacts.destroy(); }
...
double Sensor::GetBeamRange() const { return range_settings[range_index]; }
...
{
  if (nsettings < 8)
    range_settings[nsettings++] = static_cast<float>(r);
...
std::string Sensor::GetObserverName() const { return "Sensor"; }
    ...
    while (++c_iter)
    {
      ...
      else if (ship && ship->GetIFF() >= 0 && ship->GetIFF() < 5)
      {
        ...
        if (c_ship)
        {
          if (!t)
          {
            auto track = NEW Contact(c_ship, contact->d_pas, contact->d_act);
        ...
        else if (c_shot)
        {
          if (!t)
          {
            auto track = NEW Contact(c_shot, contact->d_pas, contact->d_act);
...
void Sensor::ProcessContact(Ship* c_ship, double az1, double az2)
{
  ...
  bool threat = (c_ship->Life() != 0 && c_ship->GetIFF() && c_ship->GetIFF() != ship->GetIFF() && c_ship->GetEMCON() > 2
  ...
  if (threat || vis || mode >= PST || tz > 1)
  {
    ...
    if (threat || vis || d_pas > SENSOR_THRESHOLD || d_act > SENSOR_THRESHOLD)
    {
      ...
      if (unit && ship && unit->GetIFF() != ship->GetIFF())
      {
        ...
        if (group && group->IntelLevel() < Intel::LOCATED && group->IntelLevel() > Intel::RESERVE)
      ...
      {
        c->loc = c_ship->Location();
        c->d_pas = static_cast<float>(d_pas);
        c->d_act = static_cast<float>(d_act);
        c->probe = probescan;
...
void Sensor::ProcessContact(Shot* c_shot, double az1, double az2)
{
  double sensor_range = GetBeamRange();
...
bool Sensor::IsTracking(SimObject* tgt)
{
  if (tgt && mode != GM && mode != PAS && mode != PST && IsPowerOn())
  {
    ...
    if (tgt->Type() == SimObject::SIM_SHIP)
      c = FindContact(static_cast<Ship*>(tgt));
    else
      c = FindContact(static_cast<Shot*>(tgt));

    return (c != nullptr && c->ActReturn() > SENSOR_THRESHOLD && !c->IsProbed());
  }

  return false;
}

constexpr double sensor_lock_threshold = 0.5;
...
SimObject* Sensor::LockTarget(int type, bool closest, bool hostile)
{
