...
inline int random_index() { return rand() / 3277; }
...
void ShipKiller::BeginDeathSpiral()
{
  ...
  while (++iter)
  {
    ...
    {
      auto gun = static_cast<Weapon*>(iter.value());
  ...
  if (rand() < 16000)
    loc += ship->BeamLine() * -3.0f * ship->Radius();
  else
    loc += ship->BeamLine() * 3.0f * ship->Radius();

  if (rand() < 8000)
    loc += ship->LiftLine() * -1.0f * ship->Radius();
  else
    loc += ship->LiftLine() * 2.0f * ship->Radius();
  ...
  else
  {
    ...
    if (design->explosion[exp_index].type > 0)
    {
      ...
      sim->CreateExplosion(exp_loc, ship->Velocity(), design->explosion[exp_index].type,
                           static_cast<float>(ship->Radius()), exp_scale, ship->GetRegion(), ship);
    }
  }

  ship->SetControls(nullptr);
  ship->SetupAgility();
}
...
void ShipKiller::ExecFrame(double seconds)
{
  ...
  exp_time -= static_cast<float>(seconds);
  ...
  if (time < DEATH_CAM_LINGER)
  {
    for (int i = 0; i < ShipDesign::MAX_EXPLOSIONS; i++)
    {
      if (design->explosion[i].final)
      {
        ...
        sim->CreateExplosion(exp_loc, ship->Velocity(), design->explosion[i].type, static_cast<float>(ship->Radius()),
                             exp_scale, ship->GetRegion());
      }
    }
    ...
    for (int i = 0; i < ShipDesign::MAX_DEBRIS; i++)
    {
      if (design->debris[i].model)
      {
        ...
        {
          Debris* debris = sim->CreateDebris(debris_loc, debris_vel + ship->Velocity(), design->debris[i].model,
                                             design->debris[i].mass, ship->GetRegion());
          ...
          for (int fire = 0; fire < 5; fire++)
          {
            ...
            {
              sim->CreateExplosion(fire_loc, ship->Velocity(), design->debris[i].fire_type, exp_scale, exp_scale,
                                   ship->GetRegion(), debris);
            }
            else
            {
              sim->CreateExplosion(fire_loc, ship->Velocity(), Explosion::SMALL_FIRE, exp_scale, exp_scale,
                                   ship->GetRegion(), debris);

              sim->CreateExplosion(fire_loc, ship->Velocity(), Explosion::SMOKE_TRAIL, exp_scale * 0.25f,
                                   exp_scale * 0.25f, ship->GetRegion(), debris);
            }
          }
