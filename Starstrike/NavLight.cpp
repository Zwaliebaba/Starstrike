...
NavLight::NavLight(double p, double s)
  : System(COMPUTER, 32, "Navigation Lights", 1, 0),
    ...
    nlights(0)
{
  m_name = Game::GetText("sys.nav-light");
    ...
    nlights(0)
{
  ...
  for (int i = 0; i < nlights; i++)
  {
    ...
    auto rep = NEW DriveSprite(images[beacon_type[i]]);
    rep->Scale(c.scale);
...
void NavLight::ExecFrame(double seconds)
{
  if (enable && power_on)
  {
    ...
    DWORD n = static_cast<int>(fmod(t, period) * 32 / period);
    DWORD code = 1 << n;
...
void NavLight::Enable() { enable = true; }

void NavLight::Disable() { enable = false; }

void NavLight::AddBeacon(Point l, DWORD ptn, int t)
{
  if (nlights < MAX_LIGHTS)
  {
    ...
    auto rep = NEW DriveSprite(images[t]);
    rep->Scale(scale);
...
void NavLight::SetPeriod(double p) { period = p; }

void NavLight::SetPattern(int index, DWORD ptn)
{
  if (index >= 0 && index < nlights)
    pattern[index] = ptn;
}

void NavLight::SetOffset(DWORD o) { offset = o; }
