...
class EntityGrid
{
  ...
  public:
    ...
    void UpdateObject(WorldObjectId _objectId, float _oldWorldX, float _oldWorldZ, float _newWorldX, float _newWorldZ,
                      float _radius);

    WorldObjectId* GetNeighbours(float _worldX, float _worldZ, float _range, int* _numFound, bool _includeTeam[NUM_TEAMS]);

    WorldObjectId* GetNeighbours(float _worldX, float _worldZ, float _range, int* _numFound);

    WorldObjectId* GetEnemies(float _worldX, float _worldZ, float _range, int* _numFound, unsigned char _myTeam);

    WorldObjectId GetBestEnemy(float _worldX, float _worldZ, float _minRange, float _maxRange, unsigned char _myTeam);

    WorldObjectId* GetFriends(float _worldX, float _worldZ, float _range, int* _numFound, unsigned char _myTeam);
