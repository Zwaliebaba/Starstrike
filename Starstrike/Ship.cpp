...
constexpr int HIT_NOTHING = 0;
constexpr int HIT_HULL = 1;
constexpr int HIT_SHIELD = 2;
constexpr int HIT_BOTH = 3;
constexpr int HIT_TURRET = 4;

Ship::Ship(std::string_view ship_name, std::string_view reg_num, ShipDesign* ship_dsn, int IFF, int cmd_ai, const int* load)
  : killer(nullptr),
    throttle(0),
    throttle_request(0),
    augmenter(false),
    ...
    decoy(nullptr),
    probe(nullptr),
    sensor_drone(nullptr),
    hangar(nullptr),
    combat_unit(nullptr),
    element(nullptr),
    orig_elem_index(-1),
    radio_orders(nullptr),
    launch_point(nullptr),
    director_info("Init"),
    ai_mode(2),
...
regnum(reg_num)
{
  ...
  {
    auto reactor = NEW PowerSource(*design->reactors[i]);
  ...
  {
    auto drive = NEW Drive(*design->drives[i]);
    drive->SetShip(this);
    drive->SetID(sys_id++);
  ...
  {
    auto deck = NEW FlightDeck(*design->flight_decks[i]);
    deck->SetShip(this);
  ...
  if (design->squadrons.size() > 0)
  {
    ...
    for (int i = 0; i < design->squadrons.size(); i++)
    {
      ...
      hangar->CreateSquadron(s->name, nullptr, s->design, s->count, GetIFF(), nullptr, 0, s->avail);
    }
  }
  ...
  if (design->sensor)
  {
    ...
    if (IsStarship() || IsStatic() || design->m_name.starts_with("Camera"))
  ...
  {
    auto gun = NEW Weapon(*design->weapons[i]);
    gun->SetID(sys_id++);
    gun->SetOwner(this);
  ...
  if (weapons.size() > 1)
  {
    ...
    if (weapons.size() > 4)
    {
      DebugTrace("WARNING: Ship '%s' type '%s' has %d wep groups (max=4)\n", Name(), DesignName(), weapons.size());
    }
  }
  ...
  {
    auto navlight = NEW NavLight(*design->navlights[i]);
    navlight->SetShip(this);
    navlight->SetID(sys_id++);
    navlight->SetOffset(((intptr_t)this) << 2);
  ...
  for (int i = 0; i < design->computers.size(); i++)
  {
    ...
    if (design->computers[i]->Subtype() == Computer::FLIGHT)
    {
      ...
      if (thruster) { flcs->SetTransLimit(thruster->TransXLimit(), thruster->TransYLimit(), thruster->TransZLimit()); }
      else { flcs->SetTransLimit(design->trans_x, design->trans_y, design->trans_z); }
...
{
  const float ROLL_SPEED = static_cast<float>(PI * 0.1500);
  const float PITCH_SPEED = static_cast<float>(PI * 0.0250);
  const float YAW_SPEED = static_cast<float>(PI * 0.0250);
  ...
  float rr = static_cast<float>(design->roll_rate * PI / 180);
  float pr = static_cast<float>(design->pitch_rate * PI / 180);
  float yr = static_cast<float>(design->yaw_rate * PI / 180);

  if (rr == 0)
    rr = static_cast<float>(agility) * ROLL_SPEED;
  if (pr == 0)
    pr = static_cast<float>(agility) * PITCH_SPEED;
  if (yr == 0)
    yr = static_cast<float>(agility) * YAW_SPEED * yaw_air_factor;
...
void Ship::SetRegion(SimRegion* rgn)
{
  ...
  constexpr double GRAV = 6.673e-11;
  ...
  else if (IsAirborne())
  {
    ...
    SetGravity(static_cast<float>(GRAV * m0 / (r * r)));
    SetBaseDensity(1.0f);
  }
...
int Ship::GetTextureList(List<Bitmap>& textures)
{
  ...
  for (int d = 0; d < detail.NumLevels(); d++)
  {
    for (int i = 0; i < detail.NumModels(d); i++)
    {
      ...
      if (g->IsSolid())
      {
        auto solid = static_cast<Solid*>(g);
...
{
  constexpr int DEFAULT_TRACK_LENGTH = 20; // 10 seconds
...
{
  constexpr int DEFAULT_TRACK_UPDATE = 500; // milliseconds
  constexpr int DEFAULT_TRACK_LENGTH = 20; // 10 seconds
...
std::string Ship::Abbreviation() const { return design->m_abrv; }

std::string Ship::DesignName() const { return design->DisplayName(); }

std::string Ship::DesignFileName() const { return design->m_filename; }

std::string_view Ship::ClassName() const { return ShipDesign::ClassName(design->type); }

std::string_view Ship::ClassName(int c) { return ShipDesign::ClassName(c); }

int Ship::ClassForName(std::string_view name) { return ShipDesign::ClassForName(name); }

Ship::CLASSIFICATION Ship::Class() const { return static_cast<CLASSIFICATION>(design->type); }

bool Ship::IsGroundUnit() const { return (design->type & GROUND_UNITS) ? true : false; }

bool Ship::IsStarship() const { return (design->type & STARSHIPS) ? true : false; }

bool Ship::IsDropship() const { return (design->type & DROPSHIPS) ? true : false; }

bool Ship::IsStatic() const { return design->type >= STATION; }

bool Ship::IsRogue() const { return ff_count >= 50; }
...
double Ship::RepairSpeed() const { return design->repair_speed; }

int Ship::RepairTeams() const { return design->repair_teams; }
...
int Ship::NumFlightDecks() const { return flight_decks.size(); }

FlightDeck* Ship::GetFlightDeck(int i) const
{
...
void Ship::SetCarrier(Ship* c, FlightDeck* d)
{
  carrier = c;
  dock = d;
...
void Ship::ToggleGear()
{
  if (gear)
  {
    if (gear->GetState() == LandingGear::GEAR_UP || gear->GetState() == LandingGear::GEAR_RAISE)
      LowerGear();
    else
      RaiseGear();
  }
...
int Ship::CollidesWith(Physical& o)
{
  ...
  for (int i = 0; i < detail.NumModels(detail_level); i++)
  {
    ...
    if (o.Type() == SIM_SHIP)
    {
      auto o_ship = static_cast<Ship*>(&o);
...
int Ship::HitBy(Shot* shot, Point& impact)
{
  ...
  if (shot->Owner())
  {
    ...
    if (owner_design && owner_design->scale < scale)
      scale = static_cast<float>(owner_design->scale);
  }
  ...
  if (shot->IsMissile() && rep)
  {
    ...
    {
      auto seeker = static_cast<SeekerAI*>(shot->GetDirector());
  ...
  if (hit_type != HIT_NOTHING && shot->IsArmed())
  {
    ...
    if (shot->Owner())
    {
      auto s = (Ship*)shot->Owner();

      if (!IsRogue() && s->GetIFF() == GetIFF() && s->GetDirector() && s->GetDirector()->Type() < 1000)
      {
        ...
        if (Class() > DRONE && s->Class() > DRONE)
        {
          ...
          {
            auto warn = NEW RadioMessage(s, this, RadioMessage::DECLARE_ROGUE);
    ...
    if (effective_damage > 0)
    {
      if (!shot->IsBeam() && shot->Design()->damage_type == WeaponDesign::DMG_NORMAL)
        ApplyTorque(shot->Velocity() * static_cast<float>(effective_damage) * 1e-6f);
...
int Ship::CheckShotIntersection(Shot* shot, Point& ipt, Point& hpt, Weapon** wep)
{
  int hit_type = HIT_NOTHING;
  Point shot_loc = shot->Location();
  ...
  {
    auto shipAI = static_cast<ShipAI*>(dir);
  ...
  {
    auto s = static_cast<Solid*>(detail.GetRep(detail_level, i));
    if (s)
    {
      if (easy)
...
void Ship::InflictNetDamage(double damage, Shot* shot)
{
  if (damage > 0 && !IsNetObserver())
  {
    ...
    constexpr double MAX_SHAKE = 7;

    if (shake < MAX_SHAKE)
      shake += static_cast<float>(newshake);
    if (shake > MAX_SHAKE)
      shake = static_cast<float>(MAX_SHAKE);
  }
}

void Ship::InflictNetSystemDamage(System* system, double damage, BYTE dmg_type)
{
  if (system && damage > 0 && !IsNetObserver())
  {
    ...
    if (dmg_normal || system->IsPowerCritical() && dmg_emp)
    {
      ...
      if (system->GetExplosionType() && (avail - system->Availability()) >= 50)
      {
        ...
        sim->CreateExplosion(system->MountLocation(), Velocity() * 0.7f, system->GetExplosionType(), 0.2f * scale,
                             scale, region, this, system);
      }
    }
  }
}

void Ship::SetNetSystemStatus(System* system, int status, int power, int reactor, double avail)
{
  if (system && !IsNetObserver())
  {
    ...
    if (system->GetSourceIndex() != reactor)
    {
      ...
      {
        auto reac = static_cast<PowerSource*>(s);
    ...
    if (system->Status() != status)
    {
      if (status == System::MAINT)
      {
        ...
        while (++comp)
        {
          ...
          {
            if (c->SpareCount() && c->ReplaceTime() <= 300 && (c->Availability() < 50 || c->ReplaceTime() < c->
              RepairTime()))
              c->Replace();
...
void Ship::CheckFriendlyFire()
{
  ...
  friendly_fire_time = Game::GameTime() + static_cast<DWORD>(Random(0, 500));
}
...
int Ship::GetOrigElementIndex() const { return orig_elem_index; }
...
void Ship::SetElement(Element* e)
{
  ...
  if (element)
  {
    ...
    if (combat_unit)
      integrity = static_cast<float>(design->integrity - combat_unit->GetSustainedDamage());
...
void Ship::AddNavPoint(Instruction* pt, Instruction* after)
{
...
void Ship::SetAutoNav(bool engage)
{
  if (navsys)
  {
...
void Ship::CommandMode()
{
  ...
  {
    auto msg = "Captain on the bridge";
    auto vox = NEW RadioVox(0, "1", msg);
  ...
  {
    auto msg = "Exec, you have the conn";
    auto vox = NEW RadioVox(0, "1", msg);
...
void Ship::SetNavptStatus(Instruction* navpt, int status)
{
  if (navpt && navpt->Status() != status)
  {
    if (status == Instruction::COMPLETE)
    {
      if (navpt->Action() == Instruction::ASSAULT)
        DebugTrace("Completed Assault\n");

      else if (navpt->Action() == Instruction::STRIKE)
        DebugTrace("Completed Strike\n");
    }

    navpt->SetStatus(status);

    if (status == Instruction::COMPLETE)
      sim->ProcessEventTrigger(MissionEvent::TRIGGER_NAVPT, 0, Name().c_str(), GetNavIndex(navpt));
...
void Ship::SetTarget(SimObject* targ, System* sub, bool from_net)
{
  if (targ && targ->Type() == SIM_SHIP)
  {
    auto targ_ship = static_cast<Ship*>(targ);
  ...
  if (target != targ)
  {
    ...
    if (sim && target)
      sim->ProcessEventTrigger(MissionEvent::TRIGGER_TARGET, 0, target->Name().c_str());
  }
  ...
  // track engagement:
  if (target && target->Type() == SIM_SHIP)
  {
    Element* elem = GetElement();
    Element* tgt_elem = static_cast<Ship*>(target)->GetElement();
...
void Ship::CycleSubTarget(int dir)
{
  if (!target || target->Type() != SIM_SHIP)
    return;

  auto tgt = static_cast<Ship*>(target);

  if (tgt->IsDropship())
    return;

  System* subtgt = nullptr;

  ListIter<System> sys = tgt->Systems();

  if (dir > 0)
  {
    int latch = (subtarget == nullptr);
    while (++sys)
    {
...
void Ship::ExecFrame(double seconds)
{
  ...
  if (flight_phase == LAUNCH || (flight_phase == TAKEOFF && AltitudeAGL() > Radius()))
    SetFlightPhase(ACTIVE);

  if (transition_time > 0)
  {
    transition_time -= static_cast<float>(seconds);
...
void Ship::LaunchProbe()
{
  ...
  if (probe)
  {
    sensor_drone = static_cast<Drone*>(probe->Fire());
...
void Ship::ExecNavFrame(double seconds)
{
  ...
  if (navsys)
  {
    ...
    if (navsys->AutoNavEngaged())
    {
      ...
      {
        auto navai = static_cast<NavAI*>(dir);
...
void Ship::ExecEvalFrame(double seconds)
{
  ...
  if (life == 0 || integrity < 1)
    return;

  constexpr DWORD EVAL_FREQUENCY = 1000;   // once every second
  static DWORD last_eval_frame = 0;      // one ship per game frame

  if (element && element->NumObjectives() > 0 && Game::GameTime() - last_eval_time > EVAL_FREQUENCY && last_eval_frame
    != Game::Frame())
  {
...
void Ship::ExecPhysics(double seconds)
{
  ...
  {
    thrust = static_cast<float>(Thrust(seconds));
    SetupAgility();
    ...
    if (IsAirborne())
    {
      ...
      if (seconds > 0)
        g_force = static_cast<float>(dv * cam.vup() / seconds) / 9.8f;
    }

    else if (IsDying() || flight_model < 2)
...
void Ship::ExecSystems(double seconds)
{
  ...
  for (i = 0; i < weapons.size(); i++)
  {
    ...
    if (w_group->GetTrigger() && w_group->GetFiringOrders() == Weapon::MANUAL)
    {
      ...
      if (sensor && gun->Guided() && !gun->Design()->beam && !gun_tgt)
        gun->SetTarget(sensor->AcquirePassiveTargetForMissile(), nullptr);
  ...
  if (winchester_cycle)
  {
    ...
    while (secondary != old_secondary)
    {
      ...
      if (missile && missile->CanTarget(GROUND_UNITS))
        CycleSecondary();
  ...
  if (shieldRep)
  {
    auto solid = static_cast<Solid*>(rep);
  ...
  if (cockpit)
  {
    ...
    Point cpos = cam.Pos() + cam.vrt() * bridge_vec.x + cam.vpn() * bridge_vec.y + cam.vup() * bridge_vec.z;
  ...
  else
  {
    ...
    else
    {
      if (v < 250)
      {
        ...
        dp += static_cast<float>(dp_acc * seconds);
        dy += static_cast<float>(dy_acc * seconds);
      }
...
Graphic* Ship::Cockpit() const { return cockpit; }
...
bool Ship::Update(SimObject* obj)
{
  ...
  if (obj->Type() == SIM_SHOT || obj->Type() == SIM_DRONE)
  {
    auto s = static_cast<Shot*>(obj);
...
void Ship::SetTransition(double trans_time, int trans_type, const Point& trans_loc)
{
  transition_time = static_cast<float>(trans_time);
...
void Ship::DropOrbit()
{
  if (IsDropship() && transition_type == TRANSITION_NONE && !IsAirborne())
  {
    ...
    if (dst_rgn && dst_rgn->GetOrbitalRegion()->Primary() == GetRegion()->GetOrbitalRegion()->Primary())
    {
...
void Ship::DropCam(double time, double range)
{
  transition_type = TRANSITION_DROP_CAM;

  if (time > 0)
    transition_time = static_cast<float>(time);
  else
    transition_time = 10.0f;
...
void Ship::CompleteTransition()
{
  ...
  switch (old_type)
  {
    ...
    case TRANSITION_DROP_CAM: default:
      return;
    ...
    case TRANSITION_DROP_ORBIT:
    {
      ...
      ShipStats* stats = ShipStats::Find(Name().c_str());
...
double Ship::AltitudeMSL() const { return Location().y; }

double Ship::AltitudeAGL() const
{
  if (altitude_agl < -1000)
  {
    auto pThis = (Ship*)this; // cast-away const
    Point loc = Location();
...
double Ship::GForce() const { return g_force; }

WeaponGroup* Ship::FindWeaponGroup(std::string_view name)
{
...
void Ship::SelectWeapon(int n, int w)
{
  if (n < weapons.size())
...
void Ship::SetMissileEta(int id, int eta)
{
  int index = -1;
  ...
  if (index >= 0 && index < 4)
  {
    ...
    missile_eta[index] = static_cast<BYTE>(eta);
  }
}
...
void Ship::CycleFLCSMode()
{
  ...
  {
    case FLCS_MANUAL:
      SetFLCSMode(FLCS_HELM);
      break;
    case FLCS_AUTO:
      SetFLCSMode(FLCS_MANUAL);
      break;
    case FLCS_HELM:
      SetFLCSMode(FLCS_AUTO);
      break;

    default:
      if (IsStarship())
        flcs_mode = static_cast<BYTE>(FLCS_HELM);
      else
        flcs_mode = static_cast<BYTE>(FLCS_AUTO);
      break;
  }
...
{
  flcs_mode = static_cast<BYTE>(mode);

  if (IsAirborne())
    flcs_mode = static_cast<BYTE>(FLCS_MANUAL);
  ...
  if (dir && dir->Type() < SteerAI::SEEKER)
  {
    ...
    {
      case FLCS_MANUAL:
        director_info = Game::GetText("flcs.manual");
        break;
      case FLCS_AUTO:
        director_info = Game::GetText("flcs.auto");
        break;
      case FLCS_HELM:
        director_info = Game::GetText("flcs.helm");
        break;
      default:
        director_info = Game::GetText("flcs.fault");
        break;
    }
...
int Ship::GetFLCSMode() const { return flcs_mode; }
...
void Ship::SetTransX(double t)
{
  ...
  if (thruster)
    limit = static_cast<float>(thruster->TransXLimit());

  trans_x = static_cast<float>(t);
  ...
  if (trans_x)
  {
    ...
    if (vmag > vlimit)
    {
      if (trans_x > 0 && vfwd > 0 || trans_x < 0 && vfwd < 0)
        trans_x *= static_cast<float>(pow(vlimit, 4) / pow(vmag, 4));
    }
  }
}
...
void Ship::SetTransY(double t)
{
  ...
  if (thruster)
    limit = static_cast<float>(thruster->TransYLimit());

  trans_y = static_cast<float>(t);
  ...
  if (trans_y)
  {
    ...
    if (vmag > vlimit)
    {
      ...
      if (trans_y > 0 && vfwd > 0 || trans_y < 0 && vfwd < 0)
        trans_y *= static_cast<float>(pow(vlimit, 4) / pow(vmag, 4));
    }
  }
}
...
void Ship::SetTransZ(double t)
{
  ...
  if (thruster)
    limit = static_cast<float>(thruster->TransZLimit());

  trans_z = static_cast<float>(t);
  ...
  if (trans_z)
  {
    ...
    if (vmag > vlimit)
    {
      if (trans_z > 0 && vfwd > 0 || trans_z < 0 && vfwd < 0)
        trans_z *= static_cast<float>(pow(vlimit, 4) / pow(vmag, 4));
    }
  }
}
...
void Ship::SetHelmHeading(double h)
{
  ...
  helm_heading = static_cast<float>(h);
}
...
void Ship::SetHelmPitch(double p)
{
  ...
  helm_pitch = static_cast<float>(p);
}
...
void Ship::ApplyHelmPitch(double p) { SetHelmPitch(helm_pitch - p * PI / 4); }

void Ship::ApplyPitch(double p)
{
  if (flight_model == 0)
...
bool Ship::FireDecoy()
{
  ...
  return drone != nullptr;
}

void Ship::AddActiveDecoy(Drone* drone)
{
  if (drone)
  {
    Observe(drone);
...
Weapon* Ship::GetDecoy() const { return decoy; }

List<Shot>& Ship::GetActiveDecoys() { return decoy_list; }

List<Shot>& Ship::GetThreatList() { return threat_list; }
...
{
  if (sensor)
    sensor->SetMode(static_cast<Sensor::Mode>(mode));
}
...
void Ship::LockTarget(int type, bool closest, bool hostile)
{
  if (sensor)
...
double Ship::InflictDamage(double damage, Shot* shot, int hit_type, Point impact)
{
  ...
  {
    int old_integrity = static_cast<int>((integrity + damage_applied) / design->integrity * 10);
    int new_integrity = static_cast<int>((integrity) / design->integrity * 10);
    ...
    if (new_integrity < 5 && new_integrity < old_integrity)
    {
      ...
      if (rep)
      {
        Point detonation = impact * 2.0f - Location();
...
double Ship::InflictSystemDamage(double damage, Shot* shot, Point impact)
{
  if (IsNetObserver())
    return 0;
...
int Ship::ShieldStrength() const
{
  ...
  return static_cast<int>(shield->ShieldLevel());
}

int Ship::HullStrength() const
{
  if (design)
    return static_cast<int>(Integrity() / design->integrity * 100);
...
void Ship::ExecMaintFrame(double seconds)
{
  ...
  constexpr DWORD REPAIR_FREQUENCY = 5000;  // once every five seconds
  static DWORD last_repair_frame = 0;     // one ship per game frame

  if (auto_repair && Game::GameTime() - last_repair_time > REPAIR_FREQUENCY && last_repair_frame != Game::Frame())
  {
    ...
    while (++iter)
    {
      ...
      if (sys->Status() != System::NOMINAL)
      {
        ...
        {
          auto src = static_cast<PowerSource*>(sys);
          PowerSource* dst = nullptr;
          ...
          for (int i = 0; i < reactors.size(); i++)
          {
            ...
            {
              if (!dst || (pwr->Availability() > dst->Availability() && pwr->Charge() > dst->Charge()))
                dst = pwr;
            }
          }
  ...
  if (repair_queue.size() > 0 && RepairTeams() > 0)
  {
    ...
    while (++iter && team < RepairTeams())
    {
      ...
      if (sys->Status() != System::MAINT)
      {
        ...
        if (sys->Type() == System::POWER_SOURCE && sys->Status() == System::NOMINAL)
        {
          auto src = static_cast<PowerSource*>(sys);
...
void Ship::SetNetworkControl(Director* net)
{
  net_control = net;
...
void Ship::SetControls(MotionController* m)
{
  ...
  else if (life == 0)
  {
    if (dir || navsys)
    {
      DebugTrace("Warning: dying ship '%' still has not been destroyed!\n", m_name);
      delete dir;
      dir = nullptr;
...
Color Ship::MarkerColor() const { return IFFColor(IFF_code); }
...
void Ship::SetIFF(int iff)
{
  ...
  {
    auto ai = static_cast<SteerAI*>(dir);
    ai->DropTarget();
  }
}

void Ship::SetRogue(bool r)
{
  bool rogue = IsRogue();
  ...
  if (!rogue && IsRogue())
    DebugTrace("Ship '%s' has been made rogue\n", Name());
  else if (rogue && !IsRogue())
    DebugTrace("Ship '%s' is no longer rogue\n", Name());
}
...
void Ship::SetFriendlyFire(int f)
{
  ...
  if (!rogue && IsRogue())
    DebugTrace("Ship '%s' has been made rogue with ff_count = %d\n", Name(), ff_count);
  else if (rogue && !IsRogue())
    DebugTrace("Ship '%s' is no longer rogue\n", Name());
}

void Ship::IncFriendlyFire(int f)
{
  if (f > 0)
  {
    bool rogue = IsRogue();
...
void Ship::SetEMCON(int e, bool from_net)
{
  if (e < 1)
    emcon = 1;
  else if (e > 3)
...
Instruction* Ship::GetRadioOrders() const { return radio_orders; }
...
int Ship::Value() const { return Value(design->type); }
