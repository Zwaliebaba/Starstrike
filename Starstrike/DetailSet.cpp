...
SimRegion* DetailSet::ref_rgn = nullptr;
...
DetailSet::~DetailSet() { Destroy(); }

int DetailSet::DefineLevel(double r, Graphic* g, Point* o, Point* spin_rate)
{
  if (levels < MAX_DETAIL && rep[levels].size() == 0)
  {
    ...
    if (rate.size() == 0)
    {
      if (spin_rate)
      {
        ...
        auto initial_spin = NEW Point();
        if (spin_rate->x != 0)
          initial_spin->x = Random(-PI, PI);
        if (spin_rate->y != 0)
          initial_spin->y = Random(-PI, PI);
        if (spin_rate->z != 0)
          initial_spin->z = Random(-PI, PI);
...
void DetailSet::AddToLevel(int level, Graphic* g, Point* offset, Point* spin_rate)
{
...
void DetailSet::SetLocation(SimRegion* r, const Point& p)
{
  rgn = r;
  loc = p;
}

void DetailSet::SetReference(SimRegion* r, const Point& p)
{
  ref_rgn = r;
  ref_loc = p;
}
...
Graphic* DetailSet::GetRep(int level, int n)
{
...
Point DetailSet::GetOffset(int level, int n)
{
...
Point DetailSet::GetSpin(int level, int n)
{
  if (n >= 0 && n < spin.size())
    return *(spin.at(n));
