...
Stars::Stars(int nstars)
{
  ...
  for (int i = 0; i < nstars; i++)
  {
    ...
    auto val = ColorValue(static_cast<float>(Random(0.7, 0.8)), static_cast<float>(Random(0.7, 0.8)),
                          static_cast<float>(Random(0.7, 0.8)));
    Color c = val.ToColor();
...
void Stars::Render(Video* video, DWORD flags)
{
  if (!vset || !video || (flags & RENDER_ADDITIVE) == 0)
    return;
...
static constexpr double BOUNDARY = 3000;
...
Dust::Dust(int ndust, bool b)
  : really_hidden(false),
    bright(b)
{
  radius = static_cast<float>(BOUNDARYx2);
  luminous = true;
  vset = NEW VertexSet(ndust);

  Dust::Reset(Point(0, 0, 0));
  name = "Dust";
}

Dust::~Dust() { delete vset; }

void Dust::Reset(const Point& ref)
{
  BYTE c = 0;

  for (int i = 0; i < vset->nverts; i++)
  {
    vset->loc[i] = Vec3(Random(-BOUNDARY, BOUNDARY), Random(-BOUNDARY, BOUNDARY), Random(-BOUNDARY, BOUNDARY));

    if (bright)
      c = static_cast<BYTE>(Random(96, 200));
    else
      c = static_cast<BYTE>(Random(64, 156));
...
void Dust::ExecFrame(double factor, const Point& ref)
{
  ...
  else
  {
    ...
    for (int i = 0; i < vset->nverts; i++)
    {
      ...
      if (v.x > BOUNDARY)
        v.x -= static_cast<float>(BOUNDARYx2);
      if (v.x < -BOUNDARY)
        v.x += static_cast<float>(BOUNDARYx2);
      if (v.y > BOUNDARY)
        v.y -= static_cast<float>(BOUNDARYx2);
      if (v.y < -BOUNDARY)
        v.y += static_cast<float>(BOUNDARYx2);
      if (v.z > BOUNDARY)
        v.z -= static_cast<float>(BOUNDARYx2);
      if (v.z < -BOUNDARY)
        v.z += static_cast<float>(BOUNDARYx2);
...
void Dust::Render(Video* video, DWORD flags)
{
  if (hidden || really_hidden)
    return;

  if (!vset || !video || (flags & RENDER_SOLID) == 0 || (flags & RENDER_ADD_LIGHT) != 0)
    return;
...
PlanetRep::PlanetRep(std::string_view surface_name, std::string_view glow_name, double rad, const Vec3& pos, double tscale,
                     std::string_view rngname, double minrad, double maxrad, Color atmos, std::string_view gloss_name)
  : mtl_surf(nullptr),
    mtl_limb(nullptr),
    mtl_ring(nullptr),
    star_system(nullptr)
{
  ...
  {
    DebugTrace("   invalid Planet patch - no surface texture specified\n");
    return;
  }

  DebugTrace("   constructing Planet patch {}\n", surface_name);
  name = surface_name;

  Bitmap* bmp_surf = nullptr;
  Bitmap* bmp_spec = nullptr;
  Bitmap* bmp_glow = nullptr;
  Bitmap* bmp_ring = nullptr;
  Bitmap* bmp_limb = nullptr;
  ...
  if (!glow_name.empty())
  {
    DebugTrace("   loading glow texture %s\n", glow_name);
    loader->LoadTexture(glow_name, bmp_glow, Bitmap::BMP_SOLID, true);
  }

  if (!gloss_name.empty())
  {
    DebugTrace("   loading gloss texture %s\n", gloss_name);
  ...
  {
    if (glow_name.find_first_of("light") != std::string::npos)
      mtl_surf->shader = "SimplePix/PlanetSurfNightLight";

    else if (!glow_name.empty())
  ...
  if (atmosphere != Color::Black)
  {
    ...
    DebugTrace("   loading atmospheric limb texture PlanetLimb.pcx\n");
  ...
  {
    has_ring = 1;
    radius = static_cast<float>(maxrad);
...
void PlanetRep::CreateSphere(double radius, int nrings, int nsections, double minrad, double maxrad, int rsections,
                             double tscale)
{
  ...
  {
    DebugTrace("WARNING: insufficient memory for planet '%s'\n", name);
    return;
  }
  ...
  for (i = 0; i < nrings + 3; i++)
  {
    ...
    for (j = 0; j < sect_verts; j++)
    {
      ...
      vset->tu[n] = static_cast<float>(tscale * (1 - (phi / (2.0 * PI))));
      vset->tv[n] = static_cast<float>(tscale * v);
...
int PlanetRep::CheckRayIntersection(Point Q, Point w, double len, Point& ipt, bool treat_translucent_polys_as_solid)
{
  ...
  if (delta0.length() > radius)
  {
    ...
    if (delta1 * delta2 < 0 || delta1.length() > delta2.length())
      return 0;
  }
...
void PlanetRep::SetDaytime(bool d)
{
  ...
  {
    if (mtl_surf)
      mtl_surf->blend = Material::MTL_ADDITIVE;
    if (mtl_ring)
      mtl_ring->blend = Material::MTL_ADDITIVE;
  }
...
void PlanetRep::SetStarSystem(StarSystem* system) { star_system = system; }

void PlanetRep::Render(Video* video, DWORD flags)
{
