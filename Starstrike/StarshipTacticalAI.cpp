...
constexpr double STARSHIP_TACTICAL_DROP_TIME = 15;
    ...
    bugout(false)
{
  ...
  switch (ai_level)
  {
    ...
    case 0:
      ...
      drop_time = STARSHIP_TACTICAL_DROP_TIME + Random(0, STARSHIP_TACTICAL_DROP_TIME);
      break;
  }
}
  ...
  while (++iter)
  {
    ...
    if (contact->Threat(ship) && (Game::GameTime() - contact->AcquisitionTime()) > THREAT_REACTION_TIME)
    {
      ...
      else
      {
        ...
        if (c_ship && c_ship->Class() != Ship::FREIGHTER && c_ship->Class() != Ship::FARCASTER)
        {
...
void StarshipTacticalAI::CheckBugOut(Ship* c_ship, double rng)
{
  ...
  if (dst)
  {
    ...
    if (escort)
    {
      auto msg = NEW RadioMessage(escort, ship, RadioMessage::QUANTUM_TO);
      if (msg)
      {
