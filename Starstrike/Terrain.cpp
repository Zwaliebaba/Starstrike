...
void Terrain::BuildTerrain()
{
  ...
  if (water_texture)
  {
    ...
    for (i = 0; i < 6; i++)
    {
      water[i] = NEW Water();
      int n = (1 << i) + 1;
      water[i]->Init(n, static_cast<float>(scale * patch_size), 90.0f);
    }
  }
...
void Terrain::BuildNormals()
{
  ...
  {
    for (x = 1; x < w - 1; x++)
      alt[y * w + x] = static_cast<BYTE>(pix[y * w + x].Red());
  }
  ...
  for (y = 1; y < h - 1; y++)
  {
    for (x = 1; x < w - 1; x++)
    {
      double dx = (alt[y * w + (x - 1)] - alt[y * w + (x + 1)]) * scale + (alt[(y - 1) * w + (x - 1)] - alt[(y - 1) * w
        + (x + 1)]) * scale * 0.5 + (alt[(y + 1) * w + (x - 1)] - alt[(y + 1) * w + (x + 1)]) * scale * 0.5;

      double dy = (alt[(y - 1) * w + x] - alt[(y + 1) * w + x]) * scale + (alt[(y - 1) * w + (x - 1)] - alt[(y + 1) * w
        + (x - 1)]) * scale * 0.5 + (alt[(y - 1) * w + (x + 1)] - alt[(y + 1) * w + (x + 1)]) * scale * 0.5;
      ...
      tnorm->x = static_cast<BYTE>(norm.x * 127 + 128);
      tnorm->y = static_cast<BYTE>(norm.y * 127 + 128);
      tnorm->z = static_cast<BYTE>(norm.z * 127 + 128);
...
void Terrain::SelectDetail(Projector* projector)
{
  if (!patches)
    return;
    ...
    for (x = 0; x < subdivisions; x++)
    {
      ...
      else
      {
        ...
        for (int level = 1; level <= detail_level; level++)
        {
          ...
          if (projector->ApparentRadius(loc, static_cast<float>(feature_size)) > threshold)
            ndetail = level;
        }
      }
...
double Terrain::Height(double x, double y) const
{
  ...
  if (patches)
  {
    int ix = static_cast<int>(floor(x / (patch_size * scale)));
    int iy = static_cast<int>(floor(y / (patch_size * scale)));
...
bool Terrain::IsFirstPatch(TerrainPatch* p) const { return (patches && *patches == p); }
