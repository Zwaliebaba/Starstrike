...
class NetAdminLogin : public NetAdminServlet
{
  public:
    NetAdminLogin() {}
    ~NetAdminLogin() override {}

    bool DoGet(HttpRequest& request, HttpResponse& response) override
    {
      NetServerConfig* config = NetServerConfig::GetInstance();

      std::string admin_name = "system";
      std::string admin_pass = "manager";
      ...
      std::string name = request.GetParam("user");
      std::string pass = request.GetParam("pass");
      ...
      if (CheckUser(request, response))
      {
        ...
        response.SetContent(GetHead("Login") + "<body><br>You are already logged in.<br>" + GetBodyClose());
      }
      ...
      else if (name == admin_name && pass == admin_pass)
      {
        ...
        response.SetContent(GetHead("Login") + "<body><br>You have successfully logged in.<br>" + GetBodyClose());
      }
      ...
      else
      {
        ...
        response.SetContent(
          GetHead("Login") + GetTitleBar({}, "onLoad=\"self.focus();document.loginForm.user.focus();\"") +
    ...
    {
      std::string content = "  <table border=\"0\" cellspacing=\"0\" cellpadding=\"4\" align=\"left\" width =\"100%\">\n\
    <tr>\n\
    <td width=\"100\">&nbsp;&nbsp;</td>\n\
    ...
    bool DoGet(HttpRequest& request, HttpResponse& response) override
    {
      if (CheckUser(request, response))
      {
        ...
        if (!completed)
        {
          ...
          response.SetContent(GetHead("Login") + "<body><br>Unknown Action.<br>" + GetBodyClose());
        }
      }
  ...
  public:
    ...
    bool DoGet(HttpRequest& request, HttpResponse& response) override
    {
      if (CheckUser(request, response))
      {
        ...
        response.SetContent(GetHead("User List") + GetTitleBar() + GetContent() + GetBodyClose());
      }
    ...
    std::string GetContent() override
    {
      std::string content = "<script LANGUAGE=\"JavaScript\">\n\
<!--\n\
function doConfirm() {\n\
...
<table border=\"0\"  width=\"95%\">\n\
    ...
    <td nowrap valign=\"middle\" align=\"left\">\n\
      ...
      content += "  <table border=\"0\"  width=\"95%\" class=\"std\">\n\
    <tr>\n\
  ...
  public:
    ...
    bool DoGet(HttpRequest& request, HttpResponse& response) override
    {
      if (CheckUser(request, response))
      {
        ...
        response.SetContent(
          GetHead("User List") + GetTitleBar() + "<div class=\"content\">User Banned.<br></div>" + GetBodyClose());
      }
...
static NetAdminServer* net_Admin_server = nullptr;

NetAdminServer* NetAdminServer::GetInstance(WORD port)
{
...
NetAdminServer::NetAdminServer(WORD port)
  : HttpServletExec(port) { http_server_name = std::string("Starshatter NetAdminServer ") + g_versionInfo; }

NetAdminServer::~NetAdminServer()
{
  if (net_Admin_server == this)
    net_Admin_server = nullptr;
}

HttpServlet* NetAdminServer::GetServlet(HttpRequest& request)
{
  ...
  if (path.starts_with("/ban"))
    return NEW NetAdminBanUser;

  return NEW NetAdminServlet;
}

void NetAdminServer::AddChat(NetUser* user, std::string_view msg)
{
  if (user && !msg.empty())
  {
    NetLobbyServer* lobby = NetLobbyServer::GetInstance();
...
void NetAdminServer::AddUser(NetUser* user)
{
  if (user && !admin_users.contains(user))
    admin_users.append(user);
}

void NetAdminServer::DelUser(NetUser* user)
{
  if (user)
  {
...
int NetAdminServer::NumUsers() { return admin_users.size(); }

List<NetUser>& NetAdminServer::GetUsers() { return admin_users; }

bool NetAdminServer::HasHost()
{
  bool result = false;
...
NetUser* NetAdminServer::FindUserBySession(std::string id)
{
...
void NetAdminServer::DoSyncedCheck()
{
  ListIter<NetUser> iter = admin_users;
  while (++iter)
  {
...
bool NetAdminServlet::DoGet(HttpRequest& request, HttpResponse& response)
{
  if (CheckUser(request, response))
  {
    if (request.URI() == "/home")
    ...
    response.SetContent(GetHead() + GetTitleBar(GetStatLine()) + GetContent() + GetBodyClose());
  }

  return true;
}

bool NetAdminServlet::CheckUser(HttpRequest& request, HttpResponse& response)
{
  if (!user)
  {
    if (session)
...
std::string NetAdminServlet::GetCSS()
{
  return "body      { font-family:arial,helvetica,sans-serif; color:black; background-color:white }\n\
...
std::string NetAdminServlet::GetHead(std::string_view title)
{
...
std::string NetAdminServlet::GetBody() { return GetTitleBar(GetStatLine()) + GetContent() + GetBodyClose(); }

std::string NetAdminServlet::GetTitleBar(std::string_view statline, std::string_view onload)
{
  std::string bar = "<body ";
...
std::string NetAdminServlet::GetStatLine()
{
  NetServerConfig* config = NetServerConfig::GetInstance();

  std::string line = "      <table width=\"100%\" cellspacing=\"0\" cellpadding=\"2\" border=\"0\">\n\
        <tr>\n\
  ...
  if (lobby)
  {
    ...
    {
      default: case NetServerInfo::OFFLINE:
        line += "Offline";
        break;
      case NetServerInfo::LOBBY:
        line += "Lobby";
        break;
      case NetServerInfo::BRIEFING:
        line += "Briefing";
        break;
      case NetServerInfo::ACTIVE:
        line += "Active";
        break;
      case NetServerInfo::DEBRIEFING:
        line += "Debriefing";
        break;
      case NetServerInfo::PERSISTENT:
        line += "PERSISTENT";
        break;
    }
  }
...
std::string NetAdminServlet::GetContent()
{
  std::string content = "<script LANGUAGE=\"JavaScript\">\n\
<!--\n\
function doConfirm() {\n\
...
std::string NetAdminServlet::GetBodyClose() { return "\n\n</body>\n</html>\n"; }

std::string NetAdminServlet::GetCopyright()
{
  return
    "<br><span class=\"copy\">&nbsp;&nbsp;&nbsp;&nbsp;Copyright &copy; 1997-2004 Destroyer Studios.  All rights reserved.</span><br>";
}
