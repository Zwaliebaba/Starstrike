    ...
    pkg_index(-1)
{
  ...
  mission_system = nullptr;
  mission_sector = nullptr;
  mission_time_start = nullptr;
  mission_time_target = nullptr;
  mission_time_target_label = nullptr;
...
void MsnDlg::RegisterMsnControls(FormWindow* win)
{
  ...
  sit_button = static_cast<Button*>(win->FindControl(900));
  pkg_button = static_cast<Button*>(win->FindControl(901));
  nav_button = static_cast<Button*>(win->FindControl(902));
  wep_button = static_cast<Button*>(win->FindControl(903));
  commit = static_cast<Button*>(win->FindControl(1));
  cancel = static_cast<Button*>(win->FindControl(2));
}
...
void MsnDlg::ShowMsnDlg()
{
  ...
  if (campaign)
  {
    ...
    if (!mission)
      DebugTrace("ERROR - MsnDlg::Show() no mission.\n");
    else { DebugTrace("MsnDlg::Show() mission id = %d name = '%s'\n", mission->Identity(), mission->Name()); }
  }
  else
    DebugTrace("ERROR - MsnDlg::Show() no campaign.\n");
  ...
  if (mission)
  {
    ...
    if (element)
    {
      ...
      while (++navpt)
      {
        ...
        if (navpt->Speed() > 0)
          etr = static_cast<int>(dist / navpt->Speed());
        else
          etr = static_cast<int>(dist / 500);
...
void MsnDlg::OnTabButton(AWEvent* event)
{
...
void MsnDlg::OnCommit(AWEvent* event)
{
...
void MsnDlg::OnCancel(AWEvent* event)
{
  Starshatter* stars = Starshatter::GetInstance();
