...
{
    START_PROFILE( g_app->m_profiler, "Render GameCursor 2D" );
    ...
    else if( !frame.inLocation )
    {
        ...
        if( !locAvailable ) cursorDisabled->Render( frame.screenX, frame.screenY );
    }
    else if( frame.inLocation )
    {
        if( g_app->m_taskManagerInterface->m_visible )
        {
            ...
            if( frame.somethingHighlighted )
            {
                ...
                m_cursorSelection->Render( frame.highlightedScreenX, g_app->m_renderer->ScreenH() - frame.highlightedScreenY );
            }
        }
        else if( frame.currentTask &&
                 frame.currentTask->m_state == Task::StateStarted &&
                 frame.currentTask->m_type != GlobalResearch::TypeOfficer &&
                 !frame.somethingHighlighted )
        {
        ...
        {
            if( frame.somethingHighlighted &&
                !(frame.somethingSelected && frame.highlightedId.GetUnitId() == UNIT_BUILDINGS) )
            {
            ...
            if( frame.somethingSelected && frame.selectedId.GetUnitId() != UNIT_BUILDINGS )
            {
                ...
                if( frame.radarDishEntranceValid )
                {
                    ...
                    cursorPlacement->Render( frame.radarDishEntranceScreenX, g_app->m_renderer->ScreenH() - frame.radarDishEntranceScreenY );
                }
            }
    ...
    if( !frame.suppressStandardCursor &&
    ...
    END_PROFILE( g_app->m_profiler, "Render GameCursor 2D" );
}


void GameCursor2D::RenderStandardCursor( float _screenX, float _screenY )
{
...
void GameCursor2D::RenderSelectionArrows()
{
    ...
    if( !frame.selectionArrowsValid ) return;
    ...
    {
        // _pos is onscreen
        LegacyVector3 toCam = g_app->m_camera->GetPos() - frame.selectedWorldPos;
        float camDist = toCam.Mag();
        m_updatedArrowBoost -= g_advanceTime * 0.4f;

        float distanceOut = 1000 / sqrtf( camDist );
        float alpha = min( m_updatedArrowBoost, 0.9f );
    ...
    else
    {
        ...
        RenderSelectionArrow( frame.selectedEdgeX, screenH - frame.selectedEdgeY,
                              frame.selectedEdgeNormal.x, frame.selectedEdgeNormal.y,
                              triSize * 1.5f, 0.9f );
    }
}
