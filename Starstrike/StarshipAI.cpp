    ...
    tgt_point_defense(false)
{
  ...
  else { tactical = NEW StarshipTacticalAI(this); }
...
void StarshipAI::FindObjective()
{
  ...
  bool hold = order == RadioMessage::WEP_HOLD || order == RadioMessage::FORM_UP;

  bool form = hold || (!order && !target) || (farcaster);
  ...
  if (form && element_index > 1)
  {
    ...
    if (navpt && navpt->Action() == Instruction::LAUNCH) { FindObjectiveNavPoint(); }
    else
    {
      navpt = 0;
  ...
  else { objective = Point(); }

  // transform into camera coords:
  objective = Transform(obj_w);
}

void StarshipAI::Navigator()
{
  // signifies this ship is a dead hulk:
  if (ship && ship->Design()->auto_roll < 0)
  {
  ...
  if (farcaster && distance < 25e3) { accumulator = SeekTarget(); }
  else
  {
...
void StarshipAI::HelmControl()
{
  // signifies this ship is a dead hulk:
  if (ship && ship->Design()->auto_roll < 0) { return; }
  ...
  else
  {
    ...
    if (other || target || ward || s_threat || navpt || patrol || farcaster || element_index > 1)
    {
      ...
      if (elem->Type() == Mission::FLIGHT_OPS)
      {
        ...
        if (ship->NumInbound() > 0) { ship->SetHelmHeading(ship->CompassHeading()); }
      }

      else if (accumulator.pitch > 60 * DEGREES) { ship->SetHelmPitch(60 * DEGREES); }

      else if (accumulator.pitch < -60 * DEGREES) { ship->SetHelmPitch(-60 * DEGREES); }

      else { ship->SetHelmPitch(accumulator.pitch); }
    }
    else { ship->SetHelmPitch(0); }
  }
...
void StarshipAI::ThrottleControl()
{
  // signifies this ship is a dead hulk:
  if (ship && ship->Design()->auto_roll < 0) { return; }
  ...
  {           // escort, match speed of ward
    ...
    if (speed > 0)
    {
      ...
      else if (ship_speed < speed - 10) { throttle = old_throttle + 1; }
    }
    else
    {
      throttle = 0;
      ...
      if (!farcaster)
      {
        ...
        while (++s && !farcaster)
        {
          if (s->GetFarcaster())
          {
            ...
            if (dest && dest->GetRegion() == nav_rgn) { farcaster = s->GetFarcaster(); }
          }
        }
      }
...
Steer StarshipAI::AvoidCollision()
{
...
void StarshipAI::FireControl()
{
  ...
  if (identify)
  {
    if (fabs(ship->GetHelmHeading() - ship->CompassHeading()) < 10 * DEGREES)
    {
      ...
      if (contact && !contact->ActLock()) { if (!ship->GetProbe()) { ship->LaunchProbe(); } }
    }
  ...
  if (rumor && !target && ship->GetProbeLauncher() && !ship->GetProbe())
  {
    ...
    if (dx < 10 * DEGREES && dy < 10 * DEGREES && rmr.z > 0) { ship->LaunchProbe(); }
  }
  ...
  if (ship->Class() == Ship::CORVETTE || ship->Class() == Ship::FRIGATE)
  {
    ...
    while (++iter)
    {
      ...
      while (++w_iter)
      {
        ...
        else { weapon->SetFiringOrders(Weapon::POINT_DEFENSE); }
      }
    }
  }
  ...
  else
  {
    ...
    while (++iter)
    {
      ...
      if (weapon->GetDesign()->target_type & Ship::DROPSHIPS)
      {   // anti-air weapon?
      ...
      {                               // torpedoes
        ...
        if (target && target->GetRegion() == ship->GetRegion())
        {
          ...
          if (range < weapon->GetDesign()->max_range * 0.9 && !AssessTargetPointDefense())
            weapon->SetFiringOrders(Weapon::AUTO);
...
System* StarshipAI::SelectSubtarget()
{
...
bool StarshipAI::AssessTargetPointDefense()
{
...
Point StarshipAI::Transform(const Point& point) { return point - self->Location(); }

Steer StarshipAI::Seek(const Point& point)
{
...
Steer StarshipAI::Flee(const Point& point)
{
...
Steer StarshipAI::Avoid(const Point& point, float radius)
{
  Steer result = Seek(point);
