...
class Weapon : public System, public SimObserver
{
  public:
    ...
    Weapon(WeaponDesign* d, int nmuz, const Vec3* muzzles, float az = 0.0f, float el = 0.0f);
    ...
    SimObject* GetTarget() const { return m_target; }
    System* GetSubTarget() const { return m_subtarget; }
    ...
    void SetAzimuth(double a) { aim_azimuth = static_cast<float>(a); }
    double GetAzimuth() const { return aim_azimuth; }
    void SetElevation(double e) { aim_elevation = static_cast<float>(e); }
    double GetElevation() const { return aim_elevation; }

    void SetRestAzimuth(double a) { aim_az_rest = static_cast<float>(a); }
    double GetRestAzimuth() const { return aim_az_rest; }
    void SetRestElevation(double e) { aim_el_rest = static_cast<float>(e); }
    double GetRestElevation() const { return aim_el_rest; }

    void SetAzimuthMax(double a) { aim_az_max = static_cast<float>(a); }
    double GetAzimuthMax() const { return aim_az_max; }
    void SetAzimuthMin(double a) { aim_az_min = static_cast<float>(a); }
    double GetAzimuthMin() const { return aim_az_min; }

    void SetElevationMax(double e) { aim_el_max = static_cast<float>(e); }
    double GetElevationMax() const { return aim_el_max; }
    void SetElevationMin(double e) { aim_el_min = static_cast<float>(e); }
    double GetElevationMin() const { return aim_el_min; }

    void SetGroup(std::string_view n) { group = n; }
  ...
  protected:
    ...
    bool CanLockPoint(const Point& test, double& az, double& el, Point* obj = nullptr);
