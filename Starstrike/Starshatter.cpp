...
Mission* quick_mission = nullptr;

static Mission* current_mission = nullptr;
static Mission* cutscene_mission = nullptr;
    ...
    chat_mode(0)
{
  ...
  app_name = L"Starshatter: The Gathering Storm";
...
bool Starshatter::Init(HINSTANCE hi, HINSTANCE hpi, LPSTR cmdline, int nCmdShow)
{
  if (strstr(cmdline, "-win") || strstr(cmdline, "-dbg"))
  {
    if (video_settings)
    {
      ...
      DebugTrace("   STARSHATTER RUNNING IN WINDOW MODE\n");
    }
  }
...
void Starshatter::RequestChangeVideo() { req_change_video = true; }
...
{
  if (m_gameMode != MENU_MODE && m_gameMode != CMPN_MODE)
    return;
  ...
  Campaign* c = nullptr;
  std::string saved = CampaignSaveGame::GetResumeFile();
...
{
  if (menuscreen && m_gameMode == MENU_MODE)
  {
    ...
    {
      load_activity = GetText("Starshatter.load.tac-ref");
      load_progress = 1;
      catalog_index = 0;
    }
    else
...
void Starshatter::SetGameMode(int m)
{
  if (m_gameMode == m)
    return;
  ...
  if (m >= MENU_MODE && m <= EXIT_MODE)
    DebugTrace(">>> Starshatter::SetGameMode({}) ({})\n", m, mode_name[m]);
  else
    DebugTrace(">>> Starshatter::SetGameMode({}) (UNKNOWN MODE)\n", m);
  ...
  if (m == CLOD_MODE || m == PREP_MODE || m == LOAD_MODE)
  {
    load_step = 0;
    load_progress = 0;
    load_activity = GetText("Starshatter.load.general");
    paused = true;
  }
  ...
  else if (m == CMPN_MODE)
  {
    load_step = 0;
    load_progress = 100;
    load_activity = GetText("Starshatter.load.complete");
    paused = false;
  }

  else if (m == PLAY_MODE)
  {
    DebugTrace("  Starting Game...\n");
    ...
    DebugTrace("  Stardate: %.1f\n", StarSystem::GetBaseTime());
  }

  else if (m == PLAN_MODE)
  {
    if (m_gameMode == PLAY_MODE)
    {
      DebugTrace("  Returning to Plan Mode...\n");
      if (soundcard)
  ...
  {
    DebugTrace("  Returning to Main Menu...\n");

    if (m_gameMode == PLAN_MODE || m_gameMode == PLAY_MODE)
    {
      if (soundcard)
        soundcard->StopSoundEffects();
  ...
  if (m == EXIT_MODE)
  {
    DebugTrace("  Shutting Down (Returning to Windows)...\n");
    ...
    DebugTrace("  Stardate: %.1f\n", StarSystem::GetBaseTime());
    DebugTrace("  Bitmap Cache Footprint: {} KB\n", Bitmap::CacheMemoryFootprint() / 1024);
...
void Starshatter::InstantiateMission()
{
  ...
  if (sim)
  {
    ...
    sim->ExecMission();
    sim->SetTestMode(test_mode && !dynamic);
...
{
  int k = Keyboard::KeyDownMap(action);
  return k;
}
...
void Starshatter::UpdateWorld()
{
  ...
  if (m_gameMode == PLAY_MODE || InCutscene())
  {
    if (cam_dir)
    {
    ...
    Sim* sim = Sim::GetSim();
    SimRegion* rgn = sim ? sim->GetActiveRegion() : nullptr;
...
void Starshatter::GameState()
{
  if (splash)
  {
    ...
    if (splash->Done())
    {
      splash = nullptr; // this will get deleted along with gamewin
      splash_index++;
  ...
  else if (m_gameMode == MENU_MODE)
  {
    ...
    if (MusicDirector::GetInstance() && MusicDirector::GetInstance()->GetMode() != MusicDirector::CREDITS)
      MusicDirector::SetMode(MusicDirector::MENU);

    DoMenuScreenFrame();
  }

  else if (m_gameMode == CLOD_MODE || m_gameMode == PREP_MODE || m_gameMode == LOAD_MODE)
  {
    if (menuscreen)
      menuscreen->Hide();
...
void Starshatter::DoLoadScreenFrame()
{
  ...
  if (m_gameMode == CLOD_MODE)
  {
    ...
    {
      case 0:
        load_activity = GetText("Starshatter.load.campaign");
        load_progress = 1;
        catalog_index = 0;
        break;
      ...
      case 1:
        ...
        {
          load_activity = GetText("Starshatter.load.start");
          load_progress = 80;
        }
        break;
      ...
      default:
        ...
        {
          load_activity = GetText("Starshatter.load.ready");
          load_progress = 100;
          SetGameMode(CMPN_MODE);
        }
        break;
  ...
  else
  {
    ...
    {
      case 0:
        load_activity = GetText("Starshatter.load.drives");
        load_progress = 0;
        break;
      ...
      case 1:
        ...
        load_activity = GetText("Starshatter.load.explosions");
        load_progress = 5;
        break;

      case 2:
        Explosion::Initialize();
        load_activity = GetText("Starshatter.load.systems");
        load_progress = 10;
        break;
      ...
      case 3:
        ...
        NavLight::Initialize();
        load_activity = GetText("Starshatter.load.ships");
        load_progress = 15;
        break;
      ...
      case 4:
        Ship::Initialize();
        Shot::Initialize();
        load_activity = GetText("Starshatter.load.hud");
        load_progress = 20;
        break;

      case 5:
        MFD::Initialize();
        load_activity = GetText("Starshatter.load.menus");
        load_progress = 25;
        break;
      ...
      case 6:
        ...
        if (!gamescreen && m_gameMode != PREP_MODE)
          SetupGameScreen();

        load_activity = GetText("Starshatter.load.mission");
        load_progress = 65;
        break;

      case 7:
        if (m_gameMode == PREP_MODE)
        {
          if (Campaign::GetCampaign())
            Campaign::GetCampaign()->GetMission();
          SetGameMode(PLAN_MODE);
          load_activity = GetText("Starshatter.load.loaded");
          load_progress = 100;
          Button::PlaySound(4);
        }
        else
        {
          CreateWorld();
          load_activity = GetText("Starshatter.load.simulation");
          load_progress = 75;
        }
        break;
      ...
      case 8:
        InstantiateMission();
        load_activity = GetText("Starshatter.load.viewscreen");
        load_progress = 90;
        break;
...
void Starshatter::DoGameKeys()
{
    ...
    else if (KeyDown(KEY_SELF_DESTRUCT))
    {
      ...
      if (player_ship && !player_ship->InTransition())
      {
        ...
        else
        {
          ...
          player_ship->InflictDamage(damage, nullptr, 1, scuttle_loc);
        }

        if (player_ship->Integrity() < 1)
        {
          DebugTrace("  %s 0-0-0-Destruct-0\n\n", player_ship->Name());
  ...
  if (cam_dir)
  {
    double spin = (PI / 2) * FrameTime(); // Game::GUITime();
    ...
    if (KeyDown(KEY_CAM_EXT_PLUS_RANGE))
    {
      if (!gamescreen->IsNavShown())
        cam_dir->ExternalRange(static_cast<float>(1 + 1.5 * FrameTime())); // 1.1f);
    }

    else if (KeyDown(KEY_CAM_EXT_MINUS_RANGE))
    {
      if (!gamescreen->IsNavShown())
        cam_dir->ExternalRange(static_cast<float>(1 - 1.5 * FrameTime())); // 0.9f);
    }
...
void Starshatter::DoChatMode()
{
  ...
  else if (send_chat || Keyboard::KeyDown(VK_RETURN))
  {
    switch (chat_mode)
    {
      default: case CHAT_BROADCAST: { NetUtil::SendChat(0, name, chat_text); }
        break;
  ...
  else
  {
    ...
    while (GetKeyPlus(key, shift))
    {
      ...
      {
        if (shift & 1)
          chat_text += static_cast<char>(key);
        else
          chat_text += static_cast<char>(tolower(key));
      }
      else
      {
        switch (key)
        {
          case VK_BACK:
            chat_text = chat_text.substr(0, chat_text.length() - 1);
            break;
...
void Starshatter::SetChatMode(int mode)
{
  ...
  do
  {
    ...
    if (term)
    {
      ...
      if (def)
      {
        ...
        else if (def->name()->value() == "depth" || def->name()->value() == "bpp")
        {
          int bpp;
          ...
          switch (bpp)
          {
            ...
            default:
              DebugTrace("WARNING: Invalid screen bpp ({}) in '%s'\n", bpp, filename);
              screen_depth = 8;
              break;
          }
        }
        ...
        else if (def->name()->value().starts_with("cam_range"))
        {
          double range_max = 0;
...
void Starshatter::ExecCutscene(std::string_view msn_file, std::string_view path)
{
  if (InCutscene() || msn_file.empty())
    return;
  ...
  if (cutscene_mission->Load(msn_file, path))
  {
    ...
    if (sim)
    {
      ...
      DebugTrace("  Cutscene Instantiated.\n");
...
void Starshatter::BeginCutscene()
{
  ...
  if (cutscene == 0 && !sim->IsNetGame())
  {
    ...
    if (sim->GetPlayerShip())
      sim->GetPlayerShip()->SetControls(nullptr);
...
void Starshatter::EndCutscene()
{
  ...
  if (cutscene == 0)
  {
    ...
    if (cam_dir)
    {
      cam_dir->SetViewOrbital(nullptr);
...
Mission* Starshatter::GetCutsceneMission() const { return cutscene_mission; }

std::string Starshatter::GetSubtitles() const
{
  if (cutscene_mission)
...
int Starshatter::GetLobbyMode() { return lobby_mode; }

void Starshatter::SetLobbyMode(int mode) { lobby_mode = mode; }
