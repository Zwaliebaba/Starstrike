...
constexpr int PING_SIZE = 8;
constexpr int SHIP_LOC_SIZE = 36;
constexpr int JOIN_REQ_SIZE = 116;
constexpr int JOIN_ANN_SIZE = 116;
constexpr int QUIT_ANN_SIZE = 8;
...
NetPacket::~NetPacket() { delete msg; }
...
void NetPacket::SetShipLocation(const Point& loc)
{
  if (msg && msg->Length() >= SHIP_LOC_SIZE)
  {
    long x = static_cast<long>(loc.x * 100);
    long y = static_cast<long>(loc.y * 100);
    long z = static_cast<long>(loc.z * 100);
...
void NetPacket::SetShipVelocity(const Point& vel)
{
  if (msg && msg->Length() >= SHIP_LOC_SIZE)
  {
    ...
    *(data + 0) = static_cast<short>(vel.x);
    *(data + 1) = static_cast<short>(vel.y);
    *(data + 2) = static_cast<short>(vel.z);
  }
}
...
void NetPacket::SetShipOrientation(const Point& rpy)
{
  if (msg && msg->Length() >= SHIP_LOC_SIZE)
  {
    ...
    *(data + 0) = static_cast<short>(32767 * rpy.x / (2 * PI));
    *(data + 1) = static_cast<short>(32767 * rpy.y / (2 * PI));
    *(data + 2) = static_cast<short>(32767 * rpy.z / (2 * PI));
  }
}
...
void NetPacket::SetTrigger(int i, bool trigger)
{
  if (i >= 0 && i < 8 && msg && msg->Length() >= SHIP_LOC_SIZE)
  {
    BYTE* data = (BYTE*)msg->Data() + 33;
