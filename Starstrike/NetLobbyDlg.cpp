...
NetLobbyDlg::NetLobbyDlg(Screen* s, const FormDef& def, MenuScreen* mgr)
      ...
      if (net_lobby->IsServer())
      {
        ...
        if (!user)
        {
          ...
          if (player)
          {
            ...
            ::DebugTrace("NetLobbyDlg::Show() Host mode - no current player?\n");
        }
...
void NetLobbyDlg::SendChat(std::string msg)
{
  if (msg.length() < 1)
    return;
  ...
  if (net_lobby)
  {
    ...
    if (c_index >= 0 && c_index < campaigns.size() && m_index >= 0)
    {
      ...
      if (m_index < c->missions.size())
      {
        ...
        DWORD id = (c->id << NET_CAMPAIGN_SHIFT) + (m->m_id & NET_MISSION_MASK);
