...
void Landscape::GenerateNormals()
{
    ...
    for (unsigned short z = 0; z < m_heightMap->GetNumRows(); ++z)
    {
        ...
        for (unsigned short x = 0; x < m_heightMap->GetNumColumns(); ++x)
        {
            ...
            LegacyVector3 norm = (normA + normB).Normalise();
...
void Landscape::Init(LandscapeDef *_def, bool _justMakeTheHeightMap)
{
		...
		m_worldSizeZ != _def->m_worldSizeZ )
	{
		...
		m_heightMap = new SurfaceMap2D <float>(_def->m_worldSizeX, _def->m_worldSizeZ,
											   ...
											   m_outsideHeight);
		m_normalMap = new SurfaceMap2D <LegacyVector3>(_def->m_worldSizeX, _def->m_worldSizeZ,
												 0.0f, 0.0f, _def->m_cellSize, _def->m_cellSize,
												 g_upVector);
	}
...
bool Landscape::IsInLandscape( LegacyVector3 const &_pos )
{
...
bool Landscape::RayHit(LegacyVector3 const &_rayStart, LegacyVector3 const &_rayDir, LegacyVector3 *_result) const
{
	if (!m_heightMap) return false;

	LegacyVector3 rayDirNormalised = _rayDir;
    ...
    return UnsafeRayHit(LegacyVector3(segIntersectResult.x, segIntersectResultY, segIntersectResult.y),
...
bool Landscape::UnsafeRayHit(LegacyVector3 const &_rayStart, LegacyVector3 const &_rayDir, LegacyVector3 *_result) const
{
...
bool Landscape::RayHitCell(int x0, int z0,
                           LegacyVector3 const &_rayStart, LegacyVector3 const &_rayDir,
...
float Landscape::SphereHit(LegacyVector3 const &_centre, float _radius) const
{
