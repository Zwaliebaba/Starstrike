...
void LandscapeTile::GenerateDiamondMidpoint(int _x, int _z, int _halfSize)
{
  ...
  case 0:
    ...
    break;
  case 1:
    if (darwiniaRandom() & 1) { newHeight = m_heightMap->GetData(_x - _halfSize, _z) + m_heightMap->GetData(_x + _halfSize, _z); }
...
void Landscape::Init(LandscapeDef* _def, bool _justMakeTheHeightMap)
{
    ...
    m_worldSizeX || m_worldSizeZ != _def->m_worldSizeZ)
  {
    ...
    m_heightMap = new SurfaceMap2D<float>(_def->m_worldSizeX, _def->m_worldSizeZ, 0.0f, 0.0f, _def->m_cellSize, _def->m_cellSize,
                                          m_outsideHeight);
    m_normalMap = new SurfaceMap2D<LegacyVector3>(_def->m_worldSizeX, _def->m_worldSizeZ, 0.0f, 0.0f, _def->m_cellSize, _def->m_cellSize,
                                                  g_upVector);
  }
...
bool Landscape::RayHit(const LegacyVector3& _rayStart, const LegacyVector3& _rayDir, LegacyVector3* _result) const
{
  ...
  return UnsafeRayHit(LegacyVector3(segIntersectResult.x, segIntersectResultY, segIntersectResult.y), rayDirNormalised, _result);
}
...
bool Landscape::RayHitCell(int x0, int z0, const LegacyVector3& _rayStart, const LegacyVector3& _rayDir, LegacyVector3* _result) const
{
