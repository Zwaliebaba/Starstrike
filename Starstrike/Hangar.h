...
class Hangar : public SimObserver
{
  public:
    ...
    virtual bool CreateSquadron(std::string_view squadron, CombatGroup* g, const ShipDesign* design, int count, int iff = -1,
                                int* def_load = nullptr, int maint_count = 0, int dead_count = 0);
    ...
    virtual bool GotoAlert(int squadron, int slot, FlightDeck* d, Element* elem = nullptr, int* loadout = nullptr,
                           bool pkg = false, bool expedite = false);
    ...
    std::string SquadronName(int n) const;
    ...
    std::string StatusName(const HangarSlot* s) const;
  ...
  protected:
    ...
    std::string names[MAX_SQUADRONS];
