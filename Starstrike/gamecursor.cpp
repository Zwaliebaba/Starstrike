...
bool GameCursor::GetSelectedObject(WorldObjectId& _id, LegacyVector3& _pos)
{
  ...
  if (team)
  {
    ...
    {
      Building* building = g_context->m_location->GetBuilding(team->m_currentBuildingId);
      if (building)
      {
...
bool GameCursor::GetHighlightedObject(WorldObjectId& _id, LegacyVector3& _pos, float& _radius)
{
  ...
  if (!g_context->m_taskManagerInterface->m_visible)
    somethingHighlighted = g_context->m_locationInput->GetObjectUnderMouse(id, g_context->m_globalWorld->m_myTeamId);
  else
  {
    Task* task = g_context->m_taskManager->GetTask(g_context->m_taskManagerInterface->m_highlightedTaskId);
  ...
  if (somethingHighlighted)
  {
    ...
    {
      // Found a building
      Building* building = g_context->m_location->GetBuilding(id.GetUniqueId());
      if (building->m_type == Building::TypeRadarDish || building->m_type == Building::TypeBridge || building->m_type ==
        Building::TypeGunTurret || building->m_type == Building::TypeFenceSwitch)
      {
        _id = id;
        _pos = building->m_centerPos;
    ...
    {
      // Found a unit
      Unit* unit = g_context->m_location->GetUnit(id);
      if (unit)
      {
        _id = id;
...
{
  LegacyVector3 landNormal = g_context->m_location->m_landscape.m_normalMap->GetValue(_pos.x, _pos.z);
...
{
  START_PROFILE(g_context->m_profiler, "Prepare GameCursor");
  ...
  m_frame.mousePos = g_context->m_userInput->GetMousePos3d();
  m_frame.mousePos.y = std::max(1.0f, m_frame.mousePos.y);

  // Context flags
  m_frame.isEditing = g_context->m_editing || EclGetWindows()->Size() > 0;
  m_frame.isInteractive = g_context->m_camera->IsInteractive();
  m_frame.inLocation = g_context->m_location != nullptr;
  m_frame.inGlobalWorld = g_context->m_locationId == -1;
  ...
  if (m_frame.isEditing || !m_frame.isInteractive)
  {
    END_PROFILE(g_context->m_profiler, "Prepare GameCursor");
    return;
  }
  ...
  if (!m_frame.inLocation)
  {
    ...
    GlobalLocation* highlightedLocation = g_context->m_globalWorld->GetHighlightedLocation();
    m_frame.globalWorldLocAvailable = highlightedLocation && highlightedLocation->m_missionFilename != "null" && highlightedLocation->
      m_available;
  ...
  if (g_context->m_camera->IsInMode(Camera::ModeEntityTrack) && !g_context->m_taskManagerInterface->m_visible && !(task && task->m_state ==
    Task::StateStarted && task->m_type != GlobalResearch::TypeOfficer && !m_frame.somethingHighlighted))
    m_frame.suppressStandardCursor = true;
  ...
  if (m_frame.somethingSelected && m_frame.selectedId.GetUnitId() != UNIT_BUILDINGS)
  {
    ...
    Entity* ent = g_context->m_location->GetEntity(m_frame.selectedId);
    Unit* unit = g_context->m_location->GetUnit(m_frame.selectedId);
    bool alive = (ent || unit);
    if (ent && ent->m_dead)
      alive = false;
    ...
    if (alive)
    {
      ...
      g_context->m_camera->Get2DScreenPos(m_frame.selectedWorldPos, &rawScreenX, &rawScreenY);
      ...
      LegacyVector3 toCam = g_context->m_camera->GetPos() - m_frame.selectedWorldPos;
      float angle = toCam * g_context->m_camera->GetFront();

      m_frame.selectedOnScreen = (angle <= 0.0f && m_frame.selectedScreenX >= 0 && m_frame.selectedScreenX < screenW && m_frame.
        selectedScreenY >= 0 && m_frame.selectedScreenY < screenH);
      ...
      {
        LegacyVector3 camPos = g_context->m_camera->GetPos() + g_context->m_camera->GetFront() * 1000;
  ...
  {
    g_context->m_camera->Get2DScreenPos(m_frame.highlightedWorldPos, &m_frame.highlightedScreenX, &m_frame.highlightedScreenY);
    m_frame.highlightedOnScreen = true;

    // Pre-compute camera distance for halo sizing
    m_frame.highlightedCamDist = (g_context->m_camera->GetPos() - m_frame.highlightedWorldPos).Mag();
  }

  // Radar dish entrance screen position
  if (m_frame.somethingSelected && m_frame.somethingHighlighted && m_frame.selectedId.GetUnitId() != UNIT_BUILDINGS && m_frame.highlightedId
    .GetUnitId() == UNIT_BUILDINGS)
  {
    int entityType;
    if (m_frame.selectedId.GetIndex() == -1)
      entityType = g_context->m_location->GetUnit(m_frame.selectedId)->m_troopType;
    else
      entityType = g_context->m_location->GetEntity(m_frame.selectedId)->m_type;

    if (entityType == Entity::TypeInsertionSquadie || entityType == Entity::TypeOfficer)
    {
      Building* building = g_context->m_location->GetBuilding(m_frame.highlightedId.GetUniqueId());
      if (building && building->m_type == Building::TypeRadarDish)
      {
        ...
        if (dish->Connected())
        {
          ...
          float posX, posY;
          g_context->m_camera->Get2DScreenPos(entrancePos, &posX, &posY);
...
{
  START_PROFILE(g_context->m_profiler, "Render GameCursor 3D");
  ...
  {
    if (m_frame.currentTask && m_frame.currentTask->m_state == Task::StateStarted && m_frame.currentTask->m_type !=
      GlobalResearch::TypeOfficer && !m_frame.somethingHighlighted && !g_context->m_taskManagerInterface->m_visible)
    {
      // The player is placing a task
      bool validPlacement = g_context->m_taskManager->IsValidTargetArea(m_frame.currentTask->m_id, m_frame.mousePos);
      m_cursorPlacement->SetAnimation(validPlacement);
      m_cursorPlacement->SetSize(40.0f);
      LegacyVector3 landNormal = g_context->m_location->m_landscape.m_normalMap->GetValue(m_frame.mousePos.x, m_frame.mousePos.z);
      LegacyVector3 front = (landNormal ^ g_upVector).Normalise();
    ...
    else if (!g_context->m_taskManagerInterface->m_visible && !g_context->m_camera->IsInMode(Camera::ModeEntityTrack))
    {
      if (m_frame.somethingSelected && m_frame.selectedId.GetUnitId() != UNIT_BUILDINGS)
      {
  ...
  END_PROFILE(g_context->m_profiler, "Render GameCursor 3D");
}

void GameCursor::WriteBackArrowBoost(float _boost) { m_selectionArrowBoost = _boost; }
...
void MouseCursor::Render3D(const LegacyVector3& _pos, const LegacyVector3& _front, const LegacyVector3& _up, bool _cameraScale)
{
  ...
  if (_cameraScale)
  {
    float camDist = (g_context->m_camera->GetPos() - _pos).Mag();
