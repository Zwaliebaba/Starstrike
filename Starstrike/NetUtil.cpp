...
void NetUtil::SendObjDamage(SimObject* obj, double dmg, Shot* shot)
{
  ...
  if (net_game->IsServer() && obj->GetObjID())
  {
    ...
    damage.SetDamage(static_cast<float>(dmg));
...
void NetUtil::SendSysDamage(Ship* obj, System* sys, double dmg)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !obj || !sys)
    return;
...
void NetUtil::SendSysStatus(Ship* obj, System* sys)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !obj || !sys)
    return;
  ...
  if (obj->GetObjID())
  {
    ...
    status.SetSystem(sys->GetID());
    status.SetStatus(sys->Status());
    status.SetPower(static_cast<int>(sys->GetPowerLevel()));
    status.SetReactor(sys->GetSourceIndex());
...
void NetUtil::SendObjKill(Ship* obj, const Ship* killer, int type, int deck)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !obj)
    return;
  ...
  if (type == NetObjKill::KILL_DOCK || (net_game->IsServer() && obj->GetObjID()))
  {
    ...
    {
      DebugTrace("NetObjKill preparing respawn for %s\n", obj->Name());
      ...
      DebugTrace("NetObjKill no respawn for %s\n", obj->Name());

    net_game->SendData(&kill);
  }
}

void NetUtil::SendObjHyper(Ship* obj, std::string_view rgn, const Point& loc, const Ship* fc1, const Ship* fc2,
                           int transtype)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !obj)
...
void NetUtil::SendObjTarget(Ship* obj)
{
  ...
  if (obj->GetObjID())
  {
    ...
    if (target)
    {
      ...
      if (subtarget)
      {
        auto s = static_cast<Ship*>(target);
...
void NetUtil::SendWepTrigger(Weapon* wep, int count)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !wep)
    return;
...
void NetUtil::SendWepRelease(Weapon* wep, Shot* shot)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !wep || !shot)
    return;
...
void NetUtil::SendChat(DWORD dst, std::string_view name, std::string_view text)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || name.empty() || text.empty())
    return;
  ...
  {
    auto net_game_server = dynamic_cast<NetGameServer*>(net_game);
    net_game_server->RouteChatMsg(chat_msg);
  }
}

void NetUtil::SendElemRequest(std::string_view name)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || name.empty())
    return;

  NetElemRequest elem_request;
  elem_request.SetName(name);

  DebugTrace("NetUtil::SendElemRequest name: '%s'\n", name);
  net_game->SendData(&elem_request);
}

void NetUtil::SendElemCreate(Element* elem, int squadron, int* slots, bool alert, bool in_flight)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !elem)
    return;
  ...
  if (elem->GetCarrier())
    elem_create.SetCarrier(elem->GetCarrier()->Name().c_str());

  if (elem->GetCommander())
    elem_create.SetCommander(elem->GetCommander()->Name());

  DebugTrace("NetUtil::SendElemCreate iff: %d name: '%s'\n", elem->GetIFF(), elem->Name().data());
  net_game->SendData(&elem_create);
}

void NetUtil::SendShipLaunch(Ship* carrier, int squadron, int slot)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !carrier)
    return;
...
void NetUtil::SendNavData(bool add, Element* elem, int index, Instruction* navpt)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !elem || !navpt)
    return;
...
void NetUtil::SendNavDelete(Element* elem, int index)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !elem)
    return;
...
void NetUtil::SendSelfDestruct(Ship* obj, double dmg)
{
  NetGame* net_game = NetGame::GetInstance();
  if (!net_game || !obj)
    return;
  ...
  if (obj->GetObjID())
  {
    ...
    sd.SetDamage(static_cast<float>(dmg));
