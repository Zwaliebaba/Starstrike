    ...
    m_cellSizeZ(_cellSizeZ)
{
  ...
  m_numCellsX = static_cast<int>(g_context->m_location->m_landscape.GetWorldSizeX() / _cellSizeX) + 1;
  m_numCellsZ = static_cast<int>(g_context->m_location->m_landscape.GetWorldSizeZ() / _cellSizeZ) + 1;
...
WorldObjectId* EntityGrid::GetEnemies(float _worldX, float _worldZ, float _range, int* _numFound, unsigned char _myTeam)
{
  ...
  for (int i = 0; i < NUM_TEAMS; ++i)
    include[i] = !g_context->m_location->IsFriend(i, _myTeam);
      ...
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        if (_includeTeam[t])
        {
          ...
          for (int i = 0; i < ogc->m_arraySize; i++)
          {
            ...
            Entity* obj = g_context->m_location->GetEntity(objId);
            if (!obj)
            {
...
int EntityGrid::GetNumFriends(float _worldX, float _worldZ, float _range, unsigned char _myTeam)
{
  ...
  for (int i = 0; i < NUM_TEAMS; ++i)
    includeTeam[i] = g_context->m_location->IsFriend(i, _myTeam);
...
void EntityGrid::Render()
{
  ...
  float cellSizeX = g_context->m_location->m_landscape.GetWorldSizeX() / static_cast<float>(m_numCellsX);
  float cellSizeZ = g_context->m_location->m_landscape.GetWorldSizeZ() / static_cast<float>(m_numCellsZ);
  ...
  {
    float worldX = (static_cast<float>(x) / static_cast<float>(m_numCellsX)) * g_context->m_location->m_landscape.GetWorldSizeX();
    for (z = 0; z < m_numCellsZ; ++z)
    {
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        ...
        if (numEntities > 0)
        {
          float worldZ = (static_cast<float>(z) / static_cast<float>(m_numCellsZ)) * g_context->m_location->m_landscape.GetWorldSizeZ();
          float worldY = g_context->m_location->m_landscape.m_heightMap->GetValue(worldX, worldZ) + 10.0f;
          worldY = 100.0f + t * 30.0f;

          float alpha = 128;
          RGBAColour col = g_context->m_location->m_teams[t].m_colour;
  ...
  } static float lastCheck = 0; if (GetHighResTime() > lastCheck + 1.0f)
  {
    for (int x = 0; x < m_numCellsX; ++x)
    {
      for (int z = 0; z < m_numCellsZ; ++z)
      {
        ...
        for (int i = 0; i < ogc->m_arraySize; i++)
        {
          ...
          Entity* obj = g_context->m_location->GetEntity(objId);
          if (!obj) { LogEntityGridError(objId, LegacyVector3(x * m_cellSizeX, 0.0f, z * m_cellSizeZ), 1); }
        }
      }
    }
