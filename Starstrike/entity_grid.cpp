...
void LogEntityGridError( WorldObjectId _id, LegacyVector3 const &_pos, int _error )
{
#ifdef DEBUG_ENTITY_GRID
...
void EntityGrid::RemoveObject (WorldObjectId _objectID, float _worldX, float _worldZ, float _radius )
{
    ...
    if( _radius == 0.0f )
    {
        ...
        if( !success ) LogEntityGridError( _objectID, LegacyVector3(_worldX, 0.0f, _worldZ), 2 );
    }
    else
    {
        int leftMostCell    = GetGridIndexX(_worldX - _radius/2);
        ...
        for( int x = leftMostCell; x <= rightMostCell; ++x )
        {
            for( int z = upMostCell; z <= downMostCell; ++z )
            {
                ...
                if( !success ) LogEntityGridError( _objectID, LegacyVector3(x*m_cellSizeX, 0.0f, z*m_cellSizeZ), 2 );
            }
        }
    }
}
