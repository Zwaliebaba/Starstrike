...
Hangar::Hangar(const Hangar& s)
  : ship(nullptr),
    nsquadrons(s.nsquadrons),
    last_patrol_launch(s.last_patrol_launch)
{
  ZeroMemory(nslots, sizeof(nslots));
  ZeroMemory(squadrons, sizeof(squadrons));
}
...
void Hangar::ExecFrame(double seconds)
{
        ...
        switch (slot->state)
        {
          ...
          case ALERT:
            ...
            else if (slot->deck)
            {
              ...
              {
                case FlightDeck::READY:
                  slot->state = ALERT;
                  break;
                case FlightDeck::QUEUED:
                  slot->state = QUEUED;
                  break;
                case FlightDeck::LOCKED:
                  slot->state = LOCKED;
                  break;
                case FlightDeck::LAUNCH:
                  slot->state = LAUNCH;
                  break;
                default:
                  slot->state = STORAGE;
...
bool Hangar::FinishPrep(HangarSlot* slot)
{
  if (slot->deck->SpaceLeft(slot->design->type))
  {
    Sim* sim = Sim::GetSim();

    std::string ship_name = slot->squadron;

    slot->ship = sim->CreateShip(ship_name, "", (ShipDesign*)slot->design, ship->GetRegion()->Name(), Point(0, 0, 0),
                                 slot->iff, ship->GetCommandAILevel(), slot->loadout);
    ...
    if (slot->package)
    {
      ...
      char name[64];
      sprintf_s(name, "%s %d", slot->package->Name().c_str(), slot->ship->GetElementIndex());
    ...
    DebugTrace("WARNING: Could not spot alert ship - carrier: '%s' ship '%s'\n", ship->Name(), slot->ship->Name());
  }

  return false;
}

bool Hangar::Update(SimObject* obj)
{
  bool found = false;
...
std::string Hangar::GetObserverName() const
{
  static char name[64];
  if (ship)
    sprintf_s(name, "Hangar(%s)", ship->Name().c_str());
  else
    sprintf_s(name, "Hangar");
  return name;
}

bool Hangar::CreateSquadron(std::string_view squadron, CombatGroup* group, const ShipDesign* design, int count, int iff,
                            int* def_load, int maint_count, int dead_count)
{
  if (nsquadrons < MAX_SQUADRONS && count > 0)
  {
    auto s = NEW HangarSlot[count];
...
bool Hangar::GotoActiveFlight(int squadron, int slot_index, Element* elem, int* loadout)
{
  if (elem && squadron < nsquadrons && slot_index < nslots[squadron])
  {
    ...
    if (slot->state == STORAGE)
    {
      ...
      slot->ship = sim->CreateShip(ship_name, "", (ShipDesign*)slot->design, ship->GetRegion()->Name(),
      ...
      if (slot->ship)
      {
        ...
        char name[64];
        sprintf_s(name, "%s %d", elem->Name().c_str(), slot->ship->GetElementIndex());
...
bool Hangar::GotoAlert(int squadron, int slot, FlightDeck* d, Element* elem, int* loadout, bool pkg, bool expedite)
{
...
bool Hangar::Launch(int squadron, int slot)
{
  if (squadron < nsquadrons && slot < nslots[squadron])
  {
    HangarSlot* s = &(squadrons[squadron][slot]);
...
bool Hangar::StandDown(int squadron, int slot)
{
  if (squadron < nsquadrons && slot < nslots[squadron])
  {
    HangarSlot* s = &(squadrons[squadron][slot]);

    Element* package = nullptr;
...
bool Hangar::CanStow(Ship* incoming)
{
  int squadron = -1;
  int slot = -1;
...
bool Hangar::Stow(Ship* incoming)
{
  int squadron = -1;
  int slot = -1;
  ...
  if (FindSlot(incoming, squadron, slot))
  {
    ...
    s->time = 2400;
    s->package = nullptr;   // XXX MEMORY LEAK?

    // extra maintenance time?
    if (incoming->Integrity() < incoming->Design()->integrity)
    {
      double damage = 100 * (static_cast<double>(incoming->Design()->integrity) - incoming->Integrity()) / static_cast<
...
bool Hangar::FindSlot(Ship* test, int& squadron, int& slot, int desired_state)
{
  if (test)
  {
    ...
    for (int i = 0; i < nsquadrons; i++)
    {
      if (squadron < 0 || squadron == i)
      {
        for (int j = 0; j < nslots[i]; j++)
        {
          ...
          if (avail_slot < 0 && s->ship == nullptr)
          {
            if ((desired_state > STORAGE && s->state == STORAGE) || (desired_state < STORAGE && s->state == UNAVAIL))
            {
              avail_squadron = i;
              avail_slot = j;
            }
...
bool Hangar::FindSquadronAndSlot(Ship* test, int& squadron, int& slot)
{
  if (test)
  {
    for (int i = 0; i < nsquadrons; i++)
...
bool Hangar::FindAvailSlot(const ShipDesign* design, int& squadron, int& slot)
{
  if (design)
  {
    for (int i = 0; i < nsquadrons; i++)
...
bool Hangar::Ready(int squadron, int slot, FlightDeck* d)
{
...
std::string Hangar::SquadronName(int n) const
{
  if (n >= 0 && n < nsquadrons)
    return names[n];
...
const HangarSlot* Hangar::GetSlot(int i, int j) const
{
  if (i >= 0 && i < nsquadrons)
  {
    if (j >= 0 && j < nslots[i])
...
std::string Hangar::StatusName(const HangarSlot* s) const
{
  switch (s->state)
  {
    default: case UNAVAIL:
      return Game::GetText("hangar.UNAVAIL");
    case MAINT:
      return Game::GetText("hangar.MAINT");
    case STORAGE:
      return Game::GetText("hangar.STORAGE");
    case PREP:
      return Game::GetText("hangar.PREP");
    case ALERT:
      return Game::GetText("hangar.ALERT");
    case QUEUED:
    {
      std::string state = Game::GetText("hangar.QUEUED");
      char seq[8];
      sprintf_s(seq, " %d", s->deck->Sequence(s->slot));
      return state + seq;
    }
    case LOCKED:
      return Game::GetText("hangar.LOCKED");
    case LAUNCH:
      return Game::GetText("hangar.LAUNCH");
    case ACTIVE:
      return Game::GetText("hangar.ACTIVE");
    case APPROACH:
      return Game::GetText("hangar.APPROACH");
    case RECOVERY:
      return Game::GetText("hangar.RECOVERY");
  }
}
...
int Hangar::NumShipsReady(int n) const
{
  int result = 0;
...
int Hangar::NumShipsMaint(int n) const
{
  int result = 0;
...
int Hangar::NumShipsDead(int n) const
{
  int result = 0;
...
int Hangar::GetActiveElements(List<Element>& active_list)
{
  active_list.clear();
  ...
  for (int n = 0; n < nsquadrons; n++)
  {
    if (squadrons[n] && nslots[n] > 0)
    {
      for (int i = 0; i < nslots[n]; i++)
      {
        ...
        if (slot->package != nullptr && !active_list.contains(slot->package))
...
DWORD Hangar::GetLastPatrolLaunch() const { return last_patrol_launch; }

void Hangar::SetLastPatrolLaunch(DWORD t) { last_patrol_launch = t; }

void Hangar::SetAllIFF(int iff)
{
  for (int n = 0; n < nsquadrons; n++)
  {
    if (squadrons[n] && nslots[n] > 0)
