...
void MsnPkgDlg::RegisterControls()
{
  pkg_list = static_cast<ListBox*>(FindControl(320));
  nav_list = static_cast<ListBox*>(FindControl(330));
  ...
  if (mission)
  {
    if (pkg_list)
    {
      ...
      while (++elem)
      {
        ...
        if (elem->GetIFF() == mission->Team() && !elem->IsSquadron() && elem->Region() == mission->GetRegion() && elem->
          GetDesign()->type < Ship::STATION)
        {
          char txt[256];
...
void MsnPkgDlg::DrawThreats()
{
  ...
  if (threat[0])
  {
    ...
    while (++iter)
    {
      ...
      if (mission_role == Mission::STRIKE || mission_role == Mission::INTEL || mission_role >= Mission::TRANSPORT)
        continue;
      ...
      if (mission_role == Mission::SWEEP || mission_role == Mission::INTERCEPT || mission_role == Mission::FLEET ||
      ...
      sprintf_s(txt, "%s - %d %s - %s", role, elem->Count(), elem->GetDesign()->m_abrv.c_str(), rng);
      if (threat[i])
...
void MsnPkgDlg::ExecFrame()
{
  if (Keyboard::KeyDown(VK_RETURN))
    OnCommit(nullptr);
}

void MsnPkgDlg::OnPackage(AWEvent* event)
{
  if (!pkg_list || !mission)
    return;
...
void MsnPkgDlg::OnCommit(AWEvent* event) { MsnDlg::OnCommit(event); }

void MsnPkgDlg::OnCancel(AWEvent* event) { MsnDlg::OnCancel(event); }

void MsnPkgDlg::OnTabButton(AWEvent* event) { MsnDlg::OnTabButton(event); }
