...
constexpr int MAX_DETAIL = 4;
constexpr int PATCH_SIZE = 17;
constexpr int HALF_PATCH_SIZE = 8;
constexpr int MAX_VERTS = PATCH_SIZE * PATCH_SIZE;

static bool illuminating = false;

TerrainPatch::TerrainPatch(Terrain* terr, const Bitmap* patch, const Rect& r, const Point& p1, const Point& p2)
  : terrain(terr),
    ndetail(0),
    rect(r),
    water(nullptr),
    ...
    max_height(-1e9)
{
  ...
  radius = static_cast<float>(size * 0.75);
  ...
  for (i = 0; i < PATCH_SIZE; i++)
  {
    ...
    for (j = 0; j < PATCH_SIZE; j++)
    {
      ...
      float alt = static_cast<float>(red * mtnscale);
...
TerrainPatch::TerrainPatch(Terrain* terr, const Rect& r, const Point& p1, const Point& p2, double sea_level)
  : terrain(terr),
    ndetail(0),
    rect(r),
    water(nullptr)
{
  ...
  {
    for (j = 0; j < PATCH_SIZE; j++)
      *pHeight++ = static_cast<float>(sea_level);
  }
...
void TerrainPatch::SetScales(double s, double m, double b)
{
  scale = s;
  mtnscale = m;
  base = b;
...
{
  double d1 = fabs(vset->loc[v[0]].y - vset->loc[v[3]].y);

  double d2 = fabs(vset->loc[v[1]].y - vset->loc[v[2]].y);
...
bool TerrainPatch::BuildDetailLevel(int level)
{
  ...
  constexpr int NUM_INDICES_TRI = 3;
  constexpr int NUM_INDICES_QUAD = 6;
  ...
  if (s)
  {
    ...
    double tu0 = static_cast<double>(rect.x) / rect.w / 16.0 + 1.0 / 16.0;
    double tv0 = static_cast<double>(rect.y) / rect.h / 16.0;
    ...
    for (i = 0; i < ds1; i++)
    {
      for (j = 0; j < ds1; j++)
      {
        *pVert = Vec3(static_cast<float>(j * scale * dscale - (HALF_PATCH_SIZE * scale)),
                      heights[i * dscale * PATCH_SIZE + j * dscale],
                      static_cast<float>(i * scale * dscale - (HALF_PATCH_SIZE * scale)));

        if (level >= 2)
        {
          *pTu++ = static_cast<float>(-j * dtt);
          *pTv++ = static_cast<float>(i * dtt);

          if (level >= 4 && !water)
          {
            *pTu1++ = static_cast<float>(-j * dtt * 3);
            *pTv1++ = static_cast<float>(i * dtt * 3);
          }
        ...
        {
          *pTu++ = static_cast<float>(tu0 - j * dt);
          *pTv++ = static_cast<float>(tv0 + i * dt);
        }
    ...
    if (!water)
    {
      ...
      for (i = 0; i < ds1; i += detail_size)
      {
        for (j = 0; j < ds1; j++)
        {
          auto vl = Vec3(static_cast<float>(j * scale * dscale - (HALF_PATCH_SIZE * scale)),
                         heights[i * dscale * PATCH_SIZE + j * dscale],
                         static_cast<float>(i * scale * dscale - (HALF_PATCH_SIZE * scale)));
    ...
    for (i = 0; i < detail_size; i++)
    {
      ...
      {
        int v[4] = {(ds1 * (i) + (j)), (ds1 * (i) + (j + 1)), (ds1 * (i + 1) + (j)), (ds1 * (i + 1) + (j + 1))};
    ...
    for (i = 0; i < total; i++)
    {
      ...
      plane = Plane(vset->loc[v[0]] + loc, vset->loc[v[1]] + loc, vset->loc[v[2]] + loc);
    }
    ...
    qsort(s->GetPolys(), s->NumPolys(), sizeof(Poly), mcomp);
    ...
    for (i = 0; i < ds1; i++)
    {
      for (j = 0; j < ds1; j++)
      {
        ...
        else if (dscale > 1)
        {
          ...
          for (int dy = -dscale / 2; dy < dscale / 2; dy += step)
          {
            for (int dx = -dscale / 2; dx < dscale / 2; dx += step)
            {
              ...
              if (ix < 0)
                ix = 0;
              if (ix > terrain_width - 1)
                ix = terrain_width - 1;
              if (iy < 0)
                iy = 0;
              if (iy > terrain_width - 1)
                iy = terrain_width - 1;
        ...
        else
        {
          ...
          auto normal = Point((128 - vbn.x) / 127.0, (vbn.z - 128) / 127.0, (vbn.y - 128) / 127.0);
          vset->nrm[i * ds1 + j] = normal;
        }
      }
    }
  ...
  if (terrain && y >= 0 && !water)
  {
    ...
    for (int i = 0; i < terrain->GetLayers().size(); i++)
    {
      ...
      if (y >= layer->GetMinHeight() && y < layer->GetMaxHeight())
      {
        ...
        DWORD val = static_cast<DWORD>(scale * 255);
...
int TerrainPatch::CalcLayer(Poly* poly)
{
  if (terrain && poly)
  {
    if (water)
...
int TerrainPatch::CollidesWith(Graphic& o) { return 0; }
...
void TerrainPatch::Illuminate(Color ambient, List<Light>& lights)
{
...
void TerrainPatch::Render(Video* video, DWORD flags)
{
  if (max_height < 0)
    return;
...
***/
  ...
  FLOAT fog_density = static_cast<FLOAT>(terrain->GetRegion()->FogDensity() * 2.5e-5 * 1 / visibility);
...
int TerrainPatch::CheckRayIntersection(Point obj_pos, Point dir, double len, Point& ipt, bool ttpas)
{
...
double TerrainPatch::Height(double x, double z) const
{
  if (water)
    return base;
