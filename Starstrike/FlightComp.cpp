...
void FlightComp::SetTransLimit(double x, double y, double z)
{
  trans_x_limit = 0.0f;
  trans_y_limit = 0.0f;
  trans_z_limit = 0.0f;
  ...
  if (x >= 0)
    trans_x_limit = static_cast<float>(x);
  if (y >= 0)
    trans_y_limit = static_cast<float>(y);
  if (z >= 0)
    trans_z_limit = static_cast<float>(z);
}
...
{
  throttle = static_cast<float>(ship->Throttle());
...
void FlightComp::ExecTrans()
{
  ...
  if (IsPowerOn()) { flcs_operative = Status() == NOMINAL || Status() == DEGRADED; }
    ...
    if (flcs_operative)
    {
      ...
      if (ship->Design()->auto_roll > 0)
      {
        ...
        else
        {
          ...
          // coordinate the turn:
          if (current_bank < 0 && desired_bank < 0 || current_bank > 0 && desired_bank > 0)
          {
            double coord_pitch = compass_pitch - ship->GetHelmPitch() - fabs(helm) * fabs(current_bank);
