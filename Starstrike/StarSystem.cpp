...
double StarSystem::GetBaseTime() { return base_time; }
...
static constexpr double GRAV = 6.673e-11;
static constexpr int NAMELEN = 64;

StarSystem::StarSystem(std::string sys_name, Point l, int iff, int s)
  : name(std::move(sys_name)),
    affiliation(iff),
    seq(s),
    m_loc(l),
    instantiated(false),
...
{
  center = NEW Orbital(this, "CG", Orbital::NOTHING, 1.0e35f, 0.0f, 0.0f, nullptr);
  radius = 0.0f;
}

StarSystem::~StarSystem()
{
  DebugTrace("   Destroying Star System {}\n", name);
...
static OrbitalBody* primary_star = nullptr;
static OrbitalBody* primary_planet = nullptr;
static OrbitalBody* primary_moon = nullptr;
...
void StarSystem::Load()
{
  ...
  filename = name + std::string("\\") + name + ".def";
  DebugTrace("Loading StarSystem: {}\n", filename);
  loader->LoadBuffer(filename, block, true);

  if (!block)
  {
    Fatal("ERROR: invalid star system file '{}'\n", filename);
  }
      ...
      if (def)
      {
        ...
        else if (def->name()->value() == "sky")
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: sky struct missing in '%s'\n", filename);
          else
          {
        ...
        else if (def->name()->value() == "ambient")
        {
          ...
          ambient = Color(static_cast<BYTE>(a.x), static_cast<BYTE>(a.y), static_cast<BYTE>(a.z)) * 2.5;
        }
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: star struct missing in '%s'\n", filename);
          else
          {
            TermStruct* val = def->term()->isStruct();
            ParseStar(val);
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: planet struct missing in '%s'\n", filename);
          else
          {
            TermStruct* val = def->term()->isStruct();
            ParsePlanet(val);
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: moon struct missing in '%s'\n", filename);
          else
          {
            TermStruct* val = def->term()->isStruct();
            ParseMoon(val);
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: region struct missing in '%s'\n", filename);
          else
          {
            TermStruct* val = def->term()->isStruct();
            ParseRegion(val);
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: terrain struct missing in '%s'\n", filename);
          else
          {
            TermStruct* val = def->term()->isStruct();
            ParseTerrain(val);
...
void StarSystem::ParseStar(TermStruct* val)
{
  ...
  for (int i = 0; i < val->elements()->size(); i++)
  {
    ...
    if (pdef)
    {
      ...
      else if (pdef->name()->value() == "color")
      {
        ...
        color = Color(static_cast<BYTE>(a.x), static_cast<BYTE>(a.y), static_cast<BYTE>(a.z));
      }
      ...
      else if (pdef->name()->value() == "back" || pdef->name()->value() == "back_color")
      {
        Vec3 a;
        GetDefVec(a, pdef, filename);
        back = Color(static_cast<BYTE>(a.x), static_cast<BYTE>(a.y), static_cast<BYTE>(a.z));
      }
    }
  }

  auto star = NEW OrbitalBody(this, star_name, Orbital::STAR, mass, radius, orbit, center);
  ...
  bodies.append(star);
  primary_star = star;
  primary_planet = nullptr;
  primary_moon = nullptr;

  StarSystem::radius = std::max(orbit, StarSystem::radius);
}
...
void StarSystem::ParsePlanet(TermStruct* val)
{
  ...
  std::string img_ring;
  std::string glo_name;
  std::string glo_hi_name;
  std::string gloss_name;
  ...
  for (int i = 0; i < val->elements()->size(); i++)
  {
    ...
    if (pdef)
    {
      ...
      else if (pdef->name()->value() == "atmosphere")
      {
        Vec3 a;
        GetDefVec(a, pdef, filename);
        atmos = Color(static_cast<BYTE>(a.x), static_cast<BYTE>(a.y), static_cast<BYTE>(a.z));
      }
    }
  }

  auto planet = NEW OrbitalBody(this, pln_name, Orbital::PLANET, mass, radius, orbit, primary_star);
...
void StarSystem::ParseMoon(TermStruct* val)
{
  ...
  auto moon = NEW OrbitalBody(this, pln_name, Orbital::MOON, mass, radius, orbit, primary_planet);
  ...
  {
    DebugTrace("WARNING: no planet for moon %s in '%s', deleted.\n", pln_name, filename);
    delete moon;
    moon = nullptr;
  }
...
void StarSystem::ParseRegion(TermStruct* val)
{
  ...
  if (!primary)
    primary = primary_planet;
  if (!primary)
    primary = primary_star;

  auto region = NEW OrbitalRegion(this, rgn_name, 0, size, orbit, primary);
  region->grid = grid;
  region->inclination = inclination;
  region->asteroids = asteroids;
  region->links.insert(region->links.end(), links.begin(), links.end());
...
void StarSystem::ParseTerrain(TermStruct* val)
{
  ...
  if (!primary)
  {
    DebugTrace("WARNING: Terrain region with no primary ignored in '%s'\n", filename);
    return;
  }
    ...
    if (pdef)
    {
      ...
      else if (pdef->name()->value() == "layer")
      {
        if (!pdef->term() || !pdef->term()->isStruct())
          DebugTrace("WARNING: terrain layer struct missing in '%s'\n", filename);
        else
        {
          if (!region)
...
void StarSystem::ParseLayer(TerrainRegion* rgn, TermStruct* val) const
{
  ...
  for (int i = 0; i < val->elements()->size(); i++)
  {
    ...
    if (pdef)
    {
      if (pdef->name()->value().starts_with("tile"))
        GetDefText(tile_name, pdef, filename);

      else if (pdef->name()->value().starts_with("detail"))
        GetDefText(detail_name, pdef, filename);

      else if (pdef->name()->value().contains("height") || pdef->name()->value().contains("alt"))
...
void StarSystem::Create()
{
  ...
  if (!instantiated)
  {
    DebugTrace("Creating Star System {}\n", name);
    ...
    if (sky_poly_stars.length())
    {
      ...
      DebugTrace("Celestial Sphere '%s' loaded\n", sky_poly_stars);
      DebugTrace("   radius: %f\n", poly_stars->Radius());
    }

    if (sky_stars)
    {
      DebugTrace("Point Stars: %d\n", sky_stars);
    ...
    if (sky_nebula.length())
    {
      ...
      DebugTrace("Nebular Sky '%s' loaded\n", sky_nebula);
      DebugTrace("   radius: %f\n", nebula->Radius());
    }
    ...
    if (sky_haze.length())
    {
      ...
      DebugTrace("Atmospheric Haze '%s' loaded\n", sky_haze.c_str());
      DebugTrace("   radius: %f\n", haze->Radius());
...
void StarSystem::CreateBody(OrbitalBody& body)
{
  ...
  if (body.type == Orbital::STAR)
  {
    ...
    auto rep = NEW PlanetRep(body.tex_name, {}, body.radius, starloc, body.tscale);
    ...
    {
      auto back_light = NEW Light(0.6f);
      back_light->SetColor(body.back);
      back_light->SetShadow(false);
      back_light->MoveTo(body.loc * -1.0f);
  ...
  else
  {
    ...
    std::string surface = body.tex_name;
    std::string glow = body.tex_glow;
    ...
    auto rep = NEW PlanetRep(surface, glow, body.radius, planetloc, body.tscale, body.tex_ring, rmin, rmax,
                             body.atmosphere, body.tex_gloss);
  ...
  if (instantiated)
  {
    ...
    while (++star_iter)
    {
      ...
      while (++planet)
      {
        ...
        ListIter<OrbitalBody> moon = planet->Satellites();
        while (++moon) { GRAPHIC_DESTROY(moon->rep); }
      }
    }
...
void StarSystem::ExecFrame()
{
  ...
  if (instantiated && active_region)
  {
    ...
    if (terrain)
    {
      ...
      Matrix terrain_transformation = Matrix(tvrt, tvup, tvpn);
      ...
      if (point_stars)
      {
        ...
        BYTE sky_red = static_cast<BYTE>(sky_color.Red());
        BYTE sky_green = static_cast<BYTE>(sky_color.Green());
        BYTE sky_blue = static_cast<BYTE>(sky_color.Blue());

        BYTE Max = max3(sky_red, sky_green, sky_blue);
        BYTE Min = min3(sky_red, sky_green, sky_blue);

        BYTE lum = static_cast<BYTE>(240.0 * (Max + Min) / 510.0);

        if (lum > 50)
          point_stars->Hide();
        else
        {
          auto pstars = static_cast<Stars*>(point_stars);
      ...
      if (haze)
      {
        static_cast<TerrainHaze*>(haze)->UseTerrainRegion(trgn);
        scene = haze->GetScene();
      }
    ...
    while (++star_iter)
    {
      ...
      if (terrain)
      {
        ...
        if (star->rep)
        {
          ...
          auto pr = static_cast<PlanetRep*>(star->rep);
          pr->SetDaytime(true);
        }
        ...
        if (star->back_light)
        {
          star->back_light->MoveTo(sloc * -1.0f);
      ...
      while (++planet_iter)
      {
        ...
        if (planet->rep)
        {
          auto pr = static_cast<PlanetRep*>(planet->rep);
        ...
        while (++moon_iter)
        {
          ...
          if (moon->rep)
          {
            auto pr = static_cast<PlanetRep*>(moon->rep);
...
Orbital* StarSystem::FindOrbital(std::string_view name)
{
  if (name.empty())
    return nullptr;
...
OrbitalRegion* StarSystem::FindRegion(std::string_view name)
{
  if (name.empty())
    return nullptr;
...
Color StarSystem::Ambient() const
{
  ...
  if (terrain)
  {
    auto trgn = static_cast<TerrainRegion*>(active_region);
    result = trgn->Ambient();
...
void StarSystem::SetSunlight(Color color, double brightness)
{
  ...
  while (++sun_iter)
  {
    ...
    sun_light->SetIntensity(static_cast<float>(1.1 * sun_scale));
  }
  ...
  while (++back_iter)
  {
    ...
    back_light->SetIntensity(static_cast<float>(0.5 * sun_scale));
  }
}
...
void StarSystem::SetBacklight(Color color, double brightness)
{
  ...
  while (++back_iter)
  {
    ...
    back_light->SetIntensity(static_cast<float>(0.5 * brightness));
  }
}
...
Color Star::GetColor() const { return GetColor(seq); }

int Star::GetSize() const { return GetSize(seq); }
...
Orbital::Orbital(StarSystem* s, std::string_view n, OrbitalType t, double m, double r, double o, Orbital* p)
  : name(n),
    type(t),
    subtype(0),
    mass(m),
...
Point Orbital::PredictLocation(double delta_t)
{
  ...
  if (system && primary && orbit > 0)
  {
    ...
    // orbits are(?) counter clockwise:
    double predicted_phase = -2 * PI * grade * (StarSystem::Stardate() + delta_t) / period;

    predicted_loc += Point(orbit * cos(predicted_phase), orbit * sin(predicted_phase), 0);
  }
...
OrbitalBody::OrbitalBody(StarSystem* s, std::string_view n, OrbitalType t, double m, double r, double o, Orbital* p)
  : Orbital(s, n, t, m, r, o, p),
    ...
    light_rep(nullptr),
    back_light(nullptr),
    luminous(false) { Update(); }

OrbitalBody::~OrbitalBody() { satellites.destroy(); }
...
OrbitalRegion::OrbitalRegion(StarSystem* s, std::string_view n, double m, double r, double o, Orbital* p)
  : Orbital(s, n, REGION, m, r, o, p),
    grid(25.0e3f),
    inclination(0),
    asteroids(0) {}

OrbitalRegion::~OrbitalRegion() = default;
