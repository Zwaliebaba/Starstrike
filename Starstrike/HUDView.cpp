...
static BYTE* hud_left_shade_air = nullptr;
static BYTE* hud_right_shade_air = nullptr;
static BYTE* hud_left_shade_fighter = nullptr;
static BYTE* hud_right_shade_fighter = nullptr;
static BYTE* hud_left_shade_starship = nullptr;
static BYTE* hud_right_shade_starship = nullptr;
static BYTE* instr_left_shade = nullptr;
static BYTE* instr_right_shade = nullptr;
static BYTE* warn_left_shade = nullptr;
static BYTE* warn_right_shade = nullptr;
static BYTE* lead_shade = nullptr;
static BYTE* cross_shade = nullptr;
static BYTE* cross1_shade = nullptr;
static BYTE* cross2_shade = nullptr;
static BYTE* cross3_shade = nullptr;
static BYTE* cross4_shade = nullptr;
static BYTE* fpm_shade = nullptr;
static BYTE* hpm_shade = nullptr;
static BYTE* pitch_ladder_pos_shade = nullptr;
static BYTE* pitch_ladder_neg_shade = nullptr;
static BYTE* chase_left_shade = nullptr;
static BYTE* chase_right_shade = nullptr;
static BYTE* chase_top_shade = nullptr;
static BYTE* chase_bottom_shade = nullptr;
static BYTE* icon_ship_shade = nullptr;
static BYTE* icon_target_shade = nullptr;
...
constexpr int NUM_HUD_COLORS = 4;
...
void HUDView::DrawHUDText(int index, std::string_view txt, Rect& rect, int align, int upcase, bool box)
{
  ...
  if (cockpit_hud_texture && index >= TXT_HUD_MODE && index <= TXT_TARGET_ETA && ht.font != big_font)
  {
    Sprite* s = hud_sprite[0];

    int cx = static_cast<int>(s->Location().x);
    int cy = static_cast<int>(s->Location().y);
    int w2 = s->Width() / 2;
    int h2 = s->Height() / 2;
...
HUDView::~HUDView()
{
  ...
  for (int i = 0; i < 32; i++) { GRAPHIC_DESTROY(hud_sprite[i]); }
...
void HUDView::SetTacticalMode(int mode)
{
  if (tactical != mode)
  {
    tactical = mode;
...
void HUDView::SetOverlayMode(int mode)
{
  if (overlay != mode)
    overlay = mode;
}
...
std::string HUDView::GetObserverName() const { return "HUDView"; }
...
Color HUDView::MarkerColor(Contact* contact)
{
  Color c(80, 80, 80);
...
void HUDView::DrawContactMarkers()
{
  ...
  if (CameraDirector::GetCameraMode() == CameraDirector::MODE_ORBIT && ship->TrackLength() > 0)
  {
    ...
    for (int i = 0; i < ctl - 1; i++)
    {
      ...
      if (t1 != t2)
        DrawTrackSegment(t1, t2, c * (static_cast<double>(ctl - i) / static_cast<double>(ctl)));
    }
  }
  ...
  if (CameraDirector::GetCameraMode() == CameraDirector::MODE_ORBIT && mark_pt.z > 1.0)
  {
    ...
    int x = static_cast<int>(mark_pt.x);
    int y = static_cast<int>(mark_pt.y);
  ...
  if (target && target->Type() == SimObject::SIM_SHIP && target->Rep())
  {
    ...
    if (tgt_ship == nullptr)
    {
      DebugTrace("   Null Pointer in HUDView::DrawContactMarkers(). Please investigate.");
      return;
    }
    ...
    if (mark_pt.z > 1.0)
    {
      ...
      if (x > 4 && x < width - 4 && y > 4 && y < height - 4)
      {
        ...
        int hw = static_cast<int>(BAR_LENGTH * hull_strength);
        int sw = static_cast<int>(BAR_LENGTH * (tgt_ship->ShieldStrength() / 100.0));
...
void HUDView::DrawContact(Contact* contact, int index)
{
    ...
    if (x > 4 && x < width - 4 && y > 4 && y < height - 4)
    {
      ...
      if (x < width - 8)
      {
        ...
        else
        {
          ...
          {
            if (c_ship)
              strcpy_s(contact_buf, c_ship->Name().c_str());
            else if (!mega)
              sprintf_s(contact_buf, "%c %d", code, static_cast<int>(range));
            else
          ...
          else
          {
            ...
            if (!mega)
              sprintf_s(contact_buf, "%c %d%c", code, static_cast<int>(range), closing);
            else
              sprintf_s(contact_buf, "%c %.1f M", code, range);
          }
        }
      ...
      if (NetGame::GetInstance() && c_ship)
      {
        ...
        if (netp && netp->Name() != "Server A.I. Ship")
        {
...
void HUDView::DrawTrackSegment(Point& t1, Point& t2, Color c)
{
  int x1, y1, x2, y2;
  ...
  if (t1.z >= CLIP_Z && t2.z >= CLIP_Z)
  {
    ...
    x1 = static_cast<int>(t1.x);
    y1 = static_cast<int>(t1.y);
    x2 = static_cast<int>(t2.x);
    y2 = static_cast<int>(t2.y);
...
void HUDView::DrawTrack(Contact* contact)
{
  Ship* c_ship = contact->GetShip();
...
void HUDView::DrawRect(SimObject* targ)
{
  ...
  if (targ->Type() == SimObject::SIM_SHIP)
    c = static_cast<Ship*>(targ)->MarkerColor();
  else
    c = static_cast<Shot*>(targ)->MarkerColor();
...
void HUDView::DrawBars()
{
  ...
  if (Game::Paused())
  {
    auto rc = Rect(cx - 128, cy - 60, 256, 12);
    DrawHUDText(TXT_PAUSED, Game::GetText("HUDView.PAUSED"), rc, DT_CENTER);
  }
  ...
  if (ship)
  {
    ...
    if (ship->IsAirborne())
    {
      ...
      if (alt_agl <= 1000)
        sprintf_s(txt, "R %4d", static_cast<int>(alt_agl));
      else
    ...
    else if (ship->IsStarship() && ship->GetFLCSMode() == Ship::FLCS_HELM && !arcade)
    {
      ...
      speed_rect.y -= 10;
      int heading_degrees = static_cast<int>(ship->GetHelmHeading() / DEGREES);
    ...
    if (!tactical && !arcade)
    {
      ...
      int heading_degrees = static_cast<int>(ship->CompassHeading() / DEGREES);
      if (heading_degrees < 0)
        heading_degrees += 360;
    ...
    {
      case HUD_MODE_TAC:
        strcpy_s(txt, Game::GetText("HUDView.mode.tactical").data());
        break;
      case HUD_MODE_NAV:
        strcpy_s(txt, Game::GetText("HUDView.mode.navigation").data());
        break;
      case HUD_MODE_ILS:
        strcpy_s(txt, Game::GetText("HUDView.mode.landing").data());
        break;
    }
    ...
    if (ship->IsGearDown())
    {
      auto gear_down = Game::GetText("HUDView.gear-down");
    ...
    {
      auto threat_warn = Game::GetText("HUDView.threat-warn");
      bool show_msg = true;

      if (quantum && quantum->JumpTime() > 0)
      {
        static char buf[64];
        sprintf_s(buf, "%s: %d", Game::GetText("HUDView.quantum-jump").data(), static_cast<int>(quantum->JumpTime()));
        threat_warn = buf;
      }
    ...
    else if (mode == HUD_MODE_TAC)
    {
      ...
      if (missile && missile->Ammo() > 0 && !ship->IsNetObserver())
      {
        if (!arcade)
        {
          ...
          sprintf_s(txt, "%s %d", missile->Name().c_str(), missile->Ammo());
...
void HUDView::DrawHPM()
{
  ...
  auto hvec = Point(sin(az), sin(el), cos(az));
  hvec.Normalize();
...
void HUDView::DrawCompass()
{
  ...
  auto solid = static_cast<Solid*>(ship->Rep());
...
void HUDView::DrawLCOS(SimObject* targ, double dist)
{
  ...
  if (behind || tloc.x <= 0 || tloc.x >= width - 1 || tloc.y <= 0 || tloc.y >= height - 1)
  {
    // Left side:
...
void HUDView::DrawTarget()
{
  ...
  if (target && target->Rep())
  {
    ...
    if (sensor && target->Type() == SimObject::SIM_SHIP)
      contact = sensor->FindContact(static_cast<Ship*>(target));
    ...
    if (target->Type() == SimObject::SIM_SHIP)
    {
      ...
      if (mode != HUD_MODE_ILS)
      {
        ...
        {
          range_rect.y += 10;
          sprintf_s(txt, "%s %03d", Game::GetText("HUDView.symbol.shield").data(), tgt_ship->ShieldStrength());
          DrawHUDText(TXT_TARGET_SHIELD, txt, range_rect, DT_RIGHT);
        }

        range_rect.y += 10;
        sprintf_s(txt, "%s %03d", Game::GetText("HUDView.symbol.hull").data(),
                  static_cast<int>(tgt_ship->Integrity() / tgt_ship->Design()->integrity * 100));
        ...
        if (sys)
        {
          ...
          sprintf_s(txt, "%s %03d", sys->Abbreviation().c_str(), static_cast<int>(sys->Availability()));

          switch (sys->Status())
          {
            case System::DEGRADED:
              stat = Color(255, 255, 0);
              break;
    ...
    {
      auto tgt_drone = static_cast<Drone*>(target);
...
void HUDView::DrawNavInfo()
{
  ...
  if (navpt)
  {
    ...
    if (speed > 10)
      etr = static_cast<int>(distance / speed);
    ...
    if (navpt->HoldTime() > 0)
    {
      ...
      int hold = static_cast<int>(navpt->HoldTime());
...
void HUDView::DrawDesignators()
{
  double xtarg = xcenter;
  double ytarg = ycenter;
  SimObject* t1 = nullptr;
  SimObject* t2 = nullptr;
  SimObject* t3 = nullptr;
  ...
  if (ship->IsDropship())
  {
    ...
    if (t)
    {
      ...
      {
        auto tgt_ship = static_cast<Ship*>(t);
...
{
  if (!ship || ship->GetQuantumDrive() == nullptr)
    return -1;
...
{
  if (!ship || ship->GetThruster() == nullptr)
    return -1;
...
{
  if (!ship || ship->GetSensor() == nullptr)
    return -1;
...
void HUDView::DrawWarningPanel()
{
  ...
  if (ship)
  {
    ...
    {
      int stat = -1;
      std::string abrv = Game::GetText("HUDView.UNKNOWN");

      switch (index)
      {
        case 0:
          stat = GetReactorStatus(ship);
          abrv = Game::GetText("HUDView.REACTOR");
          break;
        case 1:
          stat = GetDriveStatus(ship);
          abrv = Game::GetText("HUDView.DRIVE");
          break;
        case 2:
          stat = GetQuantumStatus(ship);
          abrv = Game::GetText("HUDView.QUANTUM");
          break;
        case 3:
          stat = GetShieldStatus(ship);
          abrv = Game::GetText("HUDView.SHIELD");
          if (ship->GetShield() == nullptr && ship->GetDecoy())
        ...
        case 8:
          stat = GetSensorStatus(ship);
          abrv = Game::GetText("HUDView.SENSOR");
          break;
        case 9:
          stat = GetComputerStatus(ship);
          abrv = Game::GetText("HUDView.COMPUTER");
          break;
        case 10:
          stat = GetThrusterStatus(ship);
          abrv = Game::GetText("HUDView.THRUSTER");
          break;
        case 11:
          stat = GetFlightDeckStatus(ship);
          abrv = Game::GetText("HUDView.FLTDECK");
          break;
      }
      ...
      if (stat >= 0)
      {
        SetStatusColor(static_cast<System::STATUS>(stat));
        ...
        if (cockpit_hud_texture)
        {
          ...
          hud_font->DrawText(abrv, -1, warn_rect, DT_CENTER | DT_SINGLELINE, cockpit_hud_texture);

          warn_rect.y -= 4;
        }
        else
        {
          DrawHUDText(TXT_CAUTION_TXT + index, abrv, warn_rect, DT_CENTER);
...
void HUDView::DrawInstructions()
{
  ...
  auto r = Rect(width / 2 - 143, height - 105, 290, 17);
  ...
  else if (nobj)
  {
    ...
    {
      char desc[256];
      sprintf_s(desc, "* %s", elem->GetObjective(i)->GetShortDescription().c_str());
      hud_text[n].color = standard_txt_colors[color];
...
std::string HUDView::FormatInstruction(const std::string& instr)
{
  if (!instr.contains('$'))
    return instr;
      ...
      if (fd && fd->IsRecoveryDeck() && fd->GetCarrier())
      {
        ...
        if (same_sector && mode == HUD_MODE_ILS && !transition && !docking)
        {
          ...
          if (dst.z > 1.0)
          {
            ...
            int x = static_cast<int>(dst.x);
            int y = static_cast<int>(dst.y);
...
void HUDView::DrawObjective()
{
  ...
  {
    auto steer = static_cast<SteerAI*>(ship->GetDirector());
...
void HUDView::DrawNavPoint(Instruction& navpt, int index, int next)
{
  ...
  if (npt.z > 1.0)
  {
    ...
    int x = static_cast<int>(npt.x);
    int y = static_cast<int>(npt.y);
...
void HUDView::SetShip(Ship* s)
{
  ...
  else if (ship && ship->Design()->hud_icon.Width())
  {
    bool update = false;
    System::STATUS s = System::NOMINAL;
    int integrity = static_cast<int>(ship->Integrity() / ship->Design()->integrity * 100);
    ...
    if (update)
    {
      SetStatusColor(static_cast<System::STATUS>(ship_status));
  ...
  {
    auto cockpit = static_cast<Solid*>(ship->Cockpit());
...
void HUDView::SetTarget(SimObject* t)
{
  ...
  {
    System::STATUS s = System::NOMINAL;
    auto tship = static_cast<Ship*>(target);
    int integrity = static_cast<int>(tship->Integrity() / tship->Design()->integrity * 100);
  ...
  if (update)
  {
    ...
    SetStatusColor(static_cast<System::STATUS>(tgt_status));
...
void HUDView::Refresh()
{
  ...
  if (ship)
  {
    ...
    sim->ShowGrid(tactical && !ship->IsAirborne() && CameraDirector::GetCameraMode() != CameraDirector::MODE_VIRTUAL);
    ...
    if (!overlay)
    {
      ...
      int fov_degrees = 180 - 2 * static_cast<int>(projector->XAngle() * 180 / PI);
...
void HUDView::DrawOrbitalBody(OrbitalBody* body)
{
  if (body)
  {
    ...
    if (p.z > 100)
    {
      ...
      window->DrawEllipse(static_cast<int>(p.x - r), static_cast<int>(p.y - r), static_cast<int>(p.x + r),
                          static_cast<int>(p.y + r), Color::Cyan);
    }
...
void HUDView::CycleMFDMode(int mfd_index)
{
  if (mfd_index < 0 || mfd_index > 2)
    return;
...
void HUDView::ShowHUDInst() { show_inst = true; }
...
void HUDView::HideHUDInst() { show_inst = false; }
...
void HUDView::ClearMessages()
{
  if (hud_view)
  {
    for (int i = 0; i < MAX_MSG - 1; i++)
    {
      hud_view->msg_text[i] = std::string();
      hud_view->msg_time[i] = 0;
    }
  }
}

void HUDView::PrepareBitmap(const char* name, Bitmap& img, BYTE*& shades)
{
  delete [] shades;
  shades = nullptr;
  ...
  {
    for (int x = 0; x < w; x++)
      shades[y * w + x] = static_cast<BYTE>(img.GetColor(x, y).Red() * 0.66);
  }
}

void HUDView::TransferBitmap(const Bitmap& src, Bitmap& img, BYTE*& shades)
{
  delete [] shades;
  shades = nullptr;
  ...
  {
    for (int x = 0; x < w; x++)
      shades[y * w + x] = static_cast<BYTE>(img.GetColor(x, y).Red() * 0.5);
  }
}

void HUDView::ColorizeBitmap(Bitmap& img, BYTE* shades, Color color, bool force_alpha)
{
  if (!shades)
    return;
...
bool HUDView::IsNameCrowded(int x, int y)
{
  for (int i = 0; i < MAX_CONTACT; i++)
  {
...
void HUDView::DrawDiamond(int x, int y, int r, Color c)
{
  POINT diamond[4];
