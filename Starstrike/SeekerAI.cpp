...
void SeekerAI::Navigator()
{
  ...
  {
    Steer s = SeekTarget();
    self->ApplyYaw(static_cast<float>(s.yaw));
    self->ApplyPitch(static_cast<float>(s.pitch));
  }
}

void SeekerAI::SetTarget(SimObject* targ, System* sub)
{
  if (!orig_target && targ && targ->Type() == SimObject::SIM_SHIP)
  {
    orig_target = static_cast<Ship*>(targ);
    Observe(orig_target);
  }
...
void SeekerAI::FindObjective()
{
  ...
  shot->SetEta(static_cast<int>(time));

  if (shot->Owner())
    ((Ship*)shot->Owner())->SetMissileEta(shot->Identity(), static_cast<int>(time));
}

void SeekerAI::CheckDecoys(double target_distance)
{
  ...
  if (target && target == orig_target && orig_target->GetActiveDecoys().size())
  {
...
bool SeekerAI::Overshot() { return overshot; }
...
bool SeekerAI::Update(SimObject* obj)
{
  if (obj == target)
  {
    if (obj->Type() == SimObject::SIM_SHOT && orig_target != nullptr)
      target = orig_target;
  }
...
std::string SeekerAI::GetObserverName() const
{
  static char name[64];
  sprintf_s(name, "SeekerAI(%s)", self->Name().c_str());
  return name;
}
