...
TerrainApron::TerrainApron(Terrain* terr, const Bitmap* patch, const Rect& r, const Point& p1, const Point& p2)
  : terrain(terr),
    rect(r)
{
  size = fabs(p2.x - p1.x);
...
TerrainApron::~TerrainApron() { delete [] heights; }
...
bool TerrainApron::BuildApron()
{
  ...
  if (s)
  {
    ...
    for (i = 0; i < ds1; i++)
    {
      for (j = 0; j < ds1; j++)
      {
        *pVert++ = Vec3((float)(j * scale - (HALF_PATCH_SIZE * scale)), (float)(heights[i * PATCH_SIZE + j]),
    ...
    for (i = 0; i < ds1; i++) { for (j = 0; j < ds1; j++) { vset->nrm[i * ds1 + j] = normal; } }
  }

  return true;
}

int TerrainApron::CollidesWith(Graphic& o) { return 0; }

void TerrainApron::Update() {}

void TerrainApron::Illuminate(Color ambient, List<Light>& lights)
{
  ...
  for (int i = 0; i < nverts; i++) { vset->diffuse[i] = aval; }
...
void TerrainApron::Render(Video* video, DWORD flags)
{
  ...
  if (scene) { Illuminate(scene->Ambient(), scene->Lights()); }
...
int TerrainApron::CheckRayIntersection(Point Q, Point w, double len, Point& ipt, bool ttpas)
{
