  ...
  : SimObject("Debris", SIM_DEBRIS)
{
  ...
  Physical::ApplyTorque(torque);
}

int Debris::HitBy(Shot* shot, Point& impact)
{
  if (!shot->IsArmed())
    return 0;
...
void Debris::ExecFrame(double seconds)
{
...
double Debris::AltitudeAGL() const
{
  Point loc = Location();
