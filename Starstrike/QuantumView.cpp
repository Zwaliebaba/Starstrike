...
void QuantumView::Close() { delete quantum_menu; }

QuantumView* QuantumView::quantum_view = nullptr;
...
std::string QuantumView::GetObserverName() const { return "QuantumView"; }
    ...
    if (show_menu)
    {
      ...
      for (int i = 0; i < max_items; i++)
      {
        if (Keyboard::KeyDown('1' + i))
        {
          ...
          {
            auto rgn = reinterpret_cast<SimRegion*>(item->GetData());
...
void QuantumView::SetColor(Color c) { hud_color = c; }

bool QuantumView::IsMenuShown() { return show_menu; }
...
void QuantumView::CloseMenu() { show_menu = false; }

Menu* QuantumView::GetQuantumMenu(Ship* s)
{
  if (s && sim)
  {
    if (s->IsStarship())
    {
      ...
      while (++iter)
      {
        ...
        if (rgn != ship->GetRegion() && !rgn->IsAirSpace() && rgn_system == current_system)
          rgn_list.append(rgn);
      }
      ...
      while (++iter)
      {
        ...
        if (rgn != ship->GetRegion() && rgn->Type() != SimRegion::AIR_SPACE && rgn_system != current_system &&
          current_region->Links().contains(rgn))
          rgn_list.append(rgn);
      }
      ...
      while (++iter)
      {
        ...
        if (rgn_system != current_system)
          sprintf_s(text, "%d. %s/%s", n++, rgn_system->Name().c_str(), rgn->Name().c_str());
        else
          sprintf_s(text, "%d. %s", n++, rgn->Name().c_str());
