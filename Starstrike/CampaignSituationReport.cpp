...
void CampaignSituationReport::GlobalSituation()
{
  if (campaign->GetTime() < 40 * 3600)
    sitrep = std::string(campaign->Name()) + std::string(" is still in its early stages and the situation is ");
  else
    sitrep = std::string("The overall outlook for ") + std::string(campaign->Name()) + std::string(" is ");
  ...
  {
    sitrep += std::string("Establishing and maintaining military control of the ") + mission->GetStarSystem()->Name() +
...
void CampaignSituationReport::MissionSituation()
{
  if (mission)
  {
    ...
    std::string threat = GetThreatInfo();
    std::string sector = mission->GetRegion();
  ...
  if (p)
  {
    if (p->Rank() > 6)
      ...
      rank = std::string(", ") + Player::RankName(p->Rank());
  ...
  {
    case 0:
      sitrep += "You'd better go get to it!";
      break;
    case 1:
      sitrep += "And let's be careful out there!";
      break;
    case 2:
      sitrep += "Good luck, sir!";
      break;
    case 3:
      sitrep += "Let's keep up the good work out there.";
      break;
    case 4:
      sitrep += "Don't lose your focus.";
      break;
    case 5:
      sitrep += "Good luck out there.";
      break;
    case 6:
      sitrep += "What are you waiting for, cocktail hour?";
      break;
    case 7:
      sitrep += std::string("Godspeed") + rank + "!";
      break;
    case 8:
      sitrep += std::string("Good luck") + rank + "!";
      break;
    case 9:
      sitrep += std::string("Good luck") + name + "!";
      break;
    case 10:
      sitrep += "If everything is clear, get your team ready and get underway.";
      break;
    case 11:
      sitrep += std::string("Go get to it") + rank + "!";
      break;
    case 12:
      sitrep += "The clock is ticking, so let's move it!";
      break;
    case 13:
      sitrep += "Stay sharp out there!";
      break;
    case 14:
      sitrep += std::string("Go get 'em") + rank + "!";
      break;
    case 15:
      sitrep += "Now get out of here and get to work!";
      break;
  }
}

MissionElement* CampaignSituationReport::FindEscort(MissionElement* player)
{
...
std::string CampaignSituationReport::GetThreatInfo()
{
  std::string threat_info;
  ...
  if (mission && mission->GetPlayer())
  {
    ...
    while (++elem)
    {
      if (elem->GetIFF() > 0 && elem->GetIFF() != iff && elem->IntelLevel() > Intel::SECRET)
      {
        ...
        if (mission_role == Mission::STRIKE || mission_role == Mission::INTEL || mission_role == Mission::CARGO ||
