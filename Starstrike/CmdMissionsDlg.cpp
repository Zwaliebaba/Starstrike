...
void CmdMissionsDlg::RegisterControls()
{
  lst_missions = static_cast<ListBox*>(FindControl(401));
  txt_desc = FindControl(402);
  btn_accept = static_cast<Button*>(FindControl(403));
  ...
  if (lst_missions) { REGISTER_CLIENT(EID_SELECT, lst_missions, CmdMissionsDlg, OnMission); }
  ...
  if (campaign)
  {
    if (lst_missions)
    {
      ...
      for (int i = 0; i < missions.size(); i++)
      {
        ...
        lst_missions->AddItemWithData(info->m_name, info->m_id);
  ...
  if (campaign)
  {
    ...
    if (missions.size() > lst_missions->NumItems())
    {
      while (missions.size() > lst_missions->NumItems())
      {
        ...
        int i = lst_missions->AddItemWithData(info->m_name, info->m_id) - 1;
    ...
    {
      uint64_t id = lst_missions->GetItemData(0);
...
void CmdMissionsDlg::OnSave(AWEvent* event) { CmdDlg::OnSave(event); }

void CmdMissionsDlg::OnExit(AWEvent* event) { CmdDlg::OnExit(event); }

void CmdMissionsDlg::OnMode(AWEvent* event) { CmdDlg::OnMode(event); }

void CmdMissionsDlg::OnMission(AWEvent* event)
{
  if (campaign && lst_missions)
  {
    ...
    btn_accept->SetEnabled((info != nullptr));

    if (info)
    {
      std::string desc("<font Limerick12><color ffff80>");
      desc += info->m_name;
...
void CmdMissionsDlg::OnAccept(AWEvent* event)
{
  if (!campaign || !mission)
  {
    ::DebugTrace("  ERROR CMD::Accept campaign=0x{:08x} mission=0x{:08x}\n", (size_t)campaign, (size_t)mission);
    return;
  }

  DebugTrace("  CMD::Accept Mission {}\n", mission->Identity());
