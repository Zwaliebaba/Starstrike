...
double Steer::Magnitude() const { return sqrt(yaw * yaw + pitch * pitch); }

Director* SteerAI::Create(SimObject* self, int type)
{
  switch (type)
  {
    case SEEKER:
...
void SteerAI::SetTarget(SimObject* targ, System* sub)
{
  if (target != targ)
  {
    target = targ;
...
void SteerAI::DropTarget(double dtime) { SetTarget(nullptr); }

bool SteerAI::Update(SimObject* obj)
{
  if (obj == target)
  {
    target = nullptr;
...
std::string SteerAI::GetObserverName() const
{
  static char name[64];
  sprintf_s(name, "SteerAI(%s)", self->Name().c_str());
  return name;
}
...
Point SteerAI::Transform(const Point& pt)
{
  ...
  if (self->FlightPathYawAngle() != 0 || self->FlightPathPitchAngle() != 0)
  {
    ...
    constexpr double MAX_ANGLE = 15 * DEGREES;
    constexpr double MIN_ANGLE = 3 * DEGREES;
    ...
    result = Point(obj_t * cam.vrt(), obj_t * cam.vup(), obj_t * cam.vpn());
  }
  else
  {
    auto& cam = (Camera&)self->Cam();   // cast away const
...
Point SteerAI::AimTransform(const Point& pt)
{
  auto& cam = (Camera&)self->Cam();   // cast away const
  Point obj_t = pt - self->Location();

  auto result = Point(obj_t * cam.vrt(), obj_t * cam.vup(), obj_t * cam.vpn());
...
int SteerAI::Accumulate(const Steer& steer)
{
  int overflow = 0;
...
Steer SteerAI::Seek(const Point& point)
{
  Steer s;
...
Steer SteerAI::Flee(const Point& pt)
{
  Steer s;
...
Steer SteerAI::Avoid(const Point& point, float radius)
{
  Steer s;
...
Steer SteerAI::Evade(const Point& point, const Point& vel)
{
  Steer evade;
