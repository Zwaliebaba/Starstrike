...
constexpr int TIME_NEVER = static_cast<int>(1e9);
constexpr int ONE_DAY = 24 * 3600;
...
MissionInfo::~MissionInfo() { delete mission; }
...
TemplateList::~TemplateList() { missions.destroy(); }

static List<Campaign> campaigns;
static Campaign* current_campaign = nullptr;

Campaign::Campaign(int id, std::string_view n)
  : campaign_id(id),
...
Campaign::Campaign(int id, std::string_view n, std::string_view p)
  : campaign_id(id),
    status(CAMPAIGN_INIT),
    m_path(p),
    m_name(n),
...
void Campaign::Close()
{
  DebugTrace("Campaign::Close() - destroying all campaigns\n");
  current_campaign = nullptr;
  campaigns.destroy();
}

Campaign* Campaign::GetCampaign() { return current_campaign; }

Campaign* Campaign::SelectCampaign(std::string_view name)
{
  Campaign* c = nullptr;
  ...
  if (c)
  {
    DebugTrace("Campaign: Selected '{}'\n", c->Name());
    current_campaign = c;
  }
  else
    DebugTrace("Campaign: could not find '{}'\n", name);

  return c;
}

List<Campaign>& Campaign::GetAllCampaigns() { return campaigns; }
...
void Campaign::Load()
{
  ...
  if (!m_path[0])
  {
    ...
    switch (campaign_id)
    {
      ...
      case MULTIPLAYER_MISSIONS:
        ...
        break;
      default:
        m_path = std::format("Campaigns\\{:02d}\\", campaign_id);
        break;
    }
  }

  DataLoader* loader = DataLoader::GetLoader();
  loader->SetDataPath(m_path);
  systems.clear();
  ...
  for (int i = 0; i < zones.size(); i++)
  {
    ...
    if (!found)
      systems.append(Galaxy::GetInstance()->GetSystem(s.c_str()));
  }
  ...
  {
    loader->LoadBitmap("image.pcx", m_image[0]);
    loader->LoadBitmap("selected.pcx", m_image[1]);
    loader->LoadBitmap("unavail.pcx", m_image[2]);
    loader->LoadBitmap("banner.pcx", m_image[3]);
  }
      ...
      if (def)
      {
        if (def->name()->value() == "name")
        {
          if (!def->term() || !def->term()->isText())
            DebugTrace("WARNING: name missing in '{}\\{}'\n", loader->GetDataPath(), filename);
          else
          {
            m_name = def->term()->isText()->value();
        ...
        {
          if (!def->term() || !def->term()->isText())
            DebugTrace("WARNING: description missing in '{}\\{}'\n", loader->GetDataPath(), filename);
          else
          {
        ...
        {
          if (!def->term() || !def->term()->isText())
            DebugTrace("WARNING: situation missing in '{}\\{}'\n", loader->GetDataPath(), filename);
          else
          {
        ...
        {
          if (!def->term() || !def->term()->isText())
            DebugTrace("WARNING: orders missing in '{}/{}'\n", loader->GetDataPath(), filename);
          else
          {
        ...
        else if (full && def->name()->value() == "combatant")
        {
          ...
          else
          {
            ...
            loader->SetDataPath(m_path);
            auto c = NEW Combatant(name, clone);
            if (c)
              combatants.append(c);
            else
            {
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: action struct missing in '{}/{}'\n", loader->GetDataPath(), filename);
          else
          {
            TermStruct* val = def->term()->isStruct();
...
void Campaign::ParseGroup(TermStruct* val, CombatGroup* force, CombatGroup* clone, std::string_view filename)
{
  if (!val)
  {
    DebugTrace("invalid combat group in campaign {}\n", m_name.data());
    return;
  }
...
void Campaign::ParseAction(TermStruct* val, std::string_view filename)
{
  if (!val)
  {
    DebugTrace("invalid action in campaign {}\n", m_name.data());
    return;
  }
  ...
  for (int i = 0; i < val->elements()->size(); i++)
  {
    ...
    if (pdef)
    {
      ...
      else if (pdef->name()->value() == "type")
      {
        ...
        type = CombatAction::TypeFromName(txt.c_str());
      }
      else if (pdef->name()->value() == "subtype")
      {
        ...
        else if (pdef->term()->isText())
        {
          ...
          if (type == CombatAction::MISSION_TEMPLATE)
            ...
            subtype = CombatEvent::TypeFromName(txt.c_str());
          else if (type == CombatAction::INTEL_EVENT)
            subtype = Intel::IntelFromName(txt.c_str());
        }
      }
      else if (pdef->name()->value() == "opp_type")
      {
        ...
        else if (pdef->term()->isText())
        {
          ...
          if (type == CombatAction::MISSION_TEMPLATE)
            opp_type = Mission::TypeFromName(txt.c_str());
        }
      }
      else if (pdef->name()->value() == "source")
      {
        std::string txt;
        GetDefText(txt, pdef, filename);
        source = CombatEvent::SourceFromName(txt.c_str());
      }
      ...
      else if (pdef->name()->value() == "min_rank")
      {
        ...
        else
        {
          ...
          min_rank = Player::RankFromName(rank_name.c_str());
        }
      }
      else if (pdef->name()->value() == "max_rank")
      {
        ...
        else
        {
          std::string rank_name;
          GetDefText(rank_name, pdef, filename);
          max_rank = Player::RankFromName(rank_name.c_str());
        }
      }
      ...
      else if (pdef->name()->value() == "asset_kill")
      {
        ...
        if (action)
        {
          std::string txt;
          GetDefText(txt, pdef, filename);
          action->AssetKills().emplace_back(txt);
        }
      }
      ...
      else if (pdef->name()->value() == "target_kill")
      {
        ...
        if (action)
        {
          std::string txt;
          GetDefText(txt, pdef, filename);
          action->TargetKills().emplace_back(txt);
        }
      }
      ...
      else if (pdef->name()->value() == "req")
      {
        ...
        if (!pdef->term() || !pdef->term()->isStruct())
          DebugTrace("WARNING: action req struct missing in '{}'\n", filename);
        else if (action)
        {
          ...
          for (int i = 0; i < val2->elements()->size(); i++)
          {
            ...
            if (pdef2)
            {
              ...
              else if (pdef2->name()->value() == "status")
              {
                ...
                stat = CombatAction::StatusFromName(txt.c_str());
              }
              ...
              else if (pdef2->name()->value() == "c1")
              {
                std::string txt;
                GetDefText(txt, pdef2, filename);
                c1 = GetCombatant(txt.c_str());
              }
              else if (pdef2->name()->value() == "c2")
              {
                std::string txt;
                GetDefText(txt, pdef2, filename);
                c2 = GetCombatant(txt.c_str());
              }
              else if (pdef2->name()->value() == "comp")
              {
                std::string txt;
                GetDefText(txt, pdef2, filename);
                comp = CombatActionReq::CompFromName(txt.c_str());
              }
              ...
              else if (pdef2->name()->value() == "intel")
              {
                ...
                else if (pdef2->term()->isText())
                {
                  std::string txt;
                  GetDefText(txt, pdef2, filename);
                  intel = Intel::IntelFromName(txt.c_str());
                }
              }
              else if (pdef2->name()->value() == "group_type")
              {
                ...
                gtype = CombatGroup::TypeFromName(type_name.c_str());
              }
...
void Campaign::LoadMissionList(DataLoader* loader)
{
  ...
  {
    DebugTrace("WARNING: invalid mission list file '{}'\n", filename);
    term->print(10);
    return;
  }
  ...
  do
  {
    ...
    if (term)
    {
      ...
      if (def)
      {
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: mission struct missing in '{}'\n", filename);
          else
          {
            TermStruct* val = def->term()->isStruct();
...
void Campaign::LoadTemplateList(DataLoader* loader)
{
  ...
  {
    DebugTrace("WARNING: invalid template list file '{}'\n", filename);
    term->print(10);
    return;
  }
...
void Campaign::SetPlayerGroup(CombatGroup* pg)
{
  if (player_group != pg)
  {
    DebugTrace("Campaign::SetPlayerGroup({})\n", pg ? pg->Name().data() : "0");
    ...
    if (IsDynamic())
    {
      DebugTrace("  destroying mission list...\n");
      missions.destroy();
    }
  }
}
...
void Campaign::SetPlayerUnit(CombatUnit* unit)
{
  if (player_unit != unit)
  {
    DebugTrace("Campaign::SetPlayerUnit({})\n", unit ? unit->Name().data() : "0");
...
CombatZone* Campaign::GetZone(std::string_view rgn)
{
...
StarSystem* Campaign::GetSystem(std::string_view sys) { return Galaxy::GetInstance()->GetSystem(sys); }

Combatant* Campaign::GetCombatant(std::string_view cname)
{
...
Mission* Campaign::GetMission() { return GetMission(mission_id); }

Mission* Campaign::GetMission(int id)
{
  if (id < 0)
  {
    DebugTrace("ERROR - Campaign::GetMission({}) invalid mission id\n", id);
    return nullptr;
  }
  ...
  if (info)
  {
    if (!info->mission)
    {
      DebugTrace("Campaign::GetMission({}) loading mission...\n", id);
      info->mission = NEW Mission(id, info->script, m_path);
      if (info->mission)
        info->mission->Load();
    }
    ...
    if (IsDynamic())
    {
      if (info->mission)
      {
        ...
        {
          DebugTrace("Campaign::GetMission({}) generating sitrep...\n", id);
          CampaignSituationReport sitrep(this, info->mission);
          sitrep.GenerateSituationReport();
        }
      }
      else
        DebugTrace("Campaign::GetMission({}) could not find/load mission.\n", id);
    }
...
Mission* Campaign::GetMissionByFile(std::string_view filename)
{
  if (filename.empty())
  {
    DebugTrace("ERROR - Campaign::GetMissionByFile() invalid filename\n");
    return nullptr;
  }
  ...
  for (int i = 0; !info && i < missions.size(); i++)
  {
    ...
    maxid = std::max<size_t>(m->m_id, maxid);
...
MissionInfo* Campaign::CreateNewMission()
{
  ...
  {
    info->m_id = maxid + 1;
    info->m_name = "New Custom Mission";
    info->script = filename;
    info->mission = NEW Mission(info->m_id, filename, m_path);
    info->mission->SetName(info->m_name);
...
void Campaign::DeleteMission(int id)
{
  if (id < 0)
  {
    DebugTrace("ERROR - Campaign::DeleteMission({}) invalid mission id\n", id);
    return;
  }
  ...
  if (m)
  {
    ...
    if (m_path[m_path.size() - 1] == '\\')
      sprintf_s(full_path, "%s%s", m_path.c_str(), m->script.c_str());
    else
      sprintf_s(full_path, "%s\\%s", m_path.c_str(), m->script.c_str());
    ...
    DebugTrace("ERROR - Campaign::DeleteMission({}) could not find mission\n", id);
}

MissionInfo* Campaign::GetMissionInfo(size_t id)
{
  MissionInfo* m = nullptr;
  for (size_t i = 0; !m && i < missions.size(); i++)
  {
    if (missions[i]->m_id == id)
      m = missions[i];
  }
  ...
  DebugTrace("ERROR - Campaign::GetMissionInfo({}) could not find mission\n", id);
...
void Campaign::LoadNetMission(int id, std::string_view net_mission_script)
{
  if (mission && mission == net_mission)
  {
    delete net_mission;
...
MissionInfo* Campaign::FindMissionTemplate(int mission_type, CombatGroup* player_group)
{
  MissionInfo* info = nullptr;
...
TemplateList* Campaign::GetTemplateList(int msn_type, int grp_type)
{
  ...
  {
    if (templates[i]->mission_type == msn_type && templates[i]->group_type == grp_type)
      return templates[i];
  }
...
{
  DebugTrace("Campaign::SetMissionId({})\n", id);

  if (id > 0)
    mission_id = id;
  else
    DebugTrace("   retaining mission id = {}\n", mission_id);
}

double Campaign::Stardate() { return StarSystem::Stardate(); }
...
void Campaign::ExecFrame()
{
  ...
  if (IsDynamic())
  {
    ...
    while (++m)
    {
      ...
      {
        DebugTrace("Campaign::ExecFrame() completed mission {} '{}'\n", m->m_id, m->m_name.data());
        completed = true;
      }
    }
    ...
    if (completed)
    {
      DebugTrace("Campaign::ExecFrame() destroying mission list after completion...\n");
      missions.destroy();
    ...
    else
    {
      ...
      while (++m)
      {
        if (m->start < time && !m->mission->IsActive())
        {
          ...
          if (info)
          {
            DebugTrace("Campaign::ExecFrame() deleting expired mission {} start: {} current: {}\n", info->m_id,
                       info->start, static_cast<int>(time));
          }
    ...
    {
      auto plan_event = static_cast<CampaignPlanEvent*>(planners.first());
...
void Campaign::LockoutEvents(int seconds) { lockout = seconds; }
...
void Campaign::StartMission()
{
  ...
  if (m)
  {
    ...
    if (!scripted)
    {
      double gtime = static_cast<double>(Game::GameTime()) / 1000.0;
      ...
      DebugTrace("  current time:  {}\n", buffer);

      FormatDayTime(buffer, m->Start());
      DebugTrace("  mission start: {}\n", buffer);
      DebugTrace("\n");
    }
  }
}
...
{
  DebugTrace("Campaign::RollbackMission()\n");
...
bool Campaign::IsDynamic() const { return campaign_id >= DYNAMIC_CAMPAIGN && campaign_id < SINGLE_MISSIONS; }

bool Campaign::IsTraining() const { return campaign_id == TRAINING_CAMPAIGN; }

bool Campaign::IsScripted() const { return scripted; }

bool Campaign::IsSequential() const { return sequential; }
...
CombatGroup* Campaign::FindGroup(int iff, int type, int id)
{
  CombatGroup* result = nullptr;
...
CombatGroup* Campaign::FindGroup(int iff, int type, CombatGroup* near_group)
{
  CombatGroup* result = nullptr;
...
{
  if (!strike_group || !strike_group->GetAssignedZone())
    return;

  if (g->IsStrikeTarget() && g->IntelLevel() > Intel::RESERVE)
  {
    if (strike_group->GetAssignedZone() == g->GetAssignedZone() || strike_group->GetAssignedZone()->HasRegion(
      g->GetRegion()))
      groups.append(g);
  }
...
CombatGroup* Campaign::FindStrikeTarget(int iff, CombatGroup* strike_group)
{
  CombatGroup* result = nullptr;
...
void Campaign::SetStatus(int s)
{
  ...
  if (status > CAMPAIGN_ACTIVE)
  {
    DebugTrace("Campaign::SetStatus() destroying mission list at campaign end\n");
    missions.destroy();
  }
}
...
int Campaign::GetAllCombatUnits(int iff, List<CombatUnit>& units)
{
  units.clear();
