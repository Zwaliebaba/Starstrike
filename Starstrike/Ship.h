...
class Ship : public SimObject, public SimObserver
{
  public:
    ...
    // CONSTRUCTORS:
    Ship(std::string_view ship_name, std::string_view reg_num, ShipDesign* design, int IFF = 0, int cmd_ai = 0,
    ...
    void AeroFrame(double seconds) override;
    virtual void StatFrame(double seconds);
    virtual void DockFrame(double seconds);
    void LinearFrame(double seconds) override;
    ...
    void SetRegion(SimRegion* rgn) override;
    virtual int GetTextureList(List<Bitmap>& textures);

    // DIRECTION:
    virtual void SetControls(MotionController* m);
    virtual void SetNetworkControl(Director* net_ctrl = nullptr);
    void SetDirectorInfo(std::string_view msg) { director_info = msg; }
    std::string GetDirectorInfo() const { return director_info; }
    void SetAIMode(int n) { ai_mode = static_cast<BYTE>(n); }
    int GetAIMode() const { return ai_mode; }
    void SetCommandAILevel(int n) { command_ai_level = static_cast<BYTE>(n); }
    ...
    void ApplyPitch(double pitch_acc) override; // override for G limiter
    ...
    void SetTransX(double t) override;
    void SetTransY(double t) override;
    void SetTransZ(double t) override;
    ...
    virtual bool CheckFire() const { return (check_fire || net_observer_mode); }
    ...
    std::string GetObserverName() const override { return m_name; }
    ...
    virtual void SetTarget(SimObject* t, System* sub = nullptr, bool from_net = false);
    ...
    virtual void LockTarget(int type = SIM_SHIP, bool closest = false, bool hostile = false);
    ...
    // SHIELD SYSTEMS:
    virtual double InflictDamage(double damage, Shot* shot = nullptr, int hit_type = 3,
                                 Point hull_impact = Point(0, 0, 0));

    virtual double InflictSystemDamage(double damage, Shot* shot, Point impact);

    virtual void InflictNetDamage(double damage, Shot* shot = nullptr);
    ...
    int CollidesWith(Physical& o) override;
    ...
    void SetName(std::string_view ident) { m_name = ident; }
    const ShipDesign* Design() const { return design; }
    std::string Abbreviation() const;
    std::string DesignName() const;
    std::string DesignFileName() const;
    static std::string_view ClassName(int c);
    ...
    void SetRespawnLoc(const Point& rl) { respawn_loc = rl; }

    double WarpFactor() const { return warp_fov; }
    void SetWarp(double w) { warp_fov = static_cast<float>(w); }
    ...
    void ClearCaution() { master_caution = false; }
  ...
  protected:
    int CheckShotIntersection(Shot* shot, Point& ipt, Point& hpt, Weapon** wep = nullptr);
    WeaponGroup* FindWeaponGroup(std::string_view name);
