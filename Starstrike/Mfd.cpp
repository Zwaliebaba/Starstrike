...
MFD::~MFD() { GRAPHIC_DESTROY(sprite); }
...
void MFD::SetRect(const Rect& r)
{
  ...
  if (sprite) { sprite->MoveTo(Point(rect.x + sprite->Width() / 2, rect.y + sprite->Height() / 2, 1)); }
}
...
void MFD::DrawSensorLabels(const char* mfd_mode)
{
  ...
  {
    case Sensor::PAS:
      strcpy_s(mode_buf, Game::GetText("MFD.mode.passive").data());
      break;
    case Sensor::STD:
      strcpy_s(mode_buf, Game::GetText("MFD.mode.standard").data());
      break;
    case Sensor::ACM:
      strcpy_s(mode_buf, Game::GetText("MFD.mode.auto-combat").data());
      break;
    case Sensor::GM:
      strcpy_s(mode_buf, Game::GetText("MFD.mode.ground").data());
      break;
    case Sensor::PST:
      strcpy_s(mode_buf, Game::GetText("MFD.mode.passive").data());
      break;
    case Sensor::CST:
      strcpy_s(mode_buf, Game::GetText("MFD.mode.combined").data());
      break;
    default:
      break;
  }
  ...
  if (beam_range >= 1e6)
    sprintf_s(range_txt, "-%dM+", static_cast<int>(beam_range / 1e6));
  else
    sprintf_s(range_txt, "-%3d+", static_cast<int>(beam_range / 1e3));
  ...
  if (Mouse::LButton() && !mouse_latch)
  {
    ...
    if (mode_rect.Contains(Mouse::X(), Mouse::Y()))
    {
      if (sensor->GetMode() < Sensor::PST)
      {
        ...
        sensor->SetMode(static_cast<Sensor::Mode>(sensor_mode));
      }
    }
...
void MFD::DrawSensorMFD()
{
  ...
  if (sensor->GetBeamLimit() > 90 * DEGREES)
    sweep_scale = static_cast<double>(r) / (90 * DEGREES);

  int az = static_cast<int>(sensor->GetBeamLimit() * sweep_scale);
  int el = az;
  int xc = static_cast<int>(scan_x + xctr);
  int yc = static_cast<int>(scan_y + yctr);
  ...
  {
    char az_txt[8];
    sprintf_s(az_txt, "%d", static_cast<int>(sensor->GetBeamLimit() / DEGREES));
  ...
  if (navpt && navpt->Region() == ship->GetRegion())
  {
    ...
    if (tz > 1.0)
    {
      ...
      if (fabs(az) < 90 * DEGREES)
      {
        ...
        int x = static_cast<int>(r + az);
        int y = static_cast<int>(r - el);
...
void MFD::DrawHSD()
{
  ...
  int xc = static_cast<int>(xctr) + scan_x;
  int yc = static_cast<int>(yctr) + scan_y;
  ...
  if (sensor->GetMode() < Sensor::PST)
  {
    ...
    int x0 = static_cast<int>(0.1 * r * s);
    int y0 = static_cast<int>(0.1 * r * c);
    int x1 = static_cast<int>(1.0 * r * s);
    int y1 = static_cast<int>(1.0 * r * c);
  ...
  double rscale = static_cast<double>(r) / (sensor->GetBeamRange());

  Camera hsd_cam = ship->Cam();
  Point look = ship->Location() + ship->Heading() * 1000.0f;
  ...
  for (int dir = 0; dir < 4; dir++)
  {
    ...
    {
      int x0 = static_cast<int>(sin(az) * r * range);
      int y0 = static_cast<int>(cos(az) * r * range);
      int x1 = static_cast<int>(sin(az) * r * (range + 0.1));
      int y1 = static_cast<int>(cos(az) * r * (range + 0.1));
  ...
  if (navpt && navpt->Region() == ship->GetRegion())
  {
    ...
    // draw:
    int x = static_cast<int>(xc + sin(az) * rng * rscale);
    int y = static_cast<int>(yc - cos(az) * rng * rscale);
  ...
  while (++contact)
  {
    ...
    // draw:
    int x = static_cast<int>(xc + sin(az) * rg * rscale);
    int y = static_cast<int>(yc - cos(az) * rg * rscale);
    int size = 2;
...
void MFD::Draw3D()
{
  ...
  if (navpt && navpt->Region() == ship->GetRegion())
  {
    ...
    // draw:
    int x = static_cast<int>(sin(az) * rng * rscale);
    int y = static_cast<int>(cos(az) * rng * rscale / 2);
    int z = static_cast<int>(ty);
  ...
  while (++contact)
  {
    ...
    // draw:
    int x = static_cast<int>(sin(az) * rg * rscale);
    int y = static_cast<int>(cos(az) * rg * rscale / 2);
    int z = static_cast<int>(ty * rscale / 2);
    int size = 1;
...
void MFD::DrawMap()
{
  auto rc = Rect(rect.x, rect.y, rect.w, 12);
  DrawMFDText(0, Game::GetText("MFD.mode.ground").data(), rc, DT_CENTER);
}

void MFD::DrawGauge(int x, int y, int percent)
{
  if (cockpit_hud_texture)
  {
...
void MFD::DrawGameMFD()
{
  ...
  if (ship)
  {
    ...
    {
      case Ship::DOCKED:
        DrawMFDText(t++, Game::GetText("MFD.phase.DOCKED").data(), txt_rect, DT_LEFT);
        break;
      case Ship::ALERT:
        DrawMFDText(t++, Game::GetText("MFD.phase.ALERT").data(), txt_rect, DT_LEFT);
        break;
      case Ship::LOCKED:
        DrawMFDText(t++, Game::GetText("MFD.phase.LOCKED").data(), txt_rect, DT_LEFT);
        break;
      case Ship::LAUNCH:
        DrawMFDText(t++, Game::GetText("MFD.phase.LAUNCH").data(), txt_rect, DT_LEFT);
        break;
      case Ship::TAKEOFF:
        DrawMFDText(t++, Game::GetText("MFD.phase.TAKEOFF").data(), txt_rect, DT_LEFT);
        break;
      case Ship::ACTIVE:
        DrawMFDText(t++, Game::GetText("MFD.phase.ACTIVE").data(), txt_rect, DT_LEFT);
        break;
      case Ship::APPROACH:
        DrawMFDText(t++, Game::GetText("MFD.phase.APPROACH").data(), txt_rect, DT_LEFT);
        break;
      case Ship::RECOVERY:
        DrawMFDText(t++, Game::GetText("MFD.phase.RECOVERY").data(), txt_rect, DT_LEFT);
        break;
      case Ship::DOCKING:
        DrawMFDText(t++, Game::GetText("MFD.phase.DOCKING").data(), txt_rect, DT_LEFT);
        break;
    }
  }
}
...
void MFD::DrawStatusMFD()
{
  ...
  if (ship)
  {
    ...
    if (drive)
    {
      ...
      DrawGauge(status_rect.x + 70, status_rect.y, static_cast<int>(ship->Throttle()));
      status_rect.y += 10;
    }
    ...
    if (quantum_drive)
    {
      ...
      DrawGauge(status_rect.x + 70, status_rect.y, quantum_drive->Charge());
      status_rect.y += 10;
    }
    ...
    DrawGauge(status_rect.x + 70, status_rect.y, static_cast<int>(hull));
    status_rect.y += 10;
    ...
    if (HUDView::IsArcade())
    {
      for (int i = 0; i < ship->Weapons().size() && i < 4; i++)
      {
        ...
        if (w->IsMissile())
        {
          ...
          DrawMFDText(row++, w->GetDesign()->name, status_rect, DT_LEFT);
          status_rect.x += 70;
...
bool MFD::IsMouseLatched() const { return mouse_in; }

void MFD::DrawMFDText(int index, std::string_view txt, Rect& txt_rect, int align, int status)
{
  if (index >= TXT_LAST)
    DebugTrace("MFD DrawMFDText() invalid mfd_text index %d '%s'\n", index, txt);
  else
  {
...
{
  if (index >= TXT_LAST)
    DebugTrace("MFD HideMFDText() invalid mfd_text index %d\n", index);
  else
