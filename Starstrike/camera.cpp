...
void Camera::Get2DScreenPos (LegacyVector3 const &_vector, float *_screenX, float *_screenY)
{
...
void Camera::AdvanceSphereWorldMode()
{
	...
	LegacyVector3 focusPos = LegacyVector3(0, m_height * -400,0);
...
float Camera::DistanceToBlockage(LegacyVector3 const &_dir, float const _maxDist)
{
...
void Camera::GetHighestTangentPoint( LegacyVector3 const &_from, LegacyVector3 const &_to, float _maxDist, LegacyVector3 &location )
{
	unsigned int const numSteps = 40;
	float const distStep = _maxDist / (float)numSteps;
...
void Camera::GetHighestPoint( LegacyVector3 const &_from, LegacyVector3 const &_to, float _maxDist, LegacyVector3 &location )
{
	unsigned int const numSteps = 40;
	float const distStep = _maxDist / (float)numSteps;
...
// as a piece of land more than 10 metres higher than the camera's current
// height. If there is no blockage FLT_MAX is returned.
float Camera::DirectDistanceToBlockage(LegacyVector3 const &_from, LegacyVector3 const &_to, float const _maxDist)
{
    if( !g_app->m_location ) return FLT_MAX;
...
bool Camera::PosInViewFrustum(LegacyVector3 const &_pos)
{
...
bool Camera::SphereInViewFrustum(LegacyVector3 const &_centre, float _radius )
{
...
void Camera::SetTarget(LegacyVector3 const &_pos, LegacyVector3 const &_front, LegacyVector3 const &_up)
{
	m_targetPos = _pos;
...
void Camera::SetTarget(LegacyVector3 const &_focusPos, float _distance, float _height )
{
...
LegacyVector3 Camera::GetControlVector()
{
