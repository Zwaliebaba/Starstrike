...
NetGameClient::NetGameClient()
  : server_id(0),
    join_req_time(0)
{
  DebugTrace("Constructing NetGameClient\n");
  ...
  if (server_name.length() && port > 0)
  {
    ...
    DebugTrace("  '%s' invalid game port number %d\n", me.Name(), port);
  else
...
NetGameClient::~NetGameClient()
{
  link->SendMessage(server_id, NET_QUIT_REQUEST, nullptr, 0, NetMsg::RELIABLE);
...
void NetGameClient::SendJoinRequest()
{
  ...
  {
    DebugTrace("  sending join request - name: '%s' elem: '%s' index: %d\n", player_name.data(),
               local_player->GetElement()->Name().data(), local_player->GetElementIndex());
...
void NetGameClient::DoJoinRequest(NetMsg* msg)
{
  ...
  if (join_req.Unpack(msg->Data()))
    DebugTrace("Client received Join Request from '%s'\n", join_req.GetName());
}
...
void NetGameClient::DoJoinAnnounce(NetMsg* msg)
{
  ...
  auto join_ann = NEW NetJoinAnnounce;
  bool saved = false;
  ...
  if (join_ann->Unpack(msg->Data()))
  {
    ...
    std::string name = join_ann->GetName();
    std::string elem_name = join_ann->GetElement();
    std::string region = join_ann->GetRegion();
    ...
    {
      HUDView::Message(Game::GetText("NetGameClient.local-accept", name, local_player->Name()).c_str());
    ...
    else
    {
      ...
      else
      {
        ...
        {
          DebugTrace("NetGameClient::DoJoinAnnounce() could not find elem %s for player '%s' objid %d\n",
        ...
        {
          strcpy_s(ship_name, ship->Name().c_str());
          ...
          auto remote_player = NEW NetPlayer(nid);
          remote_player->SetName(name);
          remote_player->SetObjID(oid);
          ...
          if (name == "Server A.I. Ship")
            DebugTrace("Remote Player '%s' has joined as '%s' with ID %d\n", name.data(), ship_name, oid);
          else
  ...
  if (nid && oid)
  {
    ...
    else
    {
      ...
      if (ship)
      {
        ...
        if (name == "Server A.I. Ship")
          DebugTrace("NetGameClient::DoJoinBacklog() Remote Player '%s' has joined as '%s' with ID %d\n", name.data(),
                     ship_name, oid);
        else
          HUDView::Message(Game::GetText("NetGameClient.remote-join").data(), name.data(), ship_name);
      }
...
void NetGameClient::DoQuitRequest(NetMsg* msg)
{
  ...
  DebugTrace("Client received Quit Request from NetID: %08X\n", msg->NetID());
}
...
void NetGameClient::DoQuitAnnounce(NetMsg* msg)
{
  ...
  if (player)
  {
    ...
    if (quit_ann.GetDisconnected())
      HUDView::Message(Game::GetText("NetGameClient.remote-discon", zombie->Name()).c_str());
    else
      HUDView::Message(Game::GetText("NetGameClient.remote-quit", zombie->Name()).c_str());
    delete zombie;
  }
  else
    DebugTrace("Quit Announce for unknown player %08X disconnected = %d\n", msg->NetID(), quit_ann.GetDisconnected());
}
  ...
  if (ship)
  {
    ...
    if (shot && sim)
    {
      ...
      if (!shot->IsBeam())
      {
        if (owner)
        {
          ShipStats* stats = ShipStats::Find(owner_name.c_str());
...
void NetGameClient::DoObjKill(NetMsg* msg)
{
  ...
  if (ship)
  {
    ...
    std::string killer_name = Game::GetText("NetGameClient.unknown");
    ...
    ShipStats* killee = ShipStats::Find(ship->Name().c_str());
    if (killee)
    {
  ...
  else
  {
    ...
    if (shot)
    {
      DebugTrace("NetGameClient::DoObjKill destroying shot '%s'\n", shot->Name());
      shot->Destroy();
    }
  }
}
...
void NetGameClient::DoObjHyper(NetMsg* msg)
{
  if (!msg)
    return;
  DebugTrace("Client received OBJ HYPER from NetID: %08x\n", msg->NetID());
...
void NetGameClient::DoElemCreate(NetMsg* msg)
{
  ...
  DebugTrace("NetGameClient::DoElemCreate name: %s iff: %d type %s\n", elem_name, elem_create.GetIFF(),
  ...
  if (elem)
  {
    DebugTrace("  element '%' already exists - ignored\n", elem_name);
    return;
  }

  elem = sim->CreateElement(elem_name, elem_create.GetIFF(), elem_create.GetType());
  ...
  else
  {
    ...
    if (shooter)
    {
      ...
      if (decoy)
        ...
        w = shooter->GetProbeLauncher();
      else
        w = shooter->GetWeaponByIndex(index);
...
void NetGameClient::DoWepDestroy(NetMsg* msg)
{
  ...
  {
    if (shot->IsBeam())
      DebugTrace("NetGameClient::DoWepDestroy shot '%s'\n", shot->Name());
...
void NetGameClient::SendData(NetData* net_data)
{
...
void NetGameClient::ExecFrame()
{
  if (local_player)
  {
    ...
    else if (active)
    {
      ...
      {
        NetMsg net_disco(0, NET_DISCONNECT, nullptr, 0, 0);
...
std::string NetGameClient::GetObserverName() const { return "NetGameClient"; }

void NetGameClient::Respawn(DWORD oid, Ship* spawn)
{
  if (!oid || !spawn)
    return;

  DebugTrace("NetGameClient::Respawn(%d, %s)\n", oid, spawn->Name());
  spawn->SetObjID(oid);
  Observe(spawn);
