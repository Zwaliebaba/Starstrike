...
MenuScreen::MenuScreen()
  : screen(nullptr),
    ...
    playdlg(nullptr),
    awarddlg(nullptr),
    msnSelectDlg(nullptr),
    msnEditDlg(nullptr),
    msnElemDlg(nullptr),
    msnEventDlg(nullptr),
    msnEditNavDlg(nullptr),
    loadDlg(nullptr),
    tacRefDlg(nullptr),
    cmpSelectDlg(nullptr),
    firstdlg(nullptr),
    netClientDlg(nullptr),
    netAddrDlg(nullptr),
    netPassDlg(nullptr),
    netLobbyDlg(nullptr),
    netServerDlg(nullptr),
    netUnitDlg(nullptr),
    current_dlg(nullptr),
    isShown(false) { loader = DataLoader::GetLoader(); }

MenuScreen::~MenuScreen() { TearDown(); }

void MenuScreen::Setup(Screen* s)
{
  if (!s)
    return;
...
void MenuScreen::TearDown()
{
  if (screen)
  {
    if (menudlg)
      screen->DelWindow(menudlg);
    if (netClientDlg)
      screen->DelWindow(netClientDlg);
    if (netAddrDlg)
      screen->DelWindow(netAddrDlg);
    if (netPassDlg)
      screen->DelWindow(netPassDlg);
    if (netLobbyDlg)
      screen->DelWindow(netLobbyDlg);
    if (netServerDlg)
      screen->DelWindow(netServerDlg);
    if (netUnitDlg)
      screen->DelWindow(netUnitDlg);

    if (cmpSelectDlg)
      screen->DelWindow(cmpSelectDlg);
    if (awarddlg)
      screen->DelWindow(awarddlg);
    if (firstdlg)
      screen->DelWindow(firstdlg);
    if (msnSelectDlg)
      screen->DelWindow(msnSelectDlg);
    if (msnEditDlg)
      screen->DelWindow(msnEditDlg);
    if (msnElemDlg)
      screen->DelWindow(msnElemDlg);
    if (msnEventDlg)
      screen->DelWindow(msnEventDlg);
    if (msnEditNavDlg)
      screen->DelWindow(msnEditNavDlg);
    if (tacRefDlg)
      screen->DelWindow(tacRefDlg);
    if (loadDlg)
      screen->DelWindow(loadDlg);
    ...
    if (exitdlg)
      screen->DelWindow(exitdlg);
    if (playdlg)
      screen->DelWindow(playdlg);
    if (confirmdlg)
      screen->DelWindow(confirmdlg);
    if (fadewin)
      screen->DelWindow(fadewin);
  }
...
bool MenuScreen::CloseTopmost()
{
  ...
  {
    netUnitDlg->OnCancel(nullptr);
    processed = true;
  }

  else if (netLobbyDlg && netLobbyDlg->IsShown())
  {
    netLobbyDlg->OnCancel(nullptr);
    processed = true;
  }

  else if (netClientDlg && netClientDlg->IsShown())
  {
    netClientDlg->OnCancel(nullptr);
    processed = true;
  }
...
void MenuScreen::ShowCmpSelectDlg()
{
  HideAll();
...
void MenuScreen::ShowMsnSelectDlg()
{
  HideAll();
  ...
  if (msnEditNavDlg)
    msnEditNavDlg->SetMission(nullptr);

  Mouse::Show(true);
}

void MenuScreen::ShowMsnEditDlg()
{
  if (msnEditDlg)
  {
...
void MenuScreen::ShowMsnElemDlg()
{
  if (msnElemDlg)
  {
...
void MenuScreen::ShowMsnEventDlg()
{
  if (msnEventDlg)
  {
    if (msnEditDlg && msnEditDlg->IsShown())
...
void MenuScreen::ShowFirstTimeDlg()
{
  HideAll();
  if (menudlg && firstdlg)
  {
...
void MenuScreen::ShowNetClientDlg()
{
  if (netClientDlg)
  {
    HideAll();
...
void MenuScreen::ShowNetAddrDlg()
{
  if (netAddrDlg)
  {
    if (netClientDlg)
...
void MenuScreen::ShowNetPassDlg()
{
  if (netPassDlg)
  {
    ShowNetClientDlg();
...
void MenuScreen::ShowNetLobbyDlg()
{
  if (netLobbyDlg)
  {
    HideAll();
...
void MenuScreen::ShowNetServerDlg()
{
  if (netServerDlg)
  {
    netServerDlg->Show();
...
void MenuScreen::ShowNetUnitDlg()
{
  if (netUnitDlg)
  {
    HideAll();
...
void MenuScreen::ShowConfirmDlg()
{
  if (confirmdlg)
  {
    if (msnSelectDlg && msnSelectDlg->IsShown())
...
void MenuScreen::HideConfirmDlg()
{
  if (confirmdlg)
    confirmdlg->Hide();
...
void MenuScreen::HideNavDlg()
{
  if (msnEditNavDlg)
    msnEditNavDlg->Hide();
}

void MenuScreen::HideMsnElemDlg()
{
  if (msnElemDlg)
    msnElemDlg->Hide();
...
void MenuScreen::HideMsnEventDlg()
{
  if (msnEventDlg)
    msnEventDlg->Hide();
...
bool MenuScreen::IsNavShown() { return msnEditNavDlg && msnEditNavDlg->IsShown(); }

void MenuScreen::HideAll()
{
  Keyboard::FlushKeys();
