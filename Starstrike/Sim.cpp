...
void Sim::CommitMission()
{
  ...
  {
    DebugTrace("\n\nFINAL SCORE '%s'\n", mission->Name());
    DebugTrace("Name              Kill1  Kill2  Died   Colls  Points  Cmd Pts\n");
    DebugTrace("----------------  -----  -----  -----  -----  ------  ------\n");
    ...
    for (int i = 0; i < ShipStats::NumStats(); i++)
    {
      ...
      DebugTrace("%-16s  %5d  %5d  %5d  %5d  %6d  %6d\n", s->GetName(), s->GetGunKills(), s->GetMissileKills(),
                 s->GetDeaths(), s->GetColls(), s->GetPoints(), s->GetCommandPoints());
      ...
      if (s->IsPlayer())
      {
        ...
        if (mission && mission->Type() == Mission::TRAINING && s->GetDeaths() == 0 && s->GetColls() == 0)
...
bool Sim::IsActive() const { return mission && mission->IsActive(); }

bool Sim::IsComplete() const { return mission && mission->IsComplete(); }

void Sim::LoadMission(Mission* m, bool preload_textures)
{
  cam_dir = CameraDirector::GetInstance();
    ...
    if (preload_textures)
    {
      ...
      while (++elem_iter)
      {
        ...
        if (design)
        {
          for (int i = 0; i < 4; i++)
          {
            auto& models = (List<Model>&)design->models[i]; // cast-away const
  ...
  if (video && all_textures.size() > 0) {
      ::DebugTrace("Preloading %d textures into video texture cache\n", all_textures.size());
...
void Sim::ExecMission()
{
  ...
  if (!mission)
  {
    DebugTrace("Sim::ExecMission() - No mission to execute.\n");
    return;
  }

  if (elements.size() || finished.size())
  {
    DebugTrace("Sim::ExecMission({}) mission is already executing.\n", mission->Name());
    return;
  }
    ...
    while (++star)
    {
      ...
      while (++planet)
      {
        ...
        while (++moon)
        {
          ...
          while (++rgn)
          {
            auto sim_region = NEW SimRegion(this, rgn.value());
...
void Sim::CreateElements()
{
  ...
  while (++e_iter)
  {
      ...
      if (carrier)
      {
        ...
        if (hangar)
        {
          ...
          if (msn_elem->Loadouts().size())
          {
            ...
            if (!m->GetName().empty())
            {
              auto dsn = (ShipDesign*)msn_elem->GetDesign();
          ...
          hangar->CreateSquadron(msn_elem->Name(), msn_elem->GetCombatGroup(), msn_elem->GetDesign(), msn_elem->Count(),
                                 msn_elem->GetIFF(), def_load, msn_elem->MaintCount(), msn_elem->DeadCount());

          Element* element = CreateElement(msn_elem->Name(), msn_elem->GetIFF(), msn_elem->MissionRole());
    ...
    else
    {
      ...
      else if (!msn_elem->Commander().empty())
      {
      ...
      {
        for (auto& instr : msn_elem->Instructions())
      ...
      while (++nav)
      {
        ...
        if (rgn)
        {
          auto npt = NEW Instruction(rgn, nav->Location(), nav->Action());
      ...
      if (hangar && element && msn_elem->Count() > 0 && msn_elem->IsAlert())
      {
        ...
        if (deck)
        {
          ...
          for (int i = 0; i < msn_elem->Count(); i++)
          {
            ...
            {
              alertPrep = alertPrep && hangar->GotoAlert(squadron, slot, deck, element, loadout,
                                                         true,    // package for launch
                                                         true);   // expedite

              auto s = (HangarSlot*)hangar->GetSlot(squadron, slot);
              ...
              if (alertShip)
              {
                ...
                if (msn_elem->Player() == i + 1)
                {
                  if (alertShip->GetRegion())
                    ...
                    DebugTrace("WARNING: alert ship '%s' region is null\n", alertShip->Name());
                }
              }
            }
          }
      ...
      if (!alertPrep)
      {
        ...
        for (int i = 0; i < msn_elem->Count(); i++)
        {
          MissionShip* msn_ship = nullptr;
          std::string sname = msn_elem->GetShipName(i);
          std::string rnum = msn_elem->GetRegistry(i);
          std::string rgn_name = msn_elem->Region();
          ...
          while (++l)
          {
            if ((l->GetShip() == i) || (l->GetShip() < 0 && loadout == nullptr))
            {
              if (l->GetName().length())
              {
          ...
          Ship* ship = CreateShip(sname, rnum, (ShipDesign*)msn_elem->GetDesign(), rgn_name, l2, msn_elem->GetIFF(),
                                  msn_elem->CommandAI(), loadout);
          ...
          if (ship)
          {
            ...
            if (!netgame)
              ship->SetRespawnLoc(RandomPoint() * 2.0f);

            if (ship->IsStarship())
              ship->SetHelmHeading(heading);

            else if (ship->IsAirborne() && ship->AltitudeAGL() > 25)
              ship->SetVelocity(ship->Heading() * 250.0f);
            ...
            if (msn_ship)
            {
              ...
              ship->SetIntegrity(static_cast<float>(msn_ship->Integrity()));
            ...
            if (ship->Class() > Ship::DRONE && ship->Class() < Ship::STATION)
            {
              ...
              {
                char design[64];
                sprintf_s(design, "%s %s", ship->Abbreviation().c_str(), ship->Design()->display_name.c_str());
...
void Sim::CopyEvents()
{
  ...
  if (mission)
  {
    ...
    while (++iter)
    {
      ...
      auto event = NEW MissionEvent(*orig);
      events.append(event);
    }
  }
}

std::string Sim::FindAvailCallsign(int IFF)
{
  std::string call = "Unidentified";
...
Element* Sim::CreateElement(std::string_view callsign, int IFF, int type)
{
  auto elem = NEW Element(callsign, IFF, type);
  elements.append(elem);
  return elem;
}

void Sim::DestroyElement(Element* elem)
{
...
Element* Sim::FindElement(std::string_view name)
{
...
int Sim::GetAssignedElements(const Element* elem, List<Element>& assigned)
{
  assigned.clear();
...
Ship* Sim::CreateShip(std::string_view name, std::string_view reg_num, ShipDesign* design, std::string_view rgn_name, const Point& loc,
                      int IFF, int cmd_ai, const int* loadout)
{
  if (!design)
  {
    DebugTrace("WARNING: CreateShip({}): invalid design\n", name);
    return nullptr;
  }
  ...
  auto ship = NEW Ship(name, reg_num, design, IFF, cmd_ai, loadout);
  ship->MoveTo(loc.OtherHand());

  if (rgn)
  {
    DebugTrace("Inserting Ship({}) into Region({}) ({})\n", ship->Name(), rgn->Name(), FormatGameTime());
...
Ship* Sim::FindShip(std::string_view name, std::string_view rgn_name)
{
  Ship* ship = nullptr;
...
void Sim::DestroyShip(Ship* ship)
{
...
void Sim::NetDockShip(Ship* ship, Ship* carrier, FlightDeck* deck)
{
  SimRegion* rgn = ship->GetRegion();
  if (rgn)
...
Shot* Sim::CreateShot(const Point& pos, const Camera& shot_cam, WeaponDesign* design, const Ship* ship, SimRegion* rgn)
{
  Shot* shot = nullptr;
...
Explosion* Sim::CreateExplosion(const Point& pos, const Point& vel, int type, float exp_scale, float part_scale,
                                SimRegion* rgn, SimObject* source, System* sys)
{
  // don't bother creating explosions that can't be seen:
  if (!rgn || !active_region || rgn != active_region)
    return nullptr;

  auto exp = NEW Explosion(type, pos, vel, exp_scale, part_scale, rgn, source);
...
Debris* Sim::CreateDebris(const Point& pos, const Point& vel, Model* model, double mass, SimRegion* rgn)
{
  auto debris = NEW Debris(model, pos, vel, mass);
...
Asteroid* Sim::CreateAsteroid(const Point& pos, int t, double mass, SimRegion* rgn)
{
  auto asteroid = NEW Asteroid(t, pos, mass);
...
void Sim::CreateSplashDamage(Ship* ship)
{
  if (ship && ship->GetRegion() && ship->Design()->splash_radius > 1)
  {
    auto splash = NEW SimSplash(ship->GetRegion(), ship->Location(), ship->Design()->integrity / 4,
...
void Sim::CreateSplashDamage(Shot* shot)
{
  if (shot && shot->GetRegion())
  {
    ...
    auto splash = NEW SimSplash(shot->GetRegion(), shot->Location(), damage, shot->Design()->lethal_radius);
...
bool Sim::GridShown() const { return grid_shown; }
...
void Sim::AddSelection(Ship* s)
{
  if (active_region)
    active_region->AddSelection(s);
}

void Sim::SetSelection(Ship* newsel)
{
  if (active_region)
    active_region->SetSelection(newsel);
}

void Sim::SetTestMode(bool t)
{
  test_mode = t;
  ...
  {
    if (IsTestMode())
      pship->SetControls(nullptr);
    else
      pship->SetControls(ctrl);
  }
}

SimRegion* Sim::FindRegion(std::string_view name)
{
  ListIter<SimRegion> rgn = regions;
  while (++rgn)
  {
...
SimRegion* Sim::FindRegion(OrbitalRegion* orgn)
{
  ListIter<SimRegion> rgn = regions;
  while (++rgn)
  {
...
SimRegion* Sim::FindNearestSpaceRegion(SimObject* object) { return FindNearestRegion(object, REAL_SPACE); }

SimRegion* Sim::FindNearestTerrainRegion(SimObject* object) { return FindNearestRegion(object, AIR_SPACE); }

SimRegion* Sim::FindNearestRegion(SimObject* object, int type)
{
  if (!object)
    return nullptr;
...
SimRegion* Sim::FindNearestSpaceRegion(Orbital* body)
{
  SimRegion* result = nullptr;
...
bool Sim::ActivateRegion(SimRegion* rgn)
{
  if (rgn && active_region != rgn && regions.contains(rgn))
  {
    ...
    if (star_system)
      ...
      DebugTrace("WARNING: Sim::ActivateRegion() No star system found for rgn '%s'", rgn->Name());
...
void Sim::RequestHyperJump(Ship* obj, SimRegion* rgn, const Point& loc, int type, Ship* fc1, Ship* fc2)
{
    ...
    while (++j_iter)
    {
      ...
      if (jumpship)
      {
        ...
        if (dest)
        {
          ...
          if (jump->type == 0 && !jump->hyperdrive)
          {
            ...
            for (int i = 0; i < riders.size(); i++)
            {
              ...
              if (jump->fc_dst)
              {
                ...
                s->SetVelocity(jump->fc_dst->Heading() * 500.0f);
              }
          ...
          if (jump->fc_src)
          {
            DebugTrace("Ship '%s' farcast to '%s'\n", jumpship->Name(), dest->Name());
            ...
            if (jump->fc_dst)
            {
              ...
              jumpship->SetVelocity(jump->fc_dst->Heading() * 500.0f);
            }
          ...
          {
            DebugTrace("Ship '%s' broke orbit to '%s'\n", jumpship->Name(), dest->Name());
          ...
          {
            DebugTrace("Ship '%s' achieved orbit '%s'\n", jumpship->Name(), dest->Name());
          ...
          {
            DebugTrace("Ship '%s' quantum to '%s'\n", jumpship->Name(), dest->Name());
        ...
        {
          DebugTrace("Warning: Unusual jump request for ship '%s'\n", jumpship->Name());
      ...
      while (++s_iter)
      {
        ...
        if (distance > 1 && distance < splash->range)
        {
          ...
          if (ship_destroyed)
          {
            ...
            DebugTrace("    {} Killed {} ({})\n", splash->owner_name, ship->Name(),FormatGameTime());
...
void Sim::ProcessEventTrigger(int type, int event_id, std::string_view ship, int param)
{
  std::string ship_name(ship);
    ...
    if (event->IsPending() && event->Trigger() == type)
    {
      switch (type)
      {
        ...
        case MissionEvent::TRIGGER_TARGET:
          ...
          {
            if (ship_name.starts_with(event->TriggerShip()))
              event->Activate();
          }
          break;
...
double Sim::MissionClock() const { return (Game::GameTime() - start_time) / 1000.0; }
...
void Sim::SkipCutscene()
{
  ...
  if (stars && stars->InCutscene())
  {
    ...
    while (++iter && !end)
    {
      ...
      {
        if (event->Event() == MissionEvent::END_SCENE || event->Event() == MissionEvent::END_MISSION)
        {
          end = true;
          end_time = event->Time();
        }
...
void Sim::ResolveTimeSkip(double seconds)
{
  ...
  if (player_ship)
  {
    ...
    if (IsTestMode())
      player_ship->SetControls(nullptr);
  }
...
MissionElement* Sim::CreateMissionElement(Element* elem)
{
  ...
  if (elem && !elem->IsNetObserver())
  {
    ...
    if (ship)
    {
      ...
      msn_elem->SetHoldTime(static_cast<int>(elem->GetHoldTime()));
    ...
    {
      std::string instr = elem->GetInstruction(i);
    ...
    while (++nav_iter)
    {
      ...
      auto npt = NEW Instruction(nav->RegionName(), nav->Location(), nav->Action());
...
Hangar* Sim::FindSquadron(std::string_view name, int& index)
{
  Hangar* hangar = nullptr;
    ...
    ai_index(0)
{
  ...
  if (orbital_region)
  {
    ...
    grid = NEW Grid(static_cast<int>(orbital_region->Radius()), static_cast<int>(orbital_region->GridSpace()));

    if (orbital_region->Type() == Orbital::TERRAIN)
    {
      auto trgn = (TerrainRegion*)orbital_region;
    ...
    else if (orbital_region->Asteroids() > 0)
    {
      ...
      {
        Point init_loc((rand() - 16384.0f) * 30, (rand() - 16384.0f) * 3, (rand() - 16384.0f) * 30);
...
void SimRegion::SetPlayerShip(Ship* ship)
{
  ...
  if (Starshatter::GetInstance())
  {
    ...
    {
      DebugTrace("SimRegion %s could not set player ship '%s' - not in region\n", name.data(),
                 ship ? ship->Name() : "(null)");
    }
  }
...
bool SimRegion::IsSelected(Ship* s) { return selection.contains(s); }

ListIter<Ship> SimRegion::GetSelection() { return selection; }

void SimRegion::SetSelection(Ship* newsel)
{
  selection.clear();
  selection.append(newsel);
}

void SimRegion::ClearSelection() { selection.clear(); }

void SimRegion::AddSelection(Ship* newsel)
{
  if (!newsel || newsel->GetFlightPhase() < Ship::ACTIVE || newsel->GetFlightPhase() >= Ship::RECOVERY)
    return;
...
void SimRegion::ExecFrame(double secs)
{
  if (!sim)
    return;
...
void SimRegion::UpdateSky(double seconds, const Point& ref)
{
  Dust* dust = sim->dust;
...
void SimRegion::UpdateShots(double seconds)
{
  ...
  while (++shot_iter)
  {
    ...
    {  // died of old age
      ...
      if (shot->IsDrone())
        drones.remove(static_cast<Drone*>(shot));
    ...
    while (shot && ++ship_iter)
    {
      ...
      if (hit)
      {
        ...
        else if (shot->Damage() > 0)
        {
          ...
          if (ship_destroyed)
          {
            NetUtil::SendObjKill(ship, owner,
                                 shot->IsMissile() ? NetObjKill::KILL_SECONDARY : NetObjKill::KILL_PRIMARY);
            Director* director = nullptr;

            DebugTrace("    %s Killed %s (%s)\n", owner_name, ship->Name(), FormatGameTime());
            ...
            {
              auto shipAI = static_cast<ShipAI*>(director);
              shipAI->Splash(ship);
            }
...
void SimRegion::CollideShips()
{
  ...
  while (++ship_iter)
  {
    ...
    if (ship->InTransition() || ship->GetFlightPhase() < Ship::ACTIVE || ship->MissionClock() < 10000 || ship->
      IsNetObserver())
      continue;
    ...
    while (++targ_iter)
    {
      ...
      if (targ->InTransition() || targ->GetFlightPhase() < Ship::ACTIVE || targ->MissionClock() < 10000 || targ->
        IsNetObserver())
        continue;

      // ignore AI fighter collisions:
      if (ship->IsDropship() && ship != player_ship && targ->IsDropship() && targ != player_ship)
        continue;
      ...
      if (ship->CollidesWith(*d))
      {
        ...
        if (ship->Integrity() < 1.0f)
        {
          if (!kill_list.contains(ship))
          {
            ShipStats* r = ShipStats::Find(ship->Name());
            if (r)
              r->AddEvent(SimEvent::COLLIDE, Game::GetText("DEBRIS"));
      ...
      if (ship->CollidesWith(*a))
      {
        ...
        if (ship->Integrity() < 1.0f)
        {
          if (!kill_list.contains(ship))
          {
            ShipStats* r = ShipStats::Find(ship->Name());
            if (r)
              r->AddEvent(SimEvent::COLLIDE, Game::GetText("ASTEROID"));
...
void SimRegion::CrashShips()
{
  ...
  while (++ship_iter)
  {
    ...
    if (!ship->IsGroundUnit() && !ship->InTransition() && ship->Class() != Ship::LCA && ship->AltitudeAGL() < ship->
      Radius() / 2)
    {
      if (ship->GetFlightPhase() == Ship::ACTIVE || ship->GetFlightPhase() == Ship::APPROACH)
      {
        ...
        if (ship->Integrity() < 1.0f)
        {
          DebugTrace("    ship destroyed by crash: %s (%s)\n", ship->Name(), FormatGameTime());
          ShipStats* r = ShipStats::Find(ship->Name());
          if (r)
            r->AddEvent(SimEvent::CRASH);
  ...
  while (++shot_iter)
  {
    ...
    if (shot->AltitudeMSL() < 5e3 && shot->AltitudeAGL() < 5)
    {
...
void SimRegion::DestroyShip(Ship* ship)
{
  ...
  auto ship_design = (ShipDesign*)ship->Design();
  ...
  if (respawn || observe)
  {
    ...
    spawn = sim->CreateShip(ship_name, ship_reg, ship_design, rgn_name, spawn_loc, ship_iff, cmd_ai,
                            observe ? nullptr : ship->GetLoadout());
    ...
    int n = ship_name.size();
    if (n > 2)
    {
      if (ship_name[n - 2] == ' ' && isdigit(ship_name[n - 1]))
        ship_name = ship_name.substr(0, n - 2);
    }
    ...
    if (elem)
      ...
      DebugTrace("Warning: No Element found for '{}' on respawn.\n", ship_name);
...
void SimRegion::NetDockShip(Ship* ship, const Ship* carrier, FlightDeck* deck)
{
  if (!ship || !carrier || !deck)
    return;

  deck->Dock(ship);
}

Ship* SimRegion::FindShip(std::string_view ship_name)
{
  Ship* ship = nullptr;

  if (!ship_name.empty())
  {
    size_t nameLen = ship_name.size();

    ListIter<Ship> ship_iter = ships;
    while (++ship_iter && !ship)
    {
      Ship* test = ship_iter.value();
      if (test->Name().starts_with(ship_name))
      { 
        // The only fuzzy match is for element indices.
        // The desired name "Alpha" matches "Alpha 1" and "Alpha 2"
        // but not "Alpha-Centauri"

        if (test->Name().size() > nameLen && test->Name()[nameLen] != ' ')
          continue;
  ...
  while (++ship_iter)
  {
    ...
    if (docked)
    {
      ...
      if (carrier)
      {
        ...
        if (c)
          c->AddEvent(SimEvent::RECOVER_SHIP, ship->Name());
      }
      ...
      if (carrier)
        DebugTrace("    {} Docked with {}\n", ship_name, carrier->Name());
    }
  }
}

void SimRegion::InsertObject(Ship* _ship)
{
  if (!_ship)
    return;

  SimRegion* orig = _ship->GetRegion();
  ...
  if (orig != this)
  {
    ...
    {
      if (orig->active)
        _ship->Deactivate(sim->scene);

      orig->ships.remove(_ship);
      orig->carriers.remove(_ship);
      orig->selection.remove(_ship);
    }

    ships.append(_ship);

    if (_ship->NumFlightDecks())
      carriers.append(_ship);

    TranslateObject(_ship);
    _ship->SetRegion(this);

    if (active)
      _ship->Activate(sim->scene);
  }
}

void SimRegion::InsertObject(Shot* _shot)
{
  if (!_shot)
    return;

  SimRegion* orig = _shot->GetRegion();

  if (orig != this)
  {
    if (orig != nullptr)
      orig->shots.remove(_shot);

    shots.append(_shot);
    if (_shot->IsDrone())
      drones.append(static_cast<Drone*>(_shot));

    TranslateObject(_shot);
    _shot->SetRegion(this);

    if (active)
      _shot->Activate(sim->scene);
  }
}

void SimRegion::InsertObject(Explosion* _exp)
{
  if (!_exp)
    return;

  SimRegion* orig = _exp->GetRegion();

  if (orig != this)
  {
    if (orig != nullptr)
      orig->explosions.remove(_exp);

    explosions.append(_exp);
    TranslateObject(_exp);
    _exp->SetRegion(this);

    if (active)
      _exp->Activate(sim->scene);
  }
}

void SimRegion::InsertObject(Debris* _d)
{
  if (!_d)
    return;

  SimRegion* orig = _d->GetRegion();

  if (orig != this)
  {
    if (orig != nullptr)
      orig->debris.remove(_d);
    ...
    if (active)
      _d->Activate(sim->scene);
  }
}

void SimRegion::InsertObject(Asteroid* _a)
{
  if (!_a)
    return;

  SimRegion* orig = _a->GetRegion();

  if (orig != this)
  {
    if (orig != nullptr)
      orig->asteroids.remove(_a);
    ...
    if (active)
      _a->Activate(sim->scene);
  }
}

void SimRegion::TranslateObject(SimObject* _object)
{
  if (orbital_region)
    location = orbital_region->Location();
  ...
  if (_object)
  {
    SimRegion* orig = _object->GetRegion();
    if (orig)
    {
      Point delta = Location() - orig->Location();
      delta = delta.OtherHand();
      _object->TranslateBy(delta);
    }
  }
}

List<Contact>& SimRegion::TrackList(const int _iff)
{
  if (_iff >= 0 && _iff < 5)
    return m_trackDatabase[_iff];

  static List<Contact> empty;
  return empty;
}

void SimRegion::UpdateTracks(double _seconds)
{
  for (int i = 0; i < 5; i++)
  {
    ListIter track_iter = m_trackDatabase[i];
  ...
  for (int i = 0; i < ships.size(); i++)
  {
    ...
    if (dist > 1 || ward)
    {
      for (int j = 0; j < ships.size(); j++)
      {
        ...
        {
          Point point = ship->Location() - test->Location();

          if (point.length() < ship->Radius() * 2 + test->Radius() * 2)
