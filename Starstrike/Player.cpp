...
class AwardInfo
{
  public:
    ...
    AwardInfo()
      : type(RANK),
        id(0),
        large_insignia(nullptr),
        small_insignia(nullptr),
        ...
        lottery(0),
        min_rank(0),
        max_rank(static_cast<int>(1e9)),
        min_ship_class(0),
        max_ship_class(static_cast<int>(1e9)) {}
    ...
    std::string desc;
    std::string grant;
    std::string desc_sound;
    std::string grant_sound;
...
Player::Player(std::string_view n)
  : uid(0),
    name(n),
    create_date(0),
    points(0),
...
void Player::SetPassword(std::string_view p)
{
  if (!p.empty())
  {
    pass = p;
...
void Player::SetSquadron(std::string_view s)
{
  if (s.size() > 0)
    squadron = s;
}

void Player::SetSignature(std::string_view s)
{
  if (s.size() > 0)
    signature = s;
}

std::string Player::ChatMacro(int n) const
{
  if (n >= 0 && n < 10)
...
void Player::SetChatMacro(int n, std::string_view m)
{
  if (n >= 0 && n < 10 && m.size() > 0)
    chat_macros[n] = m;
}
...
void Player::SetMedals(int m) { medals = m; }

void Player::SetCampaigns(int n) { campaigns = n; }
...
void Player::SetFlyingStart(int n) { flying_start = n; }
...
void Player::SetAILevel(int n) { ai_level = n; }
...
void Player::ClearShowAward() { award = nullptr; }

std::string Player::AwardName() const
{
  if (award)
    return award->name;
...
std::string Player::AwardDesc() const
{
  if (award)
    return award->grant;
...
std::string Player::RankName(int rank)
{
...
std::string Player::RankAbrv(int rank)
{
  ListIter<AwardInfo> iter = rank_table;
  while (++iter)
  {
...
Bitmap* Player::RankInsignia(int rank, int size)
{
  ListIter<AwardInfo> iter = rank_table;
  while (++iter)
  {
...
std::string Player::RankDescription(int rank)
{
  ListIter<AwardInfo> iter = rank_table;
  while (++iter)
  {
...
int Player::RankFromName(std::string_view name)
{
  ListIter<AwardInfo> iter = rank_table;
  while (++iter)
  {
...
std::string Player::MedalName(int medal)
{
...
Bitmap* Player::MedalInsignia(int medal, int size)
{
  ListIter<AwardInfo> iter = medal_table;
  while (++iter)
  {
...
std::string Player::MedalDescription(int medal)
{
  ListIter<AwardInfo> iter = medal_table;
  while (++iter)
  {
...
int Player::GetMissionPoints(ShipStats* s, DWORD start_time)
{
  int result = 0;
  ...
  if (s)
  {
    ...
    if (!s->GetDeaths() && !s->GetColls())
    {
      ...
      result -= static_cast<int>(2.5 * Ship::Value(s->GetShipClass()));
...
void Player::ProcessStats(ShipStats* s, DWORD start_time)
{
  if (!s)
    return;
...
bool Player::EarnedAward(AwardInfo* a, ShipStats* s)
{
  if (!a || !s)
    return false;
...
static Player* current_player = nullptr;

List<Player>& Player::GetRoster() { return player_roster; }

Player* Player::GetCurrentPlayer() { return current_player; }
...
Player* Player::Find(std::string_view name)
{
...
Player* Player::Create(std::string_view name)
{
  if (!name.empty())
  {
    // check for existence:
...
bool Player::ConfigExists() { return config_exists; }
      ...
      if (def)
      {
        if (def->name()->value() == "player")
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: player structure missing in '%s'\n", filename);
          else
          {
            ...
            for (int i = 0; i < val->elements()->size(); i++)
            {
              ...
              if (pdef)
              {
                GET_DEF_TEXT(name);
                  ...
                  GET_DEF_TEXT(squadron);
                  else
                    GET_DEF_TEXT(signature);

                    else
                      GET_DEF_NUM(uid);
                      else
                        GET_DEF_NUM(flight_model);
                        else
                          GET_DEF_NUM(flying_start);
                          else
                            GET_DEF_NUM(landing_model);
                            else
                              GET_DEF_NUM(ai_level);
                              else
                                GET_DEF_NUM(hud_mode);
                                else
                                  GET_DEF_NUM(hud_color);
                                  else
                                    GET_DEF_NUM(ff_level);
                                    else
                                      GET_DEF_NUM(grid);
                                      else
                                        GET_DEF_NUM(gunsight);
                                        ...
                                        else if (pdef->name()->value().starts_with("XXX_CHEAT_A1B2C3_"))
                                        {
...
void Player::Save()
{
  ...
  if (f)
  {
    ...
    while (++iter)
    {
      ...
      fprintf(f, "player: {\n");
      fprintf(f, "   uid:           %d,\n", p->uid);
      fprintf(f, "   name:          \"%s\",\n", SafeQuotes(p->name.data()).c_str());
      fprintf(f, "   squadron:      \"%s\",\n", SafeQuotes(p->squadron.data()).c_str());
      fprintf(f, "   signature:     \"%s\",\n", SafeQuotes(p->signature.data()).c_str());

      std::string stat_data = p->EncodeStats();
      ...
      for (int i = 0; i < 10; i++)
        fprintf(f, "   chat_%d:       \"%s\",\n", i, SafeQuotes(p->chat_macros[i]).c_str());
...
std::string Player::EncodeStats()
{
  ...
  sprintf_s(stat_buf,
            ...
            pass.data(), create_date, points, flight_time, missions, kills, losses, medals, campaigns, 11, 12, 13, 14,
...
void Player::DecodeStats(std::string_view stats)
{
  ZeroMemory(stat_buf, 280);
  ZeroMemory(code_buf, 280);
  ...
  if (!stats.empty())
  {
    DebugTrace("Player::DecodeStats() invalid or missing stats\n");
    create_date = NetLayer::GetUTC();
    return;
  }

  std::string plain = Encryption::Decrypt(Encryption::Decode(stats));
  ...
  {
    DebugTrace("Player::DecodeStats() invalid plain text length %d\n", plain.length());
    create_date = NetLayer::GetUTC();
    return;
  }
  ...
  if (create_date == 0)
  {
    DebugTrace("WARNING - loaded player with zero stats '%s'\n", name.data());
    create_date = NetLayer::GetUTC();
  }
}
...
void Player::LoadAwardTables()
{
  ...
  DebugTrace("Loading Ranks and Medals\n");
  ...
  do
  {
    ...
    if (term)
    {
      ...
      if (def)
      {
        ...
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: award structure missing in '%s'\n", filename);
          else
          {
            auto award = NEW AwardInfo;
            TermStruct* val = def->term()->isStruct();
            ...
            for (int i = 0; i < val->elements()->size(); i++)
            {
              ...
              if (pdef)
              {
                ...
                else if (pdef->name()->value().starts_with("large"))
                {
                  std::string txt;
                ...
                else if (pdef->name()->value().starts_with("small"))
                {
                  std::string txt;
                  GetDefText(txt, pdef, filename);
                ...
                else if (pdef->name()->value().starts_with("grant"))
