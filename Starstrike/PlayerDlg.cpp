...
PlayerDlg::PlayerDlg(Screen* s, FormDef& def, MenuScreen* mgr)
  : FormWindow(s, 0, 0, s->Width(), s->Height()),
    ...
    btn_del(nullptr),
    txt_name(nullptr),
    txt_password(nullptr),
    txt_squadron(nullptr),
    txt_signature(nullptr),
    img_rank(nullptr) { Init(def); }

PlayerDlg::~PlayerDlg() {}

void PlayerDlg::RegisterControls()
{
  lst_roster = static_cast<ListBox*>(FindControl(200));
  btn_add = static_cast<Button*>(FindControl(101));
  btn_del = static_cast<Button*>(FindControl(102));
  txt_name = static_cast<EditBox*>(FindControl(201));
  txt_password = static_cast<EditBox*>(FindControl(202));
  txt_squadron = static_cast<EditBox*>(FindControl(203));
  txt_signature = static_cast<EditBox*>(FindControl(204));
  ...
  img_rank = static_cast<ImageBox*>(FindControl(220));
  REGISTER_CLIENT(EID_CLICK, img_rank, PlayerDlg, OnRank);

  for (int i = 0; i < 15; i++)
  {
    medals[i] = -1;
    img_medals[i] = static_cast<ImageBox*>(FindControl(230 + i));
    if (img_medals[i])
      REGISTER_CLIENT(EID_CLICK, img_medals[i], PlayerDlg, OnMedal);
  }

  for (int i = 0; i < 10; i++)
    txt_chat[i] = static_cast<EditBox*>(FindControl(300 + i));
...
void PlayerDlg::ExecFrame()
{
  if (Keyboard::KeyDown(VK_RETURN))
    OnApply(nullptr);
}

void PlayerDlg::ShowPlayer()
{
  ...
  if (p)
  {
    ...
    if (txt_password)
      txt_password->SetText(p->Password());
    if (txt_squadron)
      txt_squadron->SetText(p->Squadron());
    if (txt_signature)
      txt_signature->SetText(p->Signature());
    ...
    if (lbl_createdate)
      lbl_createdate->SetText(FormatTimeString(p->CreateDate()));
    if (lbl_rank)
      lbl_rank->SetText(Player::RankName(p->Rank()));
    if (lbl_flighttime)
      lbl_flighttime->SetText(flight_time);
    if (lbl_missions)
      lbl_missions->SetText(missions);
    if (lbl_kills)
      lbl_kills->SetText(kills);
    if (lbl_losses)
      lbl_losses->SetText(losses);
    if (lbl_points)
  ...
  else
  {
    ...
    if (lbl_rank)
      lbl_rank->SetText("");
    if (lbl_flighttime)
      lbl_flighttime->SetText("");
    if (lbl_missions)
      lbl_missions->SetText("");
    if (lbl_kills)
      lbl_kills->SetText("");
    if (lbl_losses)
      lbl_losses->SetText("");
...
void PlayerDlg::UpdatePlayer()
{
  ...
  if (p)
  {
    ...
    if (txt_squadron)
      p->SetSquadron(txt_squadron->GetText());
    if (txt_signature)
...
void PlayerDlg::OnSelectPlayer(AWEvent* event)
{
  if (!lst_roster)
    return;
...
void PlayerDlg::OnAdd(AWEvent* event)
{
  Player::Create("Pilot");
  ShowPlayer();
...
void PlayerDlg::OnDel(AWEvent* event)
{
  ...
  {
    char msg[256];
    sprintf_s(msg, Game::GetText("PlayerDlg.are-you-sure").data(), Player::GetCurrentPlayer()->Name().data());
...
void PlayerDlg::OnDelConfirm(AWEvent* event)
{
...
void PlayerDlg::OnRank(AWEvent* event)
{
  Player* p = Player::GetCurrentPlayer();
...
void PlayerDlg::OnMedal(AWEvent* event)
{
  Player* p = Player::GetCurrentPlayer();
  AwardShowDlg* award_dlg = manager->GetAwardDlg();
...
void PlayerDlg::OnApply(AWEvent* event)
{
...
void PlayerDlg::OnCancel(AWEvent* event)
{
  Player::Load();
  FlushKeys();
  manager->ShowMenuDlg();
