...
NetClientDlg::~NetClientDlg() { NetClientDlg::StopNetProc(); }

void NetClientDlg::StopNetProc()
{
  if (hnet != nullptr)
  {
...
void NetClientDlg::RegisterControls()
{
  btn_add = dynamic_cast<Button*>(FindControl(101));
  btn_del = dynamic_cast<Button*>(FindControl(102));
  lst_servers = dynamic_cast<ListBox*>(FindControl(200));
  lbl_info = FindControl(210);
  btn_server = dynamic_cast<Button*>(FindControl(301));
  btn_host = dynamic_cast<Button*>(FindControl(302));
  btn_join = dynamic_cast<Button*>(FindControl(303));
...
void NetClientDlg::ExecFrame()
{
  ...
  bool del_enabled = info != nullptr;
  bool join_enabled = info != nullptr && info->status > NetServerInfo::OFFLINE;
...
void NetClientDlg::ShowServers()
{
  ...
  while (++iter)
  {
    ...
    lst_servers->AddItemWithData(info->name, static_cast<DWORD>(i));
...
void NetClientDlg::UpdateServers()
{
  if (!config || !lst_servers || lst_servers->NumItems() < 1)
    return;
  ...
  for (int i = 0; i < lst_servers->NumItems(); i++)
  {
    ...
    std::string status = Game::GetText("NetClientDlg.offline");
    ...
    if (info->ping_time > 0 && info->ping_time < 10000)
    {
      ...
      switch (info->status)
      {
        default: case NetServerInfo::OFFLINE:
          status = Game::GetText("NetClientDlg.offline");
          break;
        case NetServerInfo::LOBBY:
          status = Game::GetText("NetClientDlg.lobby");
          break;
        case NetServerInfo::BRIEFING:
          status = Game::GetText("NetClientDlg.briefing");
          break;
        case NetServerInfo::ACTIVE:
          status = Game::GetText("NetClientDlg.active");
          break;
        case NetServerInfo::DEBRIEFING:
          status = Game::GetText("NetClientDlg.debriefing");
          break;
        case NetServerInfo::PERSISTENT:
          status = Game::GetText("NetClientDlg.persistent");
          break;
      }
    }
    else
...
void NetClientDlg::PingServer(int n)
{
  if (hnet == nullptr)
  {
    ...
    DWORD thread_id = 0;
    hnet = CreateThread(nullptr, 4096, NetClientPingProc, &ping_info, 0, &thread_id);
    ...
    if (hnet == nullptr)
    {
      ...
      if (report > 0)
      {
        ::DebugTrace("WARNING: NetClientDlg() failed to create PING thread for server '%s' (err=%08x)\n",
...
bool NetClientDlg::PingComplete()
{
  if (hnet != nullptr)
  {
    DWORD result = 0;
...
DWORD WINAPI NetClientPingProc(LPVOID link)
{
  auto info = static_cast<NetServerInfo*>(link);
  if (!info)
  {
    DebugTrace("NetClientPingProc() no link\n");
    Sleep(200);
    return 1;
  }
...
void NetClientDlg::OnAdd(AWEvent* event) { manager->ShowNetAddrDlg(); }

void NetClientDlg::OnDel(AWEvent* event)
{
  if (config && server_index >= 0)
    config->DelServer(server_index);
}

void NetClientDlg::OnSelect(AWEvent* event)
{
  if (lst_servers)
  {
...
void NetClientDlg::OnServer(AWEvent* event) { manager->ShowNetServerDlg(); }

void NetClientDlg::OnHost(AWEvent* event)
{
  if (config)
  {
...
void NetClientDlg::OnJoin(AWEvent* event)
{
  if (config)
  {
    config->SetServerIndex(server_index);
...
void NetClientDlg::OnCancel(AWEvent* event)
{
  if (config)
  {
    config->Save();
