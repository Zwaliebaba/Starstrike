...
void NavAI::ExecFrame(double s)
{
  ...
  if (drop_state > 0) { ship->MakeOrbit(); }
}
...
void NavAI::HelmControl()
{
  ...
  else
  {
    ...
    self->ApplyRoll(static_cast<float>(accumulator.yaw * -0.4));
    self->ApplyYaw(static_cast<float>(accumulator.yaw * 0.2));

    if (fabs(accumulator.yaw) > 0.5 && fabs(accumulator.pitch) < 0.1)
      accumulator.pitch -= 0.1f;

    if (accumulator.pitch != 0)
      self->ApplyPitch(static_cast<float>(accumulator.pitch));

    // if not turning, roll to orient with world coords:
    if (fabs(accumulator.yaw) < 0.1)
    {
      Point vrt = self->Cam().vrt();
...
void NavAI::Disengage() { throttle = 0; }

Point NavAI::Transform(const Point& point)
{
...
Steer NavAI::Seek(const Point& point)
{
...
Steer NavAI::Flee(const Point& point)
{
  if (ship && ship->IsStarship())
  {
...
Steer NavAI::Avoid(const Point& point, float radius)
{
  if (ship && ship->IsStarship())
  {
    Steer result = Seek(point);
...
Steer NavAI::AvoidTerrain()
{
  ...
  if (!ship || !ship->GetRegion() || !ship->GetRegion()->IsActive() || takeoff || (navpt && navpt->Action() ==
    Instruction::LAUNCH))
    return avoid;
