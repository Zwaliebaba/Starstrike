...
void CmdForceDlg::RegisterControls()
{
  cmb_forces = static_cast<ComboBox*>(FindControl(400));
  lst_combat = static_cast<ListBox*>(FindControl(401));
  lst_desc = static_cast<ListBox*>(FindControl(402));
  btn_transfer = static_cast<Button*>(FindControl(403));
...
void CmdForceDlg::ExecFrame() { CmdDlg::ExecFrame(); }

void CmdForceDlg::Show()
{
  mode = MODE_FORCES;
...
bool CmdForceDlg::IsVisible(Combatant* c)
{
  int nvis = 0;
  ...
  if (c)
  {
    ...
    if (force)
    {
      ...
      for (int i = 0; i < groups.size(); i++)
      {
        ...
        if (g->Type() < CombatGroup::CIVILIAN && g->CountUnits() > 0 && g->IntelLevel() >= Intel::KNOWN)
...
void CmdForceDlg::ShowCombatant(Combatant* c)
{
  if (!lst_combat || !c)
    return;
...
void CmdForceDlg::AddCombatGroup(CombatGroup* grp, bool last_child)
{
  if (!lst_combat || !grp || grp->IntelLevel() < Intel::KNOWN)
    return;
  ...
  int index = lst_combat->AddItemWithData(std::string(pipe_stack) + std::string(prefix) + grp->GetDescription(), (intptr_t)grp);
  lst_combat->SetItemData(index - 1, 1, 0);
  blank_line = false;

  int stacklen = strlen(pipe_stack);

  if (!grp->GetParent() || grp->GetParent()->Type() == CombatGroup::FORCE); // no stack after first entry
  ...
  if (grp->IsExpanded() && grp->GetUnits().size() > 0)
  {
    ...
    while (++unit_iter)
    {
      ...
      char info[512];
      int damage = static_cast<int>(100 * unit->GetSustainedDamage() / unit->GetDesign()->integrity);
      ...
      int index = lst_combat->AddItemWithData(info, (intptr_t)unit);
    ...
    lst_combat->AddItem(std::string(pipe_stack) + std::string(prefix));
    blank_line = true;
  }
...
void CmdForceDlg::OnForces(AWEvent* event)
{
  std::string name = cmb_forces->GetSelectedItem();
...
void CmdForceDlg::OnCombat(AWEvent* event)
{
  ...
  if (lst_combat)
  {
    ...
    std::string item = lst_combat->GetItemText(index);
    auto nplus = item.find_first_of(Font::PIPE_PLUS);
    int xplus = -1;
    int dx = 10000;

    if (nplus == std::string::npos)
      nplus = item.find_first_of(Font::PIPE_MINUS);

    if (nplus != std::string::npos && nplus < 64)
    {
      char pipe[64];
    ...
    if (!expand)
    {
      ...
      if (type == 0)
      {
        ...
        if (grp->GetUnits().size() > 0)
        {
          ...
          {
            if (grp->Sorties() > 0)
              sprintf_s(txt, "%.1f", static_cast<double>(grp->Points()) / grp->Sorties());
            else
              sprintf_s(txt, "%.1f", static_cast<float>(grp->Points()));
          }
          else
            strcpy_s(txt, "Unavail");
      ...
      {
        auto unit = (CombatUnit*)data;
        current_group = unit->GetCombatGroup();
        current_unit = unit;

        char txt[64];

        int n = lst_desc->AddItem("Unit:") - 1;
  ...
  {
    btn_transfer->SetEnabled(campaign->IsActive() && CanTransfer(current_group));
  }
}

bool CmdForceDlg::CanTransfer(CombatGroup* grp)
{
  if (!grp || !campaign)
    return false;
  ...
  if (grp->Type() == CombatGroup::FLEET || grp->Type() == CombatGroup::LCA_SQUADRON)
    return false;
...
void CmdForceDlg::OnSave(AWEvent* event) { CmdDlg::OnSave(event); }

void CmdForceDlg::OnExit(AWEvent* event) { CmdDlg::OnExit(event); }

void CmdForceDlg::OnMode(AWEvent* _event) { CmdDlg::OnMode(_event); }

void CmdForceDlg::OnTransfer([[maybe_unused]] AWEvent* _event)
{
  if (campaign && current_group)
  {
    ...
    if (player->CanCommand(cmd_class))
    {
      ...
      else
      {
        ...
        sprintf_s(transfer_info,
                  "Your transfer request has been approved, %s %s.  You are now assigned to the %s.  Good luck.\n\nFleet Admiral A. Evars FORCOM\nCommanding",
                  Player::RankName(player->Rank()).c_str(), player->Name().data(), current_group->GetDescription());
      }
    ...
    else
    {
      ...
      sprintf_s(transfer_info,
                ...
                Player::RankName(player->Rank()).c_str(), player->Name().data(), current_group->GetDescription(),
                Player::RankName(Player::CommandRankRequired(cmd_class)).c_str());
