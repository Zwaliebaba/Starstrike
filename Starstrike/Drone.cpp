    ...
    decoy_type(0)
{
  ...
  if (dsn)
  {
    ...
    m_name = std::format("Drone {:04d}", Identity());
  }
}

Drone::~Drone() {}

void Drone::SeekTarget(SimObject* target, System* sub)
{
  if (!probe)
    Shot::SeekTarget(target, sub);
}

void Drone::ExecFrame(double seconds) { Shot::ExecFrame(seconds); }

void Drone::Disarm() { Shot::Disarm(); }

void Drone::Destroy() { Shot::Destroy(); }
...
std::string_view Drone::ClassName() const { return Ship::ClassName(decoy_type); }

int Drone::Class() const { return decoy_type; }

int Drone::HitBy(Shot* shot, Point& impact)
{
  if (life == 0 || !shot->IsArmed())
    return 0;
