...
constexpr int WINCHESTER_FIGHTER = 0;
constexpr int WINCHESTER_ASSAULT = 1;
constexpr int WINCHESTER_STRIKE = 2;
constexpr int WINCHESTER_STATIC = 3;
    ...
    secondary_selection_time(0)
{
  ...
  switch (ai_level)
  {
    default: case 2:
      ...
      break;
    case 1:
      THREAT_REACTION_TIME = 3000;
      break;
    case 0:
      THREAT_REACTION_TIME = 6000;
      break;
  }
}
...
bool FighterTacticalAI::CheckFlightPlan()
{
  ...
  switch (roe)
  {
    case NONE:
      ...
      break;
    case SELF_DEFENSIVE:
      ship->SetDirectorInfo(Game::GetText("ai.self-defensive"));
      break;
    case DEFENSIVE:
      ship->SetDirectorInfo(Game::GetText("ai.defensive"));
      break;
    case DIRECTED:
      ship->SetDirectorInfo(Game::GetText("ai.directed"));
      break;
    case FLEXIBLE:
      ship->SetDirectorInfo(Game::GetText("ai.flexible"));
      break;
    default:
      ship->SetDirectorInfo(Game::GetText("ai.default"));
      break;
  }

  return (navpt != nullptr);
}
...
void FighterTacticalAI::SelectTarget()
{
  ...
  if (target && (target->Type() == SimObject::SIM_SHIP) && (Game::GameTime() - secondary_selection_time) >
    THREAT_REACTION_TIME)
  {
    SelectSecondaryForTarget(static_cast<Ship*>(target));
    secondary_selection_time = Game::GameTime();
  }
}

void FighterTacticalAI::SelectTargetDirected(Ship* tgt)
{
    ...
    if (elem)
    {
      ...
      if (objective)
      {
        ...
        if (obj_sim_obj && obj_sim_obj->Type() == SimObject::SIM_SHIP)
          obj_tgt = static_cast<Ship*>(obj_sim_obj);
...
void FighterTacticalAI::SelectTargetOpportunity()
{
  ...
  Ship* potential_target = nullptr;
  Shot* current_shot_target = nullptr;
  ...
  while (++c_iter)
  {
    ...
    else if (c_shot && c_shot->IsDrone())
    {
      ...
      else
      {
        ...
        if ((c_shot->IsTracking(ward) && !current_shot_target->IsTracking(ward)) || (dist < target_dist))
        {
          current_shot_target = c_shot;
          target_dist = dist;
        }
...
int FighterTacticalAI::ListSecondariesForTarget(Ship* tgt, List<WeaponGroup>& weps)
{
  weps.clear();
...
void FighterTacticalAI::SelectSecondaryForTarget(Ship* tgt)
{
  if (tgt)
  {
    ...
    if (tgt->IsGroundUnit())
      wix = WINCHESTER_STRIKE;
    else if (tgt->IsStatic())
      wix = WINCHESTER_STATIC;
    else
      if (tgt->IsStarship())
        wix = WINCHESTER_ASSAULT;

    WeaponGroup* best = nullptr;
...
void FighterTacticalAI::FindFormationSlot(int formation)
{
  ...
  ship_ai->SetFormationDelta(delta * ship->Radius() * 2.0f);
}

void FighterTacticalAI::FindThreat()
{
  // pick the closest contact on Threat Warning System:
  Ship* threat_ship = nullptr;
  Shot* threat_missile = nullptr;
  ...
  while (++c_iter)
  {
    ...
    if (contact->Threat(ship) && (Game::GameTime() - contact->AcquisitionTime()) > THREAT_REACTION_TIME)
    {
...
bool FighterTacticalAI::IsStrikeComplete(Instruction* instr)
{
  ...
  if (!instr || !instr->GetTarget() || instr->GetTarget()->Life() == 0 || instr->GetTarget()->Type() !=
    SimObject::SIM_SHIP)
    return true;
  ...
  auto target = static_cast<Ship*>(instr->GetTarget());
