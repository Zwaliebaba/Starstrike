...
CameraDirector* CameraDirector::instance = nullptr;
...
void CameraDirector::SetShip(Ship* s)
{
  sim = Sim::GetSim();
...
void CameraDirector::SetMode(int m, double t)
{
  if (requested_mode == m)
    return;
  ...
  if (m > MODE_NONE && m < MODE_LAST)
  {
    if (m <= MODE_VIRTUAL)
    {
      ...
      if ((mode == MODE_TARGET && m == MODE_CHASE) || (mode == MODE_COCKPIT && m == MODE_VIRTUAL) || (mode ==
        MODE_VIRTUAL && m == MODE_COCKPIT))
      {
        mode = m;
        requested_mode = 0;
        transition = 0;
...
static const char* get_camera_mode_name(int m)
{
  switch (m)
  {
    default: case CameraDirector::MODE_NONE:
      return "";
    case CameraDirector::MODE_COCKPIT:
      return "";
    case CameraDirector::MODE_CHASE:
      return "Chase Cam";
    case CameraDirector::MODE_TARGET:
      return "Padlock";
    case CameraDirector::MODE_THREAT:
      return "Threatlock";
    case CameraDirector::MODE_VIRTUAL:
      return "Virtual";
    case CameraDirector::MODE_ORBIT:
    case CameraDirector::MODE_TRANSLATE:
    case CameraDirector::MODE_ZOOM:
      return "Orbit Cam";
    case CameraDirector::MODE_DOCKING:
      return "Dock Cam";
    case CameraDirector::MODE_DROP:
      return "";
  }
}

std::string CameraDirector::GetModeName()
{
  int m = GetCameraMode();
  ...
  if (instance)
  {
    if (instance->virt_az > 30 * DEGREES && instance->virt_az < 100 * DEGREES)
      return "RIGHT";
    if (instance->virt_az >= 100 * DEGREES)
      return "RIGHT-AFT";
    if (instance->virt_az < -30 * DEGREES && instance->virt_az > -100 * DEGREES)
      return "LEFT";
    if (instance->virt_az <= -100 * DEGREES)
      return "LEFT-AFT";
    if (instance->virt_el > 15 * DEGREES)
      return "UP";
    if (instance->virt_el < -15 * DEGREES)
      return "DOWN";
...
void CameraDirector::SetCameraMode(int m, double t)
{
  if (instance)
    instance->SetMode(m, t);
}

double CameraDirector::GetRangeLimit() { return range_max_limit; }
...
void CameraDirector::SetRangeLimits(double min, double max)
{
  if (instance)
  {
...
void CameraDirector::ClearGroup() { external_group.clear(); }
...
void CameraDirector::SetViewOrbital(Orbital* orb)
{
  external_body = orb;
...
void CameraDirector::SetViewObject(Ship* obj, bool quick)
{
  if (!ship)
    return;
  if (!obj)
...
void CameraDirector::SetViewObjectGroup(ListIter<Ship> group, bool quick)
{
  if (!ship)
    return;
  ...
  if (external_group.size() > 1 && external_group.size() == group.size())
  {
    bool same = true;
...
void CameraDirector::VirtualHead(double az, double el)
{
...
void CameraDirector::VirtualHeadOffset(double x, double y, double z)
{
  if (mode == MODE_VIRTUAL || mode == MODE_TARGET || mode == MODE_COCKPIT)
  {
    virt_x = x;
...
void CameraDirector::SetOrbitPoint(double a, double e, double r)
{
  azimuth = a;
  elevation = e;
  range = r;
...
void CameraDirector::SetOrbitRates(double ar, double er, double rr)
{
  az_rate = ar;
  el_rate = er;
...
bool CameraDirector::Update(SimObject* obj)
{
...
void CameraDirector::ExecFrame(double seconds)
{
  ...
  if (op_mode == MODE_TARGET && !external_ship)
  {
    ...
    if (tgt && tgt->Type() == SimObject::SIM_SHIP)
      SetViewObject(static_cast<Ship*>(tgt));
  }
  ...
  if (ship->Shake() > 0 && (op_mode < MODE_ORBIT || (op_mode == MODE_VIRTUAL && ship->Cockpit())))
  {
...
void CameraDirector::Transition(double seconds)
{
  if (transition > 0)
...
void CameraDirector::Cockpit([[maybe_unused]] double seconds)
{
  camera.Clone(ship->Cam());

  Point bridge = ship->BridgeLocation();
  Point cpos = camera.Pos() + camera.vrt() * bridge.x + camera.vpn() * bridge.y + camera.vup() * bridge.z;

  camera.MoveTo(cpos);
}

void CameraDirector::Virtual(double seconds)
{
  camera.Clone(ship->Cam());

  Point bridge = ship->BridgeLocation();
  Point cpos = camera.Pos() + camera.vrt() * (bridge.x + virt_x) + camera.vpn() * (bridge.y + virt_z) + camera.vup() * (
    bridge.z + virt_y);
...
void CameraDirector::Chase([[maybe_unused]] double seconds)
{
  double step = 1;
  ...
  if (ship->Velocity().length() > 10)
  {
    ...
    velocity.Normalize();
    velocity *= 0.25;
    velocity += camera.vpn() * 2.0;
    velocity.Normalize();
  }

  Point chase = ship->ChaseLocation();
  Point bridge = ship->BridgeLocation();
  Point cpos = camera.Pos() + camera.vrt() * bridge.x * (1 - step) + camera.vpn() * bridge.y * (1 - step) + camera.vup()
    * bridge.z * (1 - step) + velocity * chase.y * step + camera.vup() * chase.z * step;
...
void CameraDirector::Orbit(double seconds)
{
  Point cpos;
  ...
  else if (mode < MODE_ORBIT || requested_mode > 0 && requested_mode < MODE_ORBIT)
  {
    ...
    if (mode == MODE_CHASE || requested_mode == MODE_CHASE || mode == MODE_TARGET || requested_mode == MODE_TARGET)
    {
  ...
  if (external_ship == nullptr && er < 0.5 * ship->Radius())
    ship->Rep()->Hide();
...
void CameraDirector::Docking(double seconds)
{
  ...
  Point bridge = ship->BridgeLocation();
  Point cloc = ship->Location() + ship->Cam().vrt() * bridge.x + ship->Cam().vpn() * bridge.y + ship->Cam().vup() *
    bridge.z;
  ...
  else if (flight_phase == Ship::LOCKED)
  {
    ...
    cpos = base_loc * transition + cloc * (1 - transition);
  }
...
void CameraDirector::Drop(double seconds)
{
