...
Ship* ShipAI::GetWard() const { return ship->GetWard(); }
...
void ShipAI::ClearRumor() { rumor = nullptr; }
...
std::string ShipAI::GetObserverName() const
{
  static char name[64];
  sprintf_s(name, "ShipAI(%s)", self->Name().c_str());
  return name;
}

Point ShipAI::GetPatrol() const { return patrol_loc; }
...
void ShipAI::ClearPatrol() { patrol = 0; }

void ShipAI::ExecFrame(double secs)
{
  seconds = secs;
  ...
  if (target && target != ship->GetTarget())
  {
    ...
    if (target == ship->GetTarget() && target->Type() == SimObject::SIM_SHIP)
    {
      ...
      {
        // call engaging:
        auto msg = NEW RadioMessage(ship->GetElement(), ship, RadioMessage::CALL_ENGAGING);
...
void ShipAI::FindObjective()
{
  ...
  if (order == RadioMessage::QUANTUM_TO || order == RadioMessage::FARCAST_TO)
  {
  ...
  bool form = (order == RadioMessage::WEP_HOLD) || (order == RadioMessage::FORM_UP) || (order ==
    RadioMessage::MOVE_PATROL) || (order == RadioMessage::RTB) || (order == RadioMessage::DOCK_WITH) || (!order && !
    target) || (farcaster);
  ...
  if (threat && !directed)
  {
    ...
    else if (threat != target)
    {
      ...
      obj_w = ship->Location() + Point(ship->Location() - threat->Location()) * 100.0f;
      objective = Transform(obj_w);
      return;
    }
  }
...
void ShipAI::FindObjectiveTarget(SimObject* tgt)
{
  ...
  navpt = nullptr; // this tells fire control that we are chasing a target,
...
void ShipAI::FindObjectiveNavPoint()
{
  ...
  bool use_farcaster = self_rgn != nav_rgn && (navpt->Farcast() || !qdrive || !qdrive->IsPowerOn() || qdrive->Status() <
    System::DEGRADED);
...
void ShipAI::FindObjectiveQuantum()
{
  ...
  bool use_farcaster = self_rgn != nav_rgn && (orders->Farcast() || !qdrive || !qdrive->IsPowerOn() || qdrive->Status()
    < System::DEGRADED);
...
void ShipAI::FindObjectiveFarcaster(SimRegion* src_rgn, SimRegion* dst_rgn)
{
  if (!farcaster)
  {
...
void ShipAI::SetFormationDelta(const Point& point) { formation_delta = point; }

void ShipAI::FindObjectiveFormation()
{
  constexpr double prediction = 5;
  ...
  {
    auto lead_ai = static_cast<ShipAI*>(lead_dir);
...
void ShipAI::SetTarget(SimObject* targ, System* sub)
{
  if (targ != target)
    bracket = false;
...
void ShipAI::DropTarget(double dtime)
{
  SetTarget(nullptr);
...
void ShipAI::SetBracket(bool b)
{
  bracket = b;
  identify = false;
}

void ShipAI::SetIdentify(bool i)
{
  identify = i;
  bracket = false;
}
...
Steer ShipAI::AvoidCollision()
{
  ...
  if (!found)
  {
    ...
    if (other)
    {
      avoid = Avoid(obstacle, static_cast<float>(ship->Radius() + other->Radius() + avoid_dist * 0.9));
      avoid.brake = brake;
...
bool ShipAI::AvoidTestSingleObject(SimObject* obj, const Point& bearing, double avoid_dist, double& avoid_time,
                                   Steer& avoid)
{
  ...
  // will we get close?
  double time = ClosestApproachTime(ship->Location(), ship->Velocity(), obj->Location(), obj->Velocity());
  ...
  {
    auto c_ship = static_cast<Ship*>(obj);
...
Steer ShipAI::EvadeThreat() { return Steer(); }
