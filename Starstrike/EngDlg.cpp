...
EngDlg::EngDlg(Screen* s, FormDef& def, GameScreen* mgr)
  : FormWindow(s, 0, 0, s->Width(), s->Height()),
    ...
    selected_component(nullptr) { FormWindow::Init(def); }

EngDlg::~EngDlg() {}

void EngDlg::RegisterControls()
{
  for (int i = 0; i < 4; i++)
  {
    sources[i] = static_cast<Button*>(FindControl(201 + i));
    REGISTER_CLIENT(EID_CLICK, sources[i], EngDlg, OnSource);

    source_levels[i] = static_cast<Slider*>(FindControl(211 + i));

    clients[i] = static_cast<ListBox*>(FindControl(301 + i));
  ...
  selected_name = FindControl(401);
  power_off = static_cast<Button*>(FindControl(402));
  power_on = static_cast<Button*>(FindControl(403));
  override = static_cast<Button*>(FindControl(410));
  power_level = static_cast<Slider*>(FindControl(404));
  capacity = static_cast<Slider*>(FindControl(405));
  ...
  components = static_cast<ListBox*>(FindControl(501));
  ...
  auto_repair = static_cast<Button*>(FindControl(700));
  repair = static_cast<Button*>(FindControl(502));
  replace = static_cast<Button*>(FindControl(503));
  repair_time = FindControl(512);
  replace_time = FindControl(513);
  priority_increase = static_cast<Button*>(FindControl(602));
  priority_decrease = static_cast<Button*>(FindControl(603));
  ...
  repair_queue = static_cast<ListBox*>(FindControl(601));
...
void EngDlg::Hide() { FormWindow::Hide(); }

void EngDlg::SetShip(Ship* s)
{
  if (IsShown() && ship != s)
  {
    selected_source = nullptr;
  ...
  if (ship)
  {
    ...
    while (++reactor_iter)
    {
      ...
      if (sources[reactor_index] && clients[reactor_index])
      {
        ...
        source_levels[reactor_index]->SetValue(static_cast<int>(reactor->GetPowerLevel()));
        ...
        while (++client)
        {
          ...
          strcpy_s(abrv, Game::GetText(client->Name()).c_str());

          clients[reactor_index]->AddItemWithData(Game::GetText(abrv), index);
          clients[reactor_index]->SetItemText(index, 1, num);
          clients[reactor_index]->SetItemData(index, 1, static_cast<DWORD>(client->GetPowerLevel()));
...
void EngDlg::ExecFrame()
{
  if (IsShown())
    UpdateSelection();
}

void EngDlg::UpdateSelection()
{
  if (!ship)
    return;
  ...
  for (int source_index = 0; source_index < nsources; source_index++)
  {
    ...
    source_levels[source_index]->SetValue(static_cast<int>(reac->GetPowerLevel()));
    ...
    for (int i = 0; i < client_list->NumItems(); i++)
    {
      ...
      client_list->SetItemData(i, 1, static_cast<DWORD>(client->GetPowerLevel()));
  ...
  if (selected_source)
  {
    ...
    power_level->SetValue(static_cast<int>(selected_source->GetPowerLevel()));

    if (selected_source->Safety() < 100)
    {
      power_level->SetMarker(static_cast<int>(selected_source->Safety()), 0);
      power_level->SetMarker(static_cast<int>(selected_source->Safety()), 1);
    }
    else
    {
    ...
    capacity->SetValue(selected_source->Charge());
      ...
      for (int i = 0; i < components->NumItems(); i++)
      {
        ...
        switch (comp->Status())
        {
          ...
          case Component::DEGRADED:
            ...
            c = Color::Yellow;
            break;
          case Component::NOMINAL:
            stat = "OK";
            c = Color::White;
            break;
          case Component::REPLACE:
          case Component::REPAIR:
            stat = "MAINT";
            c = Color::Cyan;
            break;
        }

        stat = Game::GetText(std::string("EngDlg.") + stat);
        components->SetItemText(i, 1, stat);
        components->SetItemData(i, 1, comp->Status());
  ...
  else if (selected_clients.size() == 1)
  {
    ...
    power_level->SetValue(static_cast<int>(sink->GetPowerLevel()));

    if (sink->Safety() < 100)
    {
      power_level->SetMarker(static_cast<int>(sink->Safety()), 0);
      power_level->SetMarker(static_cast<int>(sink->Safety()), 1);
    }
    else
    {
      power_level->SetMarker(-1, 0);
  ...
  if (repair_queue)
  {
    ...
    if (repair_queue->NumItems() < ship->RepairQueue().size())
    {
      ...
      while (++iter)
      {
        ...
        FormatTime(etr, static_cast<int>(time_remaining / ship->RepairSpeed()));
    ...
    else
    {
      ...
      for (int i = 0; i < repair_queue->NumItems(); i++)
      {
        ...
        FormatTime(etr, static_cast<int>(time_remaining));
...
void EngDlg::OnSource(AWEvent* event)
{
  selected_source = nullptr;
  selected_clients.clear();
  selected_component = nullptr;
...
void EngDlg::OnClient(AWEvent* event)
{
  selected_source = nullptr;
  selected_clients.clear();
  selected_component = nullptr;
...
void EngDlg::OnRouteStart(AWEvent* event)
{
  if (!ship)
    return;
...
void EngDlg::OnRouteComplete(AWEvent* event)
{
...
void EngDlg::OnPowerOff(AWEvent* event)
{
  if (selected_source)
  {
...
void EngDlg::OnPowerOn(AWEvent* event)
{
  if (selected_source)
  {
...
void EngDlg::OnOverride(AWEvent* event)
{
  bool over = false;
...
void EngDlg::OnPowerLevel(AWEvent* event)
{
...
void EngDlg::OnComponent(AWEvent* event)
{
  selected_component = nullptr;
  ...
  if (component_index >= 0)
  {
    ...
    if (selected_source)
    {
      ...
      {
        char t[32];
        int s = static_cast<int>(selected_component->RepairTime() / ship->RepairSpeed());
        FormatTime(t, s);
      ...
      if (selected_component->SpareCount() > 0)
      {
        ...
        {
          char t[32];
          int s = static_cast<int>(selected_component->ReplaceTime() / ship->RepairSpeed());
          FormatTime(t, s);
...
void EngDlg::OnAutoRepair(AWEvent* event)
{
  if (ship)
    ship->EnableRepair(auto_repair->GetButtonState() > 0);
}

void EngDlg::OnRepair(AWEvent* event)
{
...
void EngDlg::OnReplace(AWEvent* event)
{
  if (selected_component)
  {
...
void EngDlg::OnQueue(AWEvent* event)
{
  selected_repair = nullptr;
  ...
  if (priority_increase)
    priority_increase->SetEnabled(true);
  if (priority_decrease)
    priority_decrease->SetEnabled(true);
}

void EngDlg::OnPriorityIncrease(AWEvent* event)
{
  if (ship && repair_queue)
    ship->IncreaseRepairPriority(repair_queue->GetSelection());
}

void EngDlg::OnPriorityDecrease(AWEvent* event)
{
  if (ship && repair_queue)
    ship->DecreaseRepairPriority(repair_queue->GetSelection());
}

void EngDlg::OnClose(AWEvent* event)
{
  if (manager)
