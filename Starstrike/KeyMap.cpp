...
int KeyMap::BuildDefaultKeyMap()
{
  ...
  defmap[i++] = KeyMapEntry(KEY_PLUS_X, 190, 0);    // .
  defmap[i++] = KeyMapEntry(KEY_MINUS_X, 188, 0);     // ,
  defmap[i++] = KeyMapEntry(KEY_PLUS_Y, VK_HOME);
  defmap[i++] = KeyMapEntry(KEY_MINUS_Y, VK_END);
  defmap[i++] = KeyMapEntry(KEY_PLUS_Z, VK_PRIOR, 0);
  defmap[i++] = KeyMapEntry(KEY_MINUS_Z, VK_NEXT, 0);

  defmap[i++] = KeyMapEntry(KEY_ACTION_0, VK_CONTROL, 0);
  defmap[i++] = KeyMapEntry(KEY_ACTION_1, VK_SPACE, 0);
  ...
  defmap[i++] = KeyMapEntry(KEY_LOCK_TARGET, 'T', 0);
  ...
  defmap[i++] = KeyMapEntry(KEY_DECOY, 'D', 0);
...
int KeyMap::DefaultKeyMap(int max_keys)
{
...
void KeyMap::Bind(int a, int k, int s)
{
  if (!a)
    return;
...
int KeyMap::LoadKeyMap(const char* filename, int max_keys)
{
  FILE* f;
  ...
  {
    int act = -1, key = -1, alt = 0;
    char act_str[128], key_str[128], alt_str[128];

    ZeroMemory(act_str, sizeof(act_str));
    ZeroMemory(key_str, sizeof(key_str));
    ZeroMemory(alt_str, sizeof(alt_str));

    sscanf(line, "%s %s %s", act_str, key_str, alt_str);
    ...
    if (act != -1 && key != -1)
    {
      ...
      for (int i = 0; i < max_keys && !mapped; i++)
      {
        if (map[i].act == act)
        {
          DebugTrace("  Remapping: '%s' => %s(%d) %s(%d)\n", act_str, key_str, key, alt_str, alt);

          map[i] = KeyMapEntry(act, key, alt);
          mapped = true;
        }
      }
      ...
      if (!mapped)
      {
        DebugTrace("  Mapping: '%s' => %s(%d) %s(%d)\n", act_str, key_str, key, alt_str, alt);
        map[nkeys++] = KeyMapEntry(act, key, alt);
      }
    }

    if (nkeys >= max_keys - 1)
    {
      DebugTrace("   Too many keys in configuration...\n");
      break;
    }
  }
...
int KeyMap::SaveKeyMap(const char* filename, int max_keys)
{
  FILE* f;
  ...
  for (int i = 0; i < nkeys; i++)
  {
    if (map[i].act >= KEY_CONTROL_MODEL && map[i].act <= KEY_AXIS_THROTTLE_INVERT)
    {
    ...
    else if (map[i] != defmap[i])
    {
      ...
      if (a > -1)
      {
        if (s > -1)
        {
          fprintf(f, "%-24s %-16s %-16s\n", key_action_table[a].name, key_key_table[k].name, key_key_table[s].name);
        }
        else if (k > -1) { fprintf(f, "%-24s %-16s\n", key_action_table[a].name, key_key_table[k].name); }
      }
    }
  }
...
const char* KeyMap::DescribeAction(int n)
{
  int nactions = sizeof(key_action_table) / sizeof(KeyName);
...
const char* KeyMap::DescribeKey(int n)
{
  if (n >= 0 && n < 256)
    return DescribeKey(map[n].key, map[n].alt);

  return nullptr;
}

const char* KeyMap::DescribeKey(int vk, int shift)
{
