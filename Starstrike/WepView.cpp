...
std::string WepView::GetObserverName() const { return "WepView"; }
...
void WepView::SetOverlayMode(int m)
{
  if (mode != m)
  {
    mode = m;
...
void WepView::CycleOverlayMode() { SetOverlayMode(!mode); }
...
void WepView::DrawOverlay()
{
  ...
  if (ship)
  {
    ...
    std::ranges::transform(subtxt, subtxt.begin(), ::toupper);

    hud_font->SetColor(stat);
    window->SetFont(hud_font);
    window->DrawText(subtxt, subtxt.length(), tgt_rect, DT_SINGLELINE | DT_CENTER);
  }
}
...
void WepView::DoMouseFrame()
{
  ...
  if (Mouse::LButton())
  {
    ...
    for (int i = 0; i < max_wep; i++)
    {
      ...
      if (CheckButton(index + 1, mouse_down_x, mouse_down_y))
      {
        ship->Weapons()[i]->SetFiringOrders(Weapon::MANUAL);
        return;
      }
      if (CheckButton(index + 2, mouse_down_x, mouse_down_y))
      {
        ship->Weapons()[i]->SetFiringOrders(Weapon::AUTO);
        return;
      }
      if (CheckButton(index + 3, mouse_down_x, mouse_down_y))
      {
  ...
  else if (mouse_down)
  {
    ...
    if (ship->GetTarget())
    {
      ...
      r.x = width / 2 + 180;
      if (r.Contains(x, y)) { CycleSubTarget(1); }
    }
  }
}

bool WepView::CheckButton(int index, int x, int y)
{
