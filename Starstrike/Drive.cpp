...
static int drive_value[] = {1, 1, 1, 1, 1, 1, 1, 1};
...
Drive::Drive(SUBTYPE drive_type, float max_thrust, float max_aug, bool show)
  : System(DRIVE, drive_type, "Drive", drive_value[drive_type], max_thrust * 2, max_thrust * 2, max_thrust * 2),
    thrust(max_thrust),
    augmenter(max_aug),
    scale(0.0f),
    throttle(0.0f),
    ...
    show_trail(show)
{
  ...
  {
    default: case PLASMA:
      m_name = Game::GetText("sys.drive.plasma");
      break;
    case FUSION:
      m_name = Game::GetText("sys.drive.fusion");
      break;
    case GREEN:
      m_name = Game::GetText("sys.drive.green");
      break;
    case RED:
      m_name = Game::GetText("sys.drive.red");
      break;
    case BLUE:
      m_name = Game::GetText("sys.drive.blue");
      break;
    case YELLOW:
      m_name = Game::GetText("sys.drive.yellow");
      break;
    case STEALTH:
      m_name = Game::GetText("sys.drive.stealth");
      break;
  }
...
void Drive::AddPort(const Point& loc, float flare_scale)
{
  if (flare_scale == 0)
    flare_scale = scale;
  auto port = NEW DrivePort(loc, flare_scale);
  ports.append(port);
}

void Drive::CreatePort(const Point& loc, float flare_scale)
{
  Bitmap* flare_bmp = drive_flare_bitmap[subtype];
  Bitmap* trail_bmp = drive_trail_bitmap[subtype];
  Bitmap* glow_bmp = nullptr;
  ...
  if (subtype != STEALTH && flare_scale > 0)
  {
    auto port = NEW DrivePort(loc, flare_scale);

    if (flare_bmp)
    {
      auto flare_rep = NEW DriveSprite(flare_bmp, glow_bmp);
...
void Drive::Orient(const Physical* rep)
{
  ...
  for (int i = 0; i < ports.size(); i++)
  {
    ...
    if (p->flare)
    {
      ...
      p->flare->SetFront(rep->Cam().vpn() * -10.0f * p->scale);
    }
...
void Drive::SetThrottle(double t, bool aug)
{
  ...
  if (throttle < throttle_request)
  {
    if (throttle_request - throttle < spool)
      throttle = static_cast<float>(throttle_request);
    else
      throttle += static_cast<float>(spool);
  }
  ...
  {
    augmenter_throttle += static_cast<float>(spool);
...
{
  return augmenter > 0 && augmenter_throttle > 0.05 && IsPowerOn() && status > CRITICAL;
}

int Drive::NumEngines() const { return ports.size(); }
...
float Drive::Thrust(double seconds)
{
  ...
  for (int i = 0; i < ports.size(); i++)
  {
    ...
    if (p->flare)
    {
      ...
      {
        if (fraction > 0)
          intensity += static_cast<float>(seconds);
        else
          intensity -= static_cast<float>(seconds);
        ...
        if (intensity > 0.25)
        {
          vol = static_cast<int>((intensity - 1.0) * 10000.0);
          CLAMP(vol, -10000, -1500);

          if (aug_on && intensity > 0.5)
          {
            vol_aug = static_cast<int>((5 * augmenter_throttle - 1.0) * 10000.0);
