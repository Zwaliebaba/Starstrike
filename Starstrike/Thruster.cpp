...
Thruster::Thruster(int dtype, double max_thrust, float flare_scale)
  : System(DRIVE, dtype, "Thruster", sys_value, max_thrust, max_thrust, max_thrust),
    ship(nullptr),
    thrust(1.0f),
    scale(flare_scale),
    avail_x(1.0f),
    ...
    avail_z(1.0f)
{
  m_name = Game::GetText("sys.thruster");
...
void Thruster::Initialize()
{
  ...
  constexpr int SOUND_FLAGS = Sound::LOCALIZED | Sound::LOC_3D | Sound::LOOP | Sound::LOCKED;
...
void Thruster::Orient(const Physical* rep)
{
  ...
  for (int i = 0; i < ports.size(); i++)
  {
    ...
    if (p->trail)
    {
      ...
      {
        auto t = static_cast<Bolt*>(p->trail);
        ...
        {
          case LEFT:
            t->SetEndPoints(projector, projector + rep->Cam().vrt() * len);
            break;
          case RIGHT:
            t->SetEndPoints(projector, projector - rep->Cam().vrt() * len);
            break;
          case AFT:
            t->SetEndPoints(projector, projector + rep->Cam().vpn() * len);
            break;
          case FORE:
            t->SetEndPoints(projector, projector - rep->Cam().vpn() * len);
            break;
          case BOTTOM:
            t->SetEndPoints(projector, projector + rep->Cam().vup() * len);
            break;
          case TOP:
            t->SetEndPoints(projector, projector - rep->Cam().vup() * len);
            break;
...
{
  constexpr double ROLL_SPEED = PI * 0.0400;
  constexpr double PITCH_SPEED = PI * 0.0250;
  constexpr double YAW_SPEED = PI * 0.0250;

  ship = s;

  if (ship)
  {
    auto design = (ShipDesign*)ship->Design();
    ...
    roll_rate = static_cast<float>(design->roll_rate * PI / 180);
    pitch_rate = static_cast<float>(design->pitch_rate * PI / 180);
    yaw_rate = static_cast<float>(design->yaw_rate * PI / 180);

    double agility = design->agility;

    if (roll_rate == 0)
      roll_rate = static_cast<float>(agility * ROLL_SPEED);
    if (pitch_rate == 0)
      pitch_rate = static_cast<float>(agility * PITCH_SPEED);
    if (yaw_rate == 0)
      yaw_rate = static_cast<float>(agility * YAW_SPEED);
  }
}

double Thruster::TransXLimit() { return trans_x * avail_x; }

double Thruster::TransYLimit() { return trans_y * avail_y; }

double Thruster::TransZLimit() { return trans_z * avail_z; }

void Thruster::ExecTrans(double _x, double _y, double _z)
{
  if (!ship || (ship->IsAirborne() && ship->Class() != Ship::LCA))
  {
    if (thruster_sound && thruster_sound->IsPlaying())
  ...
  if (ship->Class() == Ship::LCA && ship->IsAirborne() && ship->Velocity().length() < 250 && ship->AltitudeAGL() > ship
    ->Radius() / 2)
  {
    sound_on = true;
    IncBurn(BOTTOM, TOP);
  ...
  else if (!ship->IsAirborne())
  {
    ...
    if (r > 0)
      IncBurn(ROLL_L, ROLL_R);
    else if (r < 0)
      IncBurn(ROLL_R, ROLL_L);
    else
      DecBurn(ROLL_R, ROLL_L);
    ...
    if (p < 0)
      IncBurn(PITCH_D, PITCH_U);
    else if (p > 0)
      IncBurn(PITCH_U, PITCH_D);
    else
      DecBurn(PITCH_U, PITCH_D);
  }
  ...
  for (int i = 0; i < ports.size(); i++)
  {
    ...
    if (p->burn > 0 && thrust > 0)
    {
      ...
      if (show_flare)
      {
        auto flare_rep = static_cast<Sprite*>(p->flare);
        if (flare_rep)
        {
          flare_rep->Show();
  ...
  ship->SetTransX(_x * thrust);
  ship->SetTransY(_y * thrust);
  ship->SetTransZ(_z * thrust);
}

void Thruster::AddPort(int type, const Point& loc, DWORD fire, float flare_scale)
{
  if (flare_scale == 0)
    flare_scale = scale;
  auto port = NEW ThrusterPort(type, loc, fire, flare_scale);
  ports.append(port);
}

void Thruster::CreatePort(int type, const Point& loc, DWORD fire, float flare_scale)
{
  ...
  if (subtype != Drive::STEALTH)
  {
    auto flare_rep = NEW Sprite(flare_bmp);
    flare_rep->Scale(flare_scale * 0.667f);
    flare_rep->SetShade(0);

    auto trail_rep = NEW Bolt(flare_scale * 30, flare_scale * 8, trail_bmp, true);
...
int Thruster::NumThrusters() const { return ports.size(); }
...
void Thruster::IncBurn(int inc, int dec)
{
  burn[inc] += 0.1f;
  if (burn[inc] > 1)
    burn[inc] = 1.0f;
...
void Thruster::DecBurn(int a, int b)
{
  burn[a] -= 0.1f;
  if (burn[a] < 0)
    burn[a] = 0.0f;
