...
TacticalView* TacticalView::tac_view = nullptr;
...
std::string TacticalView::GetObserverName() const { return "TacticalView"; }

void TacticalView::UseProjector(Projector* p) { projector = p; }
...
void TacticalView::DrawSelection(Ship* seln)
{
  ...
  if (mark_pt.z > 1.0)
  {
    ...
    if (x > 4 && x < width - 4 && y > 4 && y < height - 4)
    {
      ...
      int sw = static_cast<int>(BAR_LENGTH * (seln->ShieldStrength() / 100.0));
...
void TacticalView::DrawSelectionInfo(Ship* seln)
{
  ...
  sprintf_s(name, "%s", seln->Name().c_str());

  if (show_labels)
  {
    sprintf_s(shield, "%s %03d", Game::GetText("HUDView.symbol.shield").c_str(), shield_val);
    sprintf_s(hull, "%s %03d", Game::GetText("HUDView.symbol.hull").c_str(), hull_val);
  }
  else
  {
  ...
  strcat_s(range, " km");
  sprintf_s(heading, "%03d %s", static_cast<int>(seln->CompassHeading() / DEGREES),
  ...
  if (full_info)
  {
    sprintf_s(design, "%s %s", seln->Abbreviation().c_str(), seln->Design()->display_name.c_str());
  ...
  if (full_info)
  {
    ...
    if (seln->GetIFF() == ship->GetIFF())
    {
      ...
      if (instr && instr->Action())
      {
        ...
        {
          strcat_s(orders, " ");
          strcat_s(orders, instr->RegionName().c_str());
        }
      }
      else
        *orders = 0;
  ...
  {
    sprintf_s(psv, "%03d", static_cast<int>(contact->PasReturn() * 100.0));
    sprintf_s(act, "%03d", static_cast<int>(contact->ActReturn() * 100.0));
...
void TacticalView::DoMouseFrame()
{
  ...
  else
  {
    if (sim && right_down && (Game::RealTime() - rbutton_latch < 250))
    {
      ...
      if (seln && sim->IsSelected(seln) && seln->GetIFF() == ship->GetIFF() && ship->GetElement()->
        CanCommand(seln->GetElement()))
      {
        msg_ship = seln;
        Observe(msg_ship);
      }

      else if (ship && seln == ship && (!ship->GetDirector() || ship->GetDirector()->Type() != ShipCtrl::DIR_TYPE))
        msg_ship = seln;
    ...
    else
    {
      if (mouse_down)
      {
        ...
        else
        {
          ...
          {
            int dx = static_cast<int>(fabs((double)(mouse_x - mouse_start.x)));
            int dy = static_cast<int>(fabs((double)(mouse_y - mouse_start.y)));
  ...
  for (int i = 0; i < ship->NumContacts(); i++)
  {
    ...
    if (test && test != ship)
    {
      ...
      if (test_loc.z > 1)
      {
        ...
        if (rect.Contains(static_cast<int>(test_loc.x), static_cast<int>(test_loc.y)))
        {
        ...
        if (g)
        {
          ...
          if (r.x == 2000 && r.y == 2000 && r.w == 0 && r.h == 0)
          {
            if (projector)
            {
              ...
              r.x = static_cast<int>(loc.x);
              r.y = static_cast<int>(loc.y);
            }
          }
...
void TacticalView::SetHelm(bool approach)
{
  ...
  if (delta.length() < 1)
  {
    ...
    if (projector)
    {
      ...
      delta = projector->vpn() * focal_dist + projector->vup() * -1.0f * (my - height / 2.0f) + projector->vrt() * (mx -
        width / 2.0f);
...
static Menu* view_menu = nullptr;
static Menu* emcon_menu = nullptr;
...
static Menu* quantum_menu = nullptr;
static Menu* farcast_menu = nullptr;

static Element* dst_elem = nullptr;
...
constexpr int QUANTUM = 2000;
constexpr int FARCAST = 2001;
...
void TacticalView::ProcessMenuItem(int action)
{
  ...
  switch (action)
  {
    ...
    case VIEW_FORWARD:
      stars->PlayerCam(CameraDirector::MODE_COCKPIT);
      break;
    case VIEW_CHASE:
      stars->PlayerCam(CameraDirector::MODE_CHASE);
      break;
    case VIEW_PADLOCK:
      stars->PlayerCam(CameraDirector::MODE_TARGET);
      break;
    case VIEW_ORBIT:
      stars->PlayerCam(CameraDirector::MODE_ORBIT);
      break;
      ...
      if (sim)
      {
        ...
        if (s && s->GetQuantumDrive())
        {
          ...
          if (quantum)
          {
            ...
            std::string rgn_name = menu_item->GetText();
            ...
            if (rgn)
            {
              ...
              {
                Element* elem = msg_ship->GetElement();
                auto msg = NEW RadioMessage(elem, ship, RadioMessage::QUANTUM_TO);
                if (msg)
                {
    ...
    case FARCAST:
      if (sim && msg_ship)
      {
        ...
        {
          Element* elem = msg_ship->GetElement();
          auto msg = NEW RadioMessage(elem, ship, RadioMessage::FARCAST_TO);
          if (msg)
          {
            msg->SetInfo(rgn_name);
            RadioTraffic::Transmit(msg);
...
bool TacticalView::GetMouseLoc3D()
{
  ...
  {
    double focal_dist = width / tan(projector->XAngle());
    Point focal_vect = projector->vpn() * focal_dist + projector->vup() * -1.0f * (my - height / 2.0f) + projector->vrt() * (
      mx - width / 2.0f);
    ...
    if (Keyboard::KeyDown(VK_SHIFT))
    {
      ...
      else
      {
        ...
        if (t > 0)
        {
          Point point = projector->Pos() + focal_vect * t;
          move_alt = point.y - base_alt;
        }
      }
    ...
    if (fabs(focal_vect.y) > 1e-5)
    {
      ...
      while (t <= 0 && my < height - 1)
      {
        ...
        focal_vect = projector->vpn() * focal_dist + projector->vup() * -1.0f * (my - height / 2.0f) + projector->vrt() * (mx
          - width / 2.0f);
...
void TacticalView::DrawMove()
{
  ...
  if (GetMouseLoc3D())
  {
    ...
    int x = static_cast<int>(dest.x);
    int y = static_cast<int>(dest.y);
...
void TacticalView::SendMove()
{
  ...
  {
    Element* elem = msg_ship->GetElement();
    auto msg = NEW RadioMessage(elem, ship, RadioMessage::MOVE_PATROL);
    Point dest = move_loc;
    dest.y += move_alt;
  ...
  switch (show_action)
  {
    ...
    case RadioMessage::DOCK_WITH:
      ...
      if (tgt)
      {
        ...
        if (show_action == RadioMessage::DOCK_WITH && tgt->GetHangar() == nullptr)
          r = 0;
      }
      break;
...
void TacticalView::SendAction()
{
  ...
  {
    Element* elem = msg_ship->GetElement();
    auto msg = NEW RadioMessage(elem, ship, show_action);
