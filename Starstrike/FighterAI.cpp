...
static constexpr double TIME_TO_DOCK = 30;

FighterAI::FighterAI(SimObject* s)
  : ShipAI(s),
    decoy_missile(nullptr),
    missile_time(0),
...
void FighterAI::ExecFrame(double s)
{
  if (!ship)
    return;
  ...
  if (inbound)
  {
    ...
    if (inbound->Final() && time_to_dock > 0)
    {
      ...
      if (deck)
      {
        ...
        ship->SetVelocity(carrier->Velocity() + ship->Heading() * 50.0f);
        ship->SetThrottle(50);
        ship->ExecFLCSFrame();
  ...
  else
  {
    ...
    if (orders && (orders->Action() == RadioMessage::WEP_HOLD || orders->Action() == RadioMessage::FORM_UP))
    {
      form_up = true;
      rtb_code = 0;
    }
...
void FighterAI::FindObjective()
{
  ...
  {
    auto orders = ship->GetRadioOrders();
    int action = 0;
...
void FighterAI::ReturnToBase(Ship* controller)
{
  rtb_code = 0;
  ...
  if (controller)
  {
    ...
    if (self_rgn && rtb_rgn && self_rgn != rtb_rgn)
    {
      ...
      if (rtb_rgn->GetOrbitalRegion()->Primary() == self_rgn->GetOrbitalRegion()->Primary())
      {
      ...
      else
      {
        ...
        bool use_farcaster = !qdrive || !qdrive->IsPowerOn() || qdrive->Status() < System::DEGRADED;
...
void FighterAI::FindObjectiveNavPoint()
{
  SimRegion* self_rgn = ship->GetRegion();
  ...
  {
    if (nav_rgn->GetOrbitalRegion()->Primary() == self_rgn->GetOrbitalRegion()->Primary())
    {
...
Point FighterAI::ClosingVelocity()
{
  if (ship)
  {
...
void FighterAI::Navigator()
{
  ...
  if (rtb_code == 1 && navpt && navpt->Status() < Instruction::SKIPPED && !inbound && distance < 35e3)
  { // (this should be distance to the ship)
    ...
    if (order == Instruction::RTB)
    {
      ...
      if (hangar && hangar->CanStow(ship))
      {
        for (int i = 0; i < elem->NumShips(); i++)
        {
          ...
          if (s && s->GetDirector() && s->GetDirector()->Type() >= FIGHTER)
      ...
      else
      {
        if (element_index == 1)
        {
          DebugTrace("WARNING: FighterAI NAVPT RTB, but no controller or hangar found for ship '%s'\n", ship->Name());
    ...
    {
      auto dock_target = static_cast<Ship*>(navpt->GetTarget());
      if (dock_target)
      {
        for (int i = 0; i < elem->NumShips(); i++)
        {
          ...
          if (s)
          {
            auto msg = NEW RadioMessage(dock_target, s, RadioMessage::CALL_INBOUND);
      ...
      else
      {
        if (element_index == 1)
        {
          DebugTrace("WARNING: FighterAI NAVPT DOCK, but no dock target found for ship '%s'\n", ship->Name());
          ship->SetNavptStatus(navpt, Instruction::SKIPPED);
        }
      }
    }
  ...
  hold = false;
  if ((ship->GetElement() && ship->GetElement()->GetHoldTime() > 0) || (navpt && navpt->Status() ==
    Instruction::COMPLETE && navpt->HoldTime() > 0))
    hold = true;
...
void FighterAI::HelmControl()
{
  auto cam = ((Camera*)&(ship->Cam()));
  Point vrt = cam->vrt();
  ...
  if (takeoff || navpt || farcaster || patrol || inbound || rtb_code || target || ward || threat || formation)
  {
    ...
    ship->ApplyRoll(static_cast<float>(accumulator.yaw * -0.7));
    ship->ApplyYaw(static_cast<float>(accumulator.yaw * 0.2));

    if (fabs(accumulator.yaw) > 0.5 && fabs(accumulator.pitch) < 0.1)
      accumulator.pitch -= 0.1f;

    ship->ApplyPitch(static_cast<float>(accumulator.pitch));
  }
...
void FighterAI::ThrottleControl()
{
  Element* elem = ship->GetElement();
  ...
  else if (inbound)
  {
    ...
    // holding short?
    if (inbound->Approach() == 0 && !inbound->Cleared() && distance < 2000 && !ship->IsAirborne())
      desired = 0;
  ...
  ship->SetThrottle(static_cast<int>(throttle));
...
Steer FighterAI::AvoidTerrain()
{
  Steer avoid;

  terrain_warning = false;

  if (!ship || !ship->GetRegion() || !ship->GetRegion()->IsActive() || (navpt && navpt->Action() ==
    Instruction::LAUNCH))
    return avoid;
...
Steer FighterAI::SeekTarget()
{
  ...
  if (tgt)
  {
    ...
    {
      auto tgt_ship = static_cast<Ship*>(tgt);
      ...
      else if (tgt_ship->IsStarship())
      {
        if (gap < 1000)
          return Flee(objective);
        if (ship->GetFlightModel() == Ship::FM_STANDARD && gap < 20e3)
          go_manual = true;
      }
    }
...
Steer FighterAI::SeekFormationSlot()
{
  Steer s;
...
Steer FighterAI::Seek(const Point& point)
{
  Steer s;
  ...
  if (threat && !form_up)
  {
    ...
    if (threat_dist <= threat_range)
    {
      ...
      if (threat_ship->IsStarship())
      {
        ...
        if (!target)
        {
          ...
          if (Game::GameTime() - jink_time > 1500)
          {
            ...
            jink = Point(rand() - 16384, rand() - 16384, rand() - 16384) * 15e3;
          }
        ...
        {
          jink_time = Game::GameTime();
          jink = Point(rand() - 16384, rand() - 16384, rand() - 16384);
        }
      ...
      if (!target)
      {
        ...
        {
          jink_time = Game::GameTime();
          jink = Point(rand() - 16384, rand() - 16384, rand() - 16384) * 2000.0f;
        }
...
void FighterAI::FireControl()
{
  ...
  if (target->Type() == SimObject::SIM_SHIP)
  {
    tgt_ship = static_cast<Ship*>(target);
  ...
  if (primary)
  {
    const WeaponDesign* dsgn_primary = primary->Design();
    ...
    if (use_primary)
    {
      ...
      if (primary->GetFiringOrders() == Weapon::MANUAL && dx < gun_basket && dy < gun_basket && distance > dsgn_primary
        ->min_range && distance < dsgn_primary->max_range && !primary->IsBlockedFriendly())
        ship->FirePrimary();
    }
  }
  ...
  {
    const WeaponDesign* dsgn_secondary = secondary->Design();
...
double FighterAI::CalcDefensePerimeter(Ship* starship)
{
  ...
  if (starship)
  {
    ...
    while (++g_iter)
    {
      ...
      while (++w_iter)
      {
        ...
        if (weapon->Ammo() && weapon->GetTarget() == ship && !weapon->IsBlockedFriendly())
        {
