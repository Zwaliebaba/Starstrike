...
CombatGroup::CombatGroup(int t, int n, std::string_view s, int iff_code, int e, CombatGroup* p)
  : type(t),
    id(n),
    name(s),
    iff(iff_code),
...
void CombatGroup::AddComponent(CombatGroup* g)
{
  if (g)
  {
    g->parent = this;
...
bool CombatGroup::IsTargetable() const
{
  ...
  if (type == PASSENGER || type == PRIVATE || type == MEDICAL || type == HABITAT)
    return false;
...
{
  if (type < BATTALION || type == MINEFIELD ||   // assault, not strike
    type == PASSENGER || type == PRIVATE || type == MEDICAL || type == HABITAT)
    return false;
...
CombatGroup* CombatGroup::FindGroup(int t, int n)
{
...
CombatGroup* CombatGroup::Clone(bool deep)
{
  auto clone = NEW CombatGroup(type, id, name, iff, enemy_intel);
  ...
  if (deep)
  {
    for (int i = 0; i < components.size(); i++)
    {
      ...
      if (g->Type() == FIGHTER_SQUADRON || g->Type() == INTERCEPT_SQUADRON || g->Type() == ATTACK_SQUADRON || g->Type()
        == LCA_SQUADRON)
      {
        if (units.size() > 0)
        {
          ...
          for (int u = 0; u < g->GetUnits().size(); u++)
          {
            ...
            if (unit->Type() >= Ship::FIGHTER || unit->Type() <= Ship::LCA)
            {
...
const char* CombatGroup::GetOrdinal() const
{
  ...
  if (last_two_digits > 10 && last_two_digits < 20)
  {
    ...
    std::string suffix = Game::GetText(ordinal);
...
const char* CombatGroup::GetDescription() const
{
  ...
  if (name.length())
    sprintf_s(name_desc, " \"%s\"", name.c_str());
  else
    name_desc[0] = 0;
...
void CombatGroup::MoveTo(const Point& loc) { location = loc; }

void CombatGroup::SetAssignedSystem(std::string_view s)
{
  assigned_system = s;
  assigned_zone = nullptr;
  zone_lock = false;
...
void CombatGroup::SetAssignedZone(CombatZone* z)
{
  assigned_zone = z;
...
void CombatGroup::SetZoneLock(bool lock)
{
  if (!assigned_zone)
    zone_lock = false;
  else
...
{
  if (n < Intel::RESERVE || n > Intel::TRACKED)
    return;
...
CombatGroup* CombatGroup::FindCarrier()
{
  ...
  while (p != nullptr && p->Type() != CARRIER_GROUP && p->Type() != STATION && p->Type() != STARBASE)
    p = p->GetParent();
...
CombatUnit* CombatGroup::GetRandomUnit()
{
  ...
  if (live.size() > 0)
  {
    ...
    while (!result && ntries-- > 0)
    {
      ...
      if (ship_class >= Ship::CRUISER && ship_class <= Ship::FARCASTER)
        result = nullptr;
    }
  }
...
CombatUnit* CombatGroup::FindUnit(std::string_view name)
{
  if (units.size() > 0)
  {
...
void CombatGroup::AssignRegion(std::string rgn)
{
  region = rgn;
...
int CombatGroup::TypeFromName(std::string_view type_name)
{
...
std::string_view CombatGroup::NameFromType(int type) { return group_name[type]; }

int ShipClassFromName(const char* type_name) { return Ship::ClassForName(type_name); }
...
CombatGroup* CombatGroup::LoadOrderOfBattle(std::string_view filename, int team, Combatant* combatant)
{
  ...
  if (!term)
  {
    DebugTrace("ERROR: could not parse order of battle '{}'\n", filename);
    return nullptr;
  }
  TermText* file_type = term->isText();
  if (!file_type || file_type->value() != "ORDER_OF_BATTLE")
  {
    DebugTrace("ERROR: invalid Order of Battle file '{}'\n", filename);
    term->print(10);
    return nullptr;
  }
      ...
      if (def)
      {
        if (def->name()->value() == "group")
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: group struct missing in '%s'\n", filename);
          else
          {
            ...
            int id = 0;
            int iff = -1;
            auto loc = Vec3(1.0e9f, 0.0f, 0.0f);

            List<CombatUnit> unit_list;
            std::string unit_name;
            std::string unit_regnum;
            std::string unit_design;
            std::string unit_skin;
            int unit_class = 0;
            int unit_count = 1;
            int unit_dead = 0;
            int unit_damage = 0;
                      ...
                      GET_DEF_TEXT(region);
                        ...
                        GET_DEF_TEXT(system);
                          ...
                          GET_DEF_VEC(loc);
                            ...
                            GET_DEF_TEXT(parent_type);
                            else
                              GET_DEF_NUM(parent_id);
                              else
                                GET_DEF_NUM(iff);
                                else
                                  GET_DEF_NUM(id);
                                  else
                                    GET_DEF_NUM(unit_index);

                                    else if ((iff == team || team < 0) && pdef->name()->value() == "unit")
                                    {
                                      if (!pdef->term() || !pdef->term()->isStruct())
                                        DebugTrace("WARNING: unit struct missing for group '%s' in '%s'\n", name,
                                                   filename);
                                      else
                                      {
                                        TermStruct* val = pdef->term()->isStruct();

                                        std::string unit_region;
                                        std::string design;
                                        auto unit_loc = Vec3(1.0e9f, 0.0f, 0.0f);
                                        unit_count = 1;
                                        ...
                                        {
                                          DebugTrace("ERROR: invalid design '%s' for unit '%s' in '%s'\n", unit_design,
                                                     unit_name, filename);
                                          return nullptr;
                                        }

                                        auto cu = NEW CombatUnit(unit_name, unit_regnum, unit_class, unit_design,
                                                                 unit_count, iff);
            ...
            {
              CombatGroup* parent_group = nullptr;

              if (force)
                parent_group = force->FindGroup(TypeFromName(parent_type), parent_id);

              auto g = NEW CombatGroup(TypeFromName(type), id, name, iff, Intel::IntelFromName(intel), parent_group);
              ...
              if (unit_list.size())
              {
                ...
                while (++u)
                {
                  ...
                  if (parent_group && (u->Type() == Ship::FIGHTER || u->Type() == Ship::ATTACK))
                  {
                    CombatUnit* carrier = nullptr;
...
void CombatGroup::MergeOrderOfBattle(BYTE* block, std::string_view filename, int team, Combatant* combatant,
                                     Campaign* campaign)
{
  CombatGroup* force = nullptr;
  ...
  if (!term)
  {
    DebugTrace("ERROR: could not parse order of battle '%s'\n", filename);
    return;
  }
  TermText* file_type = term->isText();
  if (!file_type || file_type->value() != "SAVEGAME")
  {
    DebugTrace("ERROR: invalid Save Game file '%s'\n", filename);
    term->print(10);
    return;
  }
        ...
        if (def->name()->value() == "group")
        {
          ...
          else
          {
                                        ...
                                        GET_DEF_NUM(points);
                                          ...
                                          else if ((iff == team || team < 0) && pdef->name()->value() == "unit")
                                          {
                                            ...
                                            else
                                            {
                                              ...
                                              std::string unit_region;
                                              std::string design;
                                              auto unit_loc = Vec3(0.0f, 0.0f, 0.0f);
                                              unit_count = 1;
                                              ...
                                              if (force)
                                              {
                                                auto cu = NEW CombatUnit(
                                                  unit_name, unit_regnum, unit_class, unit_design, unit_count, iff);
                                                cu->SetRegion(unit_region);
                                                cu->MoveTo(unit_loc);
                                                cu->Kill(unit_dead);
                                                cu->SetSustainedDamage(unit_damage);
            ...
            if (iff >= 0 && (iff == team || team < 0))
            {
              ...
              if (!g)
              {
                DebugTrace("WARNING: unexpected combat group %s %d '%s' in '%s'\n", type, id, name, filename);
                continue;
              }
              ...
              if (!zone.empty())
              {
                if (CombatZone* combat_zone = campaign->GetZone(zone))
                {
                  g->SetAssignedZone(combat_zone);
                  g->SetZoneLock(zone_locked);
                }
                else
                  DebugTrace("WARNING: could not find combat zone '{}' for group {} {} '%s' in '%s'\n", zone, type, id,
                             name, filename);
              }
...
std::string FormatNumber(double n)
{
  ...
  if (fabs(n) < 1000)
    sprintf_s(buffer, "%d", static_cast<int>(n));

  else if (fabs(n) < 1e6)
  {
    int nn = static_cast<int>(n) / 1000;
...
void SaveCombatUnit(FILE* f, CombatUnit* u)
{
  ...
  fprintf(f, " type: \"%s\",", Ship::ClassName(type).data());
  ...
  if (u->GetRegion().length() > 0)
  {
    ...
    std::string x = FormatNumber(u->Location().x);
    std::string y = FormatNumber(u->Location().y);
    std::string z = FormatNumber(u->Location().z);

    fprintf(f, " loc:(%s, %s, %s),", x.data(), y.data(), z.data());
  }

  fprintf(f, " dead_count: %d, damage: %d, heading: %d },", u->DeadCount(), static_cast<int>(u->GetSustainedDamage()),
          static_cast<int>(u->GetHeading() / DEGREES));
}

void SaveCombatGroup(FILE* f, CombatGroup* g)
{
  fprintf(f, "group: {");
  fprintf(f, " type: %s,", CombatGroup::NameFromType(g->Type()).data());
  fprintf(f, " id: %d,", g->GetID());
  fprintf(f, " name: \"%s\",", g->Name().data());
  fprintf(f, " intel: %s,", Intel::NameFromIntel(g->IntelLevel()).c_str());
  ...
  std::string x = FormatNumber(g->Location().x);
  std::string y = FormatNumber(g->Location().y);
  std::string z = FormatNumber(g->Location().z);
  ...
  if (parent)
  {
    fprintf(f, " parent_type:%s,", CombatGroup::NameFromType(parent->Type()).data());
...
void CombatGroup::SaveOrderOfBattle(std::string_view filename, CombatGroup* force)
{
  FILE* f;
  fopen_s(&f, filename.data(), "a+");
