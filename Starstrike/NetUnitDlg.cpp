...
void NetUnitDlg::RegisterControls()
{
  lst_players = static_cast<ListBox*>(FindControl(201));
  lst_units = static_cast<ListBox*>(FindControl(202));
  lst_chat = static_cast<ListBox*>(FindControl(211));
  edt_chat = static_cast<EditBox*>(FindControl(212));
  ...
  btn_select = static_cast<Button*>(FindControl(206));
  REGISTER_CLIENT(EID_CLICK, btn_select, NetUnitDlg, OnSelect);

  btn_map = static_cast<Button*>(FindControl(203));
  REGISTER_CLIENT(EID_CLICK, btn_map, NetUnitDlg, OnMap);

  btn_unmap = static_cast<Button*>(FindControl(204));
  REGISTER_CLIENT(EID_CLICK, btn_unmap, NetUnitDlg, OnUnMap);

  btn_ban = static_cast<Button*>(FindControl(205));
...
void NetUnitDlg::ExecFrame()
{
  ...
  if (btn_map)
  {
    ...
    if (lst_players && lst_units && btn_map->IsVisible())
    {
      ...
      enable = sel_unit >= 0 && sel_play >= 0 && lst_units->GetItemText(sel_unit).length() == 0;

      if (enable && !host_mode)
      {
        auto u = (NetUser*)lst_players->GetItemData(sel_play);
  ...
  if (btn_unmap)
  {
    ...
    if (lst_players && lst_units && btn_unmap->IsVisible())
    {
      ...
      if (enable && !host_mode)
      {
        auto u = (NetUser*)lst_units->GetItemData(sel_unit);
  ...
  if (btn_ban)
  {
    ...
    if (lst_players && lst_units && host_mode && btn_ban->IsVisible())
    {
      ...
      if (enable)
      {
        auto u = reinterpret_cast<NetUser*>(lst_players->GetItemData(sel_play));
        if (u && u->Name() == player_name)
          enable = false;
      }
    }
...
void NetUnitDlg::ExecLobbyFrame()
{
...
void NetUnitDlg::CheckUnitMapping()
{
  if (net_lobby && lst_units)
  {
    ...
    else if (lst_units->NumItems())
    {
      for (int i = 0; i < units.size(); i++)
      {
        ...
        std::string user_name = e->GetUserName();
        NetUser* u = net_lobby->FindUserByName(user_name);

        if (lst_units->GetItemData(i) != reinterpret_cast<uintptr_t>(u))
          assignment_change = true;
      }
    }
  }
  ...
  if (net_lobby)
  {
    ...
    if (available_users.size() != lst_players->NumItems())
    {
      ...
      for (int i = 0; i < available_users.size(); i++)
      {
        ...
        std::string name = Player::RankAbrv(u->Rank());
        name += " ";
        name += u->Name();

        lst_players->AddItemWithData(name, (intptr_t)u);
  ...
  if (net_lobby)
  {
    ...
    if (assignment_change)
    {
      ...
      for (int i = 0; i < units.size(); i++)
      {
        ...
        int count = lst_units->AddItemWithData(user_name, (intptr_t)u);
...
void NetUnitDlg::SendChat(std::string msg)
{
  if (msg.length() < 1)
    return;
  ...
  if (msg[0] >= '0' && msg[0] <= '9')
  {
    if (player)
    {
      std::string macro = player->ChatMacro(msg[0] - '0');
  ...
  if (net_lobby)
    net_lobby->AddChat(nullptr, msg);
}

void NetUnitDlg::OnUnit(AWEvent* event)
{
  if (!lst_units || host_mode)
    return;
...
void NetUnitDlg::OnSelect(AWEvent* event)
{
  if (!lst_players || !lst_units)
    return;
...
void NetUnitDlg::OnMap(AWEvent* event)
{
  if (!lst_players || !lst_units)
    return;
  ...
  if (net_lobby)
  {
    auto u = reinterpret_cast<NetUser*>(lst_players->GetItemData(sel_player));
...
void NetUnitDlg::OnUnMap(AWEvent* event)
{
  if (!lst_players || !lst_units)
    return;
  ...
  if (net_lobby)
  {
    net_lobby->MapUnit(lst_units->GetSelection(), {}, host_mode);
    lst_units->ClearItems();
  }
...
void NetUnitDlg::OnBan(AWEvent* event)
{
  if (!lst_players)
    return;
  ...
  if (net_lobby)
  {
    ...
    if (ConfirmDlg* confirm = manager->GetConfirmDlg())
    {
      char msg[512];
...
void NetUnitDlg::OnBanConfirm(AWEvent* event)
{
  if (!lst_players)
    return;
...
void NetUnitDlg::OnApply(AWEvent* event)
{
  bool ok = false;
...
void NetUnitDlg::OnCancel(AWEvent* event)
{
  if (net_lobby)
  {
