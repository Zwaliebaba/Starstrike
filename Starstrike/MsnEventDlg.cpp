...
void MsnEventDlg::RegisterControls()
{
  ...
  edt_time = static_cast<EditBox*>(FindControl(202));
  edt_delay = static_cast<EditBox*>(FindControl(203));
  cmb_event = static_cast<ComboBox*>(FindControl(204));
  cmb_event_ship = static_cast<ComboBox*>(FindControl(205));
  cmb_event_source = static_cast<ComboBox*>(FindControl(206));
  cmb_event_target = static_cast<ComboBox*>(FindControl(207));
  edt_event_param = static_cast<EditBox*>(FindControl(208));
  edt_event_chance = static_cast<EditBox*>(FindControl(220));
  edt_event_sound = static_cast<EditBox*>(FindControl(209));
  edt_event_message = static_cast<EditBox*>(FindControl(210));

  cmb_trigger = static_cast<ComboBox*>(FindControl(221));
  cmb_trigger_ship = static_cast<ComboBox*>(FindControl(222));
  cmb_trigger_target = static_cast<ComboBox*>(FindControl(223));
  edt_trigger_param = static_cast<EditBox*>(FindControl(224));
...
void MsnEventDlg::FillShipList(ComboBox* cmb, std::string_view seln)
{
  if (!cmb)
    return;
  cmb->ClearItems();
...
void MsnEventDlg::FillRgnList(ComboBox* cmb, std::string_view seln)
{
  if (!cmb)
    return;
  cmb->ClearItems();
...
void MsnEventDlg::SetMission(Mission* m) { mission = m; }

void MsnEventDlg::SetMissionEvent(MissionEvent* e) { event = e; }
...
void MsnEventDlg::OnAccept(AWEvent* e)
{
  if (mission && event)
  {
    ...
    if (edt_time)
    {
      strcpy_s(buf, edt_time->GetText().c_str());
    ...
    if (edt_delay)
    {
      strcpy_s(buf, edt_delay->GetText().c_str());
    ...
    if (edt_event_param)
    {
      strcpy_s(buf, edt_event_param->GetText().c_str());
        ...
        if (term && term->isArray())
        {
          ...
          if (val)
          {
            ...
            for (int i = 0; i < nelem; i++)
              event->event_param[i] = static_cast<int>(val->elements()->at(i)->isNumber()->value());
    ...
    if (edt_event_chance)
    {
      strcpy_s(buf, edt_event_chance->GetText().c_str());
    ...
    if (edt_trigger_param)
    {
      strcpy_s(buf, edt_trigger_param->GetText().c_str());
      ...
      else if (*buf == '(')
      {
        ...
        if (term && term->isArray())
        {
          ...
          if (val)
          {
            ...
            for (int i = 0; i < nelem; i++)
              event->trigger_param[i] = static_cast<int>(val->elements()->at(i)->isNumber()->value());
...
void MsnEventDlg::OnCancel(AWEvent* event) { manager->ShowMsnEditDlg(); }
