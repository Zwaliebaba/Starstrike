...
void Renderer::RenderLogo()
{
  ...
  g_gameFont.DrawText2D(20, m_screenH - 70, 25, Strings::Get("privatedemo1"));
  g_gameFont.DrawText2D(20, m_screenH - 40, 25, Strings::Get("privatedemo2"));
  g_gameFont.DrawText2D(20, m_screenH - 10, 10, Strings::Get("privatedemo2"));
}
...
{
  auto msg = "PAUSED";
  int x = ClientEngine::OutputSize().Width / 2;
  int y = ClientEngine::OutputSize().Height / 2;
  DX9TextRenderer& font = g_gameFont;
...
void Renderer::RenderFrame(bool withFlip)
{
  ...
  {
    int textureId = Resource::GetTexture("icons\\darwin_research_associates.bmp");
    ...
    glColor4f(alpha, alpha, alpha, 0.0f);
    g_gameFont.DrawText2DCenter(m_screenW / 2, m_screenH * 0.8f, textSize, "DARWINIA");
...
const float* Renderer::GetTotalMatrix() { return m_totalMatrix; }
...
void Renderer::MarkUsedCells(const ShapeFragment* _frag, const Matrix34& _transform)
{
  ...
  LegacyVector3 worldPos = _frag->m_center * total;
