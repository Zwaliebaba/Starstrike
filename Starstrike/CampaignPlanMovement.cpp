...
void CampaignPlanMovement::ExecFrame()
{
  ...
  {
    if (Campaign::Stardate() - m_execTime < 7200)
      return;

    m_campaign->GetAllCombatUnits(-1, all_units);
...
void CampaignPlanMovement::MoveUnit(CombatUnit* u)
{
  if (u)
  {
    // starship repair:
    ...
    {
      int percent = static_cast<int>(100 * damage / u->GetDesign()->integrity);
    ...
    constexpr double MAX_RAD = 320e3;
    constexpr double MIN_DIST = 150e3;
