...
void DeformEffect::AddPunch(LegacyVector3 pos, float range)
{
	CHECK_OK;
	Punch punch;
	punch.m_pos = pos;
...
static float Get2DScreenPos(LegacyVector3 const &_vector, float *_screenX, float *_screenY)
{
...
void DeformEffect::RenderSprite(Shader* shader, LegacyVector3 posWorld, float width, float depth, float redux)
{
	Vector2 posScreen = Vector2(0,0);
	Vector2 posScreen2 = Vector2(0,0);
	LegacyVector3 lookDir = (posWorld-g_app->m_camera->GetPos()).Normalise();
	float size = 0.001f;
	float aspect = g_app->m_renderer->ScreenW()/(float)g_app->m_renderer->ScreenH();
	if(Get2DScreenPos(posWorld,&posScreen.x,&posScreen.y)<1)
	{
		LegacyVector3 posWorld2 = posWorld + LegacyVector3(lookDir.z,0,-lookDir.x).Normalise();
...
void DeformEffect::Stop()
{
	...
	RenderSprite(m_deformFullscreenShader,LegacyVector3(0,0,0),1,0,1);
