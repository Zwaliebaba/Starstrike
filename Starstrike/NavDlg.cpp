...
static const char* filter_name[] = {"SYSTEM", "PLANET", "SECTOR", "STATION", "STARSHIP", "FIGHTER"};
...
NavDlg::NavDlg(Screen* s, FormDef& def, BaseScreen* mgr)
  : FormWindow(s, 0, 0, s->Width(), s->Height()),
    ...
    nav_edit_mode(NAV_EDIT_NONE),
    star_system(nullptr),
    ship(nullptr),
    mission(nullptr),
    editor(false) { Init(def); }
...
void NavDlg::RegisterControls()
{
  ...
  for (i = 0; i < 3; i++)
  {
    view_btn[i] = static_cast<Button*>(FindControl(101 + i));
    REGISTER_CLIENT(EID_CLICK, view_btn[i], NavDlg, OnView);
  }

  close_btn = static_cast<Button*>(FindControl(2));

  if (close_btn)
    REGISTER_CLIENT(EID_CLICK, close_btn, NavDlg, OnClose);

  zoom_in_btn = static_cast<Button*>(FindControl(110));
  zoom_out_btn = static_cast<Button*>(FindControl(111));

  for (i = 0; i < 6; i++)
  {
    filter_btn[i] = static_cast<Button*>(FindControl(401 + i));
    REGISTER_CLIENT(EID_CLICK, filter_btn[i], NavDlg, OnFilter);
  }

  commit_btn = static_cast<Button*>(FindControl(1));

  if (commit_btn) { REGISTER_CLIENT(EID_CLICK, commit_btn, NavDlg, OnEngage); }
  ...
  seln_list = static_cast<ListBox*>(FindControl(801));
  ...
  info_list = static_cast<ListBox*>(FindControl(802));
...
void NavDlg::SetSystem(StarSystem* s)
{
  if (star_system == s)
    return;
...
void NavDlg::SetMission(Mission* m)
{
  if (!m && mission == m)
    return;
  ...
  if (star_map)
  {
    ...
    star_map->SetMission(nullptr);   // prevent building map menu twice
  ...
  if (mission)
  {
    ...
    if (rgn)
    {
      SelectRegion(static_cast<OrbitalRegion*>(rgn));
      updated = true;
    }
  }
...
void NavDlg::SetEditorMode(bool e)
{
  editor = e;
...
void NavDlg::ExecFrame()
{
  ...
  if (loc_labels && ship)
  {
    ...
    strcpy_s(loc_buf, Game::GetText("NavDlg.loc-labels").c_str());
    loc_labels->SetText(loc_buf);

    if (sim->GetActiveRegion())
    {
      sprintf_s(loc_buf, "\n%s\n%s\n%s, %s, %s", star_system->Name().c_str(), sim->GetActiveRegion()->Name().c_str(), x, y, z);
    }
    else { sprintf_s(loc_buf, "\n%s\nPlanck Space?\n%s, %s, %s", star_system->Name().c_str(), x, y, z); }
    ...
    if (dst_labels)
    {
      ...
      if (navpt && navpt->Region())
      {
        ...
        sprintf_s(loc_buf, "\n%s\n%s\n%s, %s, %s\n%s", star_system->Name().c_str(), navpt->Region()->Name().c_str(), x, y, z, d);
...
void NavDlg::OnView(AWEvent* event)
{
...
void NavDlg::OnFilter(AWEvent* event)
{
  int filter_index = -1;
  for (int i = 0; i < 6; i++)
  {
...
void NavDlg::UseFilter(int filter_index)
{
...
void NavDlg::UseViewMode(int mode)
{
  if (mode >= 0 && mode < 3)
  {
    int use_filter_mode = -1;
...
void NavDlg::SelectStar(Orbital* star)
{
  UseViewMode(0);
...
void NavDlg::SelectPlanet(Orbital* planet)
{
  UseViewMode(1);
...
void NavDlg::SelectRegion(OrbitalRegion* rgn)
{
  UseViewMode(2);
...
void NavDlg::OnSelect(AWEvent* event)
{
  int index = -1;
...
void NavDlg::SelectObject(int index)
{
  std::string selected = seln_list->GetItemText(index);
...
void NavDlg::UpdateSelection()
{
  ...
  std::string units_km = std::string(" ") + Game::GetText("NavDlg.units.kilometers");
  std::string units_tonnes = std::string(" ") + Game::GetText("NavDlg.units.tonnes");
  ...
  if (seln_mode <= SELECT_REGION)
  {
    ...
    if (s)
    {
      ...
      {
        info_list->AddItem(Game::GetText(std::string("NavDlg.filter.") + filter_name[seln_mode]));
  ...
  else if (seln_mode == SELECT_STATION || seln_mode == SELECT_STARSHIP || seln_mode == SELECT_FIGHTER)
  {
    ...
    if (sel_ship)
    {
      std::string order_desc = Game::GetText("NavDlg.none");
      char shield[16];
      char hull[16];
      char range[32];
      ...
      info_list->SetItemText(
        row++, 1, std::string(sel_ship->Abbreviation()) + std::string(" ") + std::string(sel_ship->Design()->display_name));
    ...
    else if (sel_elem)
    {
      ...
      if (sel_elem->GetDesign())
        info_list->SetItemText(row++, 1, sel_elem->Abbreviation() + std::string(" ") + sel_elem->GetDesign()->m_name);
      else
...
void NavDlg::UpdateLists()
{
  ...
  switch (seln_mode)
  {
    case SELECT_SYSTEM:
    {
      seln_list->SetColumnTitle(0, Game::GetText(std::string("NavDlg.filter.") + filter_name[seln_mode]));
      int i = 0;
      ListIter<StarSystem> iter = star_map->GetGalaxy();
      while (++iter)
        seln_list->AddItemWithData(iter->Name().c_str(), i++);
    }
    break;
    ...
    case SELECT_PLANET:
    {
      ...
      while (++iter)
      {
        if (iter->Type() == Orbital::MOON)
          seln_list->AddItemWithData(std::string("- ") + std::string(iter->Name()), i++);
        else
    ...
    case SELECT_FIGHTER:
    {
      ...
      if (mission)
      {
        ...
        {
          MissionElement* e = elem.value();
          bool filter_ok = (seln_mode == SELECT_STATION && e->IsStatic()) || (seln_mode == SELECT_STARSHIP && e->
            IsStarship() && !e->IsStatic()) || (seln_mode == SELECT_FIGHTER && e->IsDropship() && !e->IsSquadron());
      ...
      else if (ship)
      {
        ...
        while (++r_iter)
        {
          ...
          {
            Ship* s = s_iter.value();
            bool filter_ok = (seln_mode == SELECT_STATION && s->IsStatic()) || (seln_mode == SELECT_STARSHIP && s->
              IsStarship() && !s->IsStatic()) || (seln_mode == SELECT_FIGHTER && s->IsDropship());

            if (filter_ok)
            {
              bool visible = s->GetIFF() == 0 || s->GetIFF() == ship->GetIFF() || s->GetElement() && s->GetElement()->
                IntelLevel() > Intel::KNOWN;
...
void NavDlg::OnEngage(AWEvent* event)
{
  bool hide = false;
...
void NavDlg::OnCommit(AWEvent* event)
{
  if (manager)
    manager->ShowNavDlg();   // also hides
}

void NavDlg::OnCancel(AWEvent* event)
{
  if (manager)
    manager->ShowNavDlg();   // also hides
}

void NavDlg::SetNavEditMode(int mode)
{
...
int NavDlg::GetNavEditMode() { return nav_edit_mode; }

void NavDlg::OnMapDown(AWEvent* event) {}

void NavDlg::OnMapMove(AWEvent* event) {}

void NavDlg::OnMapClick(AWEvent* event)
{
...
void NavDlg::CoordinateSelection()
{
  ...
  switch (seln_mode)
  {
    ...
    case SELECT_REGION:
    {
      ...
      if (selected)
      {
        for (int i = 0; i < seln_list->NumItems(); i++)
        {
          if (seln_list->GetItemText(i) == selected->Name() || seln_list->GetItemText(i) == std::string("- ") + selected->
            Name())
...
void NavDlg::OnClose(AWEvent* event)
{
  if (manager)
    manager->HideNavDlg();
}
