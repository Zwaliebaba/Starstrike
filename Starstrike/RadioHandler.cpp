...
bool RadioHandler::ProcessMessage(RadioMessage* msg, Ship* s)
{
  if (!s || !msg || !msg->Sender())
    return false;
  ...
  {
    auto sender = (Ship*)msg->Sender();  // cast-away const
    auto nak = NEW RadioMessage(sender, s, RadioMessage::NACK);
...
bool RadioHandler::IsOrder(int action)
{
  ...
  switch (action)
  {
    default: case RadioMessage::NONE:
    case RadioMessage::ACK:
    case RadioMessage::NACK:
      result = false;
      break;
    ...
    case RadioMessage::MOVE_PATROL:
      result = true;
      break;
    case RadioMessage::SKIP_NAVPOINT:
      result = false;
      break;
    ...
    case RadioMessage::CALL_WAVE_OFF:
      result = false;
      break;
  }
...
bool RadioHandler::ProcessMessageOrders(RadioMessage* msg, Ship* ship)
{
  ...
  if (msg && IsOrder(msg->Action()))
  {
    ...
    if (action >= RadioMessage::ATTACK && action <= RadioMessage::COVER_ME || action == RadioMessage::WEP_HOLD || action
      >= RadioMessage::DOCK_WITH && action <= RadioMessage::FARCAST_TO)
    {
      if (ship != msg->Sender())
        ship->DropTarget();
    ...
    if (action >= RadioMessage::GO_DIAMOND && action <= RadioMessage::GO_TRAIL)
    {
      switch (action)
      {
        case RadioMessage::GO_DIAMOND:
          instruction->SetFormation(Instruction::DIAMOND);
          break;
        case RadioMessage::GO_SPREAD:
          instruction->SetFormation(Instruction::SPREAD);
          break;
        case RadioMessage::GO_BOX:
          instruction->SetFormation(Instruction::BOX);
          break;
        case RadioMessage::GO_TRAIL:
          instruction->SetFormation(Instruction::TRAIL);
          break;
      }
    ...
    if (action >= RadioMessage::GO_EMCON1 && action <= RadioMessage::GO_EMCON3)
    {
      switch (msg->Action())
      {
        case RadioMessage::GO_EMCON1:
    ...
    if (!posture_only)
    {
      ...
      if (action == RadioMessage::ATTACK && msg->TargetList().size() > 0)
      {
        ...
        if (elem)
        {
          ...
          {
            Element* tgt = static_cast<Ship*>(msg_tgt)->GetElement();
...
bool RadioHandler::ProcessMessageAction(RadioMessage* msg, Ship* ship)
{
  if (!msg)
    return false;
...
bool RadioHandler::SkipNavpoint(RadioMessage* msg, Ship* ship)
{
...
bool RadioHandler::LaunchProbe(RadioMessage* msg, Ship* ship)
{
  if (ship && ship->GetProbeLauncher())
  {
    ship->LaunchProbe();
    return ship->GetProbe() != nullptr;
  }

  return false;
}

bool RadioHandler::Inbound(RadioMessage* msg, Ship* ship)
{
  auto inbound = (Ship*)msg->Sender();
  ...
  if (inbound->GetInbound() && inbound->GetInbound()->GetDeck() && inbound->GetInbound()->GetDeck()->GetCarrier() ==
    ship)
  {
  ...
  {
    auto wave_off = NEW RadioMessage(inbound, ship, RadioMessage::NACK);
    if (!hangar)
      wave_off->SetInfo(Game::GetText("RadioHandler.no-hangar"));

    else if (!same_rgn)
    {
      auto s = ship->GetRegion()->Name();
      auto val = std::make_format_args(s);
      std::string info = std::vformat(Game::GetText("RadioHandler.too-far-away"), val);
  ...
  auto approach = NEW RadioMessage(inbound, ship, RadioMessage::CALL_APPROACH);

  if (inbound_slot->Cleared())
  {
    char info[256];
    sprintf_s(info, Game::GetText("RadioHandler.cleared").data(), deck->Name().c_str());
    approach->SetInfo(info);
  }
  else if (sequence)
  {
    char info[256];
    sprintf_s(info, Game::GetText("RadioHandler.sequenced").data(), sequence, deck->Name().c_str());
    approach->SetInfo(info);
  }
...
bool RadioHandler::Picture(RadioMessage* msg, Ship* ship)
{
  if (!ship)
    return false;
  ...
  if (tgt)
  {
    ...
    auto response = NEW RadioMessage(sender, ship, RadioMessage::ATTACK);
  ...
  {
    auto sender = (Ship*)msg->Sender();  // cast-away const
    auto response = NEW RadioMessage(sender, ship, RadioMessage::PICTURE);
    RadioTraffic::Transmit(response);
  }
...
bool RadioHandler::Support(RadioMessage* msg, Ship* ship)
{
  if (!ship)
    return false;
  ...
  for (int i = 0; i < rgn->Ships().size(); i++)
  {
    ...
    if (e && s->IsDropship() && e->Type() == Mission::PATROL && e != baby && cmdr->CanCommand(e) && s->GetRadioOrders()
      ->Action() == RadioMessage::NONE)
    {
      help = e;
      break;
    }
  ...
  if (help)
  {
    auto escort = NEW RadioMessage(help, ship, RadioMessage::ESCORT);
    escort->TargetList().append(msg->Sender());
    RadioTraffic::Transmit(escort);

    std::string ok = Game::GetText("RadioHandler.help-enroute");
    auto sender = (Ship*)msg->Sender();  // cast-away const
    auto response = NEW RadioMessage(sender, ship, RadioMessage::ACK);
    response->SetInfo(ok);
    RadioTraffic::Transmit(response);
  }
  ...
  {
    std::string nope = Game::GetText("RadioHandler.no-help-for-you");
    auto sender = (Ship*)msg->Sender();  // cast-away const
    auto response = NEW RadioMessage(sender, ship, RadioMessage::NACK);
...
void RadioHandler::AcknowledgeMessage(RadioMessage* msg, Ship* s)
{
  if (s && msg && msg->Sender() && msg->Action())
  {
    ...
    auto sender = (Ship*)msg->Sender();  // cast-away const
    auto ack = NEW RadioMessage(sender, s, RadioMessage::ACK);
