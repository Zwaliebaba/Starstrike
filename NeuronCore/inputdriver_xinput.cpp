...
bool XInputDriver::getInput(const InputSpec& spec, InputDetails& details)
{
  ...
  switch (spec.control_id)
  {
    ...
    {
      if (COND_UP == spec.condition || COND_DOWN == spec.condition || COND_PRESSED == spec.condition || COND_REPEAT == spec.
        condition)
...
void XInputDriver::correctDeadZones(XINPUT_STATE& state)
{
  ...
  if ((state.Gamepad.sThumbRX < XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE && state.Gamepad.sThumbRX > -
    XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE) && (state.Gamepad.sThumbRY < XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE && state.Gamepad.sThumbRY
