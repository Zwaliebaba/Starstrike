...
BinaryFileReader::BinaryFileReader(const char* _filename)
  : BinaryReader()
{
  if (_filename)
  {
...
BinaryFileReader::~BinaryFileReader() { fclose(m_file); }
...
int BinaryFileReader::Seek(int _offset, int _origin) { return fseek(m_file, _offset, _origin); }

int BinaryFileReader::Tell() { return ftell(m_file); }

// ****************************************************************************
// BinaryDataReader
// ****************************************************************************

BinaryDataReader::BinaryDataReader(const unsigned char* _data, unsigned int _dataSize, const char* _filename)
  : BinaryReader(),
    m_offset(0),
    m_data(_data),
    m_dataSize(_dataSize) { strncpy(m_filename, _filename, sizeof(m_filename) - 1); }
...
int BinaryDataReader::Tell() { return m_offset; }
