...
class SortingHashTable : public HashTable<T>
{
  protected:
    short*
    m_orderedIndices;								// Stores a chain of indices that make it easy to walk through the table in alphabetical key order
    ...
    void Grow();
    short
    FindPrevKey(const char* _key) const;			// Returns the index of the table element whose key is alphabetically previous to the specified key
...
template <class T>
short SortingHashTable<T>::FindPrevKey(const char* _key) const
{
  short prevI = -1;
  ...
  while (true)
  {
    ...
    if (_stricmp(this->m_keys[i], _key) >= 0)
      return prevI;
...
template <class T>
SortingHashTable<T>::~SortingHashTable() { delete [] m_orderedIndices; }

template <class T>
int SortingHashTable<T>::PutData(const char* _key, const T& _data)
{
  //
...
template <class T>
void SortingHashTable<T>::RemoveData(const char* _key)
{
