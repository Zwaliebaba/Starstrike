...
class SurfaceMap2D : public Array2D<T>
{
  public:
    ...
    void Initialize(float _width, float _height, float _x0, float _y0, float _cellSizeX, float _cellSizeY, T _outsideValue);

    T GetValue(float _x, float _y) const;
    const T& GetValueNearest(float _x, float _y) const; // Like GetValue but without interpolation
    T* GetPointerNearest(float _x, float _y) const;
    T GetHighestValue() const;

    int GetMapIndexX(float _realX) const;
    int GetMapIndexY(float _realY) const;
    float GetRealX(int _mapIndexX) const;
    float GetRealY(int _mapIndexY) const;
};
...
template <class T>
SurfaceMap2D<T>::SurfaceMap2D(float _width, float _height, float _x0, float _y0, float _cellSizeX, float _cellSizeY,
                              T _outsideValue)
...
template <class T>
void SurfaceMap2D<T>::Initialize(float _width, float _height, float _x0, float _y0, float _cellSizeX, float _cellSizeY,
                                 T _outsideValue)
{
  Array2D<T>::Initialize(ceilf(_width / _cellSizeX), ceilf(_height / _cellSizeY), _outsideValue);
  m_x0 = _x0;
  m_y0 = _y0;
  m_cellSizeX = _cellSizeX;
  m_cellSizeY = _cellSizeY;
...
T SurfaceMap2D<T>::GetValue(float _x, float _y) const
{
  ...
  T returnVal = value11 * weight11 + value12 * weight12 + value21 * weight21 + value22 * weight22;

  return returnVal;
}

template <class T>
const T& SurfaceMap2D<T>::GetValueNearest(float _x, float _y) const
{
...
template <class T>
int SurfaceMap2D<T>::GetMapIndexX(float _realX) const { return (_realX - m_x0) * m_invCellSizeX; }

template <class T>
int SurfaceMap2D<T>::GetMapIndexY(float _realY) const { return (_realY - m_y0) * m_invCellSizeY; }

template <class T>
float SurfaceMap2D<T>::GetRealX(int _mapIndexX) const { return _mapIndexX * m_cellSizeX + m_x0; }

template <class T>
float SurfaceMap2D<T>::GetRealY(int _mapIndexY) const { return _mapIndexY * m_cellSizeY + m_y0; }
