...
InputParserState ValueInputDriver::parseInputSpecification(const InputSpecTokens& tokens, InputSpec& spec)
{
  InputParserState state = STATE_WANT_DRIVER;
  unsigned idx = 0;
  if ((tokens.length() < idx + 1) || (_stricmp(tokens[idx++].c_str(), "value") == 0))
    return state;
...
bool ValueInputDriver::getInput(const InputSpec& spec, InputDetails& details)
{
