...
InputParserState SimpleInputDriver::parseInputSpecification(const InputSpecTokens& tokens, InputSpec& spec)
{
...
bool SimpleInputDriver::acceptToken(InputParserState& state, const string& token, InputSpec& spec)
{
  switch (state)
  {
...
condition_t SimpleInputDriver::getConditionID(const string& name, inputtype_t& type) { return getDefaultConditionID(name, type); }
...
InputParserState SimpleInputDriver::parseExtraToken(const std::string& token, InputSpec& spec)
{
