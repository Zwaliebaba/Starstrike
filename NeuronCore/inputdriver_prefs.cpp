...
using KeyIt = auto_vector<string>::iterator;
...
PrefsInputDriver::PrefsInputDriver()
  : m_keys() { setName("Prefs"); }

InputParserState PrefsInputDriver::parseInputSpecification(const InputSpecTokens& tokens, InputSpec& spec)
{
...
bool PrefsInputDriver::getInput(const InputSpec& spec, InputDetails& details)
{
...
bool PrefsInputDriver::getInputDescription(const InputSpec& spec, InputDescription& desc)
{
  InputDetails details;
  return getInput(spec, details);
}

int PrefsInputDriver::keyPosition(const string& key)
{
  KeyIt i;
  ...
  if (i == m_keys.end())
  {
    m_keys.push_back(std::make_unique<string>(key));
    return m_keys.size();
  }
