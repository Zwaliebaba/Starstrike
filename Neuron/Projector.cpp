...
static constexpr float CLIP_PLANE_EPSILON = 0.0001f;
static constexpr double Z_NEAR = 1.0;
...
void Projector::UseCamera(Camera* cam)
{
  if (cam)
    camera = cam;
  else
...
void Projector::UseWindow(Window* win)
{
  Rect r = win->GetRect();
  width = r.w;
  height = r.h;
  ...
  xclip0 = 0.0f;
  xclip1 = static_cast<float>(width) - 0.5f;
  yclip0 = 0.0f;
  yclip1 = static_cast<float>(height) - 0.5f;
...
double Projector::GetFieldOfView() const { return field_of_view; }

void Projector::SetDepthScale(float scale) { depth_scale = scale; }

double Projector::GetDepthScale() const { return depth_scale; }
...
void Projector::Transform(Vec3& vec) const
{
  Vec3 tvert = vec;
...
void Projector::Transform(Point& point) const
{
  Point tvert = point;
...
float Projector::ProjectRadius(const Vec3& v, float radius) const
{
  return static_cast<float>(fabs((radius * maxscale) / v.z));
}
...
void Projector::Project(Vec3& v, bool clamp) const
{
  double zrecip;
  ...
  if (orthogonal)
  {
    ...
    v.x = static_cast<float>(xcenter + scale * v.x);
    v.y = static_cast<float>(height - (ycenter + scale * v.y));
    v.z = 0.0f;
  }
  ...
  else
  {
    ...
    zrecip = 2 / v.z;
    v.x = static_cast<float>(xcenter + maxscale * v.x * zrecip);
    v.y = static_cast<float>(height - (ycenter + maxscale * v.y * zrecip));
    v.z = static_cast<float>(1 - zrecip);
  }
...
void Projector::Project(Point& v, bool clamp) const
{
  double zrecip;
...
void Projector::Unproject(Point& v) const
{
  double zrecip = 1 / v.z;
...
void Projector::ProjectRect(Point& v, double& w, double& h) const
{
  double zrecip;
...
void Projector::SetWorldspaceClipPlane(Vec3& normal, Plane& plane)
{
  // Rotate the plane normal into worldspace
  ViewToWorld(normal, plane.normal);
  plane.distance = static_cast<float>(camera->Pos() * plane.normal + CLIP_PLANE_EPSILON);
}
...
void Projector::SetUpFrustum()
{
  ...
  // Left clip plane
  normal.x = static_cast<float>(s);
  normal.y = static_cast<float>(0);
  normal.z = static_cast<float>(c);
  ...
  // Bottom clip plane
  normal.x = static_cast<float>(0);
  normal.y = static_cast<float>(s);
  normal.z = static_cast<float>(c);
...
int Projector::IsVisible(const Vec3& v, float radius) const
{
  int visible = 1;
  int complete = 1;

  auto plane = frustum_planes;
  if (infinite)
  {
    complete = 0;
...
int Projector::IsBoxVisible(const Point* p) const
{
  int i, j, outside = 0;
...
float Projector::ApparentRadius(const Vec3& v, float radius) const
{
  Vec3 vloc = v;
...
void Projector::ViewToWorld(Point& pin, Point& pout)
{
...
void Projector::ViewToWorld(Vec3& vin, Vec3& vout)
{
  // Rotate into the world orientation
  vout.x = static_cast<float>(vin.x * camera->vrt().x + vin.y * camera->vup().x + vin.z * camera->vpn().x);
  vout.y = static_cast<float>(vin.x * camera->vrt().y + vin.y * camera->vup().y + vin.z * camera->vpn().y);
  vout.z = static_cast<float>(vin.x * camera->vrt().z + vin.y * camera->vup().z + vin.z * camera->vpn().z);
}
