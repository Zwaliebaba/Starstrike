  ...
  while (s > 0.0)
  {
    ...
    if (v > 150)
    {
      ...
      if (cos_alpha >= 1)
        ...
        alpha = static_cast<float>(acos(cos_alpha));
...
void Physical::ArcadeFrame(double s)
{
  ...
  while (s > 0.0)
  {
    ...
    // move the position by the (time-frame scaled) velocity:
    pos += arcade_velocity * seconds + velocity * seconds;
...
void Physical::AngularFrame(double seconds)
{
  if (!straight)
  {
    dr += static_cast<float>(dr_acc * seconds);
    dy += static_cast<float>(dy_acc * seconds);
    dp += static_cast<float>(dp_acc * seconds);

    dr *= static_cast<float>(exp(-dr_drg * seconds));
    dy *= static_cast<float>(exp(-dy_drg * seconds));
    dp *= static_cast<float>(exp(-dp_drg * seconds));

    roll = static_cast<float>(dr * seconds);
    pitch = static_cast<float>(dp * seconds);
    yaw = static_cast<float>(dy * seconds);

    if (shake > 0.01)
    {
      vibration = Point(random(), random(), random());
      vibration.Normalize();
      vibration *= static_cast<float>(shake * seconds);

      shake *= static_cast<float>(exp(-1.5 * seconds));
    }
    else
    {
...
void Physical::CalcFlightPath()
{
  ...
  flight_path_yaw = static_cast<float>(atan(flight_path.x / flight_path.z));
  if (flight_path.z < 0)
    flight_path_yaw -= static_cast<float>(PI);
  if (flight_path_yaw < -PI)
    flight_path_yaw += static_cast<float>(2 * PI);
  ...
  flight_path_pitch = static_cast<float>(atan(flight_path.y / flight_path.z));
}

void Physical::MoveTo(const Point& new_loc) { cam.MoveTo(new_loc); }

void Physical::TranslateBy(const Point& ref)
{
  Point new_loc = cam.Pos() - ref;
  cam.MoveTo(new_loc);
}

void Physical::ApplyForce(const Point& force) { velocity += force / mass; }

void Physical::ApplyTorque(const Point& torque)
{
  dr += static_cast<float>(torque.x / mass);
  dp += static_cast<float>(torque.y / mass);
  dy += static_cast<float>(torque.z / mass);
}

void Physical::SetThrust(double t) { thrust = static_cast<float>(t); }

void Physical::SetTransX(double t) { trans_x = static_cast<float>(t); }

void Physical::SetTransY(double t) { trans_y = static_cast<float>(t); }

void Physical::SetTransZ(double t) { trans_z = static_cast<float>(t); }
...
void Physical::LookAt(const Point& dst) { cam.LookAt(dst); }

void Physical::CloneCam(const Camera& c) { cam.Clone(c); }
...
void Physical::ApplyRoll(double r)
{
  ...
  dr_acc = static_cast<float>(r) * roll_rate;
}
...
void Physical::ApplyPitch(double p)
{
  ...
  dp_acc = static_cast<float>(p) * pitch_rate;
}
...
void Physical::ApplyYaw(double y)
{
  ...
  dy_acc = static_cast<float>(y) * yaw_rate;
}

void Physical::SetAngularRates(double r, double p, double y)
{
  roll_rate = static_cast<float>(r);
  pitch_rate = static_cast<float>(p);
  yaw_rate = static_cast<float>(y);
}
...
{
  dr_drg = static_cast<float>(r);
  dp_drg = static_cast<float>(p);
  dy_drg = static_cast<float>(y);
}
...
void Physical::GetAngularThrust(double& r, double& p, double& y)
{
  ...
  if (dr_acc > 0.05 * roll_rate)
    r = 1;
  else if (dr_acc < -0.05 * roll_rate)
    r = -1;
  else if (dr > 0.01 * roll_rate)
    r = -1;
  else if (dr < -0.01 * roll_rate)
    r = 1;

  if (dy_acc > 0.05 * yaw_rate)
    y = 1;
  else if (dy_acc < -0.05 * yaw_rate)
    y = -1;
  else if (dy > 0.01 * yaw_rate)
    y = -1;
  else if (dy < -0.01 * yaw_rate)
    y = 1;

  if (dp_acc > 0.05 * pitch_rate)
    p = 1;
  else if (dp_acc < -0.05 * pitch_rate)
    p = -1;
  else if (dp > 0.01 * pitch_rate)
    p = -1;
  else if (dp < -0.01 * pitch_rate)
    p = 1;
}

void Physical::SetPrimary(const Point& l, double m)
{
  primary_loc = l;
  primary_mass = m;
}
