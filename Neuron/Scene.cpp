...
bool Scene::IsLightObscured(const Point& obj_pos, const Point& light_pos, double obj_radius, Point* impact_point) const
{
  Point dir = light_pos - obj_pos;
  double len = dir.Normalize();

  auto pThis = (Scene*)this; // cast-away const
  Graphic* g = nullptr;
  bool obscured = false;
  ...
  while (++g_iter && !obscured)
  {
    ...
    if (g->CastsShadow() && !g->Hidden() && !g->IsInfinite())
    {
      ...
      if (gdist > 0.1 &&                     // different than object being obscured
        ...
        (g->Radius() * 400) / gdist > 10)
      {    // projects to a resonable size

        Point delta = (g->Location() - light_pos);

        if (delta.length() > g->Radius() / 100)
        { // not the object that is emitting the light
          Point impact;
      ...
      else if (obj_radius < 0 && gdist < 0.1)
      { // special case for camera (needed for cockpits)
        Point delta = (g->Location() - light_pos);
