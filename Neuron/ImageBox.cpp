...
ImageBox::ImageBox(Screen* s, int ax, int ay, int aw, int ah, DWORD aid)
  : ActiveWindow(s, ax, ay, aw, ah, aid, 0, nullptr),
    blend_mode(Video::BLEND_ALPHA)
{
  picture_loc = 1;
  text_align = DT_CENTER;
...
void ImageBox::Draw()
{
  ...
  if (picture.Width())
  {
    ...
    DrawBitmap(irect.x, irect.y, irect.x + irect.w, irect.y + irect.h, &picture, blend_mode);
  }
...
void ImageBox::DrawTabbedText()
{
  ...
  {
    Rect irect = CalcPictureRect();
    DrawBitmap(irect.x, irect.y, irect.x + irect.w, irect.y + irect.h, &picture, Video::BLEND_ALPHA);
  }
...
int ImageBox::OnMouseMove(int x, int y) { return ActiveWindow::OnMouseMove(x, y); }

int ImageBox::OnLButtonDown(int x, int y) { return ActiveWindow::OnLButtonDown(x, y); }

int ImageBox::OnLButtonUp(int x, int y) { return ActiveWindow::OnLButtonUp(x, y); }

int ImageBox::OnClick() { return ActiveWindow::OnClick(); }

int ImageBox::OnMouseEnter(int mx, int my) { return ActiveWindow::OnMouseEnter(mx, my); }

int ImageBox::OnMouseExit(int mx, int my) { return ActiveWindow::OnMouseExit(mx, my); }

void ImageBox::GetPicture(Bitmap& img) const { img.CopyBitmap(picture); }
...
int ImageBox::GetPictureLocation() const { return picture_loc; }
