...
const char* D3DErrStr(HRESULT dderr);
void VideoDX9Error(const char* msg, HRESULT dderr);
static TexCacheDX9* texcache = nullptr;
static TexCubeDX9* environment_cube = nullptr;
...
using LPDDCE = HRESULT(WINAPI *)(GUID FAR *, LPVOID*, REFIID, IUnknown FAR *);
...
static constexpr int NUM_SCREEN_VERTS = 1024;
static constexpr int NUM_SCREEN_INDICES = NUM_SCREEN_VERTS * 2;
...
DWORD VideoDX9ScreenVertex::FVF = D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1;
...
DWORD VideoDX9SolidVertex::FVF = D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEX1 | D3DFVF_TEXCOORDSIZE2(0);
...
DWORD VideoDX9LuminousVertex::FVF = D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1 | D3DFVF_TEXCOORDSIZE2(0);
...
DWORD VideoDX9DetailVertex::FVF = D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_SPECULAR | D3DFVF_TEX2;
...
DWORD VideoDX9LineVertex::FVF = D3DFVF_XYZ | D3DFVF_DIFFUSE;
...
static VideoDX9* video_dx9_instance = nullptr;
...
VideoDX9::VideoDX9(const HWND& window, VideoSettings* vs)
  : hwnd(window),
    ...
    magic_fx(nullptr),
    magic_fx_code(nullptr),
    magic_fx_code_len(0),
    current_texture(nullptr),
    use_material(nullptr),
    segment_material(nullptr),
    strategy(0),
    passes(0),
    d3dx_font(nullptr),
    nlights(0),
    ...
    screen_ibuf(nullptr),
    font_verts(nullptr),
    font_indices(nullptr),
    font_nverts(0),
    screen_line_verts(nullptr),
    line_verts(nullptr)
{
  video_dx9_instance = this;

  DebugTrace("\n********************************\n");
  DebugTrace("*   Direct 3D version 9        *\n");
  DebugTrace("********************************\n\n");
  ...
  if (d3d && dx9enum)
  {
    if (vs)
    {
      dx9enum->req_fullscreen = !(vs->is_windowed);
      dx9enum->req_windowed = vs->is_windowed;
      dx9enum->min_stencil_bits = vs->shadows ? 8 : 0;
      dx9enum->uses_depth_buffer = true;
    }
    else
    {
      dx9enum->req_fullscreen = !(video_settings.is_windowed);
      dx9enum->req_windowed = video_settings.is_windowed;
  ...
  if (VD3D_describe_things > 2)
  {
    ...
    DebugTrace("   BackBufferWidth:       {}\n", d3dparams.BackBufferWidth);
    DebugTrace("   BackBufferHeight:      {}\n", d3dparams.BackBufferHeight);
    DebugTrace("   BackBufferCount:       {}\n", d3dparams.BackBufferCount);
    DebugTrace("   BackBufferFormat:      {}\n", VideoDX9DisplayMode::D3DFormatToString(d3dparams.BackBufferFormat));
    DebugTrace("   Multisample Type:      {}\n", static_cast<size_t>(d3dparams.MultiSampleType));
    DebugTrace("   Multisample Qual:      {}\n", d3dparams.MultiSampleQuality);
    DebugTrace("   Swap Effect:           {}\n", static_cast<size_t>(d3dparams.SwapEffect));
    DebugTrace("   Device Window:         {:8x}\n", reinterpret_cast<size_t>(d3dparams.hDeviceWindow));
    DebugTrace("   Windowed:              {}\n", d3dparams.Windowed ? "true" : "false");
    DebugTrace("   Enable Depth/Stencil:  {}\n", d3dparams.EnableAutoDepthStencil ? "true" : "false");
    DebugTrace("   Depth/Stencil Format:  {}\n",
               VideoDX9DisplayMode::D3DFormatToString(d3dparams.AutoDepthStencilFormat));
    DebugTrace("   Flags:                 {:8x}\n", d3dparams.Flags);
    DebugTrace("   Fullscreen Refresh:    {} Hz\n", d3dparams.FullScreen_RefreshRateInHz);

    switch (d3dparams.PresentationInterval)
    {
      case D3DPRESENT_INTERVAL_IMMEDIATE:
        DebugTrace("   Present Interval:      IMMEDIATE\n");
        break;

      case D3DPRESENT_INTERVAL_DEFAULT:
        DebugTrace("   Present Interval:      DEFAULT\n");
        break;

      case D3DPRESENT_INTERVAL_ONE:
        DebugTrace("   Present Interval:      ONE\n");
        break;

      case D3DPRESENT_INTERVAL_TWO:
        DebugTrace("   Present Interval:      TWO\n");
        break;

      case D3DPRESENT_INTERVAL_THREE:
        DebugTrace("   Present Interval:      THREE\n");
        break;

      case D3DPRESENT_INTERVAL_FOUR:
        DebugTrace("   Present Interval:      FOUR\n");
        break;

      default:
        DebugTrace("   Present Interval:      Unknown (%d)\n", d3dparams.PresentationInterval);
        break;
    }
  ...
  DebugTrace("   Creating Video Device for HWND = {:08x}\n", (size_t)window);

  err = d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, window, D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3dparams,
                          &d3ddevice);
  ...
  if (VD3D_describe_things > 0)
  {
    ...
    DebugTrace("   Available Texture Memory: {} MB\n\n", vmf);
  }
...
VideoDX9::~VideoDX9()
{
  ...
  DebugTrace("   VideoDX9: shutdown\n");
  video_dx9_instance = nullptr;
}
...
bool VideoDX9::SetupParams()
{
  ...
  {
    DebugTrace("WARNING: VideoDX9 could not select adapter %d (max=%d)\n", adapter_index, dx9enum->NumAdapters());
  ...
  if (dx9enum->uses_depth_buffer)
  {
    ...
    d3dparams.AutoDepthStencilFormat = static_cast<D3DFORMAT>(video_settings.GetDepthStencilFormat());
  }
  else
    d3dparams.Flags = 0;
  ...
  if (video_settings.IsWindowed())
  {
    ...
    d3dparams.BackBufferFormat = static_cast<D3DFORMAT>(video_settings.GetBackBufferFormat());
...
bool VideoDX9::IsModeSupported(int w, int h, int b) const
{
  if (dx9enum)
...
bool VideoDX9::Reset(const VideoSettings* vs)
{
  ...
  bool using_x_font = (d3dx_font != nullptr);
...
bool VideoDX9::CreateBuffers()
{
  if (d3ddevice)
  {
    ...
    if (!screen_vbuf)
    {
      screen_vbuf = NEW VideoDX9VertexBuffer(this, NUM_SCREEN_VERTS, vertex_size, VideoDX9ScreenVertex::FVF,
                                             D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY);
    }

    if (!screen_ibuf)
    {
      screen_ibuf = NEW VideoDX9IndexBuffer(this, NUM_SCREEN_INDICES, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY);
    }
    ...
    hr = d3ddevice->CreateVertexDeclaration(videoDX9NormalVertexElements, &vertex_declaration);
    ...
    {
      hr = D3DXCreateEffect(d3ddevice, magic_fx_code, magic_fx_code_len, nullptr, nullptr, 0, nullptr, &magic_fx,
                            &code_buffer);

      if (code_buffer)
      {
        DebugTrace("ERROR - Failed to compile 'magic.fx'\n");
        auto errorList = code_buffer->GetBufferPointer();
        DebugTrace("{}\n", static_cast<const char*>(errorList));
        DebugTrace("\n\n");
        RELEASE(code_buffer);
      }
    }
...
int VideoDX9::MaxTexSize() const
{
  if (d3d && dx9enum && dx9enum->GetAdapterInfo())
  {
    ...
    if (dev_info)
      return static_cast<int>(dev_info->caps.MaxTextureWidth);
  }
...
int VideoDX9::MaxTexAspect() const
{
  if (d3d && dx9enum && dx9enum->GetAdapterInfo())
  {
    ...
    if (dev_info)
      return static_cast<int>(dev_info->caps.MaxTextureAspectRatio);
  }
...
{
  DebugTrace("VideoDX9::RecoverSurfaces()\n");
  ...
  if (SUCCEEDED(hr))
  {
    DebugTrace("* Invalidating Texture Cache\n");
  ...
  DebugTrace("* Vid Mem Free: {:8d}\n", VidMemFree());
  DebugTrace("* Recover Surfaces Complete.\n\n");
}
...
//
inline WORD RampValue(UINT i, double recip_gamma, double fade)
{
  return static_cast<WORD>(65535.0 * fade * pow((double)i / 255.f, recip_gamma));
}
...
//
inline float ReciprocalGamma(UINT i) { return logf(i / 255.f) / logf(0.5f); }
...
//
inline float GammaValue(UINT i) { return logf(0.5f) / logf(i / 255.f); }
...
bool VideoDX9::SetObjTransform(const Matrix& orient, const Point& loc)
{
  HRESULT hr = E_FAIL;
...
bool VideoDX9::ClearAll()
{
  ...
  err = d3ddevice->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_STENCIL | D3DCLEAR_ZBUFFER, background.Value(), 1.0f,
                         0);
...
bool VideoDX9::ClearDepthBuffer()
{
  ...
  err = d3ddevice->Clear(0, nullptr, D3DCLEAR_STENCIL | D3DCLEAR_ZBUFFER, 0, 1.0f, 0);
...
bool VideoDX9::Present()
{
  ...
  HRESULT err = d3ddevice->Present(nullptr, nullptr, nullptr, nullptr);
...
bool VideoDX9::Pause() { return true; }

bool VideoDX9::Resume() { return true; }
...
void VideoDX9::PreloadTexture(Bitmap* tex)
{
  if (texcache && tex)
...
void VideoDX9::CreateD3DMatrix(D3DMATRIX& result, const Matrix& m, const Point& p)
{
  result._11 = static_cast<float>(m.elem[0][0]);
  result._12 = static_cast<float>(m.elem[1][0]);
  result._13 = static_cast<float>(m.elem[2][0]);
  result._14 = 0.0f;

  result._21 = static_cast<float>(m.elem[0][1]);
  result._22 = static_cast<float>(m.elem[1][1]);
  result._23 = static_cast<float>(m.elem[2][1]);
  result._24 = 0.0f;

  result._31 = static_cast<float>(m.elem[0][2]);
  result._32 = static_cast<float>(m.elem[1][2]);
  result._33 = static_cast<float>(m.elem[2][2]);
  result._34 = 0.0f;

  result._41 = static_cast<float>(p.x);
  result._42 = static_cast<float>(p.y);
  result._43 = static_cast<float>(p.z);
  result._44 = 1.0f;
}

void VideoDX9::CreateD3DMatrix(D3DMATRIX& result, const Matrix& m, const Vec3& v)
{
  result._11 = static_cast<float>(m.elem[0][0]);
  result._12 = static_cast<float>(m.elem[1][0]);
  result._13 = static_cast<float>(m.elem[2][0]);
...
void VideoDX9::CreateD3DMaterial(D3DMATERIAL9& result, const Material& mtl)
{
...
bool VideoDX9::Capture(Bitmap& bmp)
{
  ...
  {
    HRESULT hr = E_FAIL;
    LPDIRECT3DSURFACE9 pSurf = nullptr, pTempSurf = nullptr;
    ...
    hr = d3ddevice->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, D3DPOOL_SYSTEMMEM, &pTempSurf,
                                                nullptr);
    ...
    if (video_settings.IsWindowed())
    {
      ...
      hr = d3ddevice->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, D3DPOOL_DEFAULT, &pSurf,
                                                  nullptr);
    ...
    hr = pTempSurf->LockRect(&lockedRect, nullptr, D3DLOCK_READONLY | D3DLOCK_NOSYSLOCK);
    ...
    auto buffer = new DWORD[desc.Width * desc.Height];
    auto src = static_cast<BYTE*>(lockedRect.pBits);
    auto dst = (BYTE*)buffer;
    Color clr;
      ...
      for (DWORD x = 0; x < desc.Width; x++)
      {
        switch (desc.Format)
        {
          case D3DFMT_R5G6B5:
            ...
            *dst++ = static_cast<BYTE>(clr.Red());
            *dst++ = static_cast<BYTE>(clr.Green());
            *dst++ = static_cast<BYTE>(clr.Blue());
            *dst++ = 255;
            break;
...
bool VideoDX9::SetWindowRect(const Rect& r) { return SetViewport(r.x, r.y, r.w, r.h); }

bool VideoDX9::SetViewport(int x, int y, int w, int h)
{
  if (!d3d || !d3ddevice)
    return false;
  ...
  {
    if (d3dstate_table[i])
      d3ddevice->SetRenderState(static_cast<D3DRENDERSTATETYPE>(d3dstate_table[i]), render_state[i]);
  }
  ...
  if (d3ddevice)
  {
    ...
    while (++iter)
    {
      ...
      if (light->IsActive())
      {
        ...
        d3d_light.Type = static_cast<D3DLIGHTTYPE>(light->Type());

        if (light->Type() == Light::LIGHT_DIRECTIONAL)
        {
          Point light_location = light->Location();
          d3d_light.Direction.x = static_cast<float>(-light_location.x);
          d3d_light.Direction.y = static_cast<float>(-light_location.y);
          d3d_light.Direction.z = static_cast<float>(-light_location.z);

          if (d3d_light.Direction.x == 0 && d3d_light.Direction.y == 0 && d3d_light.Direction.z == 0)
        ...
        {
          d3d_light.Position.x = static_cast<float>(light->Location().x);
          d3d_light.Position.y = static_cast<float>(light->Location().y);
          d3d_light.Position.z = static_cast<float>(light->Location().z);
        }
...
bool VideoDX9::SetProjection(float fov, float znear, float zfar, DWORD type)
{
  if (d3ddevice && zfar > znear)
  {
    ...
    double width = static_cast<float>(rect.w);
    double height = static_cast<float>(rect.h);
...
***/
    ...
    if (type == PROJECTION_PERSPECTIVE)
    {
      ...
      w = static_cast<float>(2 / tan(xangle));   // 1/tan(x) == cot(x)
      h = static_cast<float>(w * width / height);
      Q = zfar / (zfar - znear);
...
***/
    ...
    {
      m._11 = static_cast<float>(fov / width);
      m._22 = static_cast<float>(fov / height);
      m._33 = 1 / (zfar - znear);
      m._43 = znear / (znear - zfar);
      m._44 = static_cast<float>(1);
    }
...
bool VideoDX9::SetRenderState(RENDER_STATE state, DWORD value)
{
  if (!d3ddevice)
    return false;
  ...
  else if (state == Z_BIAS)
  {
    if (value)
    {
      ...
      FLOAT depth_bias = static_cast<FLOAT>((DW2I(value) / -10000.0));
  ...
  // all other render states:
    ...
    hr = d3ddevice->SetRenderState(static_cast<D3DRENDERSTATETYPE>(d3dstate_table[state]), value);
...
{
  float a0 = static_cast<float>((a) & 0xff) / 255.0f;
  float a1 = static_cast<float>((a >> 8) & 0xff) / 255.0f;
  float a2 = static_cast<float>((a >> 16) & 0xff) / 255.0f;
  float a3 = static_cast<float>((a >> 24) & 0xff) / 255.0f;

  float b0 = static_cast<float>((b) & 0xff) / 255.0f;
  float b1 = static_cast<float>((b >> 8) & 0xff) / 255.0f;
  float b2 = static_cast<float>((b >> 16) & 0xff) / 255.0f;
  float b3 = static_cast<float>((b >> 24) & 0xff) / 255.0f;

  return static_cast<DWORD>((BYTE)(a3 * b3 * 255.0f) << 24) | (static_cast<BYTE>(a2 * b2 * 255.0f) << 16) | (static_cast
    <BYTE>(a1 * b1 * 255.0f) << 8) | static_cast<BYTE>(a0 * b0 * 255.0f);
}

bool VideoDX9::PopulateScreenVerts(VertexSet* vset)
{
  if (!vset || !screen_vbuf)
    return false;
  ...
  auto v = (VideoDX9ScreenVertex*)screen_vbuf->Lock(num_verts);
  ...
  DebugTrace("   VideoDX9: could not lock screen vbuf for {} verts.\n", num_verts);
  return false;
}

bool VideoDX9::DrawPolys(int npolys, Poly* polys)
{
  bool result = false;
  ...
  if (d3ddevice && polys && npolys > 0)
  {
    ...
    void* verts = nullptr;
    int vsize = 0;
    auto indices = NEW WORD[npolys * 6];
    ...
    if (vset->tu1 != nullptr)
    {
      auto v = NEW VideoDX9DetailVertex[nverts];
      verts = v;
    ...
    {
      auto v = NEW VideoDX9LuminousVertex[nverts];
      verts = v;
    ...
    {
      auto v = NEW VideoDX9SolidVertex[nverts];
      verts = v;
    ...
    if (verts && indices)
    {
      ...
      hr = d3ddevice->SetVertexShader(nullptr);
      ...
      hr = d3ddevice->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, nverts, num_tris, indices, D3DFMT_INDEX16, verts,
                                             vsize);
...
bool VideoDX9::DrawScreenPolys(int npolys, Poly* polys, int blend)
{
  bool result = false;
  HRESULT hr = E_FAIL;
  ...
  if (d3ddevice && polys && npolys > 0 && screen_vbuf && screen_ibuf)
  {
    ...
    if (!screen_indices)
    {
      DebugTrace("   VideoDX9: could not lock screen ibuf for {} indices.\n", num_indices);
      return false;
    }
    ...
    hr = d3ddevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, first_vert, num_verts, first_index, num_tris);
...
bool VideoDX9::DrawSolid(Solid* s, DWORD blend_modes)
{
  bool result = false;
  HRESULT hr = E_FAIL;
  ...
  if (d3ddevice && s && s->GetModel())
  {
    ...
    while (++surf_iter)
    {
      ...
      if (PrepareSurface(surf))
      {
        ...
        auto surf_data = static_cast<VideoDX9SurfaceData*>(surf->GetVideoPrivateData());
...
bool VideoDX9::DrawSegment(Segment* segment)
{
  bool result = false;
  bool detail = false;
  bool luminous = false;
  ...
  if (segment && segment->video_data)
  {
    ...
    auto seg_data = static_cast<VideoDX9SegmentData*>(segment->video_data);
    ...
    else
    {
      ...
      else if (magic_fx && strategy < DX9_STRATEGY_BLEND && !detail)
      {
        ...
        D3DXVECTOR4 eyePos(static_cast<float>(camera->Pos().x), static_cast<float>(camera->Pos().y),
                           static_cast<float>(camera->Pos().z), 1.0f);
        ...
        {
          lightPos.x = static_cast<float>(main_light->Location().x);
          lightPos.y = static_cast<float>(main_light->Location().y);
          lightPos.z = static_cast<float>(main_light->Location().z);
        ...
        {
          lightPos.x = static_cast<float>(back_light->Location().x);
          lightPos.y = static_cast<float>(back_light->Location().y);
          lightPos.z = static_cast<float>(back_light->Location().z);
        ...
        FLOAT base_bias = static_cast<FLOAT>((DW2I(render_state[Z_BIAS]) / -10000.0));
        ...
        if (mtl->tex_diffuse)
        {
          ...
          magic_fx->SetTexture("tex_d", nullptr);
        ...
        if (mtl->tex_emissive)
        {
          ...
          magic_fx->SetTexture("tex_e", nullptr);
        ...
        if (mtl->tex_specular && IsSpecMapEnabled())
        {
          ...
          magic_fx->SetTexture("tex_s", nullptr);
        ...
        else if (mtl->tex_bumpmap && !do_bump)
        {
          ...
          magic_fx->SetTexture("tex_x", nullptr);

        const char* mtl_shader = mtl->GetShader(render_state[LIGHTING_PASS]);
        D3DXHANDLE hnd_shader = nullptr;
        ...
        if (environment_cube != nullptr && magic_fx->IsParameterUsed("env_cube", hnd_shader))
        {
...
bool VideoDX9::DrawPolyOutline(Poly* p)
{
  if (d3ddevice && p && p->nverts >= 3)
  {
    ...
    hr = d3ddevice->SetTexture(0, nullptr);
    hr = d3ddevice->DrawPrimitiveUP(D3DPT_LINESTRIP, nlines, verts, sizeof(VideoDX9LineVertex));
    return true;
  }
...
bool VideoDX9::DrawShadow(Solid* s, int nverts, Vec3* shadow_verts, bool visible)
{
  bool result = false;
  HRESULT hr = E_FAIL;
  ...
  if (d3ddevice && s && nverts && shadow_verts && IsShadowEnabled())
  {
    ...
    if (visible)
    {
      ...
      for (int i = 0; i < nverts; i += 3)
      {
        ...
        hr = d3ddevice->DrawPrimitiveUP(D3DPT_LINESTRIP, 3, verts, sizeof(VideoDX9LineVertex));
      }
...
bool VideoDX9::DrawLines(int nlines, Vec3* points, Color c, int blend)
{
  bool result = false;
  ...
  if (d3ddevice && points && nlines > 0 && nlines <= 256)
  {
    ...
    if (verts)
    {
      ...
      hr = d3ddevice->DrawPrimitiveUP(D3DPT_LINELIST, nlines, verts, sizeof(VideoDX9LineVertex));
...
bool VideoDX9::DrawScreenLines(int nlines, float* points, Color c, int blend)
{
  bool result = false;
  ...
  if (d3ddevice && points && nlines > 0 && nlines <= 64)
  {
    ...
    if (verts)
    {
      ...
      else
      {
        ...
        hr = d3ddevice->DrawPrimitiveUP(D3DPT_LINELIST, nlines, verts, sizeof(VideoDX9ScreenVertex));
...
bool VideoDX9::DrawPoints(VertexSet* vset)
{
  if (vset && vset->nverts)
  {
    HRESULT hr = E_FAIL;
    ...
    int nverts = vset->nverts;
    auto verts = NEW VideoDX9LineVertex[nverts];
    ...
    if (verts)
    {
      ...
      hr = d3ddevice->SetFVF(VideoDX9LineVertex::FVF);
      hr = d3ddevice->SetTexture(0, nullptr);
      hr = d3ddevice->DrawPrimitiveUP(D3DPT_POINTLIST, nverts, verts, sizeof(VideoDX9LineVertex));
...
bool VideoDX9::UseXFont(const char* name, int size, bool bold, bool ital)
{
  if (d3ddevice && name && *name && size > 4)
  {
    ...
    hr = D3DXCreateFontA(d3ddevice,                  // D3D device
                        nHeight,                     // Height
                        0,                           // Width
...
bool VideoDX9::DrawText(const char* text, int count, const Rect& rect, DWORD format, Color c)
{
  if (d3ddevice && text && *text && d3dx_font)
  {
    ...
    d3dx_font->DrawTextA(0, text, count, &r, format, c.Value());
  }

  return false;
}

bool VideoDX9::PrepareSurface(Surface* surf)
{
  if (surf)
  {
    int nverts = surf->NumVerts();
    int nindices = surf->NumIndices();
    bool detail = surf->GetVertexSet()->tu1 != nullptr;
    bool luminous = false;
    DWORD dynamic = 0;
    ...
    if (!surf_data)
    {
      ...
      if (surf->GetVertexSet()->tangent && surf->GetVertexSet()->binormal)
      {
        ...
        surf_data->vertex_buffer = NEW VideoDX9VertexBuffer(this, nverts, sizeof(VideoDX9NormalVertex),
      ...
      {
        surf_data->vertex_buffer = NEW VideoDX9VertexBuffer(this, nverts, sizeof(VideoDX9DetailVertex),
                                                            VideoDX9DetailVertex::FVF, dynamic | D3DUSAGE_WRITEONLY);
      }

      else if (luminous)
      {
        surf_data->vertex_buffer = NEW VideoDX9VertexBuffer(this, nverts, sizeof(VideoDX9LuminousVertex),
                                                            VideoDX9LuminousVertex::FVF, dynamic | D3DUSAGE_WRITEONLY);
      }

      else
      {
        surf_data->vertex_buffer = NEW VideoDX9VertexBuffer(this, nverts, sizeof(VideoDX9SolidVertex),
                                                            VideoDX9SolidVertex::FVF, dynamic | D3DUSAGE_WRITEONLY);
      }

      surf_data->index_buffer = NEW VideoDX9IndexBuffer(this, nindices, dynamic | D3DUSAGE_WRITEONLY);

      if (!surf_data->vertex_buffer || !surf_data->index_buffer)
      {
        DebugTrace("VideoDX9: Unable to prepare surface '%s'\n", surf->Name());
        delete surf_data;
        return false;
      }
    ...
    if (surf_data && !surf_data->IsValid())
    {
      if (detail)
      {
        auto v = (VideoDX9DetailVertex*)surf_data->vertex_buffer->Lock(nverts);
      ...
      {
        auto v = (VideoDX9LuminousVertex*)surf_data->vertex_buffer->Lock(nverts);
      ...
      {
        auto v = (VideoDX9NormalVertex*)surf_data->vertex_buffer->Lock(nverts);
      ...
      {
        auto v = (VideoDX9SolidVertex*)surf_data->vertex_buffer->Lock(nverts);
    ...
    while (++seg_iter)
    {
      ...
      {
        auto seg_data = NEW VideoDX9SegmentData;
...
int VideoDX9::PrepareMaterial(Material* m)
{
  segment_material = m;
  strategy = 0;
  passes = 0;
  ...
  if (m)
  {
    ...
    if (dev_info)
    {
      max_stages = static_cast<int>(dev_info->caps.MaxTextureBlendStages);
      max_textures = static_cast<int>(dev_info->caps.MaxSimultaneousTextures);
...
bool VideoDX9::SetupPass(int pass)
{
  ...
  {
    D3DMATERIAL9 d3dmtl;
    IDirect3DTexture9* texture_0 = nullptr;
    IDirect3DTexture9* texture_1 = nullptr;
    IDirect3DTexture9* texture_2 = nullptr;
    Bitmap* tex_bmp_0 = nullptr;
    Bitmap* tex_bmp_1 = nullptr;
    Bitmap* tex_bmp_2 = nullptr;
    ...
    else if (strategy == DX9_STRATEGY_EMISSIVE && passes == 1 || strategy == DX9_STRATEGY_SPEC_EMISSIVE && passes == 2)
    {
  ...
  else if (pass == 1)
  {
    ...
    if (strategy == DX9_STRATEGY_SPECMAP || strategy == DX9_STRATEGY_SPEC_EMISSIVE)
    {
...
void VideoDX9Error(const char* msg, HRESULT err) { DebugTrace("   VideoDX9: {}. [{}]\n", msg, D3DErrStr(err)); }

const char* D3DErrStr(HRESULT hr)
{
