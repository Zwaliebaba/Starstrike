...
void Camera::MoveTo(const Point& p)
{
  pos.x = p.x;
  pos.y = p.y;
  pos.z = p.z;
...
void Camera::MoveBy(const Point& p)
{
  pos.x += p.x;
  pos.y += p.y;
  pos.z += p.z;
...
void Camera::Clone(const Camera& cam)
{
  pos = cam.pos;
  orientation = cam.orientation;
}

void Camera::LookAt(const Point& target, const Point& eye, const Point& up)
{
  Point zaxis = target - eye;
  zaxis.Normalize();
  Point xaxis = up.cross(zaxis);
  xaxis.Normalize();
...
bool Camera::Padlock(const Point& target, double alimit, double e_lo, double e_hi)
{
  // No navel gazing:
  if (target == Pos())
    return false;
