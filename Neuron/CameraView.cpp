...
void CameraView::UseScene(Scene* s)
{
  if (s)
    scene = s;
  else
...
void CameraView::SetFieldOfView(double fov) { projector.SetFieldOfView(fov); }

double CameraView::GetFieldOfView() const { return projector.GetFieldOfView(); }
...
DWORD CameraView::GetProjectionType() const { return projection_type; }
...
void CameraView::Refresh()
{
  ...
  video->SetProjection(static_cast<float>(GetFieldOfView()), 1.0f, 1.0e6f, projection_type);
...
void CameraView::MarkVisibleObjects()
{
  ...
  while (++graphic_iter)
  {
    ...
    if (graphic->CheckVisibility(projector))
    {
      ...
      graphic->ProjectScreenRect(nullptr);
  }
}

void CameraView::MarkVisibleLights(Graphic* graphic, DWORD flags)
{
  ...
  if (graphic->IsVisible())
  {
    ...
    while (++light_iter)
    {
      ...
      bool bright_enough = light->Type() == Light::LIGHT_DIRECTIONAL || light->Intensity() >= 1e9;
...
void CameraView::RenderSceneObjects(bool distant)
{
  ...
  if (distant)
    video->SetProjection(static_cast<float>(GetFieldOfView()), 5.0e6f, 1.0e12f, projection_type);
  else
    video->SetProjection(static_cast<float>(GetFieldOfView()), 1.0f, 1.0e6f, projection_type);
...
void CameraView::Render(Graphic* g, DWORD flags)
{
  if (g && g->IsVisible() && !g->Hidden())
  {
    if (g->IsSolid())
...
void CameraView::RenderLensFlare()
{
  ...
  Vec3 sun_pos;
  Vec3 center(static_cast<float>(width) / 2.0f, static_cast<float>(height) / 2.0f, 1.0f);
  int flare_visible = 0;
  ...
  while (++light_iter)
  {
    ...
    if (distance > 1e9)
    {
      if (projector.IsVisible(light->Location(), 1.0f))
      {
        ...
        int x = static_cast<int>(sun_pos.x);
        int y = static_cast<int>(sun_pos.y);
        int w = static_cast<int>(window->Width() / 4.0);
        int h = w;
        ...
        if (elem_bitmap[0])
        {
          ...
          float vlen = static_cast<float>(vector.length());
          vector.Normalize();
          ...
          for (int elem = 0; elem < nelem; elem++)
          {
            ...
            Vec3 flare_pos = sun_pos + (vector * elem_dist[elem] * vlen);
            x = static_cast<int>(flare_pos.x);
            y = static_cast<int>(flare_pos.y);
            w = static_cast<int>(window->Width() / 8.0 * elem_size[elem]);
            h = w;
...
void CameraView::WorldPlaneToView(Plane& plane)
{
  ...
  if (!infinite)
    plane.distance -= static_cast<float>(camera->Pos() * tnormal);
