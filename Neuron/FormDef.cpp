...
ColumnDef::ColumnDef(std::string_view t, int w, int a, int s)
  : title(t),
    width(w),
    align(a),
    sort(s),
...
WinDef::WinDef(DWORD a_id, DWORD a_type, std::string_view a_text, DWORD a_style)
  : id(a_id),
    pid(0),
    type(a_type),
    text(a_text),
...
void WinDef::SetFont(std::string_view t) { font = t; }
void WinDef::SetText(std::string_view t) { text = t; }
void WinDef::SetAltText(std::string_view t) { alt_text = t; }
void WinDef::SetTexture(std::string_view t) { texture = t; }
...
CtrlDef::CtrlDef(DWORD a_id, DWORD a_type, std::string_view a_text, DWORD a_style)
...
int CtrlDef::GetOrientation() const { return orientation; }

void CtrlDef::SetOrientation(int o) { orientation = o; }

bool CtrlDef::GetActive() const { return active; }

void CtrlDef::SetActive(bool c) { active = c; }

Color CtrlDef::GetActiveColor() const { return active_color; }

void CtrlDef::SetActiveColor(Color c) { active_color = c; }

bool CtrlDef::GetAnimated() const { return ctrl_flags & CTRL_DEF_ANIMATED; }
...
short CtrlDef::GetBevelWidth() const { return bevel_width; }

void CtrlDef::SetBevelWidth(short nNewValue) { bevel_width = nNewValue; }

bool CtrlDef::GetBorder() const { return (ctrl_flags & CTRL_DEF_BORDER) ? true : false; }
...
Color CtrlDef::GetBorderColor() const { return border_color; }

void CtrlDef::SetBorderColor(Color c) { border_color = c; }

bool CtrlDef::GetDropShadow() const { return (ctrl_flags & CTRL_DEF_DROP_SHADOW) ? true : false; }
...
bool CtrlDef::GetIndent() const { return (ctrl_flags & CTRL_DEF_INDENT) ? true : false; }
...
bool CtrlDef::GetInvertLabel() const { return (ctrl_flags & CTRL_DEF_INVERT_LABEL) ? true : false; }
...
std::string CtrlDef::GetPicture() const { return picture; }

void CtrlDef::SetPicture(const std::string& img_name) { picture = img_name; }

short CtrlDef::GetPictureLocation() const { return picture_loc; }

void CtrlDef::SetPictureLocation(short nNewValue) { picture_loc = nNewValue; }

short CtrlDef::GetPictureType() const { return picture_type; }

void CtrlDef::SetPictureType(short nNewValue) { picture_type = nNewValue; }

bool CtrlDef::GetSticky() const { return (ctrl_flags & CTRL_DEF_STICKY) ? true : false; }
...
int CtrlDef::GetNumLeds() const { return num_leds; }
...
int CtrlDef::NumItems() const { return items.size(); }

std::string CtrlDef::GetItem(int i) const
{
  std::string result;
...
void CtrlDef::AddItem(std::string_view t) { items.emplace_back(t); }

int CtrlDef::NumColumns() const { return columns.size(); }

ColumnDef* CtrlDef::GetColumn(int i) const
{
  ColumnDef* result = nullptr;
...
void CtrlDef::AddColumn(std::string_view t, int w, int a, int s) { columns.append(NEW ColumnDef(t, w, a, s)); }

int CtrlDef::NumTabs() const { return ntabs; }
...
bool CtrlDef::GetShowHeadings() const { return show_headings; }

void CtrlDef::SetShowHeadings(bool bNewValue) { show_headings = bNewValue; }

int CtrlDef::GetLeading() const { return leading; }

void CtrlDef::SetLeading(int nNewValue) { leading = nNewValue; }

int CtrlDef::GetLineHeight() const { return line_height; }

void CtrlDef::SetLineHeight(int nNewValue) { line_height = nNewValue; }

int CtrlDef::GetMultiSelect() const { return multiselect; }

void CtrlDef::SetMultiSelect(int nNewValue) { multiselect = nNewValue; }

int CtrlDef::GetDragDrop() const { return dragdrop; }

void CtrlDef::SetDragDrop(int nNewValue) { dragdrop = nNewValue; }

int CtrlDef::GetScrollBarVisible() const { return scroll_bar; }

void CtrlDef::SetScrollBarVisible(int nNewValue) { scroll_bar = nNewValue; }

bool CtrlDef::GetSmoothScroll() const { return smooth_scroll; }

void CtrlDef::SetSmoothScroll(bool bNewValue) { smooth_scroll = bNewValue; }

short CtrlDef::GetItemStyle() const { return item_style; }

void CtrlDef::SetItemStyle(short nNewValue) { item_style = nNewValue; }

short CtrlDef::GetSelectedStyle() const { return selected_style; }

void CtrlDef::SetSelectedStyle(short nNewValue) { selected_style = nNewValue; }

char CtrlDef::GetPasswordChar() const { return pass_char; }

void CtrlDef::SetPasswordChar(char nNewValue) { pass_char = nNewValue; }

std::string CtrlDef::GetStandardImage() const { return standard_image; }

void CtrlDef::SetStandardImage(const std::string& img_name) { standard_image = img_name; }

std::string CtrlDef::GetActivatedImage() const { return activated_image; }

void CtrlDef::SetActivatedImage(const std::string& img_name) { activated_image = img_name; }

std::string CtrlDef::GetTransitionImage() const { return transition_image; }

void CtrlDef::SetTransitionImage(const std::string& img_name) { transition_image = img_name; }

FormDef::FormDef(const char* a_text, DWORD a_style)
  : WinDef(0, WIN_DEF_FORM, a_text, a_style) {}

FormDef::~FormDef() { controls.destroy(); }
...
void FormDef::Load(const char* fname)
{
  sprintf_s(filename, "%s.frm", fname);

  DebugTrace("Loading Form '{}'\n", fname);
  ...
  {
    DebugTrace("ERROR: invalid form file '{}'\n", filename);
    return;
  }
      ...
      if (def)
      {
        if (def->name()->value() == "form")
        {
          if (!def->term() || !def->term()->isStruct())
            DebugTrace("WARNING: form structure missing in '%s'\n", filename);
          else
          {
            auto form = this;
            ...
            for (int i = 0; i < val->elements()->size(); i++)
            {
              ...
              if (pdef)
              {
                if (pdef->name()->value() == "text" || pdef->name()->value() == "caption")
                {
                ...
                else if (pdef->name()->value() == "align" || pdef->name()->value() == "text_align")
                {
                  DWORD a = DT_LEFT;
                ...
                else if (pdef->name()->value() == "layout")
                {
                  if (!pdef->term() || !pdef->term()->isStruct())
                    DebugTrace("WARNING: layout structure missing in '%s'\n", filename);
                  else
                  {
                ...
                {
                  if (!pdef->term() || !pdef->term()->isStruct())
                    DebugTrace("WARNING: defctrl structure missing in '%s'\n", filename);
                  else
                  {
                    TermStruct* val = pdef->term()->isStruct();
                ...
                {
                  if (!pdef->term() || !pdef->term()->isStruct())
                    DebugTrace("WARNING: ctrl structure missing in '%s'\n", filename);
                  else
                  {
                    auto ctrl = NEW CtrlDef;
                    TermStruct* val = pdef->term()->isStruct();
        ...
        else { DebugTrace("WARNING: unknown object '%s' in '%s'\n", def->name()->value().data(), filename); }
      }
      else
      {
...
void FormDef::ParseCtrlDef(CtrlDef* ctrl, TermStruct* val)
{
  ...
  for (int i = 0; i < val->elements()->size(); i++)
  {
    ...
    if (pdef)
    {
      ...
      {
        if (!pdef->term() || !pdef->term()->isStruct())
          DebugTrace("WARNING: column structure missing in '%s'\n", filename);
        else
        {
          TermStruct* val = pdef->term()->isStruct();
      ...
      else if (pdef->name()->value() == "picture_loc")
      {
        int n;
        GetDefNumber(n, pdef, filename);
        ctrl->SetPictureLocation(static_cast<short>(n));
      }
      ...
      else if (pdef->name()->value() == "picture_type")
      {
        int n;
        GetDefNumber(n, pdef, filename);
        ctrl->SetPictureType(static_cast<short>(n));
      }
      ...
      else if (pdef->name()->value() == "bevel_width")
      {
        DWORD s;
        GetDefNumber(s, pdef, filename);
        ctrl->SetBevelWidth(static_cast<short>(s));
      }
      ...
      else if (pdef->name()->value() == "item_style")
      {
        int n;
        GetDefNumber(n, pdef, filename);
        ctrl->SetItemStyle(static_cast<short>(n));
      }
      ...
      else if (pdef->name()->value() == "selected_style")
      {
        int n;
        GetDefNumber(n, pdef, filename);
        ctrl->SetSelectedStyle(static_cast<short>(n));
      }
      ...
      else if (pdef->name()->value() == "password")
      {
        ...
        ctrl->SetPasswordChar(password[0]);
      }
...
void FormDef::ParseColumnDef(CtrlDef* ctrl, TermStruct* val)
{
  ...
  for (int i = 0; i < val->elements()->size(); i++)
  {
    ...
    if (pdef)
    {
      if (pdef->name()->value() == "text" || pdef->name()->value() == "title")
      {
        GetDefText(buf, pdef, filename);
...
void FormDef::ParseLayoutDef(LayoutDef* def, TermStruct* val)
{
  ...
  for (int i = 0; i < val->elements()->size(); i++)
  {
    ...
    if (pdef)
    {
      if (pdef->name()->value() == "x_mins" || pdef->name()->value() == "cols")
        GetDefArray(def->x_mins, pdef, filename);

      else if (pdef->name()->value() == "y_mins" || pdef->name()->value() == "rows")
        GetDefArray(def->y_mins, pdef, filename);

      else if (pdef->name()->value() == "x_weights" || pdef->name()->value() == "col_wts")
        GetDefArray(def->x_weights, pdef, filename);

      else if (pdef->name()->value() == "y_weights" || pdef->name()->value() == "row_wts")
