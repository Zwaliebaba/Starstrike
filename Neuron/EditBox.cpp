...
void EditBox::DrawContent(const Rect& ctrl_rect)
{
  int h = rect.h;
...
void EditBox::DrawTabbedText()
{
  if (font && !text.empty())
  {
    ...
    std::string s = text;
    if (pass_char)
      s = std::string(text.length(), pass_char);
...
Color EditBox::GetSelectedColor() { return selected_color; }
...
std::string EditBox::GetSelText()
{
  if (sel_start == 0 && sel_length == text.length())
    return text;

  std::string selection;
  auto buf = NEW char[sel_length + 1];

  if (buf)
  {
    for (int i = 0; i < sel_length; i++)
      buf[i] = text[(sel_start + i)];
...
void EditBox::EnsureCaretVisible()
{
  ...
  else if (sel_start > h_offset)
  {
    ...
    do
    {
      ...
      if (pass_char)
      {
        auto pass = std::string(pass_char, sel_start - h_offset);
        x_caret += font->StringWidth(pass);
      }
      else
      {
        std::string sub = text.substr(h_offset, sel_start - h_offset);
        x_caret += font->StringWidth(sub);
      }
...
bool EditBox::CanScroll(int dir, int nlines) { return false; }

void EditBox::Scroll(int direction, int nlines) { scrolling = SCROLL_NONE; }

void EditBox::ScrollTo(int index) {}

int EditBox::GetPageCount() { return 1; }

int EditBox::GetPageSize() { return page_size; }
...
void EditBox::KillFocus() { ActiveWindow::KillFocus(); }

int EditBox::CaretFromPoint(int x, int y) const { return 0; }

int EditBox::OnMouseMove(int x, int y) { return ActiveWindow::OnMouseMove(x, y); }

int EditBox::OnLButtonDown(int x, int y) { return ActiveWindow::OnLButtonDown(x, y); }

int EditBox::OnLButtonUp(int x, int y) { return ActiveWindow::OnLButtonUp(x, y); }
...
void EditBox::Insert(char c)
{
  ...
  if (sel_start < text.length())
  {
    ...
    {
      std::string t1 = text.substr(0, sel_start);
      std::string t2 = text.substr(sel_start, text.length() - sel_start);
      text = t1 + c + t2;
      sel_start++;
    }
  }
...
void EditBox::Delete()
{
  if (sel_start < text.length())
  {
    ...
    {
      std::string t1 = text.substr(0, sel_start);
      std::string t2 = text.substr(sel_start + 1, text.length() - sel_start - 1);
      text = t1 + t2;
    }
  }
...
void EditBox::Backspace()
{
  ...
  {
    if (sel_start == text.length())
      text = text.substr(0, sel_start - 1);
    else
    {
      std::string t1 = text.substr(0, sel_start - 1);
      std::string t2 = text.substr(sel_start, text.length() - sel_start);
      text = t1 + t2;
    }
...
int EditBox::OnKeyDown(int vk, int flags)
{
  ...
  {
    if (flags & 1)
      Insert(static_cast<char>(vk));
    else
      Insert(static_cast<char>(tolower(vk)));
  }
  else
  {
    switch (vk)
