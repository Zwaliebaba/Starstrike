...
struct AWEvent
{
  ...
  int operator ==(const AWEvent& e) const { return (window == e.window) && (eid == e.eid) && (x == e.x) && (y == e.y); }
...
using PFVAWE = void(*)(ActiveWindow*, AWEvent*);
...
struct AWMap
{
  ...
  int operator ==(const AWMap& m) const { return (eid == m.eid) && (client == m.client); }
...
class ActiveWindow : public Window, public EventTarget
{
  public:
    
    ActiveWindow(Screen* s, int ax, int ay, int aw, int ah, DWORD id = 0, DWORD style = 0,
    ...
    // Operations:
    void Paint() override; // blt to screen
    virtual void Draw();   // refresh backing store
    void Show() override;
    void Hide() override;
    void MoveTo(const Rect& r) override;
    virtual void UseLayout(const std::vector<DWORD>& min_x, const std::vector<DWORD>& min_y,
                           const std::vector<float>& weight_x, const std::vector<float>& weight_y);
    virtual void UseLayout(const std::vector<float>& min_x, const std::vector<float>& min_y,
                           const std::vector<float>& weight_x, const std::vector<float>& weight_y);
    ...
    virtual ActiveWindow* FindControl([[maybe_unused]] int x, [[maybe_unused]] int y) { return nullptr; }
    ...
    bool HasFocus() const override { return focus; }
    ...
    void SetAltText(std::string_view t) { alt_text = t; }
    ...
    void DrawText(std::string_view txt, int count, Rect& txt_rect, DWORD flags);
