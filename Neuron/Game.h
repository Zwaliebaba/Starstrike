...
public:
  ...
  virtual void ScreenCapture(const char* name = nullptr);
  virtual void AVICapture(const char* fname = nullptr);
  ...
  static void Panic(const char* msg = nullptr);
  ...
  static std::string GetText(const std::string_view key, Types&&... _args)
  {
    if (sm_content && sm_content->IsLoaded())
      return vformat(sm_content->GetText(key), std::make_format_args(_args...));
