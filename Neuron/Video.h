...
struct RenderStats
{
  ...
  void Clear() { nverts = npolys = nlines = ncalls = total_verts = total_polys = total_lines = 0; }
};
...
class Video
{
  public:
    ...
    virtual const VideoSettings* GetVideoSettings() const { return nullptr; }
    ...
    virtual bool IsModeSupported(int width, int height,
                                 int bpp) const { return true; }
    ...
    virtual bool SetViewport(int x, int y, int w,
                             int h) { return false; }
    ...
    virtual bool DrawShadow(Solid* s, int nverts, Vec3* verts, bool vis = false) { return false; }
    ...
    virtual bool DrawText(const char* text, int count, const Rect& rect, DWORD format, Color c) { return false; }
