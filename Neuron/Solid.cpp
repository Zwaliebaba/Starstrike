...
Solid::Solid()
  : Graphic(std::format("Solid {}", id)),
    model(nullptr),
    own_model(true),
    roll(0.0f),
    pitch(0.0f),
    yaw(0.0f),
    intersection_poly(nullptr) { shadow = true; }
...
void Solid::SetOrientation(const Matrix& o) { orientation = o; }
...
void Solid::SelectDetail(Projector* p) {}

void Solid::ProjectScreenRect(Projector* p)
{
  if (model && p)
  {
    Point tmp = loc;
    ...
    if (tmp.z > 1)
    {
      ...
      for (int i = 0; i < 6; i++)
      {
        ...
        if (extent.x < l)
          l = static_cast<int>(extent.x);
        if (extent.x > r)
          r = static_cast<int>(extent.x);
        if (extent.y < t)
          t = static_cast<int>(extent.y);
        if (extent.y > b)
          b = static_cast<int>(extent.y);
      }
...
int Solid::CollidesWith(Graphic& o)
{
  ...
  auto& s = static_cast<Solid&>(o);

  // use BVH to check for polygon interference:
  if (model && s.model)
  {
    ...
    // Build world transform matrices for both solids
    Matrix m1 = orientation;
    Matrix m2 = s.orientation;

    XMMATRIX world0 = XMMatrixSet(
        static_cast<float>(m1.elem[0][0]), static_cast<float>(m1.elem[0][1]), static_cast<float>(m1.elem[0][2]), 0.0f,
        static_cast<float>(m1.elem[1][0]), static_cast<float>(m1.elem[1][1]), static_cast<float>(m1.elem[1][2]), 0.0f,
        static_cast<float>(m1.elem[2][0]), static_cast<float>(m1.elem[2][1]), static_cast<float>(m1.elem[2][2]), 0.0f,
        static_cast<float>(Location().x),  static_cast<float>(Location().y),  static_cast<float>(Location().z),  1.0f
    );

    XMMATRIX world1 = XMMatrixSet(
        static_cast<float>(m2.elem[0][0]), static_cast<float>(m2.elem[0][1]), static_cast<float>(m2.elem[0][2]), 0.0f,
        static_cast<float>(m2.elem[1][0]), static_cast<float>(m2.elem[1][1]), static_cast<float>(m2.elem[1][2]), 0.0f,
        static_cast<float>(m2.elem[2][0]), static_cast<float>(m2.elem[2][1]), static_cast<float>(m2.elem[2][2]), 0.0f,
        static_cast<float>(s.Location().x), static_cast<float>(s.Location().y), static_cast<float>(s.Location().z), 1.0f
    );

    ListIter<Surface> s1_iter = model->m_surfaces;
    while (++s1_iter && !contact)
    {
      Surface* s1 = s1_iter.value();

      ListIter<Surface> s2_iter = s.model->m_surfaces;
      while (++s2_iter && !contact)
      {
        ...
        if (s1->bvh && s2->bvh)
        {
          if (s1->bvh->Intersects(*s2->bvh, world0, world1))
            contact = true;
        }
      }
    }
...
int Solid::CheckRayIntersection(Point Q, Point w, double len, Point& ipt, bool treat_translucent_polys_as_solid)
{
  int impact = 0;
...
************************************/
  ...
  // check each polygon:
  ListIter<Surface> iter = model->m_surfaces;
  while (++iter)
  {
...
bool Solid::Load(std::string_view mag_file, double scale)
{
...
bool Solid::Load(ModelFile* mod_file, double scale)
{
  // get ready to load, delete existing model:
  ClearModel();
...
bool Solid::Rescale(double scale)
{
  ...
  while (++iter)
  {
    ...
    {
      s->vertex_set->loc[v] *= static_cast<float>(scale);
      s->vloc[v] *= static_cast<float>(scale);
...
Model& Model::operator =(const Model& m)
{
  if (this != &m)
  {
    ...
    ListIter<Surface> s_iter = pmod->m_surfaces;
    while (++s_iter)
    {
      Surface* surf1 = s_iter.value();
      auto surf2 = NEW Surface;

      surf2->Copy(*surf1, this);
      m_surfaces.append(surf2);
    }
  }
...
***/
  ...
  constexpr DWORD magic_mask = 0x0fc3;

  *flags = magic_flags & magic_mask;
}

bool Model::Load(std::string_view mag_file, double scale)
{
  BYTE* block;
  ...
  if (!loader)
  {
    DebugTrace("MAG Open Failed:  no data loader for file '{}'\n", mag_file);
    return result;
  }
  ...
  if (!size)
  {
    DebugTrace("MAG Open Failed:  could not open file '{}'\n", mag_file);
    return result;
  }
  ...
  {
    DebugTrace("MAG Open Failed:  File '{}' Invalid file type '{}'\n", mag_file, file_id);
...
bool Model::Load(ModelFile* mod_file, double scale)
{
  if (mod_file)
...
bool Model::LoadMag5(BYTE* block, int len, double scale)
{
  bool result = false;
  ...
  for (int i = 0; i < ntex; i++)
  {
    ...
    if (mtl)
    {
      ...
      if (auto dot = mtl->name.find_first_of('.'); dot != std::string::npos)
        mtl->name.erase(dot);

      if (auto plus = mtl->name.find_first_of('+'); plus != std::string::npos)
        mtl->name.erase(plus);
  ...
  if (s)
  {
    ...
    for (int n = 0; n < npolys; n++)
    {
      ...
      poly.visible = static_cast<BYTE>(dummy_flags & 1);
    ...
    qsort(s->polys, s->npolys, sizeof(Poly), mcomp);
...
bool Model::LoadMag6(BYTE* block, int len, double scale)
{
  bool result = false;
  ...
  for (int i = 0; i < nsurfs; i++)
  {
    ...
    for (int v = 0; v < nverts; v++)
    {
      ...
      vset->loc[v] *= static_cast<float>(scale);
    ...
    // sort the polys by material index:
    qsort(polys, npolys, sizeof(Poly), mcomp);
...
const Material* Model::FindMaterial(std::string_view mtl_name) const
{
  if (!mtl_name.empty())
  {
    auto pThis = (Model*)this;
...
const Material* Model::ReplaceMaterial(const Material* mtl)
{
  const Material* mtl_orig = nullptr;
  ...
  if (mtl)
  {
    ...
    if (mtl_orig)
    {
      ...
      ListIter<Surface> surf_iter = m_surfaces;
      while (++surf_iter)
      {
...
Poly* Model::AddPolys(int nsurf, int np, int nv)
{
  if (nsurf >= 0 && nsurf < m_surfaces.size())
    return m_surfaces[nsurf]->AddPolys(np, nv);

  DebugTrace("WARNING: AddPolys(%d,%d,%d) invalid surface\n", nsurf, np, nv);
  return nullptr;
}

void Model::ExplodeMesh()
{
  ListIter<Surface> iter = m_surfaces;
...
void Model::SelectPolys(List<Poly>& polys, Vec3 loc)
{
  ListIter<Surface> iter = m_surfaces;
...
void Model::SelectPolys(List<Poly>& polys, Material* m)
{
  ListIter<Surface> iter = m_surfaces;
...
void Surface::Copy(Surface& s, Model* m)
{
  segments.destroy();
  ...
  while (++iter)
  {
    ...
    seg2->model = model;
    seg2->video_data = nullptr;

    segments.append(seg2);
  }
}

void Surface::SetName(std::string_view n)
{
  name = n;
}
...
{
  if (polys && vertex_set && np > 0 && np + npolys < MAX_POLYS && nv > 0 && nv + vertex_set->nverts < MAX_VERTS)
  {
...
void Surface::ExplodeMesh()
{
  ...
  auto vset = NEW VertexSet(nverts);
  v = 0;
...
constexpr double SELECT_EPSILON = 0.05;
constexpr double SELECT_TEXTURE = 0.0001;
...
void Surface::OptimizeMesh()
{
  ...
  // create vertex maps:
  auto vert_map = new BYTE[nverts];
  auto vert_dst = new WORD[nverts];
...
void Surface::ScaleBy(double factor)
{
  ...
  if (vertex_set && vertex_set->nverts)
  {
    for (int i = 0; i < vertex_set->nverts; i++)
      vertex_set->loc[i] *= static_cast<float>(factor);
  }
}
...
void Surface::Normalize()
{
  ...
  for (int i = 0; i < npolys; i++)
  {
    ...
    p->plane = Plane(vertex_set->loc[p->verts[0]], vertex_set->loc[p->verts[2]], vertex_set->loc[p->verts[1]]);
  }
  ...
  for (int i = 0; i < npolys; i++)
  {
    ...
    for (int n = 0; n < p->nverts; n++)
    {
      ...
      vertex_set->nrm[v] = vertex_set->nrm[v] * (1.0f - p->flatness) + p->plane.normal * (p->flatness);
    }
  }
}

void Surface::SelectPolys(List<Poly>& selection, Vec3 loc)
{
  constexpr double SELECT_EPSILON = 0.05;
...
void Surface::SelectPolys(List<Poly>& selection, Material* m)
{
  for (int i = 0; i < npolys; i++)
  {
    Poly* p = polys + i;
...
void Surface::CalcGradients(Poly& p, Vec3& tangent, Vec3& binormal)
{
...
Segment::Segment() { ZeroMemory(this, sizeof(Segment)); }
...
ModelFile::ModelFile(std::string_view fname)
  : filename(fname),
    model(nullptr),
    pnverts(nullptr),
    pnpolys(nullptr),
...
bool ModelFile::Load(Model* m, double scale)
{
  model = m;
