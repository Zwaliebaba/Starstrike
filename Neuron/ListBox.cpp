...
int ListBoxItem::operator <(const ListBoxItem& item) const
{
  if (listbox && listbox == item.listbox)
  {
    ...
    else if (sort_column >= 0 && sort_column <= subitems.size() && sort_column <= item.subitems.size())
    {
      switch (sort_criteria)
      {
        case ListBox::LIST_SORT_NUMERIC_DESCENDING:
...
void ListBox::DrawContent(const Rect& ctrl_rect)
{
  ...
  if (show_headings)
  {
    ...
    for (int column = 0; column <= max_column; column++)
    {
      ...
      DrawText(GetColumnTitle(column), 0, title_rect, DT_CENTER | DT_SINGLELINE);
  ...
  while (++iter && item_rect.y < h)
  {
    ...
    if (index++ >= top_index)
    {
      ...
      else
      {
        ...
        DrawText(item->text.data(), item->text.length(), text_rect, GetColumnAlign(column) | DT_SINGLELINE);
      }
      ...
      while (++sub_iter)
      {
        ...
        {
          text_rect.Deflate(2, 0);
          DrawText(sub->text.data(), sub->text.length(), text_rect, GetColumnAlign(column) | DT_SINGLELINE);
        }
      }
...
void ListBox::SizeColumns()
{
  ...
  for (int i = 0; i < columns.size(); i++)
  {
    ...
    if (i < columns.size() - 1)
      c->width = static_cast<int>(c->percent * usable_width);
    else
...
int ListBox::NumItems() const { return items.size(); }

int ListBox::NumColumns() const { return columns.size(); }
...
void ListBox::SetItemText(int _index, std::string_view _text)
{
  if (_index >= 0 && _index < items.size())
    items[_index]->text = _text;
}

uint64_t ListBox::GetItemData(int index)
{
  if (index >= 0 && index < items.size())
...
std::string ListBox::GetItemText(int index, int column)
{
  if (column == 0)
...
void ListBox::SetItemText(int _index, int _column, std::string_view _text)
{
  if (_column == 0)
  {
    SetItemText(_index, _text);
    return;
  }

  if (_index >= 0 && _index < items.size())
  {
    ListBoxItem* item = items[_index];

    _column--;
    if (_column >= 0 && _column < columns.size() - 1)
    {
      while (_column >= item->subitems.size())
      {
        auto cell = NEW ListBoxCell;
        if (cell)
          item->subitems.append(cell);
      }

      item->subitems[_column]->text = _text;
    }
  }
}

uint64_t ListBox::GetItemData(int index, int column)
{
  if (column == 0)
...
int ListBox::AddItem(std::string_view text)
{
...
int ListBox::AddItemWithData(std::string_view text, uint64_t data)
{
  auto item = NEW ListBoxItem;
...
void ListBox::InsertItem(int index, std::string_view text)
{
  if (index >= 0 && index < items.size())
  {
    auto item = NEW ListBoxItem;
...
void ListBox::InsertItemWithData(int index, std::string_view text, uint64_t data)
{
  if (index >= 0 && index < items.size())
  {
    if (auto item = NEW ListBoxItem)
    {
      item->text = text;
      item->data = data;
      item->color = fore_color;
...
void ListBox::AddColumn(std::string_view title, int width, int align, int sort)
{
  auto column = NEW ListBoxColumn;
...
std::string ListBox::GetColumnTitle(int index)
{
...
void ListBox::SetColumnTitle(int index, std::string_view title)
{
  if (index >= 0 && index < columns.size())
...
int ListBox::GetMultiSelect() { return multiselect; }
...
bool ListBox::GetShowHeadings() { return show_headings; }
...
Color ListBox::GetSelectedColor() { return selected_color; }
...
int ListBox::GetItemStyle() const { return item_style; }
...
int ListBox::GetSelectedStyle() const { return seln_style; }
...
int ListBox::GetListIndex() { return list_index; }
...
int ListBox::GetSelCount() { return selcount; }
...
int ListBox::GetSortColumn() { return sort_column; }
...
int ListBox::GetSortCriteria() { return GetColumnSort(sort_column); }

void ListBox::SetSortCriteria(SORT sort) { SetColumnSort(sort_column, sort); }
...
int ListBox::OnMouseMove(int x, int y)
{
  ...
  if (captured)
  {
    ...
    else
    {
      ...
      {
        if (dragdrop && (x < rect.x || x > rect.x + rect.w || y < rect.y || y > rect.y + rect.h))
        {
          dragging = true;
          OnDragStart(x, y);
        }
      ...
      if (scrolling == SCROLL_THUMB)
      {
        ...
        int dest = static_cast<int>((double)mouse_y / track_length * (items.size() - 1));
        ScrollTo(dest);
        dirty = true;
      }
    }
...
int ListBox::OnLButtonUp(int x, int y)
{
  if (captured)
  {
    ...
    if (dragging)
    {
      ...
      {
        auto parent_form = static_cast<FormWindow*>(form);
    ...
    Mouse::SetCursor(static_cast<Mouse::CURSOR>(old_cursor));
  }
...
int ListBox::OnMouseWheel(int wheel) { return ScrollWindow::OnMouseWheel(wheel); }
...
int ListBox::OnDragDrop(int x, int y, ActiveWindow* source)
{
  ...
  auto drag_source = static_cast<ListBox*>(source);
  ...
  if (drag_source)
  {
    ...
    for (int i = 0; i < drag_source->NumItems(); i++)
    {
      ...
      {
        AddItemWithData(drag_source->GetItemText(i), drag_source->GetItemData(i));
