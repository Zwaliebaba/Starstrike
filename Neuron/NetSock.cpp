...
NetSock::~NetSock() { close(); }

int NetSock::bind(const NetAddr& addr)
{
  if (closed || s == INVALID_SOCKET)
    return INVALID_SOCKET;
  return stat = ::bind(s, addr.GetSockAddr(), addr.GetSockAddrLength());
}

int NetSock::connect(const NetAddr& addr)
{
  if (closed || s == INVALID_SOCKET)
    return INVALID_SOCKET;
...
int NetSock::listen(int max_connections)
{
  if (closed || s == INVALID_SOCKET)
    return INVALID_SOCKET;
...
NetSock* NetSock::accept(NetAddr* addr)
{
  if (closed || s == INVALID_SOCKET)
    return nullptr;
...
int NetSock::send(std::string msg)
{
  if (closed || s == INVALID_SOCKET)
    return INVALID_SOCKET;
...
std::string NetSock::recv()
{
  ...
  if (stream)
    ...
    rlen = ::recvfrom(s, rbuf, sizeof(rbuf), 0, nullptr, nullptr);
  ...
  return std::string(rbuf, rlen);
}

int NetSock::sendto(std::string msg, const NetAddr& dest)
{
  if (closed || s == INVALID_SOCKET)
    return INVALID_SOCKET;
  return stat = ::sendto(s, msg.data(), msg.length(), 0, dest.GetSockAddr(), dest.GetSockAddrLength());
}

std::string NetSock::recvfrom(NetAddr* a)
{
  if (closed || s == INVALID_SOCKET)
    return "";
...
int NetSock::select(SELECT_TYPE t, long seconds, long microseconds)
{
  if (closed || s == INVALID_SOCKET)
    return INVALID_SOCKET;
  ...
  if (t == SELECT_READ)
    return stat = ::select(1, &fd, nullptr, nullptr, timeout);
  if (t == SELECT_WRITE)
    return stat = ::select(1, nullptr, &fd, nullptr, timeout);
  if (t == (SELECT_READ | SELECT_WRITE))
    return stat = ::select(1, &fd, &fd, nullptr, timeout);
