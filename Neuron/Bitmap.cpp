...
void WinPlot::plot(int x, int y, DWORD val, int exch)
{
  ...
  switch (pixsize)
  {
    ...
    case 2:
    {
      auto dst2 = (LPWORD)dst;
      *dst2 = static_cast<WORD>(val);
    }
    break;
    case 4:
    {
      auto dst4 = (LPDWORD)dst;
      *dst4 = val;
    }
    break;
  }
...
Bitmap::Bitmap(int w, int h, ColorIndex* p, int t)
  : type(t),
    ...
    filename(std::format("Bitmap({}, {}, index, type={})", w, h, t)) {}
...
Bitmap::Bitmap(int w, int h, Color* p, int t)
  : type(t),
    ...
    hipix(p),
    last_modified(GetRealTime()),
    filename(std::format("Bitmap({}, {}, hicolor, type={})", w, h, t)) {}
...
int Bitmap::BmpSize() const { return mapsize * PixSize(); }

int Bitmap::RowSize() const { return width; }

int Bitmap::Pitch() const { return width * PixSize(); }
...
void Bitmap::CopyAlphaRedChannel(int w, int h, DWORD* a)
{
  ...
  for (int i = 0; i < mapsize; i++)
  {
    p->SetAlpha(static_cast<BYTE>((*a & Color::RMask) >> Color::RShift));
    p++;
    a++;
  }
...
void Bitmap::ScaleTo(int w, int h)
{
  ...
  double dx = static_cast<double>(width) / static_cast<double>(w);
  double dy = static_cast<double>(height) / static_cast<double>(h);
  ...
  if (hipix)
  {
    ...
    else
    {
      for (int y = 0; y < h; y++)
      {
        int y_offset = static_cast<int>(y * dy);
        for (int x = 0; x < w; x++)
        {
          int x_offset = static_cast<int>(x * dx);
...
void Bitmap::SetFilename(std::string_view s)
{
  size_t n = s.size();
    ...
    if (def.pix)
    {
      ...
      for (int y = 0; y < 64; y++)
      {
        for (int x = 0; x < 64; x++)
        {
          ...
          if (distance > 31.0)
            distance = 31.0;
          BYTE color = 24 + static_cast<BYTE>(distance);
...
Bitmap* Bitmap::CheckCache(std::string_view filename)
{
  for (int i = 0; i < bitmap_cache.size(); i++)
  {
    if (bitmap_cache[i]->GetFilename() == filename)
      return bitmap_cache[i];
  }
...
void Bitmap::AddToCache(Bitmap* bmp) { bitmap_cache.append(bmp); }
...
void Bitmap::ClearCache() { bitmap_cache.destroy(); }
...
void Bitmap::DrawRect(int x1, int y1, int x2, int y2, Color color)
{
  ...
  if (left)
    draw_vline(s, pitch, pixsize, x1, y1, fh, color);
  if (right)
    draw_vline(s, pitch, pixsize, x2, y1, fh, color);
  if (top)
    draw_strip(s, pitch, pixsize, x1, y1, fw, color);
  if (bottom)
    draw_strip(s, pitch, pixsize, x1, y2, fw, color);
...
void Bitmap::DrawRect(const Rect& r, Color color)
{
  ...
  if (left)
    draw_vline(s, pitch, pixsize, x1, y1, r.h, color);
  if (right)
    draw_vline(s, pitch, pixsize, x2, y1, r.h, color);
  if (top)
    draw_strip(s, pitch, pixsize, x1, y1, r.w, color);
  if (bottom)
    draw_strip(s, pitch, pixsize, x1, y2, r.w, color);
...
void Bitmap::DrawEllipsePoints(int x0, int y0, int x, int y, Color c, BYTE quad)
{
  ...
  if (left >= 0 && top >= 0 && quad & 1)
  {
    ...
    {
      case 1:
        *dst = static_cast<BYTE>(cf);
        break;
      case 2:
      {
        auto sw = (LPWORD)dst;
        *sw = static_cast<WORD>(cf);
      }
      break;
      case 4:
      {
...
static void draw_strip(BYTE* s, int pitch, int pixsize, int x, int y, int len, Color color)
{
  ...
  switch (pixsize)
  {
    ...
    {
      if (len > 1)
        memset(s, static_cast<BYTE>(value), len);
    }
    break;
