#include "pch.h"
#include "FormatUtil.h"

void FormatNumber(char* txt, double n)
{
  double a = fabs(n);

  if (a < 1e3)
    sprintf(txt, "%d", static_cast<int>(n));

  else if (a < 1e6)
    sprintf(txt, "%.1f K", (n / 1e3));

  else if (a < 1e9)
    sprintf(txt, "%.1f M", (n / 1e6));

  else if (a < 1e12)
    sprintf(txt, "%.1f G", (n / 1e9));

  else if (a < 1e15)
    sprintf(txt, "%.1f T", (n / 1e12));

  else
    sprintf(txt, "%.1e", n);
}

void FormatNumberExp(char* txt, double n)
{
  double a = fabs(n);

  if (a < 100e3)
    sprintf(txt, "%d", static_cast<int>(n));

  else
    sprintf(txt, "%.1e", n);
}

constexpr int MINUTE = 60;
constexpr int HOUR = 60 * MINUTE;
constexpr int DAY = 24 * HOUR;

void FormatTime(char* txt, double time)
{
  int t = static_cast<int>(time);

  int h = (t / HOUR);
  int m = ((t - h * HOUR) / MINUTE);
  int s = (t - h * HOUR - m * MINUTE);

  if (h > 0)
    sprintf(txt, "%02d:%02d:%02d", h, m, s);
  else
    sprintf(txt, "%02d:%02d", m, s);
}

void FormatTimeOfDay(char* txt, double time)
{
  int t = static_cast<int>(time);

  if (t >= DAY)
  {
    int d = t / DAY;
    t -= d * DAY;
  }

  int h = (t / HOUR);
  int m = ((t - h * HOUR) / MINUTE);
  int s = (t - h * HOUR - m * MINUTE);

  sprintf(txt, "%02d:%02d:%02d", h, m, s);
}

void FormatDayTime(char* txt, double time, bool short_format)
{
  int t = static_cast<int>(time);
  int d = 1, h = 0, m = 0, s = 0;

  if (t >= DAY)
  {
    d = t / DAY;
    t -= d * DAY;
    d++;
  }

  if (t >= HOUR)
  {
    h = t / HOUR;
    t -= h * HOUR;
  }

  if (t >= MINUTE)
  {
    m = t / MINUTE;
    t -= m * MINUTE;
  }

  s = t;

  if (short_format)
    sprintf(txt, "%02d/%02d:%02d:%02d", d, h, m, s);
  else
    sprintf(txt, "Day %02d %02d:%02d:%02d", d, h, m, s);
}

void FormatDay(char* txt, double time)
{
  int t = static_cast<int>(time);
  int d = 1, h = 0, m = 0, s = 0;

  if (t >= DAY)
  {
    d = t / DAY;
    t -= d * DAY;
    d++;
  }

  sprintf(txt, "Day %02d", d);
}

void FormatPoint(char* txt, const Point& p)
{
  char x[16];
  char y[16];
  char z[16];

  FormatNumber(x, p.x);
  FormatNumber(y, p.y);
  FormatNumber(z, p.z);

  sprintf(txt, "(%s, %s, %s)", x, y, z);
}

std::string FormatTimeString(int utc)
{
  static const char* month[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
  static const char* meridian[2] = { "AM", "PM" };

  if (utc < 1)
    utc = static_cast<int>(time(0));

  time_t aclock = utc;                // Get time in seconds
  struct tm* t = localtime(&aclock); // Convert time to struct tm form

  char buffer[256];
  sprintf(buffer, "%d %s %d, %2d:%02d:%02d %s", t->tm_mday, month[t->tm_mon], 1900 + t->tm_year,
    t->tm_hour > 12 ? t->tm_hour - 12 : t->tm_hour, t->tm_min, t->tm_sec, meridian[t->tm_hour > 12]);

  return buffer;
}

static char safe_str[2048];

std::string SafeString(std::string_view s)
{
  ZeroMemory(safe_str, sizeof(safe_str));

  if (!s.empty())
  {
    int len = static_cast<int>(s.size());
    int n = 0;

    for (int i = 0; i < len; i++)
    {
      char c = s[i];

      if (c == '\n')
      {
        safe_str[n++] = '\\';
        safe_str[n++] = 'n';
      }

      else if (c == '\t')
      {
        safe_str[n++] = '\\';
        safe_str[n++] = 't';
      }

      else if (c == '"')
        safe_str[n++] = '\'';

      else if (c == '\\' && i < len - 1)
      {
        safe_str[n++] = s[i++];
        safe_str[n++] = s[i++];
      }

      else if (c < 32 || c > 126)
      {
        // non printing characters
      }

      else
        safe_str[n++] = c;

      if (n > 2040)
        break;
    }
  }

  return safe_str;
}

std::string SafeQuotes(std::string_view _msg)
{
  int dst = 0;

  if (!_msg.empty())
  {
    std::string_view::iterator msg = _msg.begin();
    ZeroMemory(safe_str, sizeof(safe_str));

    while (dst < 254)
    {
      if (*msg == '"')
      {
        safe_str[dst++] = '\'';
        ++msg;
      }
      else if (isspace(*msg))
      {
        safe_str[dst++] = ' ';
        ++msg;
      }
      else
        safe_str[dst++] = *msg++;
    }
  }

  safe_str[dst] = 0;
  return safe_str;
}

std::string FormatTextReplace(std::string_view msg, std::string_view tgt, std::string_view val)
{
  if (msg.empty() || tgt.empty() || val.empty())
    return "";

  if (msg.find(tgt) == std::string::npos)
    return std::string(msg);

  std::string result;
  auto buffer = new char[msg.size() + 1];
  const char* p = msg.data();
  char* q = buffer;
  int tgtlen = tgt.size();

  while (*p)
  {
    if (!strncmp(p, tgt.data(), tgtlen))
    {
      p += tgtlen;
      *q = 0;
      q = buffer;

      result += buffer;
      result += val;
    }

    else
      *q++ = *p++;
  }

  if (q != buffer)
  {
    *q = 0;
    result += buffer;
  }

  delete[] buffer;
  return result;
}

std::string FormatTextEscape(const std::string_view _msg)
{
  if (_msg.empty())
    return "";

  if (!_msg.contains('\\'))
    return std::string(_msg);

  auto buffer = new char[_msg.size() + 1];
  const char* p = _msg.data();
  char* q = buffer;

  for (size_t i = 0; i < _msg.size(); ++i, p++)
  {
    if (*p == '\\')
    {
      p++;

      if (*p == 'n')
      {
        *q++ = '\n';
        p++;
      }

      else if (*p == 't')
      {
        *q++ = '\t';
        p++;
      }

      else
        *q++ = *p++;
    }

    else
      *q++ = *p++;
  }

  *q = 0;
  std::string result = buffer;
  delete[] buffer;
  return result;
}
