...
static Sound* button_sound = nullptr;
static Sound* click_sound = nullptr;
static Sound* swish_sound = nullptr;
static Sound* chirp_sound = nullptr;
static Sound* accept_sound = nullptr;
static Sound* reject_sound = nullptr;
static Sound* confirm_sound = nullptr;
static Sound* list_select_sound = nullptr;
static Sound* list_scroll_sound = nullptr;
static Sound* list_drop_sound = nullptr;
static Sound* combo_open_sound = nullptr;
static Sound* combo_close_sound = nullptr;
static Sound* combo_hilite_sound = nullptr;
static Sound* combo_select_sound = nullptr;
static Sound* menu_open_sound = nullptr;
static Sound* menu_close_sound = nullptr;
static Sound* menu_select_sound = nullptr;
static Sound* menu_hilite_sound = nullptr;
...
static void LoadInterfaceSound(DataLoader* loader, const char* wave, Sound*& s)
{
  ...
  if (s)
    s->SetFlags(s->GetFlags() | Sound::_INTERFACE);
}
...
void Button::Draw()
{
  ...
  if (font && !text.empty())
  {
    ...
    DrawText(text, 0, label_rect, DT_CALCRECT | align);
    ...
    if (drop_shadow)
    {
      ...
      DrawText(text, text.length(), label_rect, align);
...
Rect Button::CalcLabelRect(int img_w, int img_h)
{
  ...
  if (img_h != 0)
  {
    switch (picture_loc)
    {
      default: case 0:  // the four corner positions
...
Rect Button::CalcPictureRect()
{
  ...
  switch (picture_loc)
  {
    ...
    case 1:
      img_x_offset = (w / 2 - img_w / 2);
      break;

    case 2:
      img_x_offset = w - img_w - bevel_width;
      break;
...
void Button::DrawImage(Bitmap* bmp, const Rect& irect)
{
  if (bmp) { DrawBitmap(irect.x, irect.y, irect.x + irect.w, irect.y + irect.h, bmp, Video::BLEND_ALPHA); }
}
...
void Button::PlaySound(int n)
{
  ...
  switch (n)
  {
    default: case SND_BUTTON:
      if (button_sound)
        sound = button_sound->Duplicate();
      break;
    case SND_CLICK:
      if (click_sound)
        sound = click_sound->Duplicate();
      break;
    case SND_SWISH:
      if (swish_sound)
        sound = swish_sound->Duplicate();
      break;
    case SND_CHIRP:
      if (chirp_sound)
        sound = chirp_sound->Duplicate();
      break;
    case SND_ACCEPT:
      if (accept_sound)
        sound = accept_sound->Duplicate();
      break;
    case SND_REJECT:
      if (reject_sound)
        sound = reject_sound->Duplicate();
      break;
    case SND_CONFIRM:
      if (confirm_sound)
        sound = confirm_sound->Duplicate();
      break;
    case SND_LIST_SELECT:
      if (list_select_sound)
        sound = list_select_sound->Duplicate();
      break;
    case SND_LIST_SCROLL:
      if (list_scroll_sound)
        sound = list_scroll_sound->Duplicate();
      break;
    case SND_LIST_DROP:
      if (list_drop_sound)
        sound = list_drop_sound->Duplicate();
      break;
    case SND_COMBO_OPEN:
      if (combo_open_sound)
        sound = combo_open_sound->Duplicate();
      break;
    case SND_COMBO_CLOSE:
      if (combo_close_sound)
        sound = combo_close_sound->Duplicate();
      break;
    case SND_COMBO_HILITE:
      if (combo_hilite_sound)
        sound = combo_hilite_sound->Duplicate();
      break;
    case SND_COMBO_SELECT:
      if (combo_select_sound)
        sound = combo_select_sound->Duplicate();
      break;
    case SND_MENU_OPEN:
      if (menu_open_sound)
        sound = menu_open_sound->Duplicate();
      break;
    case SND_MENU_CLOSE:
      if (menu_close_sound)
        sound = menu_close_sound->Duplicate();
      break;
    case SND_MENU_SELECT:
      if (menu_select_sound)
        sound = menu_select_sound->Duplicate();
      break;
    case SND_MENU_HILITE:
      if (menu_hilite_sound)
        sound = menu_hilite_sound->Duplicate();
      break;
  }
...
void Button::SetActivatedImage(Bitmap* img) { activated_image = img; }

void Button::SetTransitionImage(Bitmap* img) { transition_image = img; }

short Button::GetBevelWidth() { return bevel_width; }

void Button::SetBevelWidth(short nNewValue)
{
  if (nNewValue < 0)
    nNewValue = 0;
  if (nNewValue > rect.w / 2)
    nNewValue = rect.w / 2;
  bevel_width = nNewValue;
}

bool Button::GetBorder() { return border; }

void Button::SetBorder(bool bNewValue) { border = bNewValue; }

Color Button::GetBorderColor() { return border_color; }

void Button::SetBorderColor(Color newValue) { border_color = newValue; }

Color Button::GetActiveColor() { return active_color; }

void Button::SetActiveColor(Color newValue) { active_color = newValue; }

bool Button::GetAnimated() { return animated; }

void Button::SetAnimated(bool bNewValue) { animated = bNewValue; }

bool Button::GetDropShadow() { return drop_shadow; }

void Button::SetDropShadow(bool bNewValue) { drop_shadow = bNewValue; }

short Button::GetButtonState() { return button_state; }
...
void Button::GetPicture(Bitmap& img) { img.CopyBitmap(picture); }
...
short Button::GetPictureLocation() { return picture_loc; }
...
bool Button::GetSticky() { return sticky; }
