...
void Keyboard::MapKeys(KeyMapEntry* mapping, int nkeys)
{
  for (int i = 0; i < nkeys; i++)
  {
    ...
    {
      if (k.key == 0 || k.key > VK_MBUTTON && k.key <= VK_F12)
      {
        map[k.act] = k.key;
        alt[k.act] = k.alt;
      }
...
bool Keyboard::KeyDownMap(int key)
{
  if (key >= KEY_MAP_FIRST && key <= KEY_MAP_LAST && map[key])
  {
    ...
    if (alt[key] > 0 && alt[key] <= VK_F12)
      a = GetAsyncKeyState(alt[key]);
    else { a = !GetAsyncKeyState(VK_SHIFT) && !GetAsyncKeyState(VK_MENU); }
...
void Keyboard::Acquire()
{
  ...
  if (KeyDownMap(KEY_PLUS_Y))
    y = 1;
  else
    if (KeyDownMap(KEY_MINUS_Y))
      y = -1;

  if (KeyDownMap(KEY_PLUS_Z))
    z = 1;
  else
    if (KeyDownMap(KEY_MINUS_Z))
      z = -1;

  if (KeyDownMap(KEY_MINUS_X))
    x = -1;
  else
    if (KeyDownMap(KEY_PLUS_X))
      x = 1;
  ...
  if (inverted)
  {
    if (KeyDownMap(KEY_PITCH_DOWN))
    {
      if (p1 < steps)
        p1 += 1;
      p = -sqr(p1 / steps);
    }
    else if (KeyDownMap(KEY_PITCH_UP))
    {
      if (p1 < steps)
        p1 += 1;
      p = sqr(p1 / steps);
  ...
  {
    if (KeyDownMap(KEY_PITCH_UP))
    {
      if (p1 < steps)
        p1 += 1;
      p = -sqr(p1 / steps);
    }
    else if (KeyDownMap(KEY_PITCH_DOWN))
    {
      if (p1 < steps)
        p1 += 1;
      p = sqr(p1 / steps);
  ...
  if (KeyDownMap(KEY_CENTER))
    c = 1;

  // actions
  if (KeyDownMap(KEY_ACTION_0))
    action[0] = 1;
  if (KeyDownMap(KEY_ACTION_1))
    action[1] = 1;
  if (KeyDownMap(KEY_ACTION_2))
    action[2] = 1;
  if (KeyDownMap(KEY_ACTION_3))
    action[3] = 1;
}
