...
static void DescribeGUID(GUID* lpGUID)
{
  if (lpGUID)
  {
    DebugTrace("   GUID: %08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x\n", lpGUID->Data1, lpGUID->Data2,
               lpGUID->Data3, lpGUID->Data4[0], lpGUID->Data4[1], lpGUID->Data4[2], lpGUID->Data4[3], lpGUID->Data4[4],
               lpGUID->Data4[5], lpGUID->Data4[6], lpGUID->Data4[7]);
  }
  else
    DebugTrace("   GUID IS NULL!\n");
}
  ...
  if (adapter_index >= 0 && adapter_index < adapter_info_list.size())
  {
    ...
    if (adapter)
    {
      ...
      while (++iter)
      {
        ...
        if (dev_info->device_type == static_cast<D3DDEVTYPE>(devtype))
          return dev_info;
      }
    }
  }
...
bool VideoDX9Enum::IsModeSupported(int w, int h, int b) const
{
  if (adapter_index >= 0 && adapter_index < adapter_info_list.size())
  {
    VideoDX9AdapterInfo* adapter = adapter_info_list[adapter_index];
    ...
    while (++mode_iter)
    {
      ...
      if (mode->width == static_cast<UINT>(w) && mode->height == static_cast<UINT>(h) && GetBitsPerPixel(mode->format)
        == static_cast<UINT>(b))
        return true;
    }
  }
...
HRESULT VideoDX9Enum::Enumerate()
{
  ...
  {
    DebugTrace("Video DX9 Enumerate Adapters and Devices\n");
    DebugTrace("----------------------------------------\n\n");
  }
  ...
  VideoDX9AdapterInfo* adapter_info = nullptr;
  ...
  for (UINT ordinal = 0; ordinal < num_adapters; ordinal++)
  {
    ...
    for (size_t iaaf = 0; iaaf < allowed_adapter_format_list.size(); iaaf++)
    {
      ...
      for (UINT mode = 0; mode < num_adapter_modes; mode++)
      {
        ...
        if (display_mode.Width < min_width || display_mode.Height < min_height ||
          GetColorChannelBits(display_mode.Format) < min_color_bits)
          continue;

        auto dx9_display_mode = NEW VideoDX9DisplayMode(display_mode);
...
HRESULT VideoDX9Enum::EnumerateDevices(VideoDX9AdapterInfo* adapter_info, std::vector<D3DFORMAT>& adapter_format_list)
{
  HRESULT hr = E_FAIL;
  constexpr D3DDEVTYPE dtypes[3] = {D3DDEVTYPE_HAL, D3DDEVTYPE_SW, D3DDEVTYPE_REF};
  const char* dtypestr[3] = {"D3DDEVTYPE_HAL", "D3DDEVTYPE_SW", "D3DDEVTYPE_REF"};
  VideoDX9DeviceInfo* device_info = nullptr;
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    if (FAILED(d3d->GetDeviceCaps(adapter_info->adapter_ordinal, device_info->device_type, &device_info->caps)))
    {
      delete device_info;
      continue;
    }
...
HRESULT VideoDX9Enum::EnumerateDeviceCombos(VideoDX9DeviceInfo* device_info,
                                            std::vector<D3DFORMAT>& adapter_format_list)
{
  constexpr D3DFORMAT back_buffer_formats[] = {
    D3DFMT_A8R8G8B8,
    D3DFMT_X8R8G8B8,
    D3DFMT_R8G8B8,
    D3DFMT_R5G6B5,
  ...
  for (size_t i = 0; i < adapter_format_list.size(); i++)
  {
    ...
    for (int n = 0; n < 6; n++)
    {
      ...
      for (int w = 0; w < 2; w++)
      {
        ...
        if (FAILED(d3d->CheckDeviceType(device_info->adapter_ordinal, device_info->device_type, a_fmt, b_fmt, win)))
          continue;
        ...
        VideoDX9DeviceCombo* device_combo = nullptr;
...
void VideoDX9Enum::BuildDepthStencilFormatList(VideoDX9DeviceCombo* device_combo)
{
  constexpr D3DFORMAT depth_stencil_formats[] = {
    D3DFMT_D32,
    D3DFMT_D24S8,
    D3DFMT_D24X4S4,
    D3DFMT_D24X8,
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    if (SUCCEEDED(
      d3d->CheckDeviceFormat(device_combo->adapter_ordinal, device_combo->device_type, device_combo->adapter_format,
        D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, fmt)))
    {
      if (SUCCEEDED(
        d3d->CheckDepthStencilMatch(device_combo->adapter_ordinal, device_combo->device_type, device_combo->
          adapter_format, device_combo->back_buffer_format, fmt)))
        device_combo->depth_stencil_fmt_list.push_back(fmt);
    }
  }
}

void VideoDX9Enum::BuildMultiSampleTypeList(VideoDX9DeviceCombo* device_combo)
{
  constexpr D3DMULTISAMPLE_TYPE multisample_type_array[] = {
    D3DMULTISAMPLE_NONE,
  ...
  for (int i = 0; i < 17; i++)
  {
    ...
    if (SUCCEEDED(
      d3d->CheckDeviceMultiSampleType(device_combo->adapter_ordinal, device_combo->device_type, device_combo->
        back_buffer_format, device_combo->is_windowed, multisample_type, &multisample_qual)))
    {
...
void VideoDX9Enum::BuildDSMSConflictList(VideoDX9DeviceCombo* device_combo)
{
  for (size_t i = 0; i < device_combo->depth_stencil_fmt_list.size(); i++)
  {
    auto depth_format = device_combo->depth_stencil_fmt_list[i];

    for (size_t n = 0; n < device_combo->multisample_type_list.size(); n++)
    {
      auto multisample_type = device_combo->multisample_type_list[n];

      if (FAILED(d3d->CheckDeviceMultiSampleType(device_combo->adapter_ordinal,
        device_combo->device_type, depth_format, device_combo->is_windowed, multisample_type, NULL)))
      {
        auto conflict = NEW VideoDX9FormatConflict;
...
void VideoDX9Enum::BuildVertexProcessingTypeList(VideoDX9DeviceInfo* device_info, VideoDX9DeviceCombo* device_combo)
{
...
void VideoDX9Enum::BuildPresentIntervalList(VideoDX9DeviceInfo* device_info, VideoDX9DeviceCombo* device_combo)
{
  constexpr DWORD present_interval_array[] = {
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    if (interval == D3DPRESENT_INTERVAL_DEFAULT || (device_info->caps.PresentationIntervals & interval))
      device_combo->present_interval_list.push_back(interval);
  }
}

bool VideoDX9Enum::SuggestWindowSettings(VideoSettings* vs)
{
  if (!vs)
    return false;
  ...
  VideoDX9AdapterInfo* best_adapter_info = nullptr;
  VideoDX9DeviceInfo* best_device_info = nullptr;
  VideoDX9DeviceCombo* best_device_combo = nullptr;
  ...
  while (++a_iter)
  {
    ...
    while (++d_iter)
    {
      ...
      while (++c_iter)
      {
        ...
        // save it
        if (best_device_combo == nullptr || best_device_combo->device_type != D3DDEVTYPE_HAL && device_combo->
          device_type == D3DDEVTYPE_HAL || device_combo->device_type == D3DDEVTYPE_HAL && formats_match)
        {
...
EndWindowedDeviceComboSearch:
  ...
  win_device->depth_buffer_bits = GetDepthBits(best_device_combo->depth_stencil_fmt_list[0]);
...
bool VideoDX9Enum::SuggestFullscreenSettings(VideoSettings* vs)
{
  if (!vs)
    return false;
  ...
  while (++a_iter)
  {
    ...
    while (++d_iter)
    {
      ...
      while (++c_iter)
      {
        ...
        // is better (because it's a HAL, and/or because formats match better),
        // save it
        if (best_device_combo == nullptr || best_device_combo->device_type != D3DDEVTYPE_HAL && device_info->device_type
          == D3DDEVTYPE_HAL || device_combo->device_type == D3DDEVTYPE_HAL && best_device_combo->adapter_format !=
          desktop_display_mode.Format && bAdapterMatchesDesktop || device_combo->device_type == D3DDEVTYPE_HAL &&
          bAdapterMatchesDesktop && bAdapterMatchesBB)
        {
...
EndFullscreenDeviceComboSearch:
  ...
  while (++m_iter)
  {
    ...
    else if (display_mode->width == desired_width)
    { //best_desktop_display_mode.Width) {
  ...
  fs_device->depth_buffer_bits = GetDepthBits(best_device_combo->depth_stencil_fmt_list[0]);
...
int VideoDX9DisplayMode::operator<(const VideoDX9DisplayMode& m) const
{
  if (width < m.width)
    return 1;
...
int VideoDX9DisplayMode::operator<=(const VideoDX9DisplayMode& m) const
{
  ...
  // ... or equal to ...
  if (width == m.width && height == m.height && format == m.format && refresh == m.refresh)
    return 1;
...
const char* VideoDX9DisplayMode::GetDescription() const
{
  static char desc[32];

  sprintf_s(desc, "%4d x %4d %-12s %d Hz", //-V576
            width, height, D3DFormatToString(format), refresh);

  return desc;
}

const char* VideoDX9DisplayMode::D3DFormatToString(D3DFORMAT format)
{
  auto str = "Unknown Format";
...
const char* VideoDX9AdapterInfo::GetDescription() const { return adapter_identifier.Description; }
...
VideoDX9DeviceInfo::~VideoDX9DeviceInfo() { device_combo_list.destroy(); }

VideoDX9DeviceCombo::VideoDX9DeviceCombo()
  : adapter_ordinal(0),
    device_type(D3DDEVTYPE_HAL),
    adapter_format(static_cast<D3DFORMAT>(0)),
    back_buffer_format(static_cast<D3DFORMAT>(0)),
    is_windowed(false) {}

VideoDX9DeviceCombo::~VideoDX9DeviceCombo() { conflict_list.destroy(); }
