...
namespace Neuron::Math
{
  ...
  [[nodiscard]] inline XMVECTOR XM_CALLCONV SetLength(const XMVECTOR _v1, float _length) { return XMVectorScale(XMVector3Normalize(_v1), _length); }
  ...
  [[nodiscard]] inline XMMATRIX XM_CALLCONV CreateRotationMatrix(XMVECTOR _axis, float _angle) { return XMMatrixRotationAxis(_axis, _angle); }
