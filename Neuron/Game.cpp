...
constexpr double MAX_FRAME_TIME_VIDEO = 1.0 / static_cast<double>(VIDEO_FPS);
constexpr double MAX_FRAME_TIME_NORMAL = 1.0 / 5.0;
constexpr double MIN_FRAME_TIME_NORMAL = 1.0 / 60.0;
...
int Game::MaxTexSize()
{
  if (game && game->video)
  {
    ...
    return max_vid_size < game->max_tex_size ? max_vid_size : game->max_tex_size;
  }
  if (Video::GetInstance())
...
bool Game::DisplayModeSupported(int w, int h, int bpp)
{
...
bool Game::Init(HINSTANCE hi, HINSTANCE hpi, LPSTR cmdline, int nCmdShow)
{
  ...
  if (status == OK)
  {
    DebugTrace("  Initializing content...\n");
    InitContent();
...
bool Game::InitApplication(HINSTANCE hInstance)
{
  ...
  if (RegisterClass(&wc) == 0)
  {
    ...
    if (err == 1410) // class already exists, this is OK
      return true;
    DebugTrace("WARNING: Register Window Class: %08x\n", err);
  }
...
bool Game::InitInstance(HINSTANCE hInstance, int nCmdShow)
{
  ...
  window_style = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_VISIBLE;
  ...
  hwnd = CreateWindow(app_name,                   
                      ...
                      window_style, x_offset,                   
                      y_offset, rctmp.right - rctmp.left, 
                      rctmp.bottom - rctmp.top, 0,                         // Parent window (no parent)
                      0,                         // use class menu
  ...
  DebugTrace("  Instance initialized.\n");
  return true;
}
...
bool Game::InitVideo()
{
  ...
  if (video_factory)
  {
    DebugTrace("  Init Video...\n");
    DebugTrace("  Request {} mode\n", video_settings->GetModeDescription());
...
bool Game::ResetVideo()
{
  ...
  DebugTrace("  Re-created video object.\n");
  ...
  DebugTrace("  Re-created screen object.\n");

  if (!screen->SetBackgroundColor(Color::Black))
    DebugTrace("  WARNING: could not set video background color to Black\n");

  screen->ClearAllFrames(true);
  video->SetGammaLevel(gamma);

  DebugTrace("  Re-established requested video parameters.\n");

  Bitmap::CacheUpdate();
  DebugTrace("  Refreshed texture bitmaps.\n\n");
  return true;
}
...
bool Game::ResizeVideo()
{
  ...
  if (client_old.right - client_old.left != client_rect.right - client_rect.left || client_old.bottom - client_old.top
  ...
  if (video && video_settings)
  {
    ...
    if (!video->Reset(video_settings))
    {
      ...
      if (!is_windowed)
      {
        ...
        SetWindowPos(hwnd, HWND_NOTOPMOST, bounds_rect.left, bounds_rect.top, bounds_rect.right - bounds_rect.left,
                     bounds_rect.bottom - bounds_rect.top, SWP_SHOWWINDOW);
      }

      DebugTrace("Unable to toggle %s fullscreen mode.\n", is_windowed ? "into" : "out of");
    }
    ...
    else
    {
      ...
      if (is_windowed) { screen->Resize(video_settings->window_width, video_settings->window_height); }

      else { screen->Resize(video_settings->fullscreen_mode.width, video_settings->fullscreen_mode.height); }
...
{
  if (server)
    DebugTrace("  InitGame() - server mode.\n");
  ...
  else
  {
    ...
    DebugTrace("  Created video object.\n");
    ...
    DebugTrace("  Created screen object.\n");
    ...
    DebugTrace("  Established requested video parameters.\n\n");
  }
...
bool Game::InitContent()
{
  ...
  for (auto& filename : bundles)
  {
    int n = filename.find_first_of('_');

    if (n != std::string::npos)
      Locale::ParseLocale(filename.c_str() + n);  // Use c_str() to get the character pointer
    else
    {
      delete sm_content;
      sm_content = NEW ContentBundle("content", nullptr);
    }
  }
...
void Game::UseLocale(Locale* locale)
{
  if (game)
  {
    ...
    loader->SetDataPath("Content\\");
    delete sm_content;
    sm_content = NEW ContentBundle("content", locale);
...
bool Game::LoadPalette(PALETTEENTRY* pal, BYTE* inv)
{
  char palheader[32];
  struct
  {
  ...
  {
    DebugTrace("  Could not open file '%s'\n", fname);
    return false;
  }

  memcpy(&palheader, block, 24);
  memcpy(Palette.Entries, (block + 24), 256 * 4);
...
bool Game::GameLoop()
{
  ...
  double freq = static_cast<double>(perf_freq.QuadPart);
  double msec = static_cast<double>(perf_cnt2.QuadPart - perf_cnt1.QuadPart);

  msec /= freq;
  msec *= 1000.0;

  msec = std::max<double>(msec, 1);

  real_time += static_cast<DWORD>(msec);
...
Game* Game::GetInstance() { return game; }
...
void Game::ScreenCapture(const char* name)
{
  if (server || !video || !screen)
    return;
  ...
  if (loader)
  {
    ...
    {
      std::string s = shot_list[i];
      int n = 0;

      sscanf_s(s.c_str() + 1, "%d", &n);
      if (shot_num <= static_cast<DWORD>(n))
        shot_num = n + 1;
    }
  }
...
void Game::AVICapture(const char* name)
{
  if (server || !video || !screen)
    return;
  ...
  if (!avi_file)
  {
    ...
    {
      loader->SetDataPath();
      std::vector<std::string> avi_list;
      loader->ListFiles("*.avi", avi_list);

      for (int i = 0; i < avi_list.size(); i++)
      {
        std::string s = avi_list[i];
        int n = 0;

        sscanf_s(s.c_str() + 1, "%d", &n);
        if (avi_num <= static_cast<DWORD>(n))
          avi_num = n + 1;
      }
    }
    ...
    if (video && video->Capture(bmp))
    {
      avi_file = NEW AviFile(filename, Rect(0, 0, bmp.Width(), bmp.Height()), VIDEO_FPS);
    }
  }
...
void Game::ShowStats()
{
  ...
  DebugTrace("\n");
  DebugTrace("Performance Data:\n");
  DebugTrace("-----------------\n");

  DebugTrace("   Time:            %d msec\n", totaltime);
  DebugTrace("   Frames:          %d\n", stats.nframe);
  DebugTrace("   Polys Rendered:  %d\n", stats.total_polys);
  DebugTrace("   Lines Rendered:  %d\n", stats.total_lines);
  DebugTrace("   Verts Rendered:  %d\n", stats.total_verts);
  DebugTrace("   Render Calls:    %d\n", stats.ncalls);
  DebugTrace("\n");

  DebugTrace("Performance Statistics:\n");
  DebugTrace("-----------------------\n");

  DebugTrace("   Frames/Second:   %.2f\n", (stats.nframe * 1000.0) / totaltime);
  DebugTrace("   Polys/Frame:     %.2f\n", static_cast<double>(stats.total_polys) / static_cast<double>(stats.nframe));
  DebugTrace("   Polys/Call:      %.2f\n", static_cast<double>(stats.total_polys) / static_cast<double>(stats.ncalls));
  DebugTrace("   Polys/Second:    %.2f\n", (stats.total_polys * 1000.0) / totaltime);
  DebugTrace("   Lines/Second:    %.2f\n", (stats.total_lines * 1000.0) / totaltime);
  DebugTrace("   Verts/Second:    %.2f\n", (stats.total_verts * 1000.0) / totaltime);
...
*/
    ...
    case WM_MOUSEWHEEL:
    {
      int w = static_cast<int>(uParam >> 16);
      if (w > 32000)
        w -= 65536;
      Mouse::w += w;
    }
...
DWORD Game::RealTime() { return real_time; }

DWORD Game::GameTime() { return game_time; }

DWORD Game::TimeCompression() { return time_comp; }
...
void Game::ResetGameTime() { game_time = 0; }
