...
void RandomInit() { srand(timeGetTime()); }

Point RandomDirection()
{
  auto p = Point(rand() - 16384, rand() - 16384, 0);
  p.Normalize();
  return p;
}
...
{
  auto v = Vec3(rand() - 16384, rand() - 16384, rand() - 16384);
  v.Normalize();

  if (radius > 0)
    v *= static_cast<float>(radius);
  else
    v *= static_cast<float>(Random(radius / 3, radius));
...
int RandomSequence(int current, int range)
{
  if (range > 1)
  {
    int step = static_cast<int>(Random(1, range - 1));
...
int RandomShuffle(int count)
{
  ...
  if (index < 0 || index > set_size - 1)
  {
    ...
    {
      int n = static_cast<int>(Random(0, set_size));
      int tries = set_size;
      while (tmp[n] < 0 && tries--)
        n = (n + 1) % set_size;
