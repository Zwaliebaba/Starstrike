...
class Color
{
  ...
  public:
    ...
    float fRed() const { return static_cast<float>(Red() / 255.0); }
    float fGreen() const { return static_cast<float>(Green() / 255.0); }
    float fBlue() const { return static_cast<float>(Blue() / 255.0); }
    float fAlpha() const { return static_cast<float>(Alpha() / 255.0); }

    BYTE Index() const
    {
      return table[((rgba & RMask) >> (RShift + 3) << 10) | ((rgba & GMask) >> (GShift + 3) << 5) | ((rgba & BMask) >> (
        BShift + 3))];
    }
    ...
    static void SetPalette(PALETTEENTRY* pal, int palsize, BYTE* invpal = nullptr);
...
class ColorIndex
{
  ...
  public:
    ...
    ColorIndex& operator=(const ColorIndex& c)
    {
      index = c.index;
      return *this;
    }
    ...
    ColorIndex ShadeColor(int s) const { return ColorIndex(static_cast<BYTE>(shade_table[s * 256 + index])); }
