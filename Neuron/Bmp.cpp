...
BmpImage::BmpImage(short w, short h, unsigned long* hibits)
{
  ...
  file_hdr.size = sizeof(BmpFileHeader) + sizeof(BmpInfoHeader) + w * h * 3;

  file_hdr.offset = sizeof(BmpFileHeader) + sizeof(BmpInfoHeader);
...
BmpImage::~BmpImage() { delete [] image; }
...
int BmpImage::Load(char* filename)
{
  ...
  if (info_hdr.width > 32768 || info_hdr.height > 32768 || (info_hdr.width & 3) || (info_hdr.height & 3) || info_hdr.
    compression != 0)
  {
    fclose(f);
    return BMP_INVALID;
  ...
  width = static_cast<WORD>(info_hdr.width);
  height = static_cast<WORD>(info_hdr.height);
...
int BmpImage::Save(char* filename)
{
  ...
  for (int row = height - 1; row >= 0; row--)
  {
    for (int col = 0; col < width; col++)
    {
      ...
      BYTE blue = static_cast<BYTE>((pixel & 0x000000ff) >> 0);
      BYTE green = static_cast<BYTE>((pixel & 0x0000ff00) >> 8);
      BYTE red = static_cast<BYTE>((pixel & 0x00ff0000) >> 16);
