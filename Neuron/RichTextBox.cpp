...
void RichTextBox::SetText(std::string_view t)
{
  ActiveWindow::SetText(t);
  ScrollTo(0);
}

void RichTextBox::DrawContent(const Rect& ctrl_rect) { DrawTabbedText(); }
...
void RichTextBox::DrawRichText(Rect& text_rect)
{
  ...
  if (smooth_scroll)
  {
    double fraction = smooth_offset - static_cast<int>(smooth_offset);

    y_offset = static_cast<int>((1 - fraction) * (rich_font->Height() + leading));
  }
  ...
  while (block_start < count)
  {
    ...
    int next_word_end = find_next_word_end(t, block_start);
    ...
    else
    {
      ...
      length = rich_font->StringWidth(std::string_view(t + block_start, word_len));
    ...
    int block_count = curr_word_end - block_start + 1;

    if (block_count > 0)
      length = rich_font->StringWidth(std::string_view(t + block_start, block_count));
  ...
  if (captured)
  {
    ...
    else
    {
      if (scrolling == SCROLL_THUMB)
      {
        ...
        int dest = static_cast<int>((double)mouse_y / track_length * (line_count - 1));
        ScrollTo(dest);
      }
    }
  }
...
int RichTextBox::OnLButtonDown(int x, int y) { return ScrollWindow::OnLButtonDown(x, y); }

int RichTextBox::OnLButtonUp(int x, int y) { return ScrollWindow::OnLButtonUp(x, y); }

int RichTextBox::OnMouseWheel(int wheel) { return ScrollWindow::OnMouseWheel(wheel); }
