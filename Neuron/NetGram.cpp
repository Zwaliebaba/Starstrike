 ...
 */
NetGram::NetGram(const NetAddr& src, std::string msg)
 ...
 */
NetGram::NetGram(const NetAddr& dst, std::string user_data, int r)
    : addr(dst), retries(r)
{
    ...
    body = std::string((char*) buf, len+NET_GRAM_HEADER_SIZE);
}
...
NetGram::Ack()
{
    ...
    ack.body = std::string((char*) buf, NET_GRAM_HEADER_SIZE);
