...
const char* DSErrStr(HRESULT dserr);
    ...
    primary(nullptr)
{
  ...
  err = DirectSoundCreate(nullptr, &soundcard, nullptr);
  if (FAILED(err))
  {
  ...
  if (caps.dwFlags & DSCAPS_EMULDRIVER)
    DebugTrace("   WARNING: using DirectSound emulated drivers\n");
  ...
  dsbd.dwSize = sizeof(dsbd);
    ...
    if (err == DSERR_CONTROLUNAVAIL) {
        ...
        if (FAILED(err)) {
            ...
            DebugTrace("   WARNING: DirectSound3D is not available, simulating instead\n");
        }
    }
  ...
  err = soundcard->CreateSoundBuffer(&dsbd, &primary, nullptr);
  if (FAILED(err))
  {
    SoundD3DError("Could not initialize primary buffer", err);
    RELEASE(soundcard);
...
SoundCardD3D::~SoundCardD3D()
{
  ...
  DebugTrace("   SoundCardD3D: shutdown\n");

  status = SC_UNINITIALIZED;
}

bool SoundCardD3D::SetFormat(int bits, int channels, int hertz)
{
  if (!soundcard)
    return false;
...
bool SoundCardD3D::GetFormat(LPWAVEFORMATEX format)
{
  ...
  if (primary)
    err = primary->GetFormat(format, sizeof(WAVEFORMATEX), nullptr);
...
{
  DebugTrace("   SoundCardD3D Primary Buffer Format:\n");
  DebugTrace("     bits: {}\n", wfex.wBitsPerSample);
  DebugTrace("     chls: {}\n", wfex.nChannels);
  DebugTrace("     rate: {}\n\n", wfex.nSamplesPerSec);
}

Sound* SoundCardD3D::CreateSound(DWORD flags, LPWAVEFORMATEX format)
{
  if (!soundcard)
    return nullptr;
...
Sound* SoundCardD3D::CreateSound(DWORD flags, LPWAVEFORMATEX format, DWORD len, LPBYTE data)
{
  if (!soundcard)
    return nullptr;
  Sound* result = NEW SoundD3D(soundcard, flags, format, len, data);

  if (flags & (Sound::STREAMED))
  {
    if (result)
      AddSound(result);
  }
...
void SoundCardD3D::SetListener(const Camera& cam, const Vec3& vel)
{
  Point pos = cam.Pos();
...
bool SoundCardD3D::Pause()
{
  ...
  while (++iter)
  {
    ...
    if ((s->GetFlags() & Sound::_INTERFACE) == 0)
      s->Pause();
  }
...
bool SoundCardD3D::StopSoundEffects()
{
  ...
  DWORD ok_sounds = (Sound::_INTERFACE | Sound::RESOURCE);
...
void SoundD3D::Update()
{
  ...
  if (flags & STREAMED)
  {
    buffer->GetCurrentPosition(&r, nullptr);
...
void SoundD3D::RewindStream()
{
  ...
  total_time = static_cast<double>(stream_left) / static_cast<double>(wfex.nAvgBytesPerSec);
...
void SoundD3D::Localize()
{
  ...
  if ((flags & LOC_3D) && creator)
  {
    ...
    auto ears = static_cast<SoundCardD3D*>(creator);
    Camera& listener = ears->listener;
    Vec3 ear_loc = listener.Pos();
    ear_loc.SwapYZ();
    ...
    /***
DebugTrace("Localize: ears  = (%f, %f, %f)\n", ear_loc.x, ear_loc.y, ear_loc.z);
DebugTrace("          world = (%f, %f, %f)\n", location.x, location.y, location.z);
DebugTrace("          view  = (%f, %f, %f)\n", loc.x, loc.y, loc.z);
DebugTrace("          Pan=%f  Volume=%f\n", pan, vol);
/***/

    HRESULT hr = buffer->SetPan(static_cast<LONG>(pan));
    if (!SUCCEEDED(hr))
    {
      char warn[512];
    ...
    hr = buffer->SetVolume(static_cast<LONG>(vol));
    if (!SUCCEEDED(hr))
    {
      char warn[512];
    ...
    if ((flags & DOPPLER) && (d2 < maxd2))
    {
      ...
      double dist = sqrt(d2);
      DWORD roll_off = static_cast<DWORD>(80 * dist / max_dist);
  ...
  {
    buffer->SetPan(static_cast<LONG>(location.x));
...
Sound* SoundD3D::Duplicate()
{
  ...
  if (flags & RESOURCE)
  {
    sound = Create(flags & ~RESOURCE, &wfex);
    ...
    if (sound && !(flags & STREAMED))
    {
      ...
      {
        auto s3d = static_cast<SoundD3D*>(sound);
...
HRESULT SoundD3D::StreamFile(const char* name, DWORD offset)
{
...
HRESULT SoundD3D::Load(DWORD bytes, BYTE* data)
{
  status = UNINITIALIZED;
...
HRESULT SoundD3D::AllocateBuffer(DWORD bytes)
{
  ...
  if (!buffer)
  {
    ...
    if (soundcard)
    {
      hr = soundcard->CreateSoundBuffer(&dsbd, &buffer, nullptr);

      if (FAILED(hr))
      {
        SoundD3DError("AllocateBuffer: could not create buffer", hr);

        DebugTrace(L"   dsbd.dwSize         = {:8d}\n", dsbd.dwSize);
        DebugTrace(L"   dsbd.dwFlags        = {:8x}\n", dsbd.dwFlags);
        DebugTrace(L"   dsbd.dwBufferBytes  = {:8d}\n", dsbd.dwBufferBytes);
        DebugTrace(L"   dsbd.lpwfxFormat    = {:8x}\n", (size_t)dsbd.lpwfxFormat);

        if (dsbd.lpwfxFormat)
        {
          DebugTrace("   wfex.wBitsPerSample = %8d\n", dsbd.lpwfxFormat->wBitsPerSample);
          DebugTrace("   wfex.nChannels      = %8d\n", dsbd.lpwfxFormat->nChannels);
          DebugTrace("   wfex.nSamplesPerSec = %8d\n", dsbd.lpwfxFormat->nSamplesPerSec);
        }
      }
    }
    else
...
double SoundD3D::GetTimeRemaining() const
{
  ...
  if (IsPlaying() || IsReady())
    time_left = static_cast<double>(stream_left) / static_cast<double>(wfex.nAvgBytesPerSec);
...
long SoundD3D::GetPan() const
{
  ...
  if (location.z)
    p = static_cast<long>(fabs(location.x / location.z));
  if (p > 10000)
    p = 10000;
  if (location.x < 0)
    p = -p;
...
void SoundD3D::SetPan(long p)
{
  ...
  location.x = static_cast<float>(p);
  location.y = 0.0f;
  location.z = 1.0f;
  moved = true;
}

void SoundD3D::SetLocation(const Vec3& l)
{
  location = l;
  moved = true;
}

void SoundD3D::SetVelocity(const Vec3& v)
{
  velocity = v;
  moved = true;
}

float SoundD3D::GetMinDistance() const { return min_dist; }
...
float SoundD3D::GetMaxDistance() const { return max_dist; }
...
void SoundD3DError(const char* msg, HRESULT err) { DebugTrace("   SoundD3D: %s. [%s]\n", msg, DSErrStr(err)); }

const char* DSErrStr(HRESULT err)
{
  switch (err)
  {
    case DS_OK:
    ...
    case DSERR_ALLOCATED:
      return "The call failed because resources (such as a priority level) "
    ...
    case DSERR_GENERIC:
      return "An undetermined error occured inside the DirectSound subsystem";
