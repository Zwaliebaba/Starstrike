...
Font* ActiveWindow::sys_font = nullptr;
...
ActiveWindow::ActiveWindow(Screen* screen, int ax, int ay, int aw, int ah, DWORD aid, DWORD s, ActiveWindow* pParent)
  : Window(screen, ax, ay, aw, ah),
    id(aid),
    style(s),
    ...
    hide_partial(true),
    alpha(1),
    back_color(sys_back_color),
    fore_color(sys_fore_color),
    texture(nullptr),
    fixed_width(0),
    fixed_height(0),
    parent(pParent),
...
void ActiveWindow::AddChild(ActiveWindow* child)
{
  if (child)
    children.append(child);
}

void ActiveWindow::DoLayout()
{
  if (layout)
    layout->DoLayout(this);
}

void ActiveWindow::UseLayout(const std::vector<DWORD>& min_x, const std::vector<DWORD>& min_y,
                             const std::vector<float>& weight_x, const std::vector<float>& weight_y)
{
  if (!layout)
    layout = NEW Layout;
...
void ActiveWindow::UseLayout(const std::vector<float>& min_x, const std::vector<float>& min_y,
                             const std::vector<float>& weight_x, const std::vector<float>& weight_y)
{
  if (!layout)
    layout = NEW Layout;
...
void ActiveWindow::UseLayout(int nrows, int ncols, int* min_x, int* min_y, float* weight_x, float* weight_y)
{
  if (!layout)
    layout = NEW Layout;
...
void ActiveWindow::Paint() { Draw(); }

Color ActiveWindow::ShadeColor(Color c, double shade)
{
  int ishade = static_cast<int>(shade * I(Color::SHADE_LEVELS));
...
void ActiveWindow::CalcGrid()
{
  ...
  if (npolys > 0)
  {
    ...
    if (tex_w > rect.w)
      tex_w = static_cast<float>(rect.w);
    if (tex_h > rect.h)
      tex_h = static_cast<float>(rect.h);

    u_offsets[0] = 0.0f;
    u_offsets[1] = margins.left ? static_cast<float>(margins.left) : tex_w - static_cast<float>(margins.right);
    u_offsets[2] = cols == 2 ? tex_w : tex_w - static_cast<float>(margins.right);
    u_offsets[3] = tex_w;

    v_offsets[0] = 0.0f;
    v_offsets[1] = margins.top ? static_cast<float>(margins.top) : tex_h - static_cast<float>(margins.bottom);
    v_offsets[2] = rows == 2 ? tex_h : tex_h - static_cast<float>(margins.bottom);
    v_offsets[3] = tex_h;

    tex_w = static_cast<float>(texture->Width());
    tex_h = static_cast<float>(texture->Height());
    ...
    Color c = Color::White;
    c.SetAlpha(static_cast<BYTE>(alpha * 255));
    ...
    for (i = 0; i <= rows; i++)
    {
      for (j = 0; j <= cols; j++)
      {
        ...
        vset->s_loc[v].x = static_cast<float>(rect.x + x_offsets[j]) - 0.5f;
        vset->s_loc[v].y = static_cast<float>(rect.y + y_offsets[i]) - 0.5f;
        vset->s_loc[v].z = 0.0f;
        vset->rw[v] = 1.0f;
...
void ActiveWindow::DrawTextureGrid()
{
  int npolys = rows * cols;
...
void ActiveWindow::DrawStyleRect(const Rect& r, int style) { DrawStyleRect(r.x, r.y, r.x + r.w, r.y + r.h, style); }

void ActiveWindow::DrawStyleRect(int x1, int y1, int x2, int y2, int style)
{
...
void ActiveWindow::DrawTabbedText()
{
  if (shown && font && text.length())
  {
    ...
    if (text_insets.left)
    {
      ...
      label_rect.Inset(text_insets.left, text_insets.right, text_insets.top, text_insets.bottom);
    }
    else
    {
      int border_size = 4;
...
void ActiveWindow::SetTabStop(int n, int x)
{
  if (n >= 0 && n < 10)
    tab[n] = x;
}
...
void ActiveWindow::SetText(std::string_view t)
{
  if (!t.empty() && text != t)
  {
    size_t len = t.length();
...
void ActiveWindow::AddText(std::string_view t)
{
  if (!t.empty())
    text += t;
}
...
void ActiveWindow::SetTextInsets(const Insets& t) { text_insets = t; }

void ActiveWindow::SetCellInsets(const Insets& c) { cell_insets = c; }

void ActiveWindow::SetCells(int cx, int cy, int cw, int ch)
{
  cells.x = cx;
  cells.y = cy;
  cells.w = cw;
...
void ActiveWindow::DrawText(std::string_view txt, int count, Rect& txt_rect, DWORD flags)
{
  if (font)
  {
    font->SetAlpha(alpha);
...
{
  auto map = NEW AWMap(eid, client, callback);
...
ActiveWindow* ActiveWindow::GetCapture()
{
  EventDispatch* dispatch = EventDispatch::GetInstance();
  if (dispatch)
    return static_cast<ActiveWindow*>(dispatch->GetCapture());

  return nullptr;
}

bool ActiveWindow::SetCapture()
{
  EventDispatch* dispatch = EventDispatch::GetInstance();
  if (dispatch)
...
int ActiveWindow::ReleaseCapture()
{
  EventDispatch* dispatch = EventDispatch::GetInstance();
  if (dispatch)
...
ActiveWindow* ActiveWindow::FindChild(DWORD id)
{
  ListIter<ActiveWindow> iter = children;
  while (++iter)
  {
...
ActiveWindow* ActiveWindow::FindChild(int x, int y)
{
  ActiveWindow* mouse_tgt = nullptr;
