...
void TexCacheDX9Entry::Release() { RELEASE(texture); }
...
TexCacheDX9::~TexCacheDX9()
{
  ...
  DebugTrace("TexCacheDX9: final used count = %d\n", used);
}

bool TexCacheDX9::LoadTexture(Bitmap* bmp, TexCacheDX9Entry* entry)
{
  if (!d3ddevice)
    return false;
  ...
  {
    hr = d3ddevice->CreateTexture(bmp->Width(), bmp->Height(), 1,                // one mip level
                                  0,                // no specific usage
                                  D3DFMT_A8R8G8B8,  // format matching Color::rgba
                                  D3DPOOL_MANAGED, &entry->texture, nullptr);
  ...
  hr = entry->texture->LockRect(0, &locked_rect, nullptr, 0);
  ...
  {
    auto src = (BYTE*)(bmp->HiPixels() + i * bmp->Width());
...
void TexCacheDX9::CreateNormalMap(int index, float amp)
{
  if (d3ddevice && cache[index].texture && !cache[index].normal)
  {
    ...
    IDirect3DTexture9* normal_map = nullptr;

    // create the normal map texture
    hr = d3ddevice->CreateTexture(bmp->Width(), bmp->Height(), 1,                // one mip levels
                                  0,                // no specific usage
                                  D3DFMT_A8R8G8B8,  // format matching Color::rgba
                                  D3DPOOL_MANAGED, &normal_map, nullptr);
    ...
    D3DXComputeNormalMap(normal_map, entry->texture, nullptr, 0, D3DX_CHANNEL_RED, amp);
    ...
    // lock the surface for writing
    D3DLOCKED_RECT locked_rect;
    hr = normal_map->LockRect(0, &locked_rect, nullptr, 0);
...
IDirect3DTexture9* TexCacheDX9::FindTexture(Bitmap* bmp)
{
  ...
  for (int i = 0; i < DX9_TEXTURE_CACHE_SIZE; i++)
  {
    ...
    if (avail < 0 && cache[i].bitmap_id == nullptr)
      avail = i;
  }
...
IDirect3DTexture9* TexCacheDX9::FindNormalMap(Bitmap* bmp, float amp)
{
  int avail = -1;
  ...
  {
#if TEXDX9_VERBOSE
        DebugTrace("   Tex %3d: id=%2d, size=%3d, type=%d, hTex=%3d, frame=%6d   vmf=%8d\n",
...
#if TEXDX9_VERBOSE
    DebugTrace("   FreeLRU() frame=%6d unloaded=%2d vmf=%8d\n", frame_number, unloaded, vidmem);
#endif

  return unloaded;
}

int TexCacheDX9::FreeUpCache()
{
  int unloaded = 0;
  ...
  DebugTrace("   FreeUpCache() frame=%6d unloaded=%2d vmf=%8d\n", frame_number, unloaded, vidmem);
