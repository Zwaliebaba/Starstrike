...
FormWindow::FormWindow(Screen* screen, int ax, int ay, int aw, int ah, DWORD aid, DWORD s, ActiveWindow* pParent)
...
Button* FormWindow::CreateButton(std::string_view btn_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid)
{
...
ImageBox* FormWindow::CreateImageBox(std::string_view lbl_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid)
{
...
ActiveWindow* FormWindow::CreateLabel(std::string_view label_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid,
                                      DWORD astyle)
{
  ActiveWindow* label = nullptr;
  ActiveWindow* parent = this;
...
ListBox* FormWindow::CreateListBox(std::string_view lbl_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid)
{
...
ComboBox* FormWindow::CreateComboBox(std::string_view lbl_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid)
{
...
EditBox* FormWindow::CreateEditBox(std::string_view lbl_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid)
{
...
RichTextBox* FormWindow::CreateRichTextBox(std::string_view label_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid,
                                           DWORD astyle)
{
...
Slider* FormWindow::CreateSlider(std::string_view label_text, int ax, int ay, int aw, int ah, DWORD aid, DWORD pid,
                                 DWORD astyle)
{
...
{
  ActiveWindow* ctrl = CreateLabel(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                                   def.GetParentID(), def.GetStyle());
  ...
  ctrl->UseLayout(def.GetLayout().x_mins, def.GetLayout().y_mins, def.GetLayout().x_weights, def.GetLayout().y_weights);

  if (def.GetTexture().length() > 0)
  {
    Bitmap* ctrl_tex = nullptr;
    DataLoader* loader = DataLoader::GetLoader();
    loader->SetDataPath("Screens/");
...
{
  Button* ctrl = CreateButton(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                              def.GetParentID());
...
{
  ImageBox* ctrl = CreateImageBox(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                                  def.GetParentID());
...
{
  ListBox* ctrl = CreateListBox(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                                def.GetParentID());
...
{
  ComboBox* ctrl = CreateComboBox(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                                  def.GetParentID());
...
{
  EditBox* ctrl = CreateEditBox(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                                def.GetParentID());
...
{
  Slider* ctrl = CreateSlider(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                              def.GetParentID());
...
{
  RichTextBox* ctrl = CreateRichTextBox(def.GetText(), def.GetX(), def.GetY(), def.GetW(), def.GetH(), def.GetID(),
                                        def.GetParentID(), def.GetStyle());
...
ActiveWindow* FormWindow::FindControl(DWORD id) { return FindChild(id); }

ActiveWindow* FormWindow::FindControl(int x, int y)
{
