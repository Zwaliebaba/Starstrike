...
namespace Neuron
{
  ...
  {
    const hstring Title = L"Fatal Error";
    const std::string Content{ vformat(_fmt.get(), std::make_format_args(_args...)) };
    const Windows::UI::Popups::MessageDialog dialog(to_hstring(Content), Title);
    std::ignore = dialog.ShowAsync();
  ...
  {
    const hstring Title = L"Fatal Error";
    const hstring Content{ vformat(_fmt.get(), std::make_wformat_args(_args...)) };
    const Windows::UI::Popups::MessageDialog dialog(Content, Title);
    std::ignore = dialog.ShowAsync();
...
#define DEBUG_WARNING(expression, ...)       (void)((!(expression)) || (Print(__VA_ARGS__), 0))
...
inline void PrintVector(const char* _text, const DirectX::XMVECTOR& _vec)
{
  DirectX::XMFLOAT4 vec;
  DirectX::XMStoreFloat4(&vec, _vec);
  Neuron::DebugTrace("{}: {:5.2f}, {:5.2f}, {:5.2f}, {:5.2f}\n", _text, vec.x, vec.y, vec.z, vec.w);
}