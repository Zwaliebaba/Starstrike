...
void Point::SetElement(int i, double v)
{
  switch (i)
  {
    case 0:
...
Point Point::operator*(const Matrix& m) const
{
  Point result;
...
float Vec3::operator*(const Point& p) const { return (x * p.x + y * p.y + z * p.z); }

Vec3 Vec3::operator*(const Matrix& m) const
{
  Vec3 result;

  result.x = static_cast<float>((m.elem[0][0] * x) + (m.elem[1][0] * y) + (m.elem[2][0] * z));
  result.y = static_cast<float>((m.elem[0][1] * x) + (m.elem[1][1] * y) + (m.elem[2][1] * z));
  result.z = static_cast<float>((m.elem[0][2] * x) + (m.elem[1][2] * y) + (m.elem[2][2] * z));
...
inline void swap_elem(double& a, double& b)
{
  double t = a;
  a = b;
  b = t;
...
void Matrix::Rotate(double roll, double pitch, double yaw)
{
  double e[3][3];
...
void Matrix::ComputeEulerAngles(double& roll, double& pitch, double& yaw) const
{
  double cy;
...
Vec3 Matrix::operator*(const Vec3& v) const
{
  ...
  result.x = static_cast<float>((elem[0][0] * v.x) + (elem[0][1] * v.y) + (elem[0][2] * v.z));
  result.y = static_cast<float>((elem[1][0] * v.x) + (elem[1][1] * v.y) + (elem[1][2] * v.z));
  result.z = static_cast<float>((elem[2][0] * v.x) + (elem[2][1] * v.y) + (elem[2][2] * v.z));
...
void Matrix::Invert()
{
  ...
  double det = elem[0][0] * f[0][0] + elem[0][1] * f[1][0] + elem[0][2] * f[2][0];
...
void Plane::Translate(const Vec3& v0) { distance = normal * v0; }

// 3-D dot product.

double DotProduct(const Point& a, const Point& b) { return (a.x * b.x) + (a.y * b.y) + (a.z * b.z); }
...
void MConcat(double in1[3][3], double in2[3][3], double out[3][3])
{
  ...
  for (i = 0; i < 3; i++)
  {
    for (j = 0; j < 3; j++) { out[i][j] = in1[i][0] * in2[0][j] + in1[i][1] * in2[1][j] + in1[i][2] * in2[2][j]; }
  }
}
...
inline int SAME_SIGNS(double a, double b) { return ((a >= 0 && b >= 0) || (a < 0 && b < 0)); }
