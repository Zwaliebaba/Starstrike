...
DWORD* ColorIndex::texture_palette = texture_palettes[0];
...
Color& Color::operator+=(const Color& c)
{
  ...
  Set(static_cast<BYTE>(r), static_cast<BYTE>(g), static_cast<BYTE>(b));
  return *this;
}

Color Color::operator+(DWORD d) const
{
  int r = Red() + ((d & RMask) >> RShift);
  if (r > 255)
    r = 255;
  int g = Green() + ((d & GMask) >> GShift);
  if (g > 255)
    g = 255;
  int b = Blue() + ((d & BMask) >> BShift);
  if (b > 255)
    b = 255;
...
Color Color::operator+(const Color& c) const
{
  ...
  BYTE r = static_cast<BYTE>((fRed() * dst_alpha + c.fRed() * src_alpha) * 255.0f);
  BYTE g = static_cast<BYTE>((fGreen() * dst_alpha + c.fGreen() * src_alpha) * 255.0f);
  BYTE b = static_cast<BYTE>((fBlue() * dst_alpha + c.fBlue() * src_alpha) * 255.0f);
...
Color Color::operator*(const Color& c) const
{
  BYTE r = static_cast<BYTE>((fRed() * c.fRed()) * 255.0f);
  BYTE g = static_cast<BYTE>((fGreen() * c.fGreen()) * 255.0f);
  BYTE b = static_cast<BYTE>((fBlue() * c.fBlue()) * 255.0f);
...
Color Color::operator*(double scale) const
{
  ...
  return Color(static_cast<BYTE>(r), static_cast<BYTE>(g), static_cast<BYTE>(b), static_cast<BYTE>(a));
}
...
Color Color::dim(double scale) const
{
  ...
  return Color(static_cast<BYTE>(r), static_cast<BYTE>(g), static_cast<BYTE>(b), static_cast<BYTE>(Alpha()));
}
...
DWORD Color::Formatted() const
{
  ...
  if (fade != 1.0)
  {
    ...
    DWORD r = static_cast<int>((fRed() - (fRed() - fade_color.fRed()) * step) * 255.0) >> format.rdown;
    DWORD g = static_cast<int>((fGreen() - (fGreen() - fade_color.fGreen()) * step) * 255.0) >> format.gdown;
    DWORD b = static_cast<int>((fBlue() - (fBlue() - fade_color.fBlue()) * step) * 255.0) >> format.bdown;
...
Color Color::Faded() const
{
  if (fade != 1.0)
  {
    ...
    DWORD r = static_cast<int>((fRed() - (fRed() - fade_color.fRed()) * step) * 255.0);
    DWORD g = static_cast<int>((fGreen() - (fGreen() - fade_color.fGreen()) * step) * 255.0);
    DWORD b = static_cast<int>((fBlue() - (fBlue() - fade_color.fBlue()) * step) * 255.0);
    DWORD a = Alpha();
...
DWORD Color::TextureFormat(int keep_alpha) const
{
...
Color Color::ShadeColor(int shade) const
{
  ...
  return Color(static_cast<BYTE>(sr * 255.0), static_cast<BYTE>(sg * 255.0), static_cast<BYTE>(sb * 255.0),
               static_cast<BYTE>(Alpha()));
}

DWORD Color::Shaded(int shade) const { return ShadeColor(shade).Formatted(); }

Color Color::Unformat(DWORD formatted_color)
{
  if (format.pal)
    return Color(static_cast<BYTE>(formatted_color));
  if (standard_format)
  {
    Color c;
  ...
  BYTE r = static_cast<BYTE>((formatted_color & format.rmask) >> format.rshift) << format.rdown;
  BYTE g = static_cast<BYTE>((formatted_color & format.gmask) >> format.gshift) << format.gdown;
  BYTE b = static_cast<BYTE>((formatted_color & format.bmask) >> format.bshift) << format.bdown;
  BYTE a = static_cast<BYTE>((formatted_color & format.amask) >> format.ashift) << format.adown;

  return Color(r, g, b, a);
}

Color Color::Scale(const Color& c1, const Color& c2, double scale)
{
  BYTE r = static_cast<BYTE>((c1.fRed() + (c2.fRed() - c1.fRed()) * scale) * 255);
  BYTE g = static_cast<BYTE>((c1.fGreen() + (c2.fGreen() - c1.fGreen()) * scale) * 255);
  BYTE b = static_cast<BYTE>((c1.fBlue() + (c2.fBlue() - c1.fBlue()) * scale) * 255);
  BYTE a = static_cast<BYTE>((c1.fAlpha() + (c2.fAlpha() - c1.fAlpha()) * scale) * 255);

  return Color(r, g, b, a);
}

DWORD Color::FormattedBlend(DWORD c1, DWORD c2)
{
  if (format.pal)
    return ColorIndex::blend_table[static_cast<BYTE>(c1) * 256 + static_cast<BYTE>(c2)];
...
void Color::UseVideo(Video* v) { video = v; }

void Color::UseFormat(const ColorFormat& cf)
{
  format = cf;
...
void Color::UseTextureFormat(const ColorFormat& cf, int alpha_level)
{
...
void Color::SetPalette(PALETTEENTRY* pal, int palsize, BYTE* invpal)
{
...
void Color::SetFade(double f, Color c, int build_shade)
{
  ...
  if (format.pal && video)
  {
    ...
    for (int i = 0; i < 256; i++)
    {
      ...
      entry.peRed = static_cast<int>((c.fRed() - (c.fRed() - fade_color.fRed()) * step) * 255.0);
      entry.peGreen = static_cast<int>((c.fGreen() - (c.fGreen() - fade_color.fGreen()) * step) * 255.0);
      entry.peBlue = static_cast<int>((c.fBlue() - (c.fBlue() - fade_color.fBlue()) * step) * 255.0);
      entry.peFlags = 0;
    }
  }
...
void Color::BuildBlendTable()
{
  for (int src = 0; src < 256; src++)
  {
    ...
    {
      auto src_clr = ColorIndex(src);
      auto dst_clr = ColorIndex(dst);
      ...
      ColorIndex::blend_table[src * 256 + dst] = Color(static_cast<BYTE>(r), static_cast<BYTE>(g), static_cast<BYTE>(b))
        .Index();
    }
  }
}
...
void Color::SaveShadeTable(const char* basename)
{
  ...
  {
    for (int j = 0; j < 256; j++)
      *pc++ = static_cast<BYTE>(ColorIndex::shade_table[i * 256 + j]);
  }

  for (; i < 256; i++)
  {
    for (int j = 0; j < 256; j++)
      *pc++ = static_cast<BYTE>(0);
  }
...
ColorValue ColorValue::operator*(const ColorValue& c) const { return ColorValue(r * c.r, g * c.g, b * c.b); }

ColorValue ColorValue::operator*(double scale) const
{
  return ColorValue(static_cast<float>(r * scale), static_cast<float>(g * scale), static_cast<float>(b * scale),
...
inline BYTE bclamp(float x) { return static_cast<BYTE>((x < 0) ? 0 : (x > 255) ? 255 : x); }

Color ColorValue::ToColor() const
{
  return Color(bclamp(r * 255.0f), bclamp(g * 255.0f), bclamp(b * 255.0f), bclamp(a * 255.0f));
}
