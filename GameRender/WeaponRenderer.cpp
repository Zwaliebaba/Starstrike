...
void WeaponRenderer::RenderLaser(const Laser& _laser, float _predictionTime)
{
  ...
  const RGBAColour& colour = g_app->m_location->m_teams[_laser.m_fromTeamId].m_colour;
...
void WeaponRenderer::RenderShockwave(const Shockwave& _shockwave, float _predictionTime)
{
  ...
  {
    if (g_app->m_camera->PosInViewFrustum(_shockwave.m_pos))
    {
      float distance = (g_app->m_camera->GetPos() - _shockwave.m_pos).Mag();
      ...
      g_app->m_camera->CreateCameraShake(flashAlpha);
    }
  }
  ...
  {
    float distToBang = (g_app->m_camera->GetPos() - _shockwave.m_pos).Mag();
...
void WeaponRenderer::RenderTurretShell(const TurretShell& _shell, float _predictionTime)
{
  ...
  if (!s_turretShellShape)
    s_turretShellShape = g_app->m_resource->GetShapeStatic("turretshell.shp");
