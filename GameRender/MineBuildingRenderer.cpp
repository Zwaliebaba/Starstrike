...
void MineBuildingRenderer::Render(const Building& _building,
...
void MineBuildingRenderer::RenderAlphas(const Building& _building,
                                         const BuildingRenderContext& _ctx)
{
    ...
    if (mine.m_trackLink != -1)
    {
        ...
        if (trackLink)
        {
            ...
            glVertex3fv(const_cast<float*>((ourPos1 - lineOurPos1).GetData()));
            glTexCoord2f(0.1f, 1.0f);
            glVertex3fv(const_cast<float*>((ourPos1 + lineOurPos1).GetData()));
            glTexCoord2f(0.9f, 1.0f);
            glVertex3fv(const_cast<float*>((theirPos1 + lineTheirPos1).GetData()));
            ...
            glBegin(GL_QUADS);
            glTexCoord2f(0.1f, 0.0f);
            glVertex3fv(const_cast<float*>((ourPos2 - lineOurPos1).GetData()));
            glTexCoord2f(0.1f, 1.0f);
            glVertex3fv(const_cast<float*>((ourPos2 + lineOurPos1).GetData()));
            glTexCoord2f(0.9f, 1.0f);
            glVertex3fv(const_cast<float*>((theirPos2 + lineTheirPos1).GetData()));
            glTexCoord2f(0.9f, 0.0f);
            glVertex3fv(const_cast<float*>((theirPos2 - lineTheirPos1).GetData()));
            glEnd();
...
void MineBuildingRenderer::RenderCart(const MineBuilding& _mine,
                                      MineCart* _cart,
                                      float _predictionTime)
{
    ...
    if (g_app->m_camera->PosInViewFrustum(cartPos))
    {
        ...
        LegacyVector3 cartLinkLeft =
            MineBuilding::s_cartShape->GetMarkerWorldMatrix(
                MineBuilding::s_cartMarker1, transform).pos;
        LegacyVector3 cartLinkRight =
            MineBuilding::s_cartShape->GetMarkerWorldMatrix(
                MineBuilding::s_cartMarker2, transform).pos;

        LegacyVector3 camRight = g_app->m_camera->GetRight() * 0.5f;
        glBegin(GL_QUADS);
        glVertex3fv(const_cast<float*>((trackLeft - camRight).GetData()));
        glVertex3fv(const_cast<float*>((trackLeft + camRight).GetData()));
        glVertex3fv(const_cast<float*>((cartLinkLeft + camRight).GetData()));
        glVertex3fv(const_cast<float*>((cartLinkLeft - camRight).GetData()));

        glVertex3fv(const_cast<float*>((trackRight - camRight).GetData()));
        glVertex3fv(const_cast<float*>((trackRight + camRight).GetData()));
        glVertex3fv(const_cast<float*>((cartLinkRight + camRight).GetData()));
        glVertex3fv(const_cast<float*>((cartLinkRight - camRight).GetData()));
        glEnd();
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            {
                Matrix34 polyMat = MineBuilding::s_cartShape->GetMarkerWorldMatrix(
                    MineBuilding::s_cartContents[i], transform);
            ...
            if (_cart->m_primitives[i])
            {
                ...
                if (buildingDetail < 3)
                {
                    ...
                    g_app->m_camera->SetupProjectionMatrix(
                        nearPlaneStart * 1.1f, g_app->m_renderer->GetFarPlane());

                    Render3DSprite(polyMat.pos - LegacyVector3(0, 25, 0),
                                   50.0f, 50.0f,
                                   g_app->m_resource->GetTexture("textures/glow.bmp"));

                    g_app->m_camera->SetupProjectionMatrix(
                        nearPlaneStart, g_app->m_renderer->GetFarPlane());
