...
DarwiniaWindow::DarwiniaWindow(const char* name)
  : EclWindow((char*)name),
    m_currentButton(0),
...
void DarwiniaWindow::CreateValueControl(const char* name, int dataType, void* value, int y, float change, float _lowBound, float _highBound,
                                        DarwiniaButton* callback, int x, int w)
{
  ...
  if (dataType == InputField::TypeString)
    input->SetShortProperties(name, x, y, w, GetMenuSize(15));
  else
    input->SetShortProperties(name, x, y, w - 37, GetMenuSize(15));
...
void DarwiniaWindow::CreateColourControl(const char* name, int* value, int y, DarwiniaButton* callback, int x, int w)
{
  ...
  auto cw = new ColourWidget();
  cw->SetShortProperties(name, x, y, w - 13);
...
int DarwiniaWindow::GetClientRectX1() { return 2; }

int DarwiniaWindow::GetClientRectX2() { return m_w - 2; }

int DarwiniaWindow::GetClientRectY1() { return GetMenuSize(15) + 1; }

int DarwiniaWindow::GetClientRectY2() { return m_h - 2; }
